...
extern struct Descriptor *Data_0200b92c[];

struct Effect *Func_0200384e();
struct Effect *Func_020037f4();
void Func_020037fe();
void Func_02003818();
void Func_0200397a();
s32 Func_020038a4();
                   ...
                   u32 flags, const struct Options *options)
{
    ...
    party = Func_0200384e(0);
    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_020037f4(options->kind, x, y, z);
    } else {
        effect = Func_020037f4(222, x, y, z);
    }
    if (effect == 0) return;

    block = effect->sprite;
    mode_block = block;
    Func_020037fe(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02003818(effect, Data_0200b92c[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_0200397a(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        const struct Descriptor *descriptor = Data_0200b92c[table_offset >> 2];
        s32 delta;
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_020038a4(first_delta,
                                            descriptor->duration);
            ...
            effect->rate30 = Func_020038bc(first_delta,
                                            descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
            delta += (s32)0xffff0000;
        }
        effect->rate34 = Func_020038ca(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02003918(effect, 1);
        Func_02003928(effect, options->callback_arg);
    }
    if ((flags & 0x400000) != 0) {
