.syntax unified
.include "games/THE LOST AGE/SRC/COMMON/OVERLAY.INC"
	.thumb
	.set sub_02000a62, 0x02000a62
	.set sub_020017de, 0x020017de
	.set sub_0200220c, 0x0200220c
	.set sub_0200227c, 0x0200227c
	.set sub_0200228a, 0x0200228a
	.set sub_020022aa, 0x020022aa
	.set sub_020023be, 0x020023be
	.set sub_020024e4, 0x020024e4
	.set sub_02002518, 0x02002518
	.set sub_02002528, 0x02002528
	.set sub_020025da, 0x020025da
	.set sub_020025f2, 0x020025f2
	.set sub_02002600, 0x02002600
	.set sub_02002610, 0x02002610
	.set sub_02002612, 0x02002612
	.set sub_0200261e, 0x0200261e
	.set sub_02002650, 0x02002650
	.set sub_02002654, 0x02002654
	.set sub_0200268e, 0x0200268e
	.set sub_02002690, 0x02002690
	.set sub_020026c8, 0x020026c8
	.set sub_020026d8, 0x020026d8
	.set sub_020026e6, 0x020026e6
	.set sub_020026f4, 0x020026f4
	.set sub_020026f6, 0x020026f6
	.set sub_020026fc, 0x020026fc
	.set sub_02002702, 0x02002702
	.set sub_02002708, 0x02002708
	.set sub_0200270e, 0x0200270e
	.set sub_02002714, 0x02002714
	.set sub_02002724, 0x02002724
	.set sub_02002728, 0x02002728
	.set sub_0200274c, 0x0200274c
	.set sub_02002750, 0x02002750
	.set sub_0200276e, 0x0200276e
	.set sub_02002778, 0x02002778
	.set sub_0200277e, 0x0200277e
	.set sub_02002786, 0x02002786
	.set sub_02002788, 0x02002788
	.set sub_0200278a, 0x0200278a
	.set sub_0200279c, 0x0200279c
	.set sub_020027a2, 0x020027a2
	.set sub_020027ac, 0x020027ac
	.set sub_020027e0, 0x020027e0
	.set sub_020027e2, 0x020027e2
	.set sub_02002822, 0x02002822
	.set sub_0200284a, 0x0200284a
	.set sub_0200286c, 0x0200286c
	.set sub_0200287a, 0x0200287a
	.set sub_020028b4, 0x020028b4
	.set sub_020028b6, 0x020028b6
	.set sub_020028d2, 0x020028d2
	.set sub_020028dc, 0x020028dc
	.set sub_020028de, 0x020028de
	.set sub_020028ec, 0x020028ec
	.set sub_020028ee, 0x020028ee
	.set sub_02002902, 0x02002902
	.set sub_0200290a, 0x0200290a
	.set sub_0200292a, 0x0200292a
	.set sub_02002936, 0x02002936
	.set sub_02002962, 0x02002962
	.set sub_02002974, 0x02002974
	.set sub_020029a8, 0x020029a8
	.set sub_020029be, 0x020029be
	.set sub_020029d8, 0x020029d8
	.set sub_020029de, 0x020029de
	.set sub_02002a0a, 0x02002a0a
	.set sub_02002a40, 0x02002a40
	.set sub_02002a44, 0x02002a44
	.set sub_02002a48, 0x02002a48
	.set sub_02002a5a, 0x02002a5a
	.set sub_02002a72, 0x02002a72
	.set sub_02002a8c, 0x02002a8c
	.set sub_02002a94, 0x02002a94
	.set sub_02002a9a, 0x02002a9a
	.set sub_02002a9c, 0x02002a9c
	.set sub_02002ac0, 0x02002ac0
	.set sub_02002ace, 0x02002ace
	.set sub_02002b04, 0x02002b04
	.set sub_02002b06, 0x02002b06
	.set sub_02002b0e, 0x02002b0e
	.set sub_02002b36, 0x02002b36
	.set sub_02002b44, 0x02002b44
	.set sub_02002b46, 0x02002b46
	.set sub_02002b48, 0x02002b48
	.set sub_02002b92, 0x02002b92
	.set sub_02002b9a, 0x02002b9a
	.set sub_02002bbe, 0x02002bbe
	.set sub_02002bc2, 0x02002bc2
	.set sub_02002c14, 0x02002c14
	.set sub_02002c1c, 0x02002c1c
	.set sub_02002c26, 0x02002c26
	.set sub_02002c44, 0x02002c44
	.set sub_02002c90, 0x02002c90
	.set sub_02002cc8, 0x02002cc8
	.set sub_02002cd6, 0x02002cd6
	.set sub_02002d1c, 0x02002d1c
	.set sub_02002d28, 0x02002d28
	.set sub_02002d42, 0x02002d42
	.set sub_02002d8c, 0x02002d8c
	.set sub_02002d96, 0x02002d96
	.set sub_02002d9a, 0x02002d9a
	.set sub_02002dbe, 0x02002dbe
	.set sub_02002dce, 0x02002dce
	.set sub_02002dd2, 0x02002dd2
	.set sub_02002ddc, 0x02002ddc
	.set sub_02002de0, 0x02002de0
	.set sub_02002de8, 0x02002de8
	.set sub_02002df4, 0x02002df4
	.set sub_02002e24, 0x02002e24
	.set sub_02002e42, 0x02002e42
	.set sub_02002e50, 0x02002e50
	.set sub_02002e5a, 0x02002e5a
	.set sub_02002e84, 0x02002e84
	.set sub_02002ea2, 0x02002ea2
	.set sub_02002ea4, 0x02002ea4
	.set sub_02002eaa, 0x02002eaa
	.set sub_02002eca, 0x02002eca
	.set sub_02002ed4, 0x02002ed4
	.set sub_02002ee0, 0x02002ee0
	.set sub_02002f26, 0x02002f26
	.set sub_02002f38, 0x02002f38
	.set sub_02002f4e, 0x02002f4e
	.set sub_02002f54, 0x02002f54
	.set sub_02002f6c, 0x02002f6c
	.set sub_02002fe4, 0x02002fe4
	.set sub_0200305e, 0x0200305e
	.set sub_02003072, 0x02003072
	.set sub_02003078, 0x02003078
	.set sub_0200307a, 0x0200307a
	.set sub_0200307e, 0x0200307e
	.set sub_02003084, 0x02003084
	.set sub_0200309c, 0x0200309c
	.set sub_020030ba, 0x020030ba
	.set sub_02003140, 0x02003140
	.set sub_02003166, 0x02003166
	.set sub_02003180, 0x02003180
	.set sub_02003192, 0x02003192
	.set sub_020031a0, 0x020031a0
	.set sub_020031ac, 0x020031ac
	.set sub_020031b8, 0x020031b8
	.set sub_020031c4, 0x020031c4
	.set sub_020031ca, 0x020031ca
	.set sub_020031d0, 0x020031d0
	.set sub_020031e0, 0x020031e0
	.set sub_020031e8, 0x020031e8
	.set sub_020031f6, 0x020031f6
	.set sub_02003202, 0x02003202
	.set sub_02003226, 0x02003226
	.set sub_02003260, 0x02003260
	.set sub_020032ac, 0x020032ac
	.set sub_020032d8, 0x020032d8
	.set sub_020032f2, 0x020032f2
	.set sub_02003312, 0x02003312
	.set sub_0200332a, 0x0200332a
	.set sub_02003330, 0x02003330
	.set sub_02003350, 0x02003350
	.set sub_02003372, 0x02003372
	.set sub_0200345c, 0x0200345c
	.set sub_02003490, 0x02003490
	.set sub_020034a4, 0x020034a4
	.set sub_020034b2, 0x020034b2
	.set sub_020034ba, 0x020034ba
	.set sub_0200350e, 0x0200350e
	.set sub_02003546, 0x02003546
	.set sub_020035a2, 0x020035a2
	.set sub_020035a4, 0x020035a4
	.set sub_020035b4, 0x020035b4
	.set sub_020035d4, 0x020035d4
	.set sub_020035dc, 0x020035dc
	.set sub_02003676, 0x02003676
	.set sub_020036b0, 0x020036b0
	.set sub_020036b2, 0x020036b2
	.set sub_020036c6, 0x020036c6
	.set sub_0200370c, 0x0200370c
	.set sub_0200371c, 0x0200371c
	.set sub_0200371e, 0x0200371e
	.set sub_02003722, 0x02003722
	.set sub_02003740, 0x02003740
	.set sub_02003768, 0x02003768
	.set sub_02003770, 0x02003770
	.set sub_02003778, 0x02003778
	.set sub_020037d4, 0x020037d4
	.set sub_020037ee, 0x020037ee
	.set sub_020037f2, 0x020037f2
	.set sub_0200380c, 0x0200380c
	.set sub_0200382e, 0x0200382e
	.set sub_02003830, 0x02003830
	.set sub_02003834, 0x02003834
	.set sub_0200383a, 0x0200383a
	.set sub_0200383c, 0x0200383c
	.set sub_0200383e, 0x0200383e
	.set sub_02003840, 0x02003840
	.set sub_02003844, 0x02003844
	.set sub_02003848, 0x02003848
	.set sub_0200384a, 0x0200384a
	.set sub_0200384e, 0x0200384e
	.set sub_02003850, 0x02003850
	.set sub_02003854, 0x02003854
	.set sub_02003858, 0x02003858
	.set sub_0200386c, 0x0200386c
	.set sub_0200387a, 0x0200387a
	.set sub_02003886, 0x02003886
	.set sub_020038a8, 0x020038a8
	.set sub_020038c0, 0x020038c0
	.set sub_020038d4, 0x020038d4
	.set sub_020038dc, 0x020038dc
	.set sub_020038e4, 0x020038e4
	.set sub_0200392c, 0x0200392c
	.set sub_0200392e, 0x0200392e
	.set sub_0200393e, 0x0200393e
	.set sub_02003960, 0x02003960
	.set sub_02003966, 0x02003966
	.set sub_020039b2, 0x020039b2
	.set sub_020039bc, 0x020039bc
	.set sub_02003a02, 0x02003a02
	.set sub_02003a1c, 0x02003a1c
	.set sub_02003a26, 0x02003a26
	.set sub_02003a50, 0x02003a50
	.set sub_02003a64, 0x02003a64
	.set sub_02003a6e, 0x02003a6e
	.set sub_02003a74, 0x02003a74
	.set sub_02003a7c, 0x02003a7c
	.set sub_02003a92, 0x02003a92
	.set sub_02003aca, 0x02003aca
	.set sub_02003ace, 0x02003ace
	.set sub_02003bfc, 0x02003bfc
	.set sub_02003c5e, 0x02003c5e
	.set sub_02003c62, 0x02003c62
	.set sub_02003c74, 0x02003c74
	.set sub_02003c78, 0x02003c78
	.set sub_02003c98, 0x02003c98
	.set sub_02003caa, 0x02003caa
	.set sub_02003cb4, 0x02003cb4
	.set sub_02003d0e, 0x02003d0e
	.set sub_02003d38, 0x02003d38
	.set sub_02003d6e, 0x02003d6e
	.set sub_02003d9e, 0x02003d9e
	.set sub_02003db8, 0x02003db8
	.set sub_02003dda, 0x02003dda
	.set sub_02003e5e, 0x02003e5e
	.set sub_02003e62, 0x02003e62
	.set sub_02003f3c, 0x02003f3c
	.set sub_02003f5c, 0x02003f5c
	.set sub_02003f9c, 0x02003f9c
	.set sub_02003fd4, 0x02003fd4
	.set sub_020040ba, 0x020040ba
	.set sub_0200412a, 0x0200412a
	.set sub_020042d2, 0x020042d2
	.global Overlay_02000000
Overlay_02000000:
	.irp EntryTarget, 0x02008329, 0x02008039, 0x02008045, 0x0200804d, 0x02008055, 0x02008041, 0x02008a6d
	overlay_veneer \EntryTarget
	.endr
	.4byte 0x47704800
	.4byte 0x0200a49c
	.4byte 0x47702000
	.4byte 0x47704800
	.4byte 0x0200a4cc
	.4byte 0x47704800
	.4byte 0x0200a4d0
	.4byte 0x47704800
	.2byte 0xa4e8
	.2byte 0x0200
	push	{r5, r6, lr}
	movs	r0, #0
	ldr	r5, [pc, #64]	@ (0x20000a4)
	bl	sub_0200227c
	ldr	r3, [pc, #56]	@ (0x20000a0)
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #19
	adds	r2, #12
	strh	r3, [r2, #0]
	ldr	r2, [pc, #52]	@ (0x20000a8)
	movs	r3, #0
	strh	r3, [r2, #10]
	adds	r0, r5, #0
	bl	sub_0200228a
	movs	r6, #128	@ 0x80
	movs	r3, #128	@ 0x80
	movs	r2, #132	@ 0x84
	lsls	r6, r6, #1
	lsls	r3, r3, #19
	movs	r1, #160	@ 0xa0
	lsls	r2, r2, #24
	adds	r6, #255	@ 0xff
	adds	r4, r0, #0
	adds	r3, #212	@ 0xd4
	lsls	r1, r1, #19
	adds	r2, #112	@ 0x70
	stmia	r3!, {r0, r1, r2}
	subs	r3, #12
	movs	r2, #224	@ 0xe0
	lsls	r2, r2, #1
	adds	r4, r4, r2
	b.n	.L_020000ac
	.4byte 0x00000681
	.4byte 0x00000022
	.2byte 0x1120
	.2byte 0x0300
.L_020000ac:
	adds	r0, r4, #0
	ldr	r1, [pc, #148]	@ (0x2000144)
	bl	sub_020022aa
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #19
	adds	r3, #212	@ 0xd4
	ldr	r0, [pc, #136]	@ (0x2000144)
	ldr	r1, [pc, #136]	@ (0x2000148)
	ldr	r2, [pc, #140]	@ (0x200014c)
	stmia	r3!, {r0, r1, r2}
	subs	r3, #12
	ldr	r1, [pc, #136]	@ (0x2000150)
	movs	r3, #208	@ 0xd0
	lsls	r3, r3, #1
	movs	r0, #0
.L_020000cc:
	movs	r4, #0
.L_020000ce:
	adds	r2, r3, #0
	movs	r5, #128	@ 0x80
	lsls	r3, r2, #16
	lsls	r5, r5, #9
	adds	r3, r3, r5
	adds	r4, #1
	strh	r2, [r1, #0]
	asrs	r3, r3, #16
	adds	r1, #2
	cmp	r4, #29
	bls.n	.L_020000ce
	strh	r6, [r1, #0]
	adds	r0, #1
	adds	r1, #2
	strh	r6, [r1, #0]
	adds	r1, #2
	cmp	r0, #19
	bls.n	.L_020000cc
	ldr	r2, [pc, #96]	@ (0x2000154)
	movs	r0, #0
.L_020000f6:
	movs	r3, #0
	adds	r0, #1
	strh	r3, [r2, #2]
	strh	r3, [r2, #0]
	adds	r2, #4
	cmp	r0, #3
	bls.n	.L_020000f6
	movs	r3, #128	@ 0x80
	movs	r1, #128	@ 0x80
	movs	r2, #132	@ 0x84
	lsls	r3, r3, #19
	lsls	r1, r1, #19
	lsls	r2, r2, #24
	adds	r3, #212	@ 0xd4
	ldr	r0, [pc, #64]	@ (0x2000154)
	adds	r1, #16
	adds	r2, #4
	stmia	r3!, {r0, r1, r2}
	subs	r3, #12
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r2, [r3, #32]
	movs	r3, #160	@ 0xa0
	lsls	r3, r3, #5
	strh	r3, [r2, #20]
	ldr	r3, [pc, #44]	@ (0x2000158)
	movs	r2, #133	@ 0x85
	lsls	r2, r2, #2
	adds	r3, r3, r2
	ldr	r0, [r3, #0]
	bl	sub_020024e4
	ldr	r5, [pc, #8]	@ (0x2000140)
	adds	r0, #85	@ 0x55
	strb	r5, [r0, #0]
	pop	{r5, r6, pc}
	.2byte 0x0000
	.4byte 0x00000000
	.4byte 0x02010000
	.4byte 0x06006800
	.4byte 0x84002580
	.4byte 0x06003000
	.4byte 0x03001120
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, lr}
	ldr	r3, [pc, #132]	@ (0x20001e4)
	ldr	r1, [pc, #132]	@ (0x20001e8)
	ldrh	r2, [r3, #0]
	adds	r2, #1
	strh	r2, [r3, #0]
	lsls	r2, r2, #16
	lsrs	r5, r2, #17
	ldr	r0, [pc, #124]	@ (0x20001ec)
	ldrh	r3, [r0, #0]
	adds	r4, r3, #0
	strh	r0, [r0, #0]
	ldrh	r2, [r1, #0]
	cmp	r2, #31
	bgt.n	.L_0200019e
	lsls	r3, r2, #1
	adds	r3, r3, r2
	adds	r2, #1
	lsls	r3, r3, #2
	strh	r2, [r1, #0]
	movs	r2, #184	@ 0xb8
	adds	r3, r3, r1
	lsls	r2, r2, #6
	adds	r3, #4
	adds	r2, #81	@ 0x51
	stmia	r3!, {r2}
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #19
	adds	r2, #80	@ 0x50
	stmia	r3!, {r2}
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #10
	str	r2, [r3, #0]
.L_0200019e:
	strh	r4, [r0, #0]
	ldrh	r3, [r0, #0]
	adds	r4, r3, #0
	strh	r0, [r0, #0]
	ldrh	r3, [r1, #0]
	cmp	r3, #31
	bgt.n	.L_020001d4
	lsls	r2, r3, #1
	adds	r2, r2, r3
	lsls	r2, r2, #2
	adds	r3, #1
	adds	r2, r2, r1
	strh	r3, [r1, #0]
	adds	r1, r5, #0
	movs	r3, #16
	subs	r3, r3, r1
	lsls	r3, r3, #8
	adds	r2, #4
	orrs	r3, r1
	stmia	r2!, {r3}
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #19
	adds	r3, #82	@ 0x52
	stmia	r2!, {r3}
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #10
	str	r3, [r2, #0]
.L_020001d4:
	strh	r4, [r0, #0]
	cmp	r5, #15
	bls.n	.L_020001e0
	ldr	r0, [pc, #20]	@ (0x20001f0)
	bl	sub_020023be
.L_020001e0:
	pop	{r5, pc}
	.2byte 0x0000
	.4byte 0x0200a4f4
	.4byte 0x020038e0
	.4byte 0x04000208
	.2byte 0x815d
	.2byte 0x0200
	push	{r5, r6, r7, lr}
	ldr	r1, [pc, #92]	@ (0x2000254)
	movs	r2, #0
	ldrsh	r3, [r1, r2]
	mov	ip, r1
	cmp	r3, #0
	bne.n	.L_0200026a
	ldr	r6, [pc, #84]	@ (0x2000258)
	ldr	r4, [pc, #84]	@ (0x200025c)
	movs	r3, #0
	ldrsh	r5, [r6, r3]
	cmp	r5, #0
	beq.n	.L_02000218
	ldr	r3, [r4, #0]
	cmp	r3, #0
	bne.n	.L_0200026a
	strh	r3, [r6, #0]
	b.n	.L_0200026a
.L_02000218:
	ldr	r3, [r4, #0]
	cmp	r3, #0
	beq.n	.L_0200026a
	ldr	r1, [pc, #64]	@ (0x2000260)
	ldr	r7, [pc, #64]	@ (0x2000264)
	movs	r2, #0
	ldrsh	r3, [r1, r2]
	ldrh	r0, [r1, #0]
	lsls	r3, r3, #1
	ldrh	r2, [r7, r3]
	ldr	r3, [r4, #0]
	cmp	r3, r2
	bne.n	.L_02000268
	ldr	r2, [pc, #28]	@ (0x2000250)
	adds	r3, r0, #1
	strh	r3, [r1, #0]
	strh	r2, [r6, #0]
	lsls	r3, r3, #16
	asrs	r3, r3, #15
	ldrh	r3, [r7, r3]
	cmp	r3, #0
	bne.n	.L_0200026a
	mov	r3, ip
	strh	r2, [r3, #0]
	movs	r0, #110	@ 0x6e
	bl	sub_02002654
	b.n	.L_0200026a
	.4byte 0x00000001
	.4byte 0x0200a4f6
	.4byte 0x0200a4fe
	.4byte 0x03001150
	.4byte 0x0200a4fa
	.2byte 0xa410
	.2byte 0x0200
.L_02000268:
	strh	r5, [r1, #0]
.L_0200026a:
	ldr	r1, [pc, #92]	@ (0x20002c8)
	movs	r2, #0
	ldrsh	r3, [r1, r2]
	mov	ip, r1
	cmp	r3, #0
	bne.n	.L_020002de
	ldr	r6, [pc, #84]	@ (0x20002cc)
	ldr	r4, [pc, #84]	@ (0x20002d0)
	movs	r3, #0
	ldrsh	r5, [r6, r3]
	cmp	r5, #0
	beq.n	.L_0200028c
	ldr	r3, [r4, #0]
	cmp	r3, #0
	bne.n	.L_020002de
	strh	r3, [r6, #0]
	b.n	.L_020002de
.L_0200028c:
	ldr	r3, [r4, #0]
	cmp	r3, #0
	beq.n	.L_020002de
	ldr	r1, [pc, #64]	@ (0x20002d4)
	ldr	r7, [pc, #64]	@ (0x20002d8)
	movs	r2, #0
	ldrsh	r3, [r1, r2]
	ldrh	r0, [r1, #0]
	lsls	r3, r3, #1
	ldrh	r2, [r7, r3]
	ldr	r3, [r4, #0]
	cmp	r3, r2
	bne.n	.L_020002dc
	ldr	r2, [pc, #28]	@ (0x20002c4)
	adds	r3, r0, #1
	strh	r3, [r1, #0]
	strh	r2, [r6, #0]
	lsls	r3, r3, #16
	asrs	r3, r3, #15
	ldrh	r3, [r7, r3]
	cmp	r3, #0
	bne.n	.L_020002de
	mov	r3, ip
	strh	r2, [r3, #0]
	movs	r0, #110	@ 0x6e
	bl	sub_020026c8
	b.n	.L_020002de
	.4byte 0x00000001
	.4byte 0x0200a4f8
	.4byte 0x0200a500
	.4byte 0x03001150
	.4byte 0x0200a4fc
	.2byte 0xa418
	.2byte 0x0200
.L_020002dc:
	strh	r5, [r1, #0]
.L_020002de:
	pop	{r5, r6, r7, pc}
	push	{lr}
	movs	r0, #162	@ 0xa2
	lsls	r0, r0, #1
	bl	sub_02002528
	cmp	r0, #0
	bne.n	.L_020002f2
	movs	r0, #0
	b.n	.L_0200031c
.L_020002f2:
	ldr	r2, [pc, #44]	@ (0x2000320)
	movs	r1, #128	@ 0x80
	lsls	r1, r1, #2
	adds	r1, #94	@ 0x5e
	adds	r3, r2, r1
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	movs	r0, #0
	cmp	r3, #2
	beq.n	.L_0200031c
	movs	r1, #240	@ 0xf0
	lsls	r1, r1, #1
	adds	r3, r2, r1
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	ldr	r2, [pc, #16]	@ (0x2000324)
	eors	r3, r2
	negs	r0, r3
	orrs	r0, r3
	lsrs	r0, r0, #31
	negs	r0, r0
.L_0200031c:
	pop	{pc}
	.2byte 0x0000
	.4byte 0x02000240
	.2byte 0x0002
	.2byte 0x0000
	push	{r5, r6, r7, lr}
	mov	r7, sl
	mov	r6, r8
	push	{r6, r7}
	movs	r1, #0
	mov	sl, r1
	bl	.L_02000392
	movs	r1, #144	@ 0x90
	lsls	r1, r1, #3
	ldr	r0, [pc, #872]	@ (0x20006a8)
	bl	sub_02002518
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #108]	@ 0x6c
	movs	r2, #214	@ 0xd6
	lsls	r2, r2, #1
	adds	r3, r3, r2
	mov	r1, sl
	str	r1, [r3, #0]
	bl	sub_02002714
	bl	sub_02002728
	ldr	r3, [pc, #848]	@ (0x20006ac)
	movs	r2, #241	@ 0xf1
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	cmp	r3, #2
	bne.n	.L_020003ee
.L_0200036a:
	ldr	r5, [pc, #836]	@ (0x20006b0)
	movs	r1, #5
	adds	r0, r5, #0
	bl	sub_020025da
	movs	r0, #1
	bl	sub_02002750
	adds	r6, r0, #0
	bl	sub_0200261e
	cmp	r6, #0
	bne.n	.L_020003cc
	movs	r1, #1
	adds	r0, r5, #1
	bl	sub_020025f2
	ldr	r3, [pc, #796]	@ (0x20006ac)
	movs	r2, #152	@ 0x98
	lsls	r2, r2, #1
.L_02000392:
	adds	r2, #255	@ 0xff
	adds	r3, r3, r2
	movs	r2, #1
	strb	r2, [r3, #0]
	movs	r0, #5
	bl	sub_020026f6
	movs	r0, #6
	bl	sub_020026fc
	movs	r0, #7
	bl	sub_02002702
	movs	r0, #1
	bl	sub_02002708
	movs	r0, #2
	bl	sub_0200270e
	movs	r0, #3
	bl	sub_02002714
	bl	sub_02002690
	movs	r3, #1
	adds	r6, r0, #0
	negs	r3, r3
	cmp	r6, r3
	beq.n	.L_0200036a
.L_020003cc:
	movs	r0, #60	@ 0x3c
	bl	sub_02002778
	bl	sub_0200279c
	movs	r0, #78	@ 0x4e
	bl	sub_020027e2
	movs	r0, #150	@ 0x96
	lsls	r0, r0, #1
	bl	sub_0200278a
	ldr	r0, [pc, #716]	@ (0x20006b4)
	movs	r1, #1
	bl	sub_020027a2
	b.n	.L_02000a2c
.L_020003ee:
	ldr	r3, [pc, #712]	@ (0x20006b8)
	mov	r1, sl
	strb	r1, [r3, #0]
.L_020003f4:
	bl	sub_020026e6
	adds	r5, r0, #0
	cmp	r5, #0
	bge.n	.L_02000422
	ldr	r3, [pc, #700]	@ (0x20006bc)
	ldrb	r3, [r3, #0]
	cmp	r3, #0
	beq.n	.L_02000422
	ldr	r3, [pc, #676]	@ (0x20006ac)
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #2
	adds	r2, #74	@ 0x4a
	adds	r3, r3, r2
	movs	r2, #1
	strb	r2, [r3, #0]
	ldr	r3, [pc, #680]	@ (0x20006c0)
	ldr	r0, [pc, #684]	@ (0x20006c4)
	strb	r2, [r3, #0]
	movs	r1, #1
	movs	r2, #8
	bl	sub_020026d8
.L_02000422:
	cmp	r5, #0
	bne.n	.L_02000446
	mov	r3, sl
	cmp	r3, #0
	bne.n	.L_02000446
	movs	r0, #30
	bl	sub_02002600
	movs	r1, #144	@ 0x90
	lsls	r1, r1, #3
	ldr	r0, [pc, #656]	@ (0x20006c8)
	bl	sub_02002612
	movs	r0, #1
	bl	sub_02002610
	movs	r1, #1
	mov	sl, r1
.L_02000446:
	cmp	r5, #0
	ble.n	.L_02000452
	bl	sub_020026f4
	adds	r5, r0, #0
	b.n	.L_02000454
.L_02000452:
	movs	r5, #0
.L_02000454:
	cmp	r5, #0
	beq.n	.L_0200045a
	b.n	.L_020006fc
.L_0200045a:
	bl	sub_020027ac
	ldr	r3, [pc, #588]	@ (0x20006ac)
	movs	r1, #147	@ 0x93
	lsls	r1, r1, #1
	adds	r1, #255	@ 0xff
	adds	r2, r3, r1
	ldrb	r0, [r2, #0]
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #2
	adds	r2, #38	@ 0x26
	adds	r3, r3, r2
	ldrb	r1, [r3, #0]
	bl	sub_02002786
	movs	r6, #4
	movs	r7, #5
.L_0200047c:
	movs	r0, #6
	bl	sub_02002650
	adds	r0, r6, #0
	bl	sub_0200277e
	movs	r3, #1
	adds	r5, r0, #0
	negs	r3, r3
	cmp	r5, r3
	bne.n	.L_0200049a
	cmp	r6, #4
	beq.n	.L_020003f4
	subs	r6, #1
	b.n	.L_0200047c
.L_0200049a:
	ldr	r3, [pc, #560]	@ (0x20006cc)
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	cmp	r3, #0
	bne.n	.L_020004ae
	ldr	r3, [pc, #552]	@ (0x20006d0)
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #0
	beq.n	.L_020004b0
.L_020004ae:
	movs	r7, #8
.L_020004b0:
	adds	r6, #1
	cmp	r6, r7
	blt.n	.L_0200047c
.L_020004b6:
	ldr	r0, [pc, #540]	@ (0x20006d4)
	movs	r1, #13
	bl	sub_02002724
	movs	r0, #1
	movs	r1, #0
	movs	r2, #0
	movs	r3, #0
	bl	sub_020027e0
	adds	r5, r0, #0
	bl	sub_0200276e
	cmp	r5, #0
	bne.n	.L_02000570
.L_020004d4:
	movs	r0, #0
	bl	sub_02002788
	movs	r3, #1
	negs	r3, r3
	adds	r5, r0, #0
	mov	r8, r3
	cmp	r5, r8
	beq.n	.L_020003f4
	cmp	r5, #1
	bne.n	.L_0200052a
	ldr	r5, [pc, #492]	@ (0x20006d8)
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r6, [pc, #488]	@ (0x20006dc)
	ldr	r0, [pc, #488]	@ (0x20006e0)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0xf000
	.2byte 0xfd91
	.2byte 0x1c07
	cmp	r7, r8
	beq.n	.L_02000508
	bl	sub_020028ee
.L_02000508:
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r0, [pc, #468]	@ (0x20006e4)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x22c0
	adds	r0, r5, #0
	ldr	r1, [pc, #452]	@ (0x20006e0)
	lsls	r2, r2, #6
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x4547
	beq.n	.L_020004d4
	ldr	r0, [pc, #444]	@ (0x20006e4)
	movs	r1, #0
	b.n	.L_02000568
.L_0200052a:
	cmp	r5, #0
	bne.n	.L_02000574
	ldr	r5, [pc, #424]	@ (0x20006d8)
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r6, [pc, #420]	@ (0x20006dc)
	ldr	r0, [pc, #420]	@ (0x20006e0)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0xf002
	.2byte 0xf9a7
	.2byte 0xf000
	.2byte 0xff21
	.2byte 0x22c0
	adds	r7, r0, #0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r0, [pc, #404]	@ (0x20006e4)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x22c0
	adds	r0, r5, #0
	ldr	r1, [pc, #388]	@ (0x20006e0)
	lsls	r2, r2, #6
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x4547
	beq.n	.L_020004d4
	ldr	r0, [pc, #380]	@ (0x20006e4)
	movs	r1, #1
.L_02000568:
	movs	r2, #0
	bl	sub_020028dc
	b.n	.L_02000574
.L_02000570:
	bl	sub_0200290a
.L_02000574:
	movs	r6, #0
	movs	r7, #1
.L_02000578:
	movs	r0, #6
	bl	sub_0200274c
	adds	r0, r6, #0
	bl	sub_0200287a
	movs	r1, #1
	adds	r5, r0, #0
	negs	r1, r1
	cmp	r5, r1
	bne.n	.L_02000596
	cmp	r6, #0
	beq.n	.L_020004b6
	subs	r6, #1
	b.n	.L_02000578
.L_02000596:
	ldr	r3, [pc, #312]	@ (0x20006d0)
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #0
	beq.n	.L_020005a2
	movs	r7, #4
.L_020005a2:
	adds	r6, #1
	cmp	r6, r7
	blt.n	.L_02000578
	ldr	r3, [pc, #316]	@ (0x20006e8)
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	cmp	r3, #0
	beq.n	.L_0200069c
.L_020005b2:
	ldr	r5, [pc, #312]	@ (0x20006ec)
	movs	r1, #13
	adds	r0, r5, #0
	bl	sub_02002822
	ldr	r1, [pc, #304]	@ (0x20006f0)
	movs	r0, #1
	movs	r2, #8
	movs	r3, #0
	bl	sub_020028de
	adds	r6, r0, #0
	bl	sub_0200286c
	cmp	r6, #0
	bne.n	.L_02000648
	ldr	r3, [pc, #276]	@ (0x20006e8)
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #0
	bne.n	.L_020005e6
	adds	r0, r5, #1
	movs	r1, #1
	bl	sub_0200284a
	b.n	.L_02000648
.L_020005e6:
	ldr	r6, [pc, #240]	@ (0x20006d8)
	movs	r2, #192	@ 0xc0
	adds	r1, r6, #0
	lsls	r2, r2, #6
	ldr	r7, [pc, #236]	@ (0x20006dc)
	ldr	r0, [pc, #236]	@ (0x20006e0)
	mov	lr, r7
	.2byte 0xf800
	.2byte 0x2005
	bl	sub_020028d2
	adds	r5, r0, #0
	cmp	r5, #0
	bne.n	.L_020005b2
	ldr	r5, [pc, #168]	@ (0x20006ac)
	movs	r1, #147	@ 0x93
	lsls	r1, r1, #1
	movs	r2, #128	@ 0x80
	adds	r1, #255	@ 0xff
	lsls	r2, r2, #2
	adds	r3, r5, r1
	adds	r2, #38	@ 0x26
	ldrb	r0, [r3, #0]
	adds	r3, r5, r2
	ldrb	r1, [r3, #0]
	bl	sub_0200292a
	movs	r2, #192	@ 0xc0
	adds	r1, r6, #0
	lsls	r2, r2, #6
	ldr	r0, [pc, #192]	@ (0x20006e4)
	mov	lr, r7
	.2byte 0xf800
	.2byte 0x22c0
	ldr	r1, [pc, #180]	@ (0x20006e0)
	lsls	r2, r2, #6
	adds	r0, r6, #0
	mov	lr, r7
	.2byte 0xf800
	.2byte 0x482b
	bl	sub_020029d8
	movs	r3, #196	@ 0xc4
	lsls	r3, r3, #1
	adds	r3, #255	@ 0xff
	adds	r5, r5, r3
	movs	r3, #1
	strb	r3, [r5, #0]
	b.n	.L_0200069c
.L_02000648:
	ldr	r0, [pc, #168]	@ (0x20006f4)
	movs	r1, #13
	bl	sub_020028b6
	ldr	r5, [pc, #156]	@ (0x20006f0)
	movs	r0, #1
	adds	r1, r5, #0
	movs	r2, #8
	movs	r3, #0
	bl	sub_02002974
	adds	r6, r0, #0
	bl	sub_02002902
	cmp	r6, #0
	bne.n	.L_0200067e
	movs	r0, #46	@ 0x2e
	bl	sub_020028b4
	ldr	r3, [pc, #60]	@ (0x20006ac)
	movs	r1, #196	@ 0xc4
	lsls	r1, r1, #1
	adds	r1, #255	@ 0xff
	adds	r3, r3, r1
	movs	r2, #2
	strb	r2, [r3, #0]
	b.n	.L_0200069c
.L_0200067e:
	ldr	r0, [pc, #120]	@ (0x20006f8)
	movs	r1, #13
	bl	sub_020028ec
	movs	r0, #1
	adds	r1, r5, #0
	movs	r2, #8
	movs	r3, #0
	bl	sub_020029a8
	adds	r6, r0, #0
	bl	sub_02002936
	cmp	r6, #0
	bne.n	.L_020005b2
.L_0200069c:
	ldr	r0, [pc, #20]	@ (0x20006b4)
	movs	r1, #8
	bl	sub_02002a5a
	b.n	.L_020009fe
	.2byte 0x0000
	.4byte 0x020081f5
	.4byte 0x02000240
	.4byte 0x00000008
	.4byte 0x00000000
	.4byte 0x03001180
	.4byte 0x03001238
	.4byte 0x03001200
	.4byte 0x0000000b
	.4byte 0x0200815d
	.4byte 0x0200a4f6
	.4byte 0x0200a4f8
	.4byte 0x00000024
	.4byte 0x02000000
	.4byte 0x03000730
	.4byte 0x02010000
	.4byte 0x02013000
	.4byte 0x02003860
	.4byte 0x00000025
	.4byte 0x00000029
	.4byte 0x00000027
	.2byte 0x0028
	.2byte 0x0000
.L_020006fc:
	cmp	r5, #1
	beq.n	.L_02000702
	b.n	.L_02000850
.L_02000702:
	movs	r0, #1
	bl	sub_020029de
	movs	r2, #1
	adds	r5, r0, #0
	negs	r2, r2
	cmp	r5, r2
	bne.n	.L_02000714
	b.n	.L_020003f4
.L_02000714:
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02002962
	ldr	r5, [pc, #792]	@ (0x2000a38)
	movs	r1, #147	@ 0x93
	lsls	r1, r1, #1
	movs	r2, #128	@ 0x80
	adds	r1, #255	@ 0xff
	lsls	r2, r2, #2
	adds	r3, r5, r1
	adds	r2, #38	@ 0x26
	ldrb	r0, [r3, #0]
	adds	r3, r5, r2
	ldrb	r1, [r3, #0]
	bl	sub_02002a44
	bl	sub_02002ac0
	ldr	r3, [r5, #0]
	cmp	r3, r0
	beq.n	.L_02000772
	movs	r1, #242	@ 0xf2
	lsls	r1, r1, #1
	adds	r3, r5, r1
	ldrh	r0, [r3, #0]
	movs	r2, #240	@ 0xf0
	lsls	r2, r2, #1
	adds	r3, r5, r2
	strh	r0, [r3, #0]
	adds	r1, #2
	adds	r3, r5, r1
	ldrh	r1, [r3, #0]
	adds	r2, #2
	adds	r3, r5, r2
	strh	r1, [r3, #0]
	lsls	r0, r0, #16
	lsls	r1, r1, #16
	asrs	r0, r0, #16
	asrs	r1, r1, #16
	bl	sub_02002b46
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_020029be
	b.n	.L_02000846
.L_02000772:
	ldr	r3, [pc, #712]	@ (0x2000a3c)
	movs	r2, #130	@ 0x82
	ldr	r3, [r3, #0]
	lsls	r2, r2, #2
	ands	r3, r2
	cmp	r3, r2
	bne.n	.L_020007c6
	bl	sub_02000a62
	cmp	r0, #0
	beq.n	.L_0200078c
	ldr	r0, [pc, #692]	@ (0x2000a40)
	b.n	.L_02000806
.L_0200078c:
	movs	r1, #242	@ 0xf2
	lsls	r1, r1, #1
	adds	r3, r5, r1
	ldrh	r0, [r3, #0]
	movs	r2, #240	@ 0xf0
	lsls	r2, r2, #1
	adds	r3, r5, r2
	strh	r0, [r3, #0]
	adds	r1, #2
	adds	r3, r5, r1
	ldrh	r1, [r3, #0]
	adds	r2, #2
	adds	r3, r5, r2
	strh	r1, [r3, #0]
	lsls	r0, r0, #16
	lsls	r1, r1, #16
	asrs	r0, r0, #16
	asrs	r1, r1, #16
	bl	sub_02002b92
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02002a0a
	movs	r0, #159	@ 0x9f
	lsls	r0, r0, #1
	bl	sub_02002a0a
	b.n	.L_02000846
.L_020007c6:
	ldr	r3, [pc, #636]	@ (0x2000a44)
	ldr	r2, [r5, #4]
	ldr	r3, [r3, #0]
	cmp	r2, r3
	beq.n	.L_02000846
	ldr	r6, [pc, #628]	@ (0x2000a48)
	movs	r1, #9
	adds	r0, r6, #0
	bl	sub_02002a40
	adds	r0, r6, #1
	movs	r1, #13
	bl	sub_02002a48
	movs	r0, #1
	movs	r1, #0
	movs	r2, #0
	movs	r3, #0
	bl	sub_02002b04
	cmp	r0, #0
	beq.n	.L_020007f8
	bl	sub_02002a94
	b.n	.L_020003f4
.L_020007f8:
	bl	sub_02002a9a
	bl	.L_02000ade
	cmp	r0, #0
	beq.n	.L_0200080e
	adds	r0, r6, #2
.L_02000806:
	movs	r1, #9
	bl	sub_02002a72
	b.n	.L_020003f4
.L_0200080e:
	movs	r1, #242	@ 0xf2
	lsls	r1, r1, #1
	adds	r3, r5, r1
	ldrh	r0, [r3, #0]
	movs	r2, #240	@ 0xf0
	lsls	r2, r2, #1
	adds	r3, r5, r2
	strh	r0, [r3, #0]
	adds	r1, #2
	adds	r3, r5, r1
	ldrh	r1, [r3, #0]
	adds	r2, #2
	adds	r3, r5, r2
	strh	r1, [r3, #0]
	lsls	r0, r0, #16
	lsls	r1, r1, #16
	asrs	r0, r0, #16
	asrs	r1, r1, #16
	bl	sub_02002c14
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02002a8c
	movs	r0, #64	@ 0x40
	adds	r0, #255	@ 0xff
	bl	sub_02002a8c
.L_02000846:
	movs	r0, #131	@ 0x83
	lsls	r0, r0, #1
	bl	sub_02002a9c
	b.n	.L_020009fe
.L_02000850:
	cmp	r5, #2
	bne.n	.L_0200085a
	bl	sub_02002b36
	b.n	.L_020003f4
.L_0200085a:
	cmp	r5, #3
	bne.n	.L_02000864
	bl	sub_02002b48
	b.n	.L_020003f4
.L_02000864:
	cmp	r5, #4
	bne.n	.L_020008dc
	movs	r0, #4
	bl	sub_02002b44
	movs	r3, #1
	adds	r5, r0, #0
	negs	r3, r3
	cmp	r5, r3
	bne.n	.L_0200087a
	b.n	.L_020003f4
.L_0200087a:
	ldr	r5, [pc, #444]	@ (0x2000a38)
	movs	r1, #133	@ 0x85
	lsls	r1, r1, #2
	movs	r0, #144	@ 0x90
	adds	r3, r5, r1
	movs	r2, #4
	lsls	r0, r0, #4
	str	r2, [r3, #0]
	adds	r0, #255	@ 0xff
	bl	sub_02002ace
	cmp	r0, #0
	beq.n	.L_02000898
	bl	sub_02002c26
.L_02000898:
	movs	r2, #147	@ 0x93
	lsls	r2, r2, #1
	movs	r1, #128	@ 0x80
	adds	r2, #255	@ 0xff
	lsls	r1, r1, #2
	adds	r3, r5, r2
	adds	r1, #38	@ 0x26
	ldrb	r0, [r3, #0]
	adds	r3, r5, r1
	ldrb	r1, [r3, #0]
	bl	sub_02002bbe
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02002b06
	movs	r0, #131	@ 0x83
	lsls	r0, r0, #1
	bl	sub_02002b0e
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #2
	adds	r3, #118	@ 0x76
	adds	r2, r5, r3
	movs	r3, #0
	strh	r3, [r2, #0]
	ldr	r2, [pc, #380]	@ (0x2000a4c)
	movs	r3, #1
	strb	r3, [r2, #0]
	ldr	r0, [pc, #380]	@ (0x2000a50)
	movs	r1, #1
	bl	sub_02002c90
	b.n	.L_020009fe
.L_020008dc:
	cmp	r5, #5
	beq.n	.L_020008e2
	b.n	.L_020003f4
.L_020008e2:
	movs	r0, #6
	bl	sub_02002bbe
	movs	r1, #1
	adds	r5, r0, #0
	negs	r1, r1
	cmp	r5, r1
	bne.n	.L_020008f4
	b.n	.L_020003f4
.L_020008f4:
	ldr	r3, [pc, #320]	@ (0x2000a38)
	movs	r1, #147	@ 0x93
	lsls	r1, r1, #1
	adds	r1, #255	@ 0xff
	adds	r2, r3, r1
	ldrb	r0, [r2, #0]
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #2
	adds	r2, #38	@ 0x26
	adds	r3, r3, r2
	ldrb	r1, [r3, #0]
	bl	sub_02002c1c
.L_0200090e:
	movs	r0, #2
	bl	sub_02002bc2
	movs	r3, #1
	negs	r3, r3
	adds	r5, r0, #0
	mov	r8, r3
	cmp	r5, r8
	beq.n	.L_020008e2
	cmp	r5, #1
	bne.n	.L_0200096a
	ldr	r5, [pc, #300]	@ (0x2000a54)
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r6, [pc, #296]	@ (0x2000a58)
	ldr	r0, [pc, #300]	@ (0x2000a5c)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0xf000
	.2byte 0xfd91
	.2byte 0x1c07
	cmp	r7, r8
	beq.n	.L_02000942
	bl	sub_02002d28
.L_02000942:
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r0, [pc, #276]	@ (0x2000a60)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x22c0
	adds	r0, r5, #0
	ldr	r1, [pc, #264]	@ (0x2000a5c)
	lsls	r2, r2, #6
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x4547
	beq.n	.L_0200090e
	ldr	r0, [pc, #256]	@ (0x2000a60)
	movs	r1, #0
	movs	r2, #1
	bl	sub_02002cd6
	b.n	.L_020009ae
.L_0200096a:
	cmp	r5, #0
	bne.n	.L_020009ae
	ldr	r5, [pc, #228]	@ (0x2000a54)
	movs	r2, #192	@ 0xc0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r6, [pc, #224]	@ (0x2000a58)
	ldr	r0, [pc, #224]	@ (0x2000a5c)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0xf002
	.2byte 0xf9a7
	.2byte 0xf000
	.2byte 0xff21
	.2byte 0x22c0
	adds	r7, r0, #0
	adds	r1, r5, #0
	lsls	r2, r2, #6
	ldr	r0, [pc, #208]	@ (0x2000a60)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x22c0
	adds	r0, r5, #0
	ldr	r1, [pc, #192]	@ (0x2000a5c)
	lsls	r2, r2, #6
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x4547
	beq.n	.L_0200090e
	ldr	r0, [pc, #184]	@ (0x2000a60)
	movs	r1, #1
	movs	r2, #1
	bl	sub_02002d1c
.L_020009ae:
	ldr	r3, [pc, #180]	@ (0x2000a64)
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	cmp	r3, #0
	bne.n	.L_020009c2
	ldr	r3, [pc, #172]	@ (0x2000a68)
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #0
	beq.n	.L_020009f6
.L_020009c2:
	movs	r6, #0
	movs	r7, #1
.L_020009c6:
	movs	r0, #6
	bl	sub_02002b9a
	adds	r0, r6, #0
	bl	sub_02002cc8
	movs	r3, #1
	adds	r5, r0, #0
	negs	r3, r3
	cmp	r5, r3
	bne.n	.L_020009e4
	cmp	r6, #0
	beq.n	.L_020008e2
	subs	r6, #1
	b.n	.L_020009c6
.L_020009e4:
	ldr	r3, [pc, #128]	@ (0x2000a68)
	movs	r1, #0
	ldrsh	r3, [r3, r1]
	cmp	r3, #0
	beq.n	.L_020009f0
	movs	r7, #4
.L_020009f0:
	adds	r6, #1
	cmp	r6, r7
	blt.n	.L_020009c6
.L_020009f6:
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02002c44
.L_020009fe:
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #108]	@ 0x6c
	movs	r2, #172	@ 0xac
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r2, #186	@ 0xba
	lsls	r2, r2, #2
	adds	r2, #255	@ 0xff
	strh	r2, [r3, #0]
	movs	r0, #30
	bl	sub_02002dbe
	movs	r0, #78	@ 0x4e
	bl	sub_02002e24
	bl	sub_02002de8
	bl	sub_02002df4
	movs	r0, #60	@ 0x3c
	bl	sub_02002dd2
.L_02000a2c:
	movs	r0, #0
	pop	{r3, r5}
	mov	r8, r3
	mov	sl, r5
	pop	{r5, r6, r7, pc}
	.2byte 0x0000
	.4byte 0x02000240
	.4byte 0x03001150
	.4byte 0x00000007
	.4byte 0x02001000
	.4byte 0x00000005
	.4byte 0x03001180
	.4byte 0x00000138
	.4byte 0x02000000
	.4byte 0x03000730
	.4byte 0x02010000
	.4byte 0x02013000
	.4byte 0x0200a4f6
	.4byte 0x0200a4f8
	.2byte 0x2000
	.2byte 0x4770
	push	{lr}
	ldr	r3, [pc, #36]	@ (0x2000a98)
	strh	r3, [r1, #2]
	strh	r3, [r1, #4]
	cmp	r0, #7
	bgt.n	.L_02000a82
	adds	r3, r0, #0
	adds	r3, #65	@ 0x41
	b.n	.L_02000b1a
.L_02000a82:
	cmp	r0, #12
	bgt.n	.L_02000a8c
	adds	r3, r0, #0
	adds	r3, #66	@ 0x42
	b.n	.L_02000b1a
.L_02000a8c:
	cmp	r0, #23
	bgt.n	.L_02000a9c
	adds	r3, r0, #0
	adds	r3, #67	@ 0x43
	b.n	.L_02000b1a
	.2byte 0x0000
	.2byte 0x0000
	.2byte 0x0000
.L_02000a9c:
	cmp	r0, #31
	bgt.n	.L_02000aa6
	adds	r3, r0, #0
	adds	r3, #26
	b.n	.L_02000b1a
.L_02000aa6:
	cmp	r0, #42	@ 0x2a
	bgt.n	.L_02000ab0
	adds	r3, r0, #0
	adds	r3, #65	@ 0x41
	b.n	.L_02000b1a
.L_02000ab0:
	cmp	r0, #44	@ 0x2c
	bgt.n	.L_02000aba
	adds	r3, r0, #0
	adds	r3, #66	@ 0x42
	b.n	.L_02000b1a
.L_02000aba:
	cmp	r0, #55	@ 0x37
	bgt.n	.L_02000ac4
	adds	r3, r0, #0
	adds	r3, #67	@ 0x43
	b.n	.L_02000b1a
.L_02000ac4:
	cmp	r0, #56	@ 0x38
	bne.n	.L_02000acc
	ldr	r3, [pc, #24]	@ (0x2000ae4)
	b.n	.L_02000b1a
.L_02000acc:
	cmp	r0, #57	@ 0x39
	bne.n	.L_02000ad4
	ldr	r3, [pc, #20]	@ (0x2000ae8)
	b.n	.L_02000b1a
.L_02000ad4:
	cmp	r0, #58	@ 0x3a
	bne.n	.L_02000adc
	ldr	r3, [pc, #16]	@ (0x2000aec)
	b.n	.L_02000b1a
.L_02000adc:
	cmp	r0, #59	@ 0x3b
.L_02000ade:
	bne.n	.L_02000af4
	ldr	r3, [pc, #12]	@ (0x2000af0)
	b.n	.L_02000b1a
	.4byte 0x00000021
	.4byte 0x0000003f
	.4byte 0x00000023
	.2byte 0x0026
	.2byte 0x0000
.L_02000af4:
	cmp	r0, #60	@ 0x3c
	bne.n	.L_02000afc
	ldr	r3, [pc, #16]	@ (0x2000b0c)
	b.n	.L_02000b1a
.L_02000afc:
	cmp	r0, #61	@ 0x3d
	bne.n	.L_02000b04
	ldr	r3, [pc, #12]	@ (0x2000b10)
	b.n	.L_02000b1a
.L_02000b04:
	cmp	r0, #62	@ 0x3e
	bne.n	.L_02000b18
	ldr	r3, [pc, #8]	@ (0x2000b14)
	b.n	.L_02000b1a
	.4byte 0x00000024
	.4byte 0x00000025
	.2byte 0x002b
	.2byte 0x0000
.L_02000b18:
	ldr	r3, [pc, #4]	@ (0x2000b20)
.L_02000b1a:
	strh	r3, [r1, #0]
	pop	{pc}
	.2byte 0x0000
	.2byte 0x003d
	.2byte 0x0000
	push	{r5, r6, r7, lr}
	mov	r7, sl
	mov	r6, r9
	mov	r5, r8
	push	{r5, r6, r7}
	sub	sp, #4
	movs	r3, #2
	str	r3, [sp, #0]
	movs	r1, #5
	movs	r2, #18
	movs	r3, #8
	movs	r0, #6
	bl	sub_02002d96
	ldr	r5, [pc, #172]	@ (0x2000bf0)
	adds	r6, r0, #0
	adds	r1, r6, #0
	adds	r0, r5, #0
	movs	r2, #0
	movs	r3, #4
	bl	sub_02002dce
	adds	r0, r5, #1
	adds	r1, r6, #0
	movs	r2, #0
	movs	r3, #16
	adds	r5, #3
	bl	sub_02002ddc
	adds	r0, r5, #0
	adds	r1, r6, #0
	movs	r2, #0
	movs	r3, #36	@ 0x24
	bl	sub_02002de8
	bl	sub_02002d8c
	movs	r0, #10
	bl	sub_02002d42
.L_02000b74:
	movs	r5, #3
	b.n	.L_02000bcc
.L_02000b78:
	ldr	r3, [pc, #120]	@ (0x2000bf4)
	ldrh	r2, [r3, #0]
	adds	r3, r5, #0
	ands	r3, r2
	cmp	r3, r5
	bne.n	.L_02000bc6
	ldr	r3, [pc, #112]	@ (0x2000bf8)
	movs	r2, #1
	ldr	r3, [r3, #0]
	lsls	r3, r3, #26
	lsrs	r3, r3, #30
	eors	r3, r2
	lsls	r2, r3, #1
	adds	r2, r2, r3
	ldr	r3, [pc, #100]	@ (0x2000bfc)
	lsls	r2, r2, #3
	adds	r2, r2, r3
	ldrh	r3, [r2, #0]
	cmp	r3, #48	@ 0x30
	bne.n	.L_02000bc6
	ldrh	r3, [r2, #2]
	cmp	r3, #48	@ 0x30
	bne.n	.L_02000bc6
	ldrh	r3, [r2, #4]
	cmp	r3, #48	@ 0x30
	bne.n	.L_02000bc6
	ldrh	r3, [r2, #6]
	cmp	r3, #48	@ 0x30
	bne.n	.L_02000bc6
	ldr	r2, [pc, #76]	@ (0x2000c00)
	ldr	r3, [pc, #40]	@ (0x2000be0)
	strh	r3, [r2, #0]
	ldr	r3, [pc, #40]	@ (0x2000be4)
	strh	r3, [r2, #2]
	ldr	r3, [pc, #40]	@ (0x2000be8)
	strh	r3, [r2, #4]
	ldr	r3, [pc, #40]	@ (0x2000bec)
	strh	r3, [r2, #6]
	b.n	.L_02000c08
.L_02000bc6:
	movs	r0, #1
	bl	sub_02002d9a
.L_02000bcc:
	ldr	r3, [pc, #52]	@ (0x2000c04)
	movs	r2, #2
	ldr	r3, [r3, #4]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02000b78
	movs	r0, #113	@ 0x71
	bl	sub_02002fe4
	b.n	.L_02000ce8
	.4byte 0x00000055
	.4byte 0x00000056
	.4byte 0x00000054
	.4byte 0x00000053
	.4byte 0x00001184
	.4byte 0x0300124c
	.4byte 0x04000128
	.4byte 0x02003874
	.4byte 0x02003a74
	.2byte 0x1150
	.2byte 0x0300
.L_02000c08:
	movs	r0, #10
	bl	sub_02002ddc
	ldr	r0, [pc, #364]	@ (0x2000d7c)
	bl	sub_02002e42
	movs	r0, #10
	bl	sub_02002de8
	movs	r1, #3
	movs	r2, #1
	movs	r3, #0
	mov	sl, r1
	movs	r7, #0
	mov	r9, r2
	mov	r8, r3
	movs	r5, #0
.L_02000c2a:
	mov	r1, r8
	cmp	r1, #0
	bne.n	.L_02000c62
	ldr	r3, [pc, #332]	@ (0x2000d80)
	ldrh	r3, [r3, #0]
	cmp	r3, #0
	beq.n	.L_02000c62
	movs	r2, #1
	adds	r0, r6, #0
	movs	r1, #2
	mov	r8, r2
	bl	sub_02002ea2
	movs	r3, #2
	str	r3, [sp, #0]
	movs	r1, #10
	movs	r2, #20
	movs	r3, #4
	movs	r0, #5
	bl	sub_02002eaa
	adds	r6, r0, #0
	adds	r1, r6, #0
	ldr	r0, [pc, #296]	@ (0x2000d84)
	movs	r2, #0
	movs	r3, #4
	bl	sub_02002ee0
.L_02000c62:
	cmp	r5, #9
	bne.n	.L_02000c8e
	ldr	r3, [pc, #280]	@ (0x2000d80)
	ldrh	r3, [r3, #0]
	cmp	r3, #0
	bne.n	.L_02000c8e
	ldr	r2, [pc, #280]	@ (0x2000d88)
	strh	r3, [r2, #0]
	strh	r3, [r2, #2]
	strh	r3, [r2, #4]
	strh	r3, [r2, #6]
	bl	sub_02002ea2
	movs	r0, #10
	bl	sub_02002e50
	bl	sub_02002ea4
	movs	r0, #10
	bl	sub_02002e5a
	b.n	.L_02000b74
.L_02000c8e:
	ldr	r3, [pc, #252]	@ (0x2000d8c)
	adds	r7, #1
	ldrh	r2, [r3, #0]
	mov	r3, sl
	ands	r3, r2
	cmp	r3, sl
	bne.n	.L_02000c9e
	movs	r7, #0
.L_02000c9e:
	cmp	r7, #10
	bne.n	.L_02000ca8
	movs	r3, #0
	mov	r9, r3
	b.n	.L_02000cbe
.L_02000ca8:
	ldr	r3, [pc, #228]	@ (0x2000d90)
	ldr	r3, [r3, #0]
	cmp	r3, #0
	beq.n	.L_02000cbe
	movs	r0, #1
	bl	sub_02002e84
	ldr	r1, [pc, #220]	@ (0x2000d94)
	adds	r5, #1
	cmp	r5, r1
	ble.n	.L_02000c2a
.L_02000cbe:
	mov	r2, r9
	cmp	r2, #0
	bne.n	.L_02000cf6
	adds	r0, r6, #0
	bl	sub_02002f38
	ldr	r0, [pc, #204]	@ (0x2000d98)
	adds	r1, r6, #0
	movs	r2, #0
	movs	r3, #4
	bl	sub_02002f54
.L_02000cd6:
	movs	r0, #1
	bl	sub_02002eaa
	ldr	r3, [pc, #188]	@ (0x2000d9c)
	movs	r2, #1
	ldr	r3, [r3, #4]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02000cd6
.L_02000ce8:
	adds	r0, r6, #0
	movs	r1, #2
	bl	sub_02002f4e
	movs	r0, #1
	negs	r0, r0
	b.n	.L_02000d6e
.L_02000cf6:
	movs	r0, #10
	bl	sub_02002eca
	bl	sub_02002f26
	movs	r0, #10
	bl	sub_02002ed4
	adds	r0, r6, #0
	movs	r1, #2
	bl	sub_02002f6c
	ldr	r5, [pc, #144]	@ (0x2000da0)
	movs	r2, #133	@ 0x85
	ldr	r6, [pc, #144]	@ (0x2000da4)
	ldr	r1, [pc, #144]	@ (0x2000da8)
	lsls	r2, r2, #2
	adds	r0, r5, #0
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x2388
	lsls	r3, r3, #2
	movs	r2, #210	@ 0xd2
	adds	r0, r5, r3
	ldr	r1, [pc, #132]	@ (0x2000dac)
	lsls	r2, r2, #4
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x2000
	bl	sub_02003072
	movs	r0, #1
	bl	sub_02003078
	movs	r0, #2
	bl	sub_0200307e
	movs	r0, #3
	bl	sub_02003084
	movs	r2, #244	@ 0xf4
	adds	r1, r5, #0
	lsls	r2, r2, #4
	ldr	r0, [pc, #88]	@ (0x2000da8)
	mov	lr, r6
	.2byte 0xf800
	.2byte 0x4b17
	movs	r1, #147	@ 0x93
	lsls	r1, r1, #1
	adds	r1, #255	@ 0xff
	adds	r2, r3, r1
	ldrb	r0, [r2, #0]
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #2
	adds	r2, #38	@ 0x26
	adds	r3, r3, r2
	ldrb	r1, [r3, #0]
	bl	sub_0200307a
	movs	r0, #1
.L_02000d6e:
	add	sp, #4
	pop	{r3, r5, r6}
	mov	r8, r3
	mov	r9, r5
	mov	sl, r6
	pop	{r5, r6, r7, pc}
	.2byte 0x0000
	.4byte 0x02013000
	.4byte 0x02005354
	.4byte 0x00001186
	.4byte 0x02003a74
	.4byte 0x0300124c
	.4byte 0x020055d0
	.4byte 0x000927bf
	.4byte 0x00001188
	.4byte 0x03001150
	.4byte 0x02000040
	.4byte 0x03000730
	.4byte 0x02013040
	.4byte 0x02013240
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, r6, r7, lr}
	adds	r7, r3, #0
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #60]	@ 0x3c
	adds	r5, r1, #0
	mov	ip, r3
	movs	r4, #12
	ldrsh	r3, [r0, r4]
	ldr	r1, [sp, #16]
	adds	r3, r5, r3
	adds	r5, r3, #1
	movs	r4, #14
	ldrsh	r3, [r0, r4]
	adds	r3, r2, r3
	adds	r2, r3, #1
	ldr	r3, [sp, #20]
	lsls	r3, r3, #12
	str	r3, [sp, #20]
	cmp	r5, #0
	bge.n	.L_02000de2
	adds	r7, r7, r5
	movs	r5, #0
.L_02000de2:
	adds	r3, r5, r7
	cmp	r3, #29
	ble.n	.L_02000dec
	movs	r3, #30
	subs	r7, r3, r5
.L_02000dec:
	cmp	r2, #0
	bge.n	.L_02000df4
	adds	r1, r1, r2
	movs	r2, #0
.L_02000df4:
	adds	r3, r2, r1
	cmp	r3, #29
	ble.n	.L_02000dfe
	movs	r3, #20
	subs	r1, r3, r2
.L_02000dfe:
	cmp	r7, #0
	ble.n	.L_02000e3c
	cmp	r1, #0
	ble.n	.L_02000e3c
	lsls	r3, r2, #6
	mov	r4, ip
	adds	r6, r3, r4
.L_02000e0c:
	lsls	r3, r5, #1
	adds	r3, r6, r3
	adds	r4, r3, #0
	adds	r0, r7, #0
	adds	r4, #8
	cmp	r0, #0
	beq.n	.L_02000e2e
.L_02000e1a:
	ldrh	r3, [r4, #0]
	ldr	r2, [pc, #32]	@ (0x2000e40)
	subs	r0, #1
	ands	r3, r2
	ldr	r2, [sp, #20]
	orrs	r3, r2
	strh	r3, [r4, #0]
	adds	r4, #2
	cmp	r0, #0
	bne.n	.L_02000e1a
.L_02000e2e:
	subs	r1, #1
	adds	r6, #64	@ 0x40
	cmp	r1, #0
	bne.n	.L_02000e0c
	movs	r3, #1
	mov	r4, ip
	strb	r3, [r4, #3]
.L_02000e3c:
	pop	{r5, r6, r7, pc}
	.2byte 0x0000
	.2byte 0xefff
	.2byte 0xffff
	push	{r5, r6, r7, lr}
	mov	r7, fp
	mov	r6, sl
	mov	r5, r9
	push	{r5, r6, r7}
	mov	r7, r8
	push	{r7}
	sub	sp, #124	@ 0x7c
	bl	sub_0200305e
	ldr	r3, [pc, #112]	@ (0x2000ecc)
	movs	r0, #147	@ 0x93
	movs	r1, #128	@ 0x80
	lsls	r0, r0, #1
	lsls	r1, r1, #2
	adds	r0, #255	@ 0xff
	adds	r1, #38	@ 0x26
	adds	r2, r3, r0
	adds	r3, r3, r1
	ldrb	r0, [r2, #0]
	ldrb	r1, [r3, #0]
	bl	sub_02003180
	ldr	r4, [pc, #92]	@ (0x2000ed0)
	movs	r3, #128	@ 0x80
	movs	r2, #132	@ 0x84
	lsls	r3, r3, #19
	lsls	r2, r2, #24
	adds	r3, #212	@ 0xd4
	ldr	r0, [pc, #84]	@ (0x2000ed4)
	adds	r1, r4, #0
	adds	r2, #8
	stmia	r3!, {r0, r1, r2}
	subs	r3, #12
	ldr	r3, [pc, #60]	@ (0x2000ec8)
	movs	r0, #1
	strh	r3, [r4, #8]
	bl	sub_02003140
	movs	r2, #1
	negs	r2, r2
	str	r0, [sp, #64]	@ 0x40
	cmp	r0, r2
	bne.n	.L_02000ea0
	bl	sub_0200268e
.L_02000ea0:
	movs	r0, #71	@ 0x47
	bl	sub_020032ac
	movs	r0, #238	@ 0xee
	movs	r3, #0
	lsls	r0, r0, #1
	str	r3, [sp, #60]	@ 0x3c
	str	r3, [sp, #52]	@ 0x34
	str	r3, [sp, #48]	@ 0x30
	bl	sub_0200309c
	movs	r4, #0
	str	r0, [sp, #36]	@ 0x24
	ldr	r0, [pc, #28]	@ (0x2000ed8)
	str	r4, [sp, #8]
	bl	sub_020031e0
	ldr	r1, [sp, #64]	@ 0x40
	b.n	.L_02000edc
	.2byte 0x0000
	.4byte 0x00006318
	.4byte 0x02000240
	.4byte 0x050001c0
	.4byte 0x050001e0
	.2byte 0x6000
	.2byte 0x0600
.L_02000edc:
	movs	r0, #130	@ 0x82
	lsls	r0, r0, #1
	str	r0, [sp, #44]	@ 0x2c
	cmp	r1, #0
	beq.n	.L_02000ef4
	ldr	r3, [sp, #64]	@ 0x40
	movs	r2, #61	@ 0x3d
	str	r2, [sp, #44]	@ 0x2c
	cmp	r3, #1
	beq.n	.L_02000ef4
	movs	r4, #16
	str	r4, [sp, #44]	@ 0x2c
.L_02000ef4:
	ldr	r0, [sp, #44]	@ 0x2c
	movs	r1, #50	@ 0x32
	bl	sub_020030ba
	adds	r0, #1
	str	r0, [sp, #40]	@ 0x28
	movs	r5, #2
	movs	r1, #13
	movs	r2, #29
	movs	r3, #6
	movs	r0, #1
	str	r5, [sp, #0]
	bl	sub_02003166
	mov	r9, r0
	mov	r1, r9
	ldr	r0, [pc, #584]	@ (0x2001160)
	movs	r2, #0
	movs	r3, #0
	bl	sub_020031ac
	ldr	r0, [pc, #580]	@ (0x2001164)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #8
	bl	sub_020031b8
	ldr	r0, [pc, #572]	@ (0x2001168)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #16
	bl	sub_020031c4
	ldr	r0, [pc, #564]	@ (0x200116c)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #24
	bl	sub_020031d0
	ldr	r6, [pc, #556]	@ (0x2001170)
	movs	r3, #5
	str	r3, [sp, #0]
	mov	r0, r9
	movs	r1, #20
	movs	r2, #0
	movs	r3, #20
	bl	sub_020031ca
	adds	r0, r6, #0
	mov	r1, r9
	adds	r6, #1
	movs	r2, #160	@ 0xa0
	movs	r3, #0
	bl	sub_020031e8
	adds	r0, r6, #0
	mov	r1, r9
	adds	r6, #1
	movs	r2, #160	@ 0xa0
	movs	r3, #8
	bl	sub_020031f6
	adds	r0, r6, #0
	mov	r1, r9
	movs	r2, #160	@ 0xa0
	movs	r3, #24
	bl	sub_02003202
	movs	r0, #5
	movs	r1, #1
	movs	r2, #20
	movs	r3, #12
	str	r5, [sp, #0]
	bl	sub_020031e0
	str	r0, [sp, #56]	@ 0x38
	movs	r6, #0
.L_02000f8e:
	ldr	r2, [sp, #36]	@ 0x24
	movs	r0, #224	@ 0xe0
	lsls	r0, r0, #1
	adds	r1, r6, r0
	adds	r2, #2
	movs	r3, #0
	adds	r6, #1
	strb	r3, [r2, r1]
	cmp	r6, #26
	bne.n	.L_02000f8e
	ldr	r2, [pc, #464]	@ (0x2001174)
	movs	r6, #0
.L_02000fa6:
	movs	r1, #130	@ 0x82
	movs	r3, #99	@ 0x63
	adds	r6, #1
	lsls	r1, r1, #1
	strb	r3, [r2, #0]
	adds	r2, #1
	cmp	r6, r1
	bne.n	.L_02000fa6
	ldr	r2, [sp, #36]	@ 0x24
	movs	r3, #200	@ 0xc8
	lsls	r3, r3, #1
	movs	r6, #0
	adds	r2, r2, r3
	mov	sl, r2
	mov	fp, r6
.L_02000fc4:
	movs	r1, #10
	adds	r0, r6, #0
	bl	sub_02003192
	adds	r5, r0, #0
	lsls	r3, r5, #1
	adds	r3, r3, r5
	lsls	r7, r3, #2
	movs	r4, #8
	adds	r4, r4, r7
	adds	r0, r6, #0
	movs	r1, #10
	mov	r8, r4
	bl	sub_020031a0
	lsls	r0, r0, #4
	adds	r3, r0, #2
	cmp	r5, #4
	ble.n	.L_02000fee
	adds	r7, #18
	mov	r8, r7
.L_02000fee:
	ldr	r2, [sp, #36]	@ 0x24
	mov	r0, fp
	ldr	r1, [r0, r2]
	ldr	r0, [sp, #56]	@ 0x38
	mov	r2, r8
	bl	sub_0200332a
	ldr	r3, [sp, #36]	@ 0x24
	mov	r2, fp
	adds	r2, #200	@ 0xc8
	str	r0, [r2, r3]
	mov	r4, sl
	movs	r3, #99	@ 0x63
	strb	r3, [r4, #0]
	ldr	r1, [sp, #36]	@ 0x24
	movs	r0, #1
	add	sl, r0
	ldr	r3, [pc, #356]	@ (0x2001178)
	ldr	r0, [r2, r1]
	movs	r1, #128	@ 0x80
	mov	lr, r3
	.2byte 0xf800
	.2byte 0x3601
	movs	r2, #4
	add	fp, r2
	cmp	r6, #50	@ 0x32
	bne.n	.L_02000fc4
	bl	sub_02003226
	adds	r5, r0, #0
	cmp	r5, #95	@ 0x5f
	bgt.n	.L_0200105e
	bl	sub_02003330
	movs	r1, #128	@ 0x80
	movs	r3, #0
	lsls	r1, r1, #23
	mov	r2, r9
	adds	r0, r5, #0
	str	r3, [sp, #0]
	bl	sub_020032d8
	adds	r1, r0, #0
	add	r0, sp, #108	@ 0x6c
	str	r1, [r0, #0]
	mov	r4, r9
	movs	r3, #12
	ldrsh	r1, [r4, r3]
	movs	r3, #14
	ldrsh	r2, [r4, r3]
	lsls	r1, r1, #3
	lsls	r2, r2, #3
	subs	r1, #2
	adds	r2, #12
	bl	sub_0200345c
.L_0200105e:
	bl	sub_02003260
	adds	r5, r0, #0
	cmp	r5, #95	@ 0x5f
	bgt.n	.L_020010a6
	bl	sub_02003372
	movs	r1, #128	@ 0x80
	movs	r3, #0
	lsls	r1, r1, #23
	mov	r2, r9
	adds	r0, r5, #0
	str	r3, [sp, #0]
	bl	sub_02003312
	adds	r1, r0, #0
	add	r0, sp, #92	@ 0x5c
	str	r1, [r0, #0]
	movs	r3, #255	@ 0xff
	ldrb	r2, [r1, #25]
	strb	r3, [r1, #15]
	movs	r3, #13
	negs	r3, r3
	ands	r3, r2
	strb	r3, [r1, #25]
	ldr	r2, [sp, #56]	@ 0x38
	movs	r4, #12
	ldrsh	r1, [r2, r4]
	movs	r3, #14
	ldrsh	r2, [r2, r3]
	lsls	r1, r1, #3
	lsls	r2, r2, #3
	subs	r1, #4
	adds	r2, #12
	bl	sub_020034a4
.L_020010a6:
	movs	r4, #0
	movs	r0, #1
	str	r0, [sp, #24]
	str	r4, [sp, #32]
	str	r4, [sp, #28]
	mov	r2, r9
	movs	r1, #12
	ldrsh	r3, [r2, r1]
	movs	r0, #0
	lsls	r3, r3, #3
	subs	r3, #8
	str	r3, [sp, #20]
	movs	r4, #14
	ldrsh	r3, [r2, r4]
	str	r0, [sp, #12]
	lsls	r3, r3, #3
	adds	r3, #11
	str	r3, [sp, #16]
.L_020010ca:
	ldr	r2, [sp, #12]
	movs	r1, #1
	negs	r1, r1
	mov	sl, r1
	cmp	r2, #2
	bne.n	.L_020010da
	movs	r3, #1
	str	r3, [sp, #12]
.L_020010da:
	movs	r3, #15
	movs	r4, #1
	str	r3, [sp, #4]
	mov	r0, r9
	ldr	r1, [sp, #32]
	ldr	r2, [sp, #28]
	ldr	r3, [sp, #24]
	str	r4, [sp, #0]
	bl	.L_02001ea0
	ldr	r0, [sp, #12]
	cmp	r0, #0
	beq.n	.L_020010f6
	b.n	.L_020011fc
.L_020010f6:
	ldr	r3, [pc, #132]	@ (0x200117c)
	movs	r2, #16
	ldr	r3, [r3, #12]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_0200112e
	movs	r0, #111	@ 0x6f
	bl	sub_0200350e
	ldr	r0, [sp, #52]	@ 0x34
	ldr	r1, [sp, #48]	@ 0x30
	adds	r0, #1
	cmp	r1, #2
	bne.n	.L_02001126
	adds	r3, r0, #0
	cmp	r0, #0
	bge.n	.L_0200111c
	ldr	r3, [sp, #52]	@ 0x34
	adds	r3, #16
.L_0200111c:
	asrs	r3, r3, #4
	str	r3, [sp, #52]	@ 0x34
	lsls	r3, r3, #4
	subs	r0, r0, r3
	b.n	.L_0200112c
.L_02001126:
	movs	r1, #17
	bl	sub_020032f2
.L_0200112c:
	str	r0, [sp, #52]	@ 0x34
.L_0200112e:
	ldr	r3, [pc, #76]	@ (0x200117c)
	movs	r2, #32
	ldr	r3, [r3, #12]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_0200118c
	movs	r0, #111	@ 0x6f
	bl	sub_02003546
	ldr	r2, [sp, #48]	@ 0x30
	cmp	r2, #2
	bne.n	.L_02001180
	ldr	r2, [sp, #52]	@ 0x34
	adds	r2, #15
	adds	r3, r2, #0
	cmp	r2, #0
	bge.n	.L_02001154
	ldr	r3, [sp, #52]	@ 0x34
	adds	r3, #30
.L_02001154:
	asrs	r3, r3, #4
	str	r3, [sp, #52]	@ 0x34
	lsls	r3, r3, #4
	subs	r2, r2, r3
	str	r2, [sp, #52]	@ 0x34
	b.n	.L_0200118c
	.4byte 0x0200a440
	.4byte 0x0200a454
	.4byte 0x0200a468
	.4byte 0x0200a47c
	.4byte 0x00000d8d
	.4byte 0x0200a74a
	.4byte 0x03000258
	.2byte 0x1150
	.2byte 0x0300
.L_02001180:
	ldr	r0, [sp, #52]	@ 0x34
	movs	r1, #17
	adds	r0, #16
	bl	sub_02003350
	str	r0, [sp, #52]	@ 0x34
.L_0200118c:
	ldr	r3, [pc, #624]	@ (0x2001400)
	movs	r2, #128	@ 0x80
	ldr	r3, [r3, #12]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_020011c4
	movs	r0, #111	@ 0x6f
	bl	sub_020035a4
	ldr	r1, [sp, #48]	@ 0x30
	adds	r1, #1
	adds	r3, r1, #0
	cmp	r1, #0
	bge.n	.L_020011ac
	ldr	r3, [sp, #48]	@ 0x30
	adds	r3, #4
.L_020011ac:
	asrs	r3, r3, #2
	str	r3, [sp, #48]	@ 0x30
	lsls	r3, r3, #2
	subs	r1, r1, r3
	ldr	r3, [sp, #52]	@ 0x34
	str	r1, [sp, #48]	@ 0x30
	cmp	r3, #16
	bne.n	.L_020011c4
	cmp	r1, #2
	bne.n	.L_020011c4
	movs	r4, #3
	str	r4, [sp, #48]	@ 0x30
.L_020011c4:
	ldr	r3, [pc, #568]	@ (0x2001400)
	movs	r2, #64	@ 0x40
	ldr	r3, [r3, #12]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_020011fc
	movs	r0, #111	@ 0x6f
	bl	sub_020035dc
	ldr	r1, [sp, #48]	@ 0x30
	adds	r1, #3
	adds	r3, r1, #0
	cmp	r1, #0
	bge.n	.L_020011e4
	ldr	r3, [sp, #48]	@ 0x30
	adds	r3, #6
.L_020011e4:
	asrs	r3, r3, #2
	ldr	r0, [sp, #52]	@ 0x34
	str	r3, [sp, #48]	@ 0x30
	lsls	r3, r3, #2
	subs	r1, r1, r3
	str	r1, [sp, #48]	@ 0x30
	cmp	r0, #16
	bne.n	.L_02001206
	cmp	r1, #2
	bne.n	.L_020011fc
	movs	r1, #1
	str	r1, [sp, #48]	@ 0x30
.L_020011fc:
	ldr	r3, [sp, #52]	@ 0x34
	movs	r2, #6
	str	r2, [sp, #24]
	cmp	r3, #16
	beq.n	.L_0200120a
.L_02001206:
	movs	r4, #1
	str	r4, [sp, #24]
.L_0200120a:
	ldr	r0, [sp, #52]	@ 0x34
	ldr	r1, [sp, #48]	@ 0x30
	str	r0, [sp, #32]
	str	r1, [sp, #28]
	cmp	r0, #4
	ble.n	.L_0200121c
	adds	r2, r0, #0
	adds	r2, #1
	str	r2, [sp, #32]
.L_0200121c:
	ldr	r3, [sp, #52]	@ 0x34
	cmp	r3, #9
	ble.n	.L_02001228
	ldr	r4, [sp, #32]
	adds	r4, #1
	str	r4, [sp, #32]
.L_02001228:
	ldr	r0, [sp, #52]	@ 0x34
	cmp	r0, #14
	ble.n	.L_02001234
	ldr	r1, [sp, #32]
	adds	r1, #1
	str	r1, [sp, #32]
.L_02001234:
	ldr	r2, [sp, #52]	@ 0x34
	cmp	r2, #15
	ble.n	.L_02001240
	ldr	r3, [sp, #32]
	adds	r3, #1
	str	r3, [sp, #32]
.L_02001240:
	movs	r3, #14
	movs	r4, #1
	mov	r0, r9
	str	r3, [sp, #4]
	ldr	r1, [sp, #32]
	ldr	r2, [sp, #48]	@ 0x30
	ldr	r3, [sp, #24]
	str	r4, [sp, #0]
	bl	.L_02002006
	mov	r1, r9
	movs	r0, #12
	ldrsh	r3, [r1, r0]
	ldr	r2, [sp, #52]	@ 0x34
	mov	r0, r9
	adds	r3, r3, r2
	lsls	r1, r3, #3
	movs	r4, #14
	ldrsh	r3, [r0, r4]
	ldr	r4, [sp, #48]	@ 0x30
	adds	r2, r1, #0
	adds	r3, r3, r4
	lsls	r3, r3, #3
	adds	r0, r3, #0
	ldr	r3, [sp, #52]	@ 0x34
	subs	r2, #8
	adds	r0, #11
	cmp	r3, #4
	ble.n	.L_0200127c
	adds	r2, r1, #0
.L_0200127c:
	ldr	r4, [sp, #52]	@ 0x34
	cmp	r4, #9
	ble.n	.L_02001284
	adds	r2, #8
.L_02001284:
	ldr	r1, [sp, #52]	@ 0x34
	cmp	r1, #14
	ble.n	.L_0200128c
	adds	r2, #8
.L_0200128c:
	ldr	r3, [sp, #52]	@ 0x34
	cmp	r3, #15
	ble.n	.L_02001294
	adds	r2, #8
.L_02001294:
	ldr	r4, [sp, #20]
	ldr	r1, [sp, #16]
	subs	r3, r2, r4
	lsrs	r2, r3, #31
	adds	r3, r3, r2
	asrs	r3, r3, #1
	adds	r4, r4, r3
	subs	r3, r0, r1
	lsrs	r2, r3, #31
	adds	r3, r3, r2
	asrs	r3, r3, #1
	adds	r1, r1, r3
	str	r1, [sp, #16]
	ldr	r2, [sp, #16]
	add	r0, sp, #108	@ 0x6c
	adds	r1, r4, #0
	movs	r3, #1
	str	r4, [sp, #20]
	bl	sub_020036b2
	add	r0, sp, #108	@ 0x6c
	bl	sub_020036b0
	movs	r1, #50	@ 0x32
	ldr	r0, [sp, #60]	@ 0x3c
	bl	sub_02003490
	movs	r1, #10
	adds	r5, r0, #0
	bl	sub_02003490
	ldr	r4, [sp, #56]	@ 0x38
	lsls	r0, r0, #1
	movs	r2, #14
	ldrsh	r3, [r4, r2]
	movs	r1, #10
	adds	r0, r0, r3
	ldr	r3, [pc, #292]	@ (0x2001404)
	lsls	r0, r0, #3
	ldr	r3, [r3, #0]
	subs	r7, r0, #2
	lsrs	r6, r3, #1
	movs	r0, #7
	mov	r8, r0
	ands	r6, r0
	adds	r0, r5, #0
	bl	sub_020034ba
	ldr	r4, [sp, #56]	@ 0x38
	lsls	r2, r0, #1
	movs	r1, #12
	ldrsh	r3, [r4, r1]
	adds	r2, r2, r0
	lsls	r2, r2, #2
	lsls	r3, r3, #3
	adds	r2, r2, r3
	adds	r1, r2, #0
	adds	r1, #16
	cmp	r0, #4
	ble.n	.L_0200130e
	adds	r1, #10
.L_0200130e:
	ldr	r2, [pc, #248]	@ (0x2001408)
	mov	r0, r8
	ldrsb	r3, [r2, r6]
	adds	r1, r1, r3
	adds	r3, r6, #5
	ands	r3, r0
	ldrsb	r3, [r2, r3]
	add	r0, sp, #92	@ 0x5c
	adds	r7, r7, r3
	adds	r2, r7, #0
	movs	r3, #1
	bl	sub_0200371e
	add	r0, sp, #92	@ 0x5c
	bl	sub_0200371c
	ldr	r1, [sp, #8]
	cmp	r1, #0
	ble.n	.L_02001346
	adds	r1, #1
	str	r1, [sp, #8]
	cmp	r1, #8
	bne.n	.L_02001346
	ldr	r3, [sp, #60]	@ 0x3c
	movs	r2, #0
	adds	r3, #1
	str	r2, [sp, #8]
	str	r3, [sp, #60]	@ 0x3c
.L_02001346:
	ldr	r4, [sp, #8]
	cmp	r4, #0
	beq.n	.L_0200134e
	b.n	.L_02001692
.L_0200134e:
	ldr	r0, [sp, #52]	@ 0x34
	ldr	r3, [pc, #172]	@ (0x2001400)
	cmp	r0, #16
	bne.n	.L_0200136a
	ldr	r1, [sp, #48]	@ 0x30
	cmp	r1, #0
	bne.n	.L_02001360
	ldr	r3, [r3, #12]
	b.n	.L_0200136c
.L_02001360:
	ldr	r2, [sp, #48]	@ 0x30
	cmp	r2, #1
	bne.n	.L_0200136a
	ldr	r3, [r3, #12]
	b.n	.L_0200136c
.L_0200136a:
	ldr	r3, [r3, #4]
.L_0200136c:
	lsls	r3, r3, #16
	asrs	r2, r3, #16
	movs	r3, #128	@ 0x80
	lsls	r2, r2, #16
	lsls	r3, r3, #9
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02001418
	ldr	r3, [sp, #12]
	cmp	r3, #0
	bne.n	.L_02001418
	ldr	r0, [sp, #52]	@ 0x34
	movs	r4, #0
	mov	sl, r4
	cmp	r0, #16
	bne.n	.L_020013b0
	ldr	r1, [sp, #48]	@ 0x30
	cmp	r1, #0
	bne.n	.L_02001398
	movs	r2, #1
	mov	sl, r2
	b.n	.L_02001418
.L_02001398:
	ldr	r3, [sp, #48]	@ 0x30
	cmp	r3, #1
	bne.n	.L_020013a4
	movs	r4, #2
	mov	sl, r4
	b.n	.L_02001418
.L_020013a4:
	ldr	r0, [sp, #48]	@ 0x30
	cmp	r0, #3
	bne.n	.L_02001418
	movs	r1, #3
	mov	sl, r1
	b.n	.L_02001418
.L_020013b0:
	ldr	r2, [sp, #60]	@ 0x3c
	ldr	r3, [sp, #44]	@ 0x2c
	cmp	r2, r3
	bge.n	.L_02001418
	ldr	r4, [sp, #48]	@ 0x30
	ldr	r0, [sp, #52]	@ 0x34
	ldr	r2, [pc, #76]	@ (0x200140c)
	lsls	r3, r4, #4
	adds	r3, r0, r3
	ldrb	r5, [r2, r3]
	cmp	r5, #63	@ 0x3f
	bgt.n	.L_02001418
	movs	r0, #112	@ 0x70
	bl	sub_020037d4
	ldr	r3, [pc, #64]	@ (0x2001410)
	ldr	r1, [sp, #60]	@ 0x3c
	strb	r5, [r3, r1]
	ldr	r0, [sp, #60]	@ 0x3c
	movs	r1, #10
	bl	sub_020035a2
	ldr	r3, [sp, #44]	@ 0x2c
	ldr	r2, [sp, #60]	@ 0x3c
	subs	r3, #1
	cmp	r2, r3
	bge.n	.L_02001414
	adds	r0, r2, #0
	movs	r1, #50	@ 0x32
	bl	sub_020035b4
	cmp	r0, #49	@ 0x31
	bne.n	.L_020013f8
	movs	r3, #1
	str	r3, [sp, #8]
	b.n	.L_02001418
.L_020013f8:
	ldr	r4, [sp, #60]	@ 0x3c
	adds	r4, #1
	str	r4, [sp, #60]	@ 0x3c
	b.n	.L_02001418
	.4byte 0x03001150
	.4byte 0x03001100
	.4byte 0x0200a438
	.4byte 0x0200a65e
	.2byte 0xa74a
	.2byte 0x0200
.L_02001414:
	movs	r0, #2
	str	r0, [sp, #12]
.L_02001418:
	mov	r1, sl
	cmp	r1, #0
	bne.n	.L_02001426
	ldr	r2, [sp, #12]
	cmp	r2, #1
	beq.n	.L_02001426
	b.n	.L_02001692
.L_02001426:
	ldr	r1, [pc, #816]	@ (0x2001758)
	movs	r2, #8
	ldr	r3, [r1, #4]
	ands	r3, r2
	cmp	r3, #0
	bne.n	.L_02001440
	mov	r3, sl
	cmp	r3, #3
	beq.n	.L_02001440
	ldr	r4, [sp, #12]
	cmp	r4, #1
	beq.n	.L_02001440
	b.n	.L_02001602
.L_02001440:
	ldr	r7, [pc, #792]	@ (0x200175c)
	ldr	r1, [sp, #44]	@ 0x2c
	adds	r2, r7, #0
	ldr	r0, [pc, #792]	@ (0x2001760)
	bl	sub_020034b2
	mov	sl, r0
	mov	r6, sl
	subs	r6, #2
	adds	r1, r7, #0
	adds	r0, r6, #0
	bl	sub_020035d4
	movs	r3, #2
	adds	r5, r0, #0
	movs	r0, #0
	str	r0, [sp, #12]
	str	r3, [sp, #0]
	movs	r1, #12
	movs	r3, #3
	movs	r0, #5
	movs	r2, #20
	bl	sub_020036c6
	lsls	r5, r5, #16
	asrs	r5, r5, #16
	ldrb	r3, [r7, r6]
	lsls	r5, r5, #16
	lsrs	r1, r5, #16
	lsrs	r5, r5, #24
	mov	r8, r0
	cmp	r3, r5
	bne.n	.L_0200155a
	mov	r3, sl
	subs	r3, #1
	ldrb	r3, [r7, r3]
	movs	r2, #255	@ 0xff
	ands	r2, r1
	cmp	r3, r2
	bne.n	.L_0200155a
	ldr	r0, [pc, #720]	@ (0x2001764)
	mov	r1, r8
	movs	r2, #0
	movs	r3, #0
	bl	sub_02003722
	movs	r0, #112	@ 0x70
	bl	sub_020038a8
.L_020014a2:
	movs	r0, #1
	bl	sub_02003676
	ldr	r3, [pc, #684]	@ (0x2001758)
	movs	r2, #1
	ldr	r3, [r3, #4]
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_020014a2
	movs	r0, #112	@ 0x70
	bl	sub_020038c0
	bl	sub_0200380c
	ldr	r5, [pc, #680]	@ (0x2001768)
	movs	r1, #133	@ 0x85
	lsls	r1, r1, #2
	adds	r2, r5, r1
	movs	r3, #0
	str	r3, [r2, #0]
	movs	r0, #4
	bl	sub_0200382e
	movs	r0, #5
	bl	sub_02003834
	movs	r0, #6
	bl	sub_0200383a
	movs	r0, #7
	bl	sub_02003840
	movs	r0, #0
	bl	sub_0200383e
	movs	r0, #1
	bl	sub_02003844
	movs	r0, #2
	bl	sub_0200384a
	movs	r0, #3
	bl	sub_02003850
	ldr	r1, [pc, #608]	@ (0x200175c)
	ldr	r0, [sp, #64]	@ 0x40
	bl	sub_02002de0
	mov	r0, r8
	movs	r1, #2
	bl	sub_02003768
	movs	r1, #2
	ldr	r0, [sp, #56]	@ 0x38
	bl	sub_02003770
	mov	r0, r9
	movs	r1, #2
	bl	sub_02003778
	movs	r2, #147	@ 0x93
	lsls	r2, r2, #1
	adds	r2, #255	@ 0xff
	adds	r3, r5, r2
	ldrb	r0, [r3, #0]
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #2
	adds	r3, #38	@ 0x26
	adds	r5, r5, r3
	ldrb	r1, [r5, #0]
	bl	sub_02003840
	movs	r0, #74	@ 0x4a
	bl	sub_0200393e
	movs	r0, #6
	bl	sub_0200370c
	movs	r0, #0
	bl	sub_0200383a
	bl	sub_0200392e
	movs	r2, #244	@ 0xf4
	ldr	r3, [pc, #544]	@ (0x200176c)
	ldr	r0, [pc, #544]	@ (0x2001770)
	ldr	r1, [pc, #548]	@ (0x2001774)
	lsls	r2, r2, #4
	mov	lr, r3
	.2byte 0xf800
	.2byte 0x2000
	b.n	.L_020017f0
.L_0200155a:
	movs	r0, #113	@ 0x71
	bl	sub_02003966
	ldr	r0, [pc, #532]	@ (0x2001778)
	mov	r1, r8
	movs	r2, #0
	movs	r3, #0
	bl	sub_020037f2
.L_0200156c:
	movs	r0, #1
	bl	sub_02003740
	ldr	r1, [pc, #484]	@ (0x2001758)
	movs	r2, #1
	ldr	r3, [r1, #4]
	ands	r3, r2
	cmp	r3, #0
	bne.n	.L_02001588
	ldr	r3, [r1, #4]
	movs	r2, #2
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_0200156c
.L_02001588:
	movs	r1, #2
	mov	r0, r8
	bl	sub_020037ee
	mov	r0, r9
	bl	sub_02003854
	ldr	r0, [pc, #484]	@ (0x200177c)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #0
	bl	sub_02003830
	ldr	r0, [pc, #476]	@ (0x2001780)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #8
	bl	sub_0200383c
	ldr	r0, [pc, #468]	@ (0x2001784)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #16
	bl	sub_02003848
	ldr	r0, [pc, #460]	@ (0x2001788)
	mov	r1, r9
	movs	r2, #0
	movs	r3, #24
	bl	sub_02003854
	ldr	r6, [pc, #452]	@ (0x200178c)
	movs	r3, #5
	str	r3, [sp, #0]
	mov	r0, r9
	movs	r1, #20
	movs	r2, #0
	movs	r3, #20
	bl	sub_0200384e
	adds	r0, r6, #0
	mov	r1, r9
	adds	r6, #1
	movs	r2, #160	@ 0xa0
	movs	r3, #0
	bl	sub_0200386c
	adds	r0, r6, #0
	mov	r1, r9
	movs	r2, #160	@ 0xa0
	movs	r3, #8
	adds	r6, #1
	bl	sub_0200387a
	adds	r0, r6, #0
	mov	r1, r9
	movs	r2, #160	@ 0xa0
	movs	r3, #24
	bl	sub_02003886
	b.n	.L_02001692
.L_02001602:
	ldr	r3, [r1, #12]
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #2
	ands	r3, r2
	cmp	r3, #0
	bne.n	.L_02001614
	mov	r4, sl
	cmp	r4, #2
	bne.n	.L_02001628
.L_02001614:
	ldr	r0, [sp, #60]	@ 0x3c
	cmp	r0, #0
	ble.n	.L_02001692
	movs	r0, #111	@ 0x6f
	bl	sub_02003a26
	ldr	r1, [sp, #60]	@ 0x3c
	subs	r1, #1
	str	r1, [sp, #60]	@ 0x3c
	b.n	.L_02001692
.L_02001628:
	ldr	r3, [r1, #12]
	movs	r2, #128	@ 0x80
	lsls	r2, r2, #1
	ands	r3, r2
	cmp	r3, #0
	bne.n	.L_0200163a
	mov	r2, sl
	cmp	r2, #1
	bne.n	.L_02001652
.L_0200163a:
	ldr	r3, [sp, #44]	@ 0x2c
	ldr	r4, [sp, #60]	@ 0x3c
	subs	r3, #1
	cmp	r4, r3
	bge.n	.L_02001692
	movs	r0, #111	@ 0x6f
	bl	sub_02003a50
	ldr	r0, [sp, #60]	@ 0x3c
	adds	r0, #1
	str	r0, [sp, #60]	@ 0x3c
	b.n	.L_02001692
.L_02001652:
	ldr	r3, [r1, #4]
	movs	r2, #2
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02001692
	ldr	r1, [sp, #60]	@ 0x3c
	cmp	r1, #0
	ble.n	.L_02001670
	movs	r0, #111	@ 0x6f
	bl	sub_02003a6e
	ldr	r2, [sp, #60]	@ 0x3c
	subs	r2, #1
	str	r2, [sp, #60]	@ 0x3c
	b.n	.L_02001692
.L_02001670:
	movs	r0, #113	@ 0x71
	bl	sub_02003a7c
	movs	r1, #2
	ldr	r0, [sp, #56]	@ 0x38
	bl	sub_020038dc
	mov	r0, r9
	movs	r1, #2
	bl	sub_020038e4
	movs	r0, #74	@ 0x4a
	bl	sub_02003a92
	movs	r0, #1
	negs	r0, r0
	b.n	.L_020017f0
.L_02001692:
	ldr	r0, [sp, #60]	@ 0x3c
	movs	r1, #50	@ 0x32
	bl	sub_02003858
	mov	fp, r0
	ldr	r0, [sp, #56]	@ 0x38
	bl	sub_02003960
	movs	r6, #0
	movs	r5, #2
.L_020016a6:
	adds	r2, r5, #0
	ldr	r0, [sp, #56]	@ 0x38
	movs	r1, #0
	movs	r3, #18
	adds	r6, #1
	str	r5, [sp, #0]
	bl	sub_0200392c
	adds	r5, #2
	cmp	r6, #4
	bne.n	.L_020016a6
	ldr	r3, [sp, #40]	@ 0x28
	movs	r6, #0
	cmp	r3, #0
	beq.n	.L_020016f6
	negs	r3, r3
	adds	r5, r3, #0
	adds	r5, #18
.L_020016ca:
	movs	r4, #243	@ 0xf3
	lsls	r4, r4, #8
	adds	r4, #1
	adds	r1, r6, r4
	cmp	r6, fp
	bne.n	.L_020016de
	movs	r0, #243	@ 0xf3
	lsls	r0, r0, #8
	adds	r0, #11
	adds	r1, r6, r0
.L_020016de:
	movs	r3, #0
	str	r3, [sp, #0]
	adds	r2, r5, #0
	ldr	r0, [sp, #56]	@ 0x38
	movs	r3, #10
	bl	sub_020039b2
	ldr	r1, [sp, #40]	@ 0x28
	adds	r6, #1
	adds	r5, #1
	cmp	r6, r1
	bne.n	.L_020016ca
.L_020016f6:
	ldr	r2, [sp, #44]	@ 0x2c
	mov	sl, r2
	cmp	r2, #49	@ 0x31
	ble.n	.L_02001716
	ldr	r3, [sp, #40]	@ 0x28
	subs	r3, #1
	cmp	r3, fp
	bne.n	.L_02001712
	ldr	r0, [sp, #44]	@ 0x2c
	movs	r1, #50	@ 0x32
	bl	sub_020038d4
	mov	sl, r0
	b.n	.L_02001716
.L_02001712:
	movs	r3, #50	@ 0x32
	mov	sl, r3
.L_02001716:
	mov	r4, sl
	movs	r6, #0
	cmp	r4, #0
	beq.n	.L_020017b4
	ldr	r0, [sp, #36]	@ 0x24
	ldr	r1, [sp, #36]	@ 0x24
	movs	r2, #200	@ 0xc8
	adds	r0, #200	@ 0xc8
	lsls	r2, r2, #1
	mov	r8, r0
	adds	r7, r1, r2
.L_0200172c:
	movs	r3, #50	@ 0x32
	mov	r0, fp
	muls	r0, r3
	ldr	r4, [pc, #44]	@ (0x2001760)
	adds	r3, r0, #0
	adds	r0, r6, r3
	ldrb	r2, [r4, r0]
	ldrb	r3, [r7, #0]
	adds	r1, r2, #0
	cmp	r1, r3
	beq.n	.L_020017a8
	strb	r2, [r7, #0]
	cmp	r1, #99	@ 0x63
	bne.n	.L_02001794
	mov	r1, r8
	ldr	r0, [r1, #0]
	ldr	r3, [pc, #64]	@ (0x2001790)
	movs	r1, #128	@ 0x80
	mov	lr, r3
	.2byte 0xf800
	.2byte 0xe028
	.2byte 0x0000
	.4byte 0x03001150
	.4byte 0x0200a88a
	.4byte 0x0200a74a
	.4byte 0x00000d91
	.4byte 0x02000240
	.4byte 0x03000730
	.4byte 0x02013040
	.4byte 0x02000040
	.4byte 0x00000d90
	.4byte 0x0200a440
	.4byte 0x0200a454
	.4byte 0x0200a468
	.4byte 0x0200a47c
	.4byte 0x00000d8d
	.2byte 0x0258
	.2byte 0x0300
.L_02001794:
	add	r5, sp, #68	@ 0x44
	ldrb	r0, [r4, r0]
	adds	r1, r5, #0
	bl	sub_0200220c
	mov	r2, r8
	ldrh	r0, [r5, #0]
	ldr	r1, [r2, #0]
	bl	sub_02003ace
.L_020017a8:
	movs	r3, #4
	adds	r6, #1
	add	r8, r3
	adds	r7, #1
	cmp	r6, sl
	bne.n	.L_0200172c
.L_020017b4:
	cmp	r6, #50	@ 0x32
	beq.n	.L_020017e8
	ldr	r4, [sp, #36]	@ 0x24
	lsls	r3, r6, #2
	movs	r0, #200	@ 0xc8
	adds	r3, r3, r4
	lsls	r0, r0, #1
	adds	r7, r3, #0
	adds	r3, r6, r0
	adds	r7, #200	@ 0xc8
	adds	r5, r3, r4
.L_020017ca:
	ldrb	r3, [r5, #0]
	cmp	r3, #99	@ 0x63
	beq.n	sub_020017de
	movs	r3, #99	@ 0x63
	strb	r3, [r5, #0]
	ldr	r0, [r7, #0]
	ldr	r3, [pc, #40]	@ (0x2001800)
	movs	r1, #128	@ 0x80
	mov	lr, r3
	.2byte 0xf800
	.2byte 0x3601
	adds	r7, #4
	adds	r5, #1
	cmp	r6, #50	@ 0x32
	bne.n	.L_020017ca
.L_020017e8:
	movs	r0, #1
	bl	sub_020039bc
	b.n	.L_020010ca
.L_020017f0:
	add	sp, #124	@ 0x7c
	pop	{r3, r5, r6, r7}
	mov	r8, r3
	mov	r9, r5
	mov	sl, r6
	mov	fp, r7
	pop	{r5, r6, r7, pc}
	.2byte 0x0000
	.2byte 0x0258
	.2byte 0x0300
	push	{r5, r6, r7, lr}
	mov	r7, r8
	push	{r7}
	adds	r7, r0, #0
	movs	r3, #192	@ 0xc0
	movs	r0, #192	@ 0xc0
	lsls	r3, r3, #18
	lsls	r0, r0, #2
	ldr	r5, [r3, #60]	@ 0x3c
	adds	r6, r1, #0
	bl	sub_02003a02
	mov	r8, r0
	mov	r1, r8
	adds	r0, r6, #0
	bl	sub_02003a1c
	movs	r1, #14
	ldrsh	r3, [r7, r1]
	movs	r1, #12
	ldrsh	r2, [r7, r1]
	lsls	r3, r3, #5
	adds	r3, r3, r2
	ldr	r2, [pc, #56]	@ (0x200186c)
	lsls	r3, r3, #1
	adds	r5, r5, r3
	adds	r1, r3, r2
	adds	r5, #8
	movs	r4, #0
.L_0200183e:
	movs	r0, #0
.L_02001840:
	ldrh	r3, [r7, #8]
	ldr	r2, [pc, #36]	@ (0x2001868)
	muls	r3, r4
	adds	r3, r3, r0
	orrs	r3, r2
	lsls	r3, r3, #16
	asrs	r3, r3, #16
	adds	r0, #1
	strh	r3, [r1, #0]
	strh	r3, [r5, #0]
	adds	r1, #2
	adds	r5, #2
	cmp	r0, #15
	ble.n	.L_02001840
	adds	r4, #1
	adds	r1, #32
	adds	r5, #32
	cmp	r4, #7
	ble.n	.L_0200183e
	b.n	.L_02001870
	.4byte 0xfc34f002
	.2byte 0x2000
	.2byte 0x0600
.L_02001870:
	mov	r0, r8
	bl	sub_02003a64
	pop	{r3}
	mov	r8, r3
	pop	{r5, r6, r7, pc}
	push	{r5, r6, lr}
	adds	r6, r0, #0
	movs	r3, #192	@ 0xc0
	movs	r0, #192	@ 0xc0
	lsls	r3, r3, #18
	lsls	r0, r0, #2
	ldr	r5, [r3, #60]	@ 0x3c
	bl	sub_02003a74
	movs	r1, #14
	ldrsh	r3, [r6, r1]
	movs	r1, #12
	ldrsh	r2, [r6, r1]
	lsls	r3, r3, #5
	adds	r3, r3, r2
	ldr	r2, [pc, #56]	@ (0x20018d4)
	lsls	r3, r3, #1
	adds	r5, r5, r3
	adds	r1, r3, r2
	adds	r5, #8
	movs	r6, #0
.L_020018a6:
	movs	r4, #0
.L_020018a8:
	lsls	r3, r6, #4
	ldr	r2, [pc, #36]	@ (0x20018d0)
	adds	r3, r3, r4
	adds	r3, #32
	orrs	r3, r2
	lsls	r3, r3, #16
	asrs	r3, r3, #16
	adds	r4, #1
	strh	r3, [r1, #0]
	strh	r3, [r5, #0]
	adds	r1, #2
	adds	r5, #2
	cmp	r4, #15
	ble.n	.L_020018a8
	adds	r6, #1
	adds	r1, #32
	adds	r5, #32
	cmp	r6, #7
	ble.n	.L_020018a6
	b.n	.L_020018d8
	.4byte 0xfc68f002
	.2byte 0x2000
	.2byte 0x0600
.L_020018d8:
	bl	sub_02003aca
	pop	{r5, r6, pc}
	.2byte 0x0000
	push	{r5, r6, r7, lr}
	mov	r7, fp
	mov	r6, sl
	mov	r5, r9
	push	{r5, r6, r7}
	mov	r7, r8
	push	{r7}
	mov	sl, r1
	mov	r2, sl
	ldrb	r3, [r2, #1]
	ldrb	r1, [r1, #0]
	lsls	r3, r3, #8
	orrs	r1, r3
	ldrb	r3, [r2, #2]
	ldrb	r2, [r2, #3]
	lsls	r3, r3, #16
	orrs	r1, r3
	movs	r3, #240	@ 0xf0
	ands	r3, r2
	lsls	r3, r3, #20
	orrs	r1, r3
	movs	r3, #15
	mov	r9, r3
	mov	r8, r1
	mov	r1, r9
	ands	r1, r2
	mov	r2, sl
	ldrb	r3, [r2, #4]
	sub	sp, #40	@ 0x28
	lsls	r3, r3, #4
	orrs	r1, r3
	ldrb	r3, [r2, #5]
	ldrb	r6, [r2, #7]
	lsls	r3, r3, #12
	orrs	r1, r3
	ldrb	r3, [r2, #6]
	mov	fp, r0
	lsls	r3, r3, #20
	orrs	r1, r3
	ldrb	r3, [r2, #8]
	mov	r9, r1
	str	r3, [sp, #4]
	cmp	r0, #2
	beq.n	.L_020019fe
	negs	r3, r0
	orrs	r3, r0
	lsrs	r3, r3, #31
	adds	r3, #8
	movs	r7, #0
	movs	r4, #4
	adds	r0, r3, r2
.L_02001946:
	ldrb	r2, [r0, #0]
	ldrb	r3, [r0, #1]
	lsls	r2, r2, #24
	lsls	r3, r3, #16
	orrs	r2, r3
	ldrb	r3, [r0, #2]
	add	r5, sp, #8
	lsls	r3, r3, #8
	orrs	r2, r3
	ldrb	r3, [r0, #3]
	lsls	r1, r7, #4
	orrs	r2, r3
	str	r2, [r5, r1]
	adds	r7, #1
	ldrb	r2, [r0, #4]
	ldrb	r3, [r0, #5]
	lsls	r2, r2, #24
	lsls	r3, r3, #16
	orrs	r2, r3
	ldrb	r3, [r0, #6]
	ldrb	r1, [r0, #7]
	lsls	r3, r3, #8
	orrs	r2, r3
	orrs	r2, r1
	str	r2, [r5, r4]
	lsls	r1, r1, #28
	ldrb	r3, [r0, #8]
	adds	r2, r4, #4
	lsls	r3, r3, #20
	orrs	r1, r3
	ldrb	r3, [r0, #9]
	lsls	r3, r3, #12
	orrs	r1, r3
	ldrb	r3, [r0, #10]
	lsls	r3, r3, #4
	orrs	r1, r3
	ldrb	r3, [r0, #11]
	lsrs	r3, r3, #4
	orrs	r1, r3
	str	r1, [r5, r2]
	adds	r1, r4, #0
	ldrb	r2, [r0, #11]
	ldrb	r3, [r0, #12]
	lsls	r2, r2, #28
	lsls	r3, r3, #20
	orrs	r2, r3
	ldrb	r3, [r0, #13]
	adds	r1, #8
	lsls	r3, r3, #12
	orrs	r2, r3
	ldrb	r3, [r0, #14]
	adds	r4, #16
	lsls	r3, r3, #4
	orrs	r2, r3
	str	r2, [r5, r1]
	adds	r0, #15
	cmp	r7, #2
	bne.n	.L_02001946
	movs	r7, #0
.L_020019bc:
	ldr	r2, [pc, #92]	@ (0x2001a1c)
	lsls	r3, r7, #2
	ldr	r0, [r2, r3]
	bl	sub_02003bfc
	lsls	r1, r7, #3
	ldr	r2, [r5, r1]
	movs	r4, #128	@ 0x80
	lsls	r4, r4, #1
	adds	r0, #16
	lsrs	r3, r2, #21
	adds	r4, #255	@ 0xff
	strh	r3, [r0, #0]
	adds	r1, #4
	lsls	r3, r2, #11
	ands	r2, r4
	strh	r2, [r0, #8]
	ldr	r2, [r5, r1]
	lsrs	r3, r3, #21
	strh	r3, [r0, #2]
	lsrs	r3, r2, #22
	ands	r3, r4
	strh	r3, [r0, #10]
	lsrs	r3, r2, #12
	ands	r3, r4
	strh	r3, [r0, #12]
	lsrs	r2, r2, #4
	movs	r3, #127	@ 0x7f
	ands	r2, r3
	adds	r7, #1
	strb	r2, [r0, #14]
	cmp	r7, #4
	bne.n	.L_020019bc
.L_020019fe:
	movs	r7, #0
.L_02001a00:
	adds	r2, r6, #0
	movs	r3, #1
	asrs	r2, r7
	ands	r2, r3
	adds	r5, r7, #1
	ldr	r3, [pc, #20]	@ (0x2001a20)
	lsls	r0, r7, #1
	cmp	r2, #0
	beq.n	.L_02001a24
	ldrh	r0, [r3, r0]
	bl	sub_02003c5e
	b.n	.L_02001a2a
	.2byte 0x0000
	.4byte 0x0200a6a0
	.2byte 0xa490
	.2byte 0x0200
.L_02001a24:
	ldrh	r0, [r3, r0]
	bl	sub_02003c78
.L_02001a2a:
	adds	r7, r5, #0
	cmp	r7, #6
	bne.n	.L_02001a00
	movs	r7, #0
	movs	r6, #0
.L_02001a34:
	ldr	r2, [pc, #320]	@ (0x2001b78)
	lsls	r3, r7, #2
	ldr	r0, [r2, r3]
	bl	sub_02003c74
	mov	r3, r8
	lsrs	r3, r6
	movs	r2, #127	@ 0x7f
	adds	r5, r0, #0
	ands	r3, r2
	strb	r3, [r5, #15]
	adds	r0, r7, #0
	ldrb	r1, [r5, #15]
	bl	sub_02003db8
	movs	r1, #146	@ 0x92
	lsls	r1, r1, #1
	adds	r5, r5, r1
	adds	r7, #1
	str	r0, [r5, #0]
	adds	r6, #7
	cmp	r7, #4
	bne.n	.L_02001a34
	movs	r0, #0
	bl	sub_02003d9e
	movs	r2, #148	@ 0x94
	lsls	r2, r2, #1
	adds	r0, r0, r2
	movs	r3, #0
	str	r3, [r0, #0]
	movs	r7, #0
.L_02001a74:
	ldr	r3, [pc, #256]	@ (0x2001b78)
	lsls	r2, r7, #2
	ldr	r0, [r3, r2]
	bl	sub_02003cb4
	movs	r6, #0
	adds	r5, r0, #0
.L_02001a82:
	adds	r0, r5, #0
	ldr	r3, [pc, #244]	@ (0x2001b7c)
	adds	r0, #248	@ 0xf8
	movs	r1, #40	@ 0x28
	adds	r6, #1
	mov	lr, r3
	.2byte 0xf800
	.2byte 0x2e04
	bne.n	.L_02001a82
	adds	r7, #1
	cmp	r7, #4
	bne.n	.L_02001a74
	movs	r7, #0
.L_02001a9c:
	adds	r0, r7, #0
	movs	r1, #7
	bl	sub_02003c62
	movs	r1, #7
	adds	r5, r0, #0
	adds	r0, r7, #0
	bl	sub_02003c74
	adds	r3, r0, #0
	lsls	r0, r5, #2
	adds	r0, r0, r5
	lsls	r0, r0, #2
	adds	r0, r0, r3
	adds	r0, #48	@ 0x30
	adds	r7, #1
	bl	sub_02003d0e
	cmp	r7, #28
	bne.n	.L_02001a9c
	movs	r7, #0
.L_02001ac6:
	mov	r3, r9
	lsrs	r3, r7
	movs	r2, #1
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02001aec
	movs	r1, #7
	adds	r0, r7, #0
	bl	sub_02003c98
	movs	r1, #7
	adds	r5, r0, #0
	adds	r0, r7, #0
	bl	sub_02003caa
	adds	r1, r0, #0
	adds	r0, r5, #0
	bl	sub_02003e62
.L_02001aec:
	adds	r7, #1
	cmp	r7, #28
	bne.n	.L_02001ac6
	mov	r3, fp
	cmp	r3, #0
	beq.n	.L_02001b26
	ldr	r3, [pc, #124]	@ (0x2001b78)
	movs	r7, #0
	ldr	r0, [r3, #0]
	bl	sub_02003d38
.L_02001b02:
	ldr	r3, [sp, #4]
	movs	r2, #1
	asrs	r3, r7
	ands	r3, r2
	cmp	r3, #0
	beq.n	.L_02001b18
	ldr	r3, [pc, #112]	@ (0x2001b80)
	lsls	r2, r7, #1
	ldrh	r0, [r3, r2]
	bl	sub_02003e5e
.L_02001b18:
	adds	r7, #1
	cmp	r7, #8
	bne.n	.L_02001b02
	mov	r1, fp
	cmp	r1, #0
	beq.n	.L_02001b26
	b.n	.L_02001c4a
.L_02001b26:
	movs	r2, #39	@ 0x27
	mov	r8, r2
	movs	r5, #0
	movs	r7, #0
.L_02001b2e:
	ldr	r2, [pc, #72]	@ (0x2001b78)
	lsls	r3, r7, #2
	ldr	r0, [r2, r3]
	bl	sub_02003d6e
	mov	r4, r8
	adds	r0, #216	@ 0xd8
	movs	r6, #0
	mov	ip, r0
	add	r4, sl
.L_02001b42:
	ldrb	r1, [r4, #1]
	ldrb	r2, [r4, #0]
	movs	r3, #7
	subs	r3, r3, r5
	asrs	r1, r3
	adds	r0, r5, #1
	ldr	r3, [pc, #36]	@ (0x2001b74)
	lsls	r2, r0
	orrs	r2, r1
	ands	r2, r3
	mov	r3, ip
	strh	r2, [r3, #0]
	movs	r1, #2
	movs	r2, #1
	adds	r5, r0, #0
	add	ip, r1
	adds	r4, #1
	add	r8, r2
	cmp	r5, #7
	bne.n	.L_02001b84
	movs	r5, #0
	adds	r4, #1
	add	r8, r2
	b.n	.L_02001b84
	.2byte 0x0000
	.4byte 0x000001ff
	.4byte 0x0200a6a0
	.4byte 0x03000258
	.2byte 0xa6b0
	.2byte 0x0200
.L_02001b84:
	adds	r6, #1
	cmp	r6, #15
	bne.n	.L_02001b42
	adds	r7, #1
	cmp	r7, #4
	bne.n	.L_02001b2e
	movs	r3, #107	@ 0x6b
	movs	r5, #1
	mov	r8, r3
	negs	r5, r5
	movs	r7, #0
.L_02001b9a:
	ldr	r3, [pc, #212]	@ (0x2001c70)
	lsls	r2, r7, #2
	ldr	r0, [r3, r2]
	bl	sub_02003dda
	mov	r2, r8
	movs	r1, #0
	add	r2, sl
	mov	fp, r0
	mov	r9, r1
	mov	ip, r2
.L_02001bb0:
	movs	r1, #0
	cmp	r5, #0
	bge.n	.L_02001bca
	mov	r3, ip
	ldrb	r1, [r3, #0]
	negs	r3, r5
	lsls	r1, r3
	movs	r2, #1
	movs	r3, #31
	ands	r1, r3
	add	ip, r2
	add	r8, r2
	adds	r5, #8
.L_02001bca:
	mov	r2, ip
	ldrb	r3, [r2, #0]
	movs	r2, #31
	asrs	r3, r5
	ands	r3, r2
	adds	r1, r1, r3
	movs	r3, #5
	subs	r5, #5
	negs	r3, r3
	cmp	r5, r3
	bne.n	.L_02001be8
	movs	r3, #1
	add	ip, r3
	add	r8, r3
	movs	r5, #3
.L_02001be8:
	ands	r1, r2
	ldr	r2, [pc, #136]	@ (0x2001c74)
	movs	r6, #0
	mov	lr, r2
	movs	r0, #216	@ 0xd8
.L_02001bf2:
	mov	r3, fp
	ldrh	r4, [r0, r3]
	mov	r3, r9
	lsls	r2, r3, #1
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #1
	adds	r3, #255	@ 0xff
	ands	r3, r4
	str	r3, [sp, #0]
	mov	r3, lr
	ldrh	r2, [r3, r2]
	ldr	r3, [sp, #0]
	cmp	r3, r2
	bne.n	.L_02001c16
	lsls	r3, r1, #11
	orrs	r3, r4
	mov	r2, fp
	strh	r3, [r0, r2]
.L_02001c16:
	adds	r6, #1
	adds	r0, #2
	cmp	r6, #15
	bne.n	.L_02001bf2
	movs	r3, #1
	add	r9, r3
	mov	r1, r9
	cmp	r1, #23
	bne.n	.L_02001bb0
	adds	r7, #1
	cmp	r7, #4
	bne.n	.L_02001b9a
	mov	r3, sl
	adds	r3, #165	@ 0xa5
	ldrb	r2, [r3, #0]
	adds	r3, #1
	ldrb	r3, [r3, #0]
	lsls	r2, r2, #16
	lsls	r3, r3, #8
	orrs	r2, r3
	mov	r3, sl
	adds	r3, #167	@ 0xa7
	ldrb	r3, [r3, #0]
	ldr	r1, [pc, #48]	@ (0x2001c78)
	orrs	r2, r3
	str	r2, [r1, #16]
.L_02001c4a:
	movs	r7, #0
.L_02001c4c:
	ldr	r6, [pc, #32]	@ (0x2001c70)
	lsls	r5, r7, #2
	ldr	r0, [r6, r5]
	bl	sub_02003fd4
	adds	r7, #1
	ldr	r0, [r6, r5]
	bl	sub_02003f9c
	cmp	r7, #4
	bne.n	.L_02001c4c
	add	sp, #40	@ 0x28
	pop	{r3, r5, r6, r7}
	mov	r8, r3
	mov	r9, r5
	mov	sl, r6
	mov	fp, r7
	pop	{r5, r6, r7, pc}
	.4byte 0x0200a6a0
	.4byte 0x0200a6c0
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, r6, r7, lr}
	mov	r7, fp
	mov	r6, sl
	mov	r5, r9
	push	{r5, r6, r7}
	mov	r7, r8
	push	{r7}
	mov	fp, r1
	sub	sp, #60	@ 0x3c
	mov	r8, r2
	movs	r1, #11
	mov	r2, fp
	str	r1, [sp, #24]
	cmp	r2, #1
	beq.n	.L_02001cb2
	cmp	r2, #1
	bgt.n	.L_02001ca4
	cmp	r2, #0
	beq.n	.L_02001cac
	b.n	.L_02001cbc
.L_02001ca4:
	mov	r3, fp
	cmp	r3, #2
	beq.n	.L_02001cb8
	b.n	.L_02001cbc
.L_02001cac:
	movs	r4, #173	@ 0xad
	str	r4, [sp, #24]
	b.n	.L_02001cbc
.L_02001cb2:
	movs	r5, #39	@ 0x27
	str	r5, [sp, #24]
	b.n	.L_02001cbc
.L_02001cb8:
	movs	r1, #9
	str	r1, [sp, #24]
.L_02001cbc:
	ldr	r2, [sp, #24]
	movs	r6, #0
	cmp	r2, #0
	beq.n	.L_02001cd4
	mov	r2, r8
.L_02001cc6:
	movs	r3, #0
	strb	r3, [r2, #0]
	ldr	r3, [sp, #24]
	adds	r6, #1
	adds	r2, #1
	cmp	r6, r3
	bne.n	.L_02001cc6
.L_02001cd4:
	movs	r4, #0
	str	r4, [sp, #20]
	str	r4, [sp, #16]
	str	r4, [sp, #12]
	str	r4, [sp, #8]
	movs	r6, #0
.L_02001ce0:
	mov	r5, sp
	adds	r5, #28
	lsls	r2, r6, #2
	movs	r3, #0
	adds	r6, #1
	str	r5, [sp, #4]
	str	r3, [r5, r2]
	cmp	r6, #8
	bne.n	.L_02001ce0
	movs	r6, #0
.L_02001cf4:
	ldr	r2, [pc, #332]	@ (0x2001e44)
	lsls	r3, r6, #1
	ldrh	r0, [r2, r3]
	bl	sub_02003f3c
	cmp	r0, #0
	beq.n	.L_02001d10
	ldr	r1, [sp, #8]
	movs	r3, #1
	lsls	r3, r6
	orrs	r1, r3
	lsls	r3, r1, #24
	lsrs	r3, r3, #24
	str	r3, [sp, #8]
.L_02001d10:
	adds	r6, #1
	cmp	r6, #6
	bne.n	.L_02001cf4
	movs	r2, #4
	movs	r6, #0
	mov	sl, r2
.L_02001d1c:
	ldr	r2, [pc, #296]	@ (0x2001e48)
	lsls	r3, r6, #2
	ldr	r0, [r2, r3]
	bl	sub_02003f5c
	adds	r1, r0, #0
	adds	r1, #16
	movs	r2, #218	@ 0xda
	movs	r4, #0
	ldrsh	r3, [r1, r4]
	lsls	r2, r2, #3
	adds	r2, #255	@ 0xff
	cmp	r3, r2
	ble.n	.L_02001d3a
	strh	r2, [r1, #0]
.L_02001d3a:
	movs	r5, #0
	ldrsh	r3, [r1, r5]
	cmp	r3, #0
	bge.n	.L_02001d46
	movs	r3, #0
	strh	r3, [r1, #0]
.L_02001d46:
	movs	r4, #2
	ldrsh	r3, [r1, r4]
	cmp	r3, r2
	ble.n	.L_02001d50
	strh	r2, [r1, #2]
.L_02001d50:
	movs	r5, #2
	ldrsh	r3, [r1, r5]
	cmp	r3, #0
	bge.n	.L_02001d5c
	movs	r3, #0
	strh	r3, [r1, #2]
.L_02001d5c:
	movs	r2, #186	@ 0xba
	ldrh	r3, [r1, #8]
	lsls	r2, r2, #2
	adds	r2, #255	@ 0xff
	cmp	r3, r2
	bls.n	.L_02001d6a
	strh	r2, [r1, #8]
.L_02001d6a:
	ldrh	r3, [r1, #8]
	cmp	r3, #0
	bge.n	.L_02001d72
	movs	r3, #0
.L_02001d72:
	strh	r3, [r1, #8]
	ldrh	r3, [r1, #10]
	cmp	r3, r2
	bls.n	.L_02001d7c
	strh	r2, [r1, #10]
.L_02001d7c:
	ldrh	r3, [r1, #10]
	cmp	r3, #0
	bge.n	.L_02001d84
	movs	r3, #0
.L_02001d84:
	strh	r3, [r1, #10]
	ldrh	r3, [r1, #12]
	cmp	r3, r2
	bls.n	.L_02001d8e
	strh	r2, [r1, #12]
.L_02001d8e:
	ldrh	r3, [r1, #12]
	cmp	r3, #0
	bge.n	.L_02001d96
	movs	r3, #0
.L_02001d96:
	strh	r3, [r1, #12]
	ldrb	r3, [r1, #14]
	cmp	r3, #99	@ 0x63
	bls.n	.L_02001da2
	movs	r3, #99	@ 0x63
	strb	r3, [r1, #14]
.L_02001da2:
	ldrb	r3, [r1, #14]
	cmp	r3, #0
	bge.n	.L_02001daa
	movs	r3, #0
.L_02001daa:
	strb	r3, [r1, #14]
	movs	r3, #0
	ldrsh	r2, [r1, r3]
	movs	r5, #2
	ldrsh	r3, [r1, r5]
	lsls	r2, r2, #21
	lsls	r3, r3, #10
	orrs	r2, r3
	ldrh	r3, [r1, #8]
	lsls	r4, r6, #3
	orrs	r2, r3
	ldr	r3, [sp, #4]
	str	r2, [r3, r4]
	ldrh	r2, [r1, #10]
	ldrh	r3, [r1, #12]
	lsls	r2, r2, #22
	lsls	r3, r3, #12
	orrs	r2, r3
	ldrb	r3, [r1, #14]
	ldr	r5, [sp, #4]
	lsls	r3, r3, #4
	orrs	r2, r3
	mov	r1, sl
	str	r2, [r5, r1]
	ldrb	r3, [r0, #15]
	cmp	r3, #99	@ 0x63
	bls.n	.L_02001de4
	movs	r3, #99	@ 0x63
	strb	r3, [r0, #15]
.L_02001de4:
	ldrb	r3, [r0, #15]
	cmp	r3, #0
	bne.n	.L_02001dee
	movs	r3, #1
	strb	r3, [r0, #15]
.L_02001dee:
	ldrb	r3, [r0, #15]
	subs	r2, r4, r6
	lsls	r3, r2
	ldr	r2, [sp, #20]
	adds	r1, r0, #0
	orrs	r2, r3
	str	r2, [sp, #20]
	movs	r4, #0
	adds	r1, #248	@ 0xf8
	movs	r2, #0
.L_02001e02:
	ldmia	r1!, {r3}
	ldr	r5, [sp, #16]
	lsls	r3, r2
	adds	r5, r5, r3
	adds	r4, #1
	str	r5, [sp, #16]
	adds	r2, #7
	cmp	r4, #4
	bne.n	.L_02001e02
	ldr	r7, [pc, #52]	@ (0x2001e4c)
	movs	r4, #0
	adds	r0, #216	@ 0xd8
.L_02001e1a:
	ldr	r3, [pc, #36]	@ (0x2001e40)
	ldrh	r2, [r0, #0]
	adds	r1, r3, #0
	ands	r1, r2
	mov	ip, r1
	adds	r2, r7, #0
	movs	r1, #0
.L_02001e28:
	ldrh	r3, [r2, #0]
	adds	r2, #2
	cmp	ip, r3
	bne.n	.L_02001e50
	ldr	r5, [sp, #12]
	movs	r3, #1
	lsls	r3, r1
	orrs	r5, r3
	lsls	r3, r5, #24
	lsrs	r3, r3, #24
	str	r3, [sp, #12]
	b.n	.L_02001e50
	.4byte 0x000001ff
	.4byte 0x0200a700
	.4byte 0x0200a6f0
	.2byte 0xa70c
	.2byte 0x0200
.L_02001e50:
	adds	r1, #1
	cmp	r1, #8
	bne.n	.L_02001e28
	adds	r4, #1
	adds	r0, #2
	cmp	r4, #15
	bne.n	.L_02001e1a
	movs	r1, #8
	adds	r6, #1
	add	sl, r1
	cmp	r6, #4
	beq.n	.L_02001e6a
	b.n	.L_02001d1c
.L_02001e6a:
	mov	r2, fp
	cmp	r2, #0
	beq.n	.L_02001e72
	b.n	.L_02001f8e
.L_02001e72:
	movs	r3, #39	@ 0x27
	mov	sl, r3
	movs	r5, #0
	movs	r6, #0
.L_02001e7a:
	ldr	r2, [pc, #80]	@ (0x2001ecc)
	lsls	r3, r6, #2
	ldr	r0, [r2, r3]
	bl	sub_020040ba
	movs	r4, #0
	mov	ip, r4
	adds	r7, r0, #0
	mov	r4, sl
	adds	r7, #216	@ 0xd8
	add	r4, r8
.L_02001e90:
	ldrh	r1, [r7, #0]
	ldr	r3, [pc, #52]	@ (0x2001ec8)
	adds	r0, r5, #1
	ands	r1, r3
	ldrb	r3, [r4, #0]
	adds	r2, r1, #0
	asrs	r2, r0
	adds	r3, r3, r2
.L_02001ea0:
	strb	r3, [r4, #0]
	movs	r3, #7
	subs	r3, r3, r5
	lsls	r1, r3
	ldrb	r3, [r4, #1]
	movs	r5, #1
	adds	r3, r3, r1
	add	sl, r5
	adds	r5, r0, #0
	strb	r3, [r4, #1]
	adds	r7, #2
	adds	r4, #1
	cmp	r5, #7
	bne.n	.L_02001ed0
	movs	r1, #1
	movs	r5, #0
	adds	r4, #1
	add	sl, r1
	b.n	.L_02001ed0
	.2byte 0x0000
	.4byte 0x000001ff
	.2byte 0xa6f0
	.2byte 0x0200
.L_02001ed0:
	movs	r2, #1
	add	ip, r2
	mov	r3, ip
	cmp	r3, #15
	bne.n	.L_02001e90
	adds	r6, #1
	cmp	r6, #4
	bne.n	.L_02001e7a
	movs	r4, #107	@ 0x6b
	movs	r5, #1
	mov	sl, r4
	negs	r5, r5
	movs	r6, #0
.L_02001eea:
	ldr	r3, [pc, #368]	@ (0x200205c)
	lsls	r2, r6, #2
	ldr	r0, [r3, r2]
	bl	sub_0200412a
	movs	r1, #0
	str	r0, [sp, #0]
	mov	lr, r1
	mov	r1, sl
	add	r1, r8
.L_02001efe:
	ldr	r4, [sp, #0]
	ldr	r7, [pc, #348]	@ (0x2002060)
	movs	r2, #0
	mov	r9, r2
	mov	ip, r2
	adds	r4, #216	@ 0xd8
.L_02001f0a:
	mov	r3, lr
	ldrh	r0, [r4, #0]
	lsls	r2, r3, #1
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #1
	ldrh	r2, [r7, r2]
	adds	r3, #255	@ 0xff
	ands	r3, r0
	adds	r4, #2
	cmp	r3, r2
	bne.n	.L_02001f24
	lsrs	r0, r0, #11
	mov	r9, r0
.L_02001f24:
	movs	r2, #1
	add	ip, r2
	mov	r3, ip
	cmp	r3, #15
	bne.n	.L_02001f0a
	mov	r2, r9
	lsls	r4, r2, #16
	cmp	r5, #0
	bge.n	.L_02001f4a
	negs	r2, r5
	lsrs	r3, r4, #16
	asrs	r3, r2
	ldrb	r2, [r1, #0]
	adds	r5, #8
	adds	r2, r2, r3
	movs	r3, #1
	strb	r2, [r1, #0]
	add	sl, r3
	adds	r1, #1
.L_02001f4a:
	ldrb	r3, [r1, #0]
	lsrs	r2, r4, #16
	lsls	r2, r5
	movs	r4, #5
	adds	r3, r3, r2
	subs	r5, #5
	negs	r4, r4
	strb	r3, [r1, #0]
	cmp	r5, r4
	bne.n	.L_02001f66
	movs	r5, #1
	add	sl, r5
	adds	r1, #1
	movs	r5, #3
.L_02001f66:
	movs	r2, #1
	add	lr, r2
	mov	r3, lr
	cmp	r3, #23
	bne.n	.L_02001efe
	adds	r6, #1
	cmp	r6, #4
	bne.n	.L_02001eea
	ldr	r2, [pc, #236]	@ (0x2002064)
	mov	r1, r8
	ldrh	r3, [r2, #18]
	adds	r1, #165	@ 0xa5
	strb	r3, [r1, #0]
	adds	r1, #1
	ldr	r3, [r2, #16]
	lsrs	r3, r3, #8
	strb	r3, [r1, #0]
	adds	r1, #1
	ldr	r3, [r2, #16]
	strb	r3, [r1, #0]
.L_02001f8e:
	mov	r4, fp
	cmp	r4, #2
	beq.n	.L_02002006
	negs	r3, r4
	orrs	r3, r4
	lsrs	r3, r3, #31
	adds	r3, #8
	mov	r5, r8
	movs	r6, #0
	movs	r4, #4
	adds	r0, r3, r5
.L_02001fa4:
	ldr	r1, [sp, #4]
	lsls	r3, r6, #4
	ldr	r2, [r1, r3]
	adds	r6, #1
	lsrs	r3, r2, #24
	strb	r3, [r0, #0]
	lsrs	r3, r2, #16
	strb	r3, [r0, #1]
	lsrs	r3, r2, #8
	strb	r3, [r0, #2]
	strb	r2, [r0, #3]
	ldr	r2, [r1, r4]
	lsrs	r3, r2, #24
	strb	r3, [r0, #4]
	lsrs	r3, r2, #16
	strb	r3, [r0, #5]
	lsrs	r3, r2, #8
	strb	r2, [r0, #7]
	strb	r3, [r0, #6]
	adds	r3, r4, #4
	ldr	r3, [r1, r3]
	lsrs	r1, r3, #28
	orrs	r2, r1
	strb	r2, [r0, #7]
	lsrs	r2, r3, #20
	strb	r2, [r0, #8]
	lsrs	r2, r3, #12
	strb	r2, [r0, #9]
	lsrs	r2, r3, #4
	lsls	r3, r3, #4
	strb	r3, [r0, #11]
	strb	r2, [r0, #10]
	ldr	r5, [sp, #4]
	adds	r2, r4, #0
	adds	r2, #8
	ldr	r1, [r5, r2]
	adds	r4, #16
	lsrs	r2, r1, #28
	orrs	r3, r2
	strb	r3, [r0, #11]
	lsrs	r3, r1, #20
	strb	r3, [r0, #12]
	lsrs	r3, r1, #12
	lsrs	r1, r1, #4
	strb	r3, [r0, #13]
	strb	r1, [r0, #14]
	adds	r0, #15
	cmp	r6, #2
	bne.n	.L_02001fa4
.L_02002006:
	add	r1, sp, #20
	ldrb	r1, [r1, #0]
	mov	r2, r8
	strb	r1, [r2, #0]
	ldr	r2, [sp, #20]
	mov	r4, r8
	lsrs	r3, r2, #8
	strb	r3, [r4, #1]
	lsrs	r3, r2, #16
	strb	r3, [r4, #2]
	ldr	r5, [sp, #20]
	movs	r3, #240	@ 0xf0
	lsrs	r2, r5, #20
	ands	r2, r3
	ldr	r3, [sp, #16]
	movs	r1, #15
	ands	r3, r1
	orrs	r2, r3
	strb	r2, [r4, #3]
	ldr	r1, [sp, #16]
	add	r2, sp, #8
	lsrs	r3, r1, #4
	strb	r3, [r4, #4]
	lsrs	r3, r1, #12
	strb	r3, [r4, #5]
	lsrs	r3, r1, #20
	strb	r3, [r4, #6]
	mov	r3, fp
	ldrb	r2, [r2, #0]
	strb	r2, [r4, #7]
	cmp	r3, #0
	beq.n	.L_0200204c
	add	r5, sp, #12
	ldrb	r5, [r5, #0]
	strb	r5, [r4, #8]
.L_0200204c:
	ldr	r0, [sp, #24]
	add	sp, #60	@ 0x3c
	pop	{r3, r5, r6, r7}
	mov	r8, r3
	mov	r9, r5
	mov	sl, r6
	mov	fp, r7
	pop	{r5, r6, r7, pc}
	.4byte 0x0200a6f0
	.4byte 0x0200a71c
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, r6, r7, lr}
	mov	r7, fp
	mov	r6, sl
	mov	r5, r9
	push	{r5, r6, r7}
	mov	r7, r8
	push	{r7}
	sub	sp, #268	@ 0x10c
	mov	sl, r2
	movs	r3, #11
	movs	r2, #0
	adds	r7, r1, #0
	mov	fp, r0
	movs	r4, #2
	str	r2, [sp, #4]
	mov	r8, r2
	mov	r9, r3
	cmp	r7, #16
	beq.n	.L_0200209a
	movs	r2, #41	@ 0x29
	mov	r9, r2
	cmp	r7, #61	@ 0x3d
	beq.n	.L_0200209a
	movs	r3, #175	@ 0xaf
	mov	r9, r3
.L_0200209a:
	mov	r2, r9
	movs	r6, #0
	cmp	r2, #0
	beq.n	.L_020020b0
	mov	r2, sl
.L_020020a4:
	movs	r3, #0
	adds	r6, #1
	strb	r3, [r2, #0]
	adds	r2, #1
	cmp	r6, r9
	bne.n	.L_020020a4
.L_020020b0:
	movs	r6, #0
	cmp	r7, #0
	beq.n	.L_020020ce
	mov	r1, fp
.L_020020b8:
	ldrb	r3, [r1, #0]
	add	r2, sp, #8
	strb	r3, [r2, r6]
	subs	r3, r3, r6
	movs	r2, #63	@ 0x3f
	ands	r3, r2
	adds	r6, #1
	strb	r3, [r1, #0]
	adds	r1, #1
	cmp	r6, r7
	bne.n	.L_020020b8
.L_020020ce:
	mov	r5, r8
	movs	r6, #0
	add	r5, sl
	b.n	.L_020020fe
.L_020020d6:
	adds	r4, #8
.L_020020d8:
	ldrb	r3, [r5, #0]
	lsls	r1, r4
	adds	r3, r3, r1
	strb	r3, [r5, #0]
	movs	r3, #6
	subs	r4, #6
	negs	r3, r3
	cmp	r4, r3
	bne.n	.L_020020f6
	movs	r2, #1
	add	r8, r2
	movs	r4, #2
	adds	r5, #1
	cmp	r8, r9
	beq.n	.L_02002132
.L_020020f6:
	ldr	r3, [sp, #4]
	adds	r6, #1
	adds	r3, #1
	str	r3, [sp, #4]
.L_020020fe:
	cmp	r6, r7
	beq.n	.L_02002132
	adds	r0, r6, #0
	movs	r1, #10
	str	r4, [sp, #0]
	bl	sub_020042d2
	ldr	r4, [sp, #0]
	cmp	r0, #8
	bgt.n	.L_020020f6
	ldr	r3, [sp, #4]
	mov	r2, fp
	ldrb	r1, [r2, r3]
	cmp	r4, #0
	bge.n	.L_020020d8
	negs	r3, r4
	adds	r2, r1, #0
	asrs	r2, r3
	ldrb	r3, [r5, #0]
	adds	r3, r3, r2
	movs	r2, #1
	add	r8, r2
	strb	r3, [r5, #0]
	adds	r5, #1
	cmp	r8, r9
	bne.n	.L_020020d6
.L_02002132:
	mov	r3, sl
	add	r3, r8
	mov	r0, r8
	subs	r3, #1
	subs	r0, #1
	ldrb	r4, [r3, #0]
	movs	r6, #0
	cmp	r0, #0
	beq.n	.L_02002156
	mov	r1, sl
.L_02002146:
	ldrb	r2, [r1, #0]
	adds	r3, r4, #0
	eors	r3, r2
	adds	r6, #1
	strb	r3, [r1, #0]
	adds	r1, #1
	cmp	r6, r0
	bne.n	.L_02002146
.L_02002156:
	movs	r6, #0
	cmp	r7, #0
	beq.n	.L_0200216c
	mov	r0, fp
.L_0200215e:
	add	r3, sp, #8
	ldrb	r3, [r3, r6]
	adds	r6, #1
	strb	r3, [r0, #0]
	adds	r0, #1
	cmp	r6, r7
	bne.n	.L_0200215e
.L_0200216c:
	mov	r0, r8
	add	sp, #268	@ 0x10c
	pop	{r3, r5, r6, r7}
	mov	r8, r3
	mov	r9, r5
	mov	sl, r6
	mov	fp, r7
	pop	{r5, r6, r7, pc}
	push	{r5, r6, r7, lr}
	adds	r6, r0, #0
	movs	r0, #255	@ 0xff
	lsls	r0, r0, #8
	adds	r0, #255	@ 0xff
	movs	r5, #0
	cmp	r6, #0
	beq.n	.L_020021b4
.L_0200218c:
	ldrb	r3, [r1, #0]
	movs	r4, #0
	lsls	r3, r3, #8
	eors	r0, r3
.L_02002194:
	movs	r3, #128	@ 0x80
	ldr	r7, [pc, #36]	@ (0x20021bc)
	lsls	r3, r3, #8
	ands	r3, r0
	lsls	r2, r0, #1
	adds	r4, #1
	adds	r0, r2, r7
	cmp	r3, #0
	bne.n	.L_020021a8
	adds	r0, r2, #0
.L_020021a8:
	cmp	r4, #8
	bne.n	.L_02002194
	adds	r5, #1
	adds	r1, #1
	cmp	r5, r6
	bne.n	.L_0200218c
.L_020021b4:
	mvns	r0, r0
	lsls	r0, r0, #16
	lsrs	r0, r0, #16
	pop	{r5, r6, r7, pc}
	.4byte 0xffffefdf
	.irp EntryTarget, 0x03000528, 0x03000508, 0x080000c1, 0x080000d1, 0x080000d9, 0x08000171, 0x08000179, 0x080001a9, 0x080001d1, 0x080001f1, 0x08000291, 0x080002a9, 0x08000301, 0x08000371, 0x08000389, 0x080003c1, 0x080003c9, 0x080003d1, 0x080003d9, 0x08038011, 0x08038019, 0x08038041, 0x08038061, 0x08038071, 0x08038079, 0x08038081, 0x08038099, 0x080380c9, 0x08038141, 0x080381f1, 0x080381f9, 0x08038211, 0x08038261, 0x08038271, 0x080382c1, 0x080382c9, 0x080382d1, 0x080382d9, 0x080382f1, 0x080382f9, 0x08038301, 0x08038309, 0x08038349, 0x08038379, 0x080383b9, 0x08038429, 0x08038431, 0x080ad001, 0x080ad009, 0x080ad029, 0x080ad091, 0x080ad0f9, 0x080ad111, 0x080ad201, 0x080ad239, 0x080ad249, 0x080ad299, 0x080ad2c9, 0x080ad2e1, 0x080ad321, 0x080ad331, 0x080c8011, 0x080c8089, 0x080c8269, 0x080c83a9, 0x080c83b1, 0x080c83b9, 0x080c8559, 0x080c8941, 0x080f8011, 0x08108031, 0x08108041, 0x08108049, 0x081c0011
	overlay_veneer \EntryTarget
	.endr
	.4byte 0x00040004
	.4byte 0x00000004
	.4byte 0x00800040
	.4byte 0x00800040
	.4byte 0x00100020
	.4byte 0x00100020
	.4byte 0x00100040
	.4byte 0x00200080
	.4byte 0x00040040
	.4byte 0x00000000
	.4byte 0x03020100
	.4byte 0x00010203
	.4byte 0x44434241
	.4byte 0x47462045
	.4byte 0x204b4a48
	.4byte 0x504e4d4c
	.4byte 0x00522051
	.4byte 0x56555453
	.4byte 0x59582057
	.4byte 0x2033325a
	.4byte 0x37363534
	.4byte 0x00392038
	.4byte 0x64636261
	.4byte 0x67662065
	.4byte 0x206a6968
	.4byte 0x706e6d6b
	.4byte 0x00722071
	.4byte 0x76757473
	.4byte 0x79782077
	.4byte 0x203f217a
	.4byte 0x25242623
	.4byte 0x003d202b
	.4byte 0x09510941
	.4byte 0x08d108b3
	.4byte 0x0868081e
	.4byte 0xffff0000
	.4byte 0x00000000
	.4byte 0x40000000
	.4byte 0xffff0000
	.4byte 0xffffffff
	.4byte 0x0000ffff
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x000001ff
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0xffffffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x8fc22100
	.4byte 0x250d09f0
	.4byte 0x3f08b7c2
	.4byte 0xf0c97c32
	.4byte 0x0ca7c327
	.4byte 0x907c32bf
	.4byte 0x5fc3d5f8
	.4byte 0xfc3d9f0f
	.4byte 0xb9ddf0f6
	.4byte 0x1f0c3fc2
	.4byte 0xf0c4fc31
	.4byte 0xd45fc315
	.4byte 0xfc7c32f5
	.4byte 0x78652df0
	.4byte 0x6d4b4f21
	.4byte 0x0ccfc331
	.4byte 0xf7ef535f
	.4byte 0xc3e304f0
	.4byte 0x5e7f0f97
	.4byte 0x6df51936
	.4byte 0x0751d1d4
	.4byte 0xc79c1b9b
	.4byte 0xc339f0cd
	.4byte 0x33df0cef
	.4byte 0xe9bd4ffc
	.4byte 0xd1d7ac75
	.4byte 0x863e025e
	.4byte 0x7fa5ab0f
	.4byte 0x1a7efff8
	.4byte 0x87821ebc
	.4byte 0x78be1e1f
	.4byte 0x93e1e3f8
	.4byte 0xb97bc1cb
	.4byte 0x7ebe1f9a
	.4byte 0x031efbf8
	.4byte 0x9f1e98b6
	.4byte 0xe1e59f81
	.4byte 0x1e7f879b
	.4byte 0xe9f87a3e
	.4byte 0x68ebf37a
	.4byte 0x0fbf63a8
	.4byte 0x9f9f83c1
	.4byte 0x7fc1e6f8
	.4byte 0xe60cc1f3
	.4byte 0x65f0d8c1
	.4byte 0x9f0d9fc3
	.4byte 0xc95afc36
	.4byte 0xe5fc3950
	.4byte 0x6fc399f0
	.4byte 0x6a59df0e
	.4byte 0x351f0d3c
	.4byte 0x55f0d4fc
	.4byte 0x09f55fc3
	.4byte 0xb41e6208
	.4byte 0x7c36ff0d
	.4byte 0xc373f0dc
	.4byte 0x39fbd5d7
	.4byte 0xa3f0e87c
	.4byte 0x7f0e97c3
	.4byte 0xd564025a
	.4byte 0x574755b7
	.4byte 0x676f41e7
	.4byte 0x37707982
	.4byte 0x7bf0de7c
	.4byte 0xfa10f7c6
	.4byte 0x1d6a6f57
	.4byte 0xd6b475ab
	.4byte 0xe027d5fe
	.4byte 0x1da27e1f
	.4byte 0x083c79ce
	.4byte 0xf8707e1c
	.4byte 0x870fe1c2
	.4byte 0x5e072c4f
	.4byte 0x7e1d8f87
	.4byte 0xe1daf876
	.4byte 0xd410fb6f
	.4byte 0xc1a818f2
	.4byte 0xbe1c583c
	.4byte 0xe1c7f871
	.4byte 0xac9f8723
	.4byte 0xa68eb72f
	.4byte 0x14fb763a
	.4byte 0x90ed8394
	.4byte 0x270fc791
	.4byte 0x09f0a7c2
	.4byte 0xfbc22b0d
	.4byte 0x01000000
	.4byte 0x05040302
	.4byte 0x09080706
	.4byte 0x0d0c0b0a
	.4byte 0x11100f0e
	.4byte 0x15141312
	.4byte 0x19181716
	.4byte 0x1d1c1b1a
	.4byte 0x21201f1e
	.4byte 0x25242322
	.4byte 0x29282726
	.4byte 0x2d2c2b2a
	.4byte 0x31302f2e
	.4byte 0x35343332
	.4byte 0x39383736
	.4byte 0x3d3c3b3a
	.4byte 0x00003f3e
	.4byte 0x00000000
	.4byte 0x00000001
	.4byte 0x00000002
	.4byte 0x00000003
	.4byte 0x00c900c8
	.4byte 0x00cb00ca
	.4byte 0x00cd00cc
	.4byte 0x00cf00ce
	.4byte 0x00b500b4
	.4byte 0x00b700b6
	.4byte 0x00bb00ba
	.4byte 0x00bd00bc
	.4byte 0x00c000bf
	.4byte 0x00c200c1
	.4byte 0x00c400c3
	.4byte 0x00e300e2
	.4byte 0x00e500e4
	.4byte 0x00ee00ec
	.4byte 0x00f000ef
	.4byte 0x000000f1
	.4byte 0x00000000
	.4byte 0x00000001
	.4byte 0x00000002
	.4byte 0x00000003
	.4byte 0x09510941
	.4byte 0x08d108b3
	.4byte 0x0868081e
	.4byte 0x00c900c8
	.4byte 0x00cb00ca
	.4byte 0x00cd00cc
	.4byte 0x00cf00ce
	.4byte 0x00b500b4
	.4byte 0x00b700b6
	.4byte 0x00bb00ba
	.4byte 0x00bd00bc
	.4byte 0x00c000bf
	.4byte 0x00c200c1
	.4byte 0x00c400c3
	.4byte 0x00e300e2
	.4byte 0x00e500e4
	.4byte 0x00ee00ec
	.4byte 0x00f000ef
	.2byte 0x00f1
