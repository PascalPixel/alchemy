...
fn quoted_field(line: &str, key: &str) -> Option<String> {
    ...
    line.trim().starts_with(&format!("\"{key}\"")).then(|| {
        value
            .trim()
            .trim_end_matches(',')
            .trim_matches('"')
...
fn integer_field(line: &str, key: &str) -> Option<usize> {
    ...
    line.trim()
        ...
        .then(|| value.trim().trim_end_matches(',').parse::<usize>().ok())?
}
...
fn registered_overlay_span(name: &str, address: i64) -> Result<usize, String> {
    ...
    for line in text.lines() {
        ...
        if line.trim().starts_with('}')
            && overlay.as_deref() == Some(name)
            && entry == Some(address)
        {
            return span.ok_or_else(|| {
                format!(
                    "{name} 0x{address:08x} has no span_bytes in {}",
                    path.display()
                )
            });
        }
    ...
    Err(format!(
        "{name} 0x{address:08x} has no registered owner span in {}",
        path.display()
    ))
}
    ...
    pub fn prepare(original: PathBuf, base_source: &str) -> Result<Self, String> {
        ...
        if let Some(name) = overlay_name(&candidate_stem) {
            ...
            if expected.len() != span {
                return Err(format!(
                    "{} registered span extends beyond {name} reference image",
                    original.display()
                ));
            }
...
pub fn self_test() -> Result<(), String> {
    ...
    if overlay_name("resource_373_c_02005b48") != Some("resource_373".into())
        || overlay_name("08012345").is_some()
    {
