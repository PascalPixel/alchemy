...
//
//   bun tools/semantic/index.ts <subcommand> [args...]
//   bun tools/semantic/index.ts --list
...
const RUST_SUBCOMMANDS: Record<string, string> = {
  semantic_owner_scope: "tools-rs/semantic-owner-scope/target/release/semantic-owner-scope",
  semantic_queue: "tools-rs/semantic-queue/target/release/semantic-queue",
  semantic_superseded: "tools-rs/semantic-superseded/target/release/semantic-superseded",
};
...
async function main(): Promise<void> {
  ...
  if (subcommand === undefined || subcommand === "--list" || subcommand === "--help") {
    console.log("usage: semantic <subcommand> [args...]");
    for (const name of available) console.log(`  ${name}`);
    return;
  }
  if (subcommand === "--self-test") {
    if (available.length === 0) throw new Error("semantic has no subcommands");
    console.log(`semantic self-test ok (${available.length} subcommands)`);
    return;
  }
