...
use crate::{cohort_digest, cohort_summary, member_report_path, root, self_test, Report};
...
mod tests {
    ...
    fn console_block_matches_the_native_shape() {
        let reports = crate::self_test_reports().unwrap();
    ...
    fn collate_is_reachable_from_the_binary() {
        assert_eq!(crate::collate("a-b", "a+b"), std::cmp::Ordering::Less);
    }
