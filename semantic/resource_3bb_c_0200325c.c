#include "types.h"

/* STILL-OPEN: adopt --where differing_bytes=33/148. alchemist.ts exhausted
 * (tiers: original-order, priority, unaligned; 34 licensed compiles tried,
 * no improving move found). overlay_mode_cohort singles sweep: no config
 * closes it either. Genuinely unfixed within budget. */








/*
 * resource_3bb owner at 0x0200325c, 148 bytes: occasionally clone an object
 * at a randomized nearby position.  The final eight bytes are this owner's
 * object-kind and callback pool words; the next prologue is at 0x020032f0.
 */
extern u32 Func_02007030(void);
extern u32 Func_0200704c(void);
extern u32 Func_02007052(void);
extern void Func_0200706e();
extern u8 * Func_0200710a();
extern void Func_02007166();
extern void Func_0200711e();
extern void Func_02007126();
void Func_0200325c(u8 *object)
{
    s32 position[3];
    u32 random_x;
    u32 random_z;
    u8 *clone;

    if ((u32)(*(s32 *)(object + 40) + 255) <= 510)
        object[0x55] = 0;

    if (((Func_02007030() * 100) >> 16) > 9)
        return;

    position[0] = *(s32 *)(object + 8);
    position[1] = *(s32 *)(object + 12);
    position[2] = *(s32 *)(object + 16);
    random_x = Func_0200704c();
    random_z = Func_02007052();
    Func_0200706e(random_x << 4, random_z, position);

    clone = Func_0200710a(0x11d, position[0], position[1], position[2]);
    if (clone != 0) {
        clone[0x55] = 0;
        Func_02007166(clone, 0);
        Func_0200711e(clone, (void *)0x0200c264);
        Func_0200711e(clone, 1);
        Func_02007126(clone, 0);
    }
}
