...
extern u8 Data_03001eec[];   /* heap-block cache: [0]=work,[1]=canvas,[2]=extra,[46]=rect46,[47]=rect47 */
extern u8 Value_00000085;
extern u8 Value_00000073;
extern u8 Value_00000086;
extern u8 Value_00000087;    /* kind==2 palette id: unresolved placeholder */
extern u8 Data_080ee2a9[];   /* per-slot exit-frame-threshold byte table */
extern u8 Data_080ee29a[];   /* active-member-count byte table, indexed by kind-ish field */
extern s16 Data_080ede48[];  /* signed halfword lookup used by the draw-position curve */
extern u8 Data_080ede9f[];   /* per-cell frame-window byte table (puff_arc-style) */
...
void Func_080d4ce8(void *object)
{
    ...
    rectangle[0] = (void *)heap[46];
    status = Func_080ed408(47, 7, 7, 7, 2);
    second_rectangle = (void *)heap[47];
    ...
    for (i = 0; i != 4; i++) {
        ...
        anchor->unk0C = 0;
        anchor->bias =
            (s32)Data_080ede9f[(efx->unk1C << 2) + i] << 16;
    ...
    for (frame = 0; frame != 96; frame++) {
        ...
        if (frame <= 63) {
            if (Data_080ee29a[efx->unk1C] != 0) {
                flag = frame & 1;
                cursor = work + 0x7080;
                cursor2 = work + 0x6e80;
                cell = &Data_080ede9f[0];
                slot = 0;
                ...
                do {
                    ...
                    cursor = (u8 *)cursor + 28;
                    cell++;
                } while (slot != Data_080ee29a[efx->unk1C]);
