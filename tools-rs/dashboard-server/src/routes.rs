...
fn text_response(status: u16, reason: &'static str, body: &str, content_type: &str) -> Response {
    Response::new(status, reason, header_block(Some(content_type), "no-store"), body.as_bytes().to_vec())
}

/// Read a file for a route.
///
/// PORT NOTE: `new Response(Bun.file(path))` on a MISSING file is a 200 with
/// an empty body in Bun, not a 404. Reproduced: an unreadable file yields an
/// empty 200 body rather than an error page.
...
pub fn svg_tree_id(path: &str) -> Option<&'static str> {
    ...
    state::TREE_LABELS.iter().map(|(id, _)| *id).find(|id| *id == rest)
}

pub fn respond(path: &str, method: &str) -> RouteOutcome {
    if method != "GET" && method != "HEAD" {
        // Not a route the oracle has; the oracle answers every method with
        // the same body. Reproduced by ignoring the method for dispatch and
        // suppressing the body for HEAD at the writer.
    }
    match path {
            ...
            Some(tree) => {
                let svg = state::with_state(|current| {
                    current
                        ...
                        .as_ref()
                        .and_then(|live| {
                            live.trees.iter().find(|(id, _)| *id == tree).map(|(_, svg)| svg.clone())
                        })
                        .ok_or_else(|| {
                            current
                                .scan_error
...
mod tests {
    ...
    fn encode_uri_component_matches_the_javascript_unreserved_set() {
        assert_eq!(encode_uri_component("1785756380214.9888:17857.5"), "1785756380214.9888%3A17857.5");
