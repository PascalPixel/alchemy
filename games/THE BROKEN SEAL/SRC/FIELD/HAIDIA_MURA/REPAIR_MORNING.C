#include "TYPES.H"
#include "FIELD_EVENT.H"
#include "FIELD_SCENE.H"

/*
 * Outside the protagonist's house in Vale. He boards up three holes in the
 * wall while his mother watches, then she talks with him about his father
 * until Garet and Jenna arrive to fetch him for the trip to Mt. Aleph with
 * Kraden.
 */

enum HouseActor {
    ACTOR_JENNA = 5,
    ACTOR_MOTHER = 21,
    ACTOR_BOARD = 23,
    ACTOR_LAST_BOARD = 24
};

enum {
    ANIM_HAMMER = 11
};

extern const u8 gLeaderHammerAction[];
extern const u8 gGaretAction[];
extern const u8 gJennaAction[];

void SceneActor_ResetActorRun(s32 first, u32 count, s32 mode);

/*
 * An actor climbs between the yard and the ledge in front of the house at
 * x 392, playing one animation for each half of the climb.
 */
void HouseScene_ClimbUp(s32 actor, s32 animation, s32 next_animation, s32 grounded);
void HouseScene_ClimbDown(s32 actor, s32 animation, s32 next_animation, s32 grounded);

void HouseScene_RunRepairMorning(void)
{
    struct FieldActor *leader;
    struct FieldActor *actor;
    u8 leader_motion_flags;
    u8 *leader_motion;
    s32 i;

    Event_Begin();
    Camera_MoveTo(-1, -1, -1, 0);
    actor = Event_GetViewCenter();
    actor->motion_flags = 0;
    Camera_MoveTo(PIXELS(383), PIXELS(160), PIXELS(877), 0);
    Event_Wait(1);
    Map_Redraw();
    Map_CopyCellAttributes(49, 41, 7, 3, 20, 50);
    Map_CopyCellsTo(2, 102, 84, 41, 2, 1);
    Map_CopyCellsTo(1, 102, 83, 41, 1, 1);
    Map_CopyCellsTo(0, 103, 82, 42, 1, 1);
    leader = Actor_Get(ACTOR_PARTY_LEADER);
    leader_motion = &leader->motion_flags;
    leader_motion_flags = *leader_motion;
    *leader_motion = 0;
    Actor_SetPosition(ACTOR_PARTY_LEADER, PIXELS(407), PIXELS(690));
    Actor_SetPosition(ACTOR_MOTHER, PIXELS(392), PIXELS(896));
    Actor_SetPosition(ACTOR_GARET, PIXELS(298), PIXELS(736));
    Actor_SetPosition(ACTOR_JENNA, PIXELS(298), PIXELS(760));
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_NORTH, 0);
    Actor_FaceDirection(ACTOR_GARET, FACING_WEST, 0);
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST, 0);
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_HAMMER);
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, gLeaderHammerAction);
    SceneActor_ResetActorRun(ACTOR_BOARD, 2, 1);
    gEventWork->start_transition = 0;
    gEventWork->transition_frames = 32;
    Event_OpenScreen();
    Actor_SetSpeed(ACTOR_JENNA, 0x8000, 0x4000);
    Actor_SetSpeed(ACTOR_GARET, 0x8000, 0x4000);
    Actor_EnableActionCallback(ACTOR_JENNA, gJennaAction);
    Actor_EnableActionCallback(ACTOR_GARET, gGaretAction);

    /* The first hole. */
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, ACTION_TABLE_STOP);
    leader->scale_x = 0x10000;
    leader->scale_y = 0x10000;
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTHWEST + FACING_STEP, 40);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(10);
    Actor_SetSpeed(ACTOR_PARTY_LEADER, 0x4ccc, 0x2666);
    Actor_WalkToAndWait(ACTOR_PARTY_LEADER, 404, 843);
    Event_Wait(10);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 30);
    Actor_RunRepeatedMotion(ACTOR_PARTY_LEADER, 1);
    Event_Wait(20);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_WEST, 40);
    Actor_RunRepeatedMotion(ACTOR_PARTY_LEADER, 2);
    Event_Wait(20);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 2);
    Actor_SetChildValue(ACTOR_BOARD, 2);
    Actor_SetSpeed(ACTOR_BOARD, 0x4ccc, 0x2666);
    Actor_MoveToAndWait(ACTOR_BOARD, 390, 832);
    Event_Wait(80);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_MoveToAndWait(ACTOR_BOARD, 402, 828);
    Event_Wait(80);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 0);
    Actor_SetChildValue(ACTOR_BOARD, 0);
    Actor_SetPosition(ACTOR_BOARD, PIXELS(390), PIXELS(842));
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_HAMMER);
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, gLeaderHammerAction);
    Event_Wait(200);
    Map_CopyCellsTo(7, 102, 84, 41, 2, 1);

    /* The second hole. */
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, ACTION_TABLE_STOP);
    leader->scale_x = 0x10000;
    leader->scale_y = 0x10000;
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_STAND);
    Event_Wait(30);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(20);
    Actor_WalkToAndWait(ACTOR_PARTY_LEADER, 377, 843);
    Event_Wait(10);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 30);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_EAST, 20);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 2);
    Actor_SetChildValue(ACTOR_BOARD, 2);
    Actor_SetSpeed(ACTOR_BOARD, 0x4ccc, 0x2666);
    Actor_MoveToAndWait(ACTOR_BOARD, 390, 832);
    Event_Wait(80);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_MoveToAndWait(ACTOR_BOARD, 377, 828);
    Event_Wait(80);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 0);
    Actor_SetChildValue(ACTOR_BOARD, 0);
    Actor_SetPosition(ACTOR_BOARD, 0, 0);
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_HAMMER);
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, gLeaderHammerAction);
    Event_Wait(200);
    Map_CopyCellsTo(6, 102, 83, 41, 1, 1);

    /* The third hole. */
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, ACTION_TABLE_STOP);
    leader->scale_x = 0x10000;
    leader->scale_y = 0x10000;
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_STAND);
    Event_Wait(30);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(20);
    Actor_WalkToAndWait(ACTOR_PARTY_LEADER, 360, 855);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_NORTHWEST + FACING_STEP, 10);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 30);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 20);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 2);
    Actor_SetChildValue(ACTOR_LAST_BOARD, 2);
    Actor_SetSpeed(ACTOR_LAST_BOARD, 0x4ccc, 0x2666);
    Actor_MoveToAndWait(ACTOR_LAST_BOARD, 390, 832);
    Event_Wait(80);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_MoveToAndWait(ACTOR_LAST_BOARD, 360, 842);
    Event_Wait(80);
    Actor_SetChildValue(ACTOR_PARTY_LEADER, 0);
    Actor_SetChildValue(ACTOR_LAST_BOARD, 0);
    Actor_SetPosition(ACTOR_LAST_BOARD, 0, 0);
    Actor_SetAnimation(ACTOR_PARTY_LEADER, ANIM_HAMMER);
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, gLeaderHammerAction);
    Event_Wait(200);
    Map_CopyCellsTo(5, 103, 82, 42, 1, 1);
    Actor_EnableActionCallback(ACTOR_PARTY_LEADER, ACTION_TABLE_STOP);
    leader->scale_x = 0x10000;
    leader->scale_y = 0x10000;

    /* His mother praises the work and talks about his father. */
    Event_SetMessage(0xf03);
    Actor_Jump(ACTOR_MOTHER, 2, 20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_STEP, 20);
    HouseScene_ClimbUp(ACTOR_MOTHER, 5, 6, 0);
    Actor_SetSpeed(ACTOR_MOTHER, 0x4ccc, 0x2666);
    Actor_WalkToAndWait(ACTOR_MOTHER, 397, 832);
    Event_Wait(20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_NORTH, 60);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_WalkToAndWait(ACTOR_MOTHER, 372, 832);
    Event_Wait(20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 40);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_WEST, 40);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 30);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_RunRepeatedMotion(ACTOR_PARTY_LEADER, 2);
    Event_Wait(40);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(20);
    Event_OpenMessage(ACTOR_MOTHER, 0);
    if (Event_ChooseYesNo(ACTOR_PARTY_LEADER, 0) == 0) {
        gEventWork->message++;
    }
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Event_SetMessage(0xf0a);
    Actor_WalkToAndWait(ACTOR_MOTHER, 386, 841);
    Event_Wait(10);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_NORTH + FACING_STEP, 60);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 30);
    Event_OpenMessage(ACTOR_MOTHER, 0);
    if (Event_ChooseYesNo(ACTOR_PARTY_LEADER, 0) == 1) {
        gEventWork->message++;
    }
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_NORTH + FACING_STEP, 60);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(20);
    Event_SetMessage(0xf0e);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_WalkToAndWait(ACTOR_MOTHER, 386, 825);
    Event_Wait(10);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(60);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 10);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_WalkToAndWait(ACTOR_MOTHER, 372, 832);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 20);
    Actor_RunRepeatedMotion(ACTOR_PARTY_LEADER, 2);
    Event_Wait(20);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_ShowEmote(ACTOR_PARTY_LEADER, EMOTE_IN_FRONT | 2, 60);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(30);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 10);

    /* Garet and Jenna arrive. */
    Camera_SetSpeed(0x6666, 0xccc);
    Camera_MoveTo(PIXELS(377), PIXELS(160), PIXELS(860), 1);
    Actor_SetSpeed(ACTOR_JENNA, 0x10000, 0x8000);
    Actor_SetSpeed(ACTOR_GARET, 0x10000, 0x8000);
    Actor_WalkTo(ACTOR_GARET, 369, 904);
    Actor_WalkToAndWait(ACTOR_JENNA, 392, 904);
    Actor_SetAnimation(ACTOR_GARET, ANIM_STAND);
    HouseScene_ClimbUp(ACTOR_JENNA, 10, 11, 0);
    Actor_FaceDirection(ACTOR_JENNA, FACING_NORTHWEST, 0);
    Event_ShowMessageAndWait(ACTOR_JENNA, 0, 10);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(10);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTHEAST + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_STEP, 30);
    Actor_Jump(ACTOR_JENNA, 4, 0);
    Actor_WalkToAndWait(ACTOR_JENNA, 392, 843);
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTHEAST + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 40);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_JENNA, 0, 20);
    Actor_SetAnimation(ACTOR_MOTHER, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    HouseScene_ClimbUp(ACTOR_GARET, 10, 11, 0);
    Actor_SetSpeed(ACTOR_JENNA, 0x4ccc, 0x2666);
    Actor_SetSpeed(ACTOR_GARET, 0x4ccc, 0x2666);
    Actor_WalkTo(ACTOR_GARET, 392, 843);
    Actor_Get(ACTOR_JENNA)->unknown_5a &= 0xfe;
    Actor_WalkToAndWait(ACTOR_JENNA, 408, 843);
    Event_Wait(1);
    Actor_Get(ACTOR_JENNA)->unknown_5a |= 1;
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST, 0);
    Actor_WaitForMove(ACTOR_GARET);
    Actor_SetAnimation(ACTOR_GARET, ANIM_STAND);
    Actor_FaceDirection(ACTOR_GARET, FACING_WEST, 30);
    Actor_Jump(ACTOR_MOTHER, 4, 30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimationAndWait(ACTOR_GARET, ANIM_NOD);
    Event_Wait(30);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 0);
    Actor_Jump(ACTOR_PARTY_LEADER, 2, 30);
    Actor_SetAttachedEffect(ACTOR_PARTY_LEADER, EMOTE_IN_FRONT | 2);
    Event_Wait(60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTHEAST + FACING_STEP, 40);
    Actor_RunRepeatedMotion(ACTOR_GARET, 2);
    Event_ShowMessageAndWait(ACTOR_GARET, 0, 20);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 1, 80);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 30);
    Actor_ShowEmote(ACTOR_PARTY_LEADER, EMOTE_IN_FRONT | 2, 80);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTHEAST + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 40);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimationAndWait(ACTOR_GARET, ANIM_NOD);
    Event_Wait(100);
    Actor_FaceEachOther(ACTOR_JENNA, ACTOR_GARET, 30);
    Actor_StartRepeatedMotion(ACTOR_GARET, 2);
    Actor_RunRepeatedMotion(ACTOR_JENNA, 2);
    Event_Wait(10);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 5, 60);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST, 0);
    Actor_FaceDirection(ACTOR_GARET, FACING_WEST, 30);
    Actor_RunRepeatedMotion(ACTOR_JENNA, 2);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_JENNA, 0, 20);
    Actor_StartRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimation(ACTOR_GARET, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(30);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimation(ACTOR_GARET, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(10);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimation(ACTOR_GARET, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(10);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Camera_SetSpeed(0x9999, 0x1333);
    Camera_MoveTo(PIXELS(373), PIXELS(160), PIXELS(837), 1);
    Actor_WalkToAndWait(ACTOR_MOTHER, 364, 816);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTHEAST + FACING_STEP, 30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 40);
    Actor_FaceEachOther(ACTOR_JENNA, ACTOR_GARET, 30);
    Actor_StartRepeatedMotion(ACTOR_GARET, 2);
    Actor_RunRepeatedMotion(ACTOR_JENNA, 2);
    Event_Wait(30);
    Actor_FaceDirection(ACTOR_GARET, FACING_WEST, 0);
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST, 30);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH + FACING_STEP, 30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 30);
    Actor_ShowEmote(ACTOR_PARTY_LEADER, EMOTE_IN_FRONT | 5, 60);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(10);
    Event_OpenMessage(ACTOR_MOTHER, 0);
    if (Event_ChooseYesNo(ACTOR_PARTY_LEADER, 0) == 1) {
        gEventWork->message++;
    }
    Event_Wait(40);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);

    /* She slips from the step; Garet catches her. */
    Event_SetMessage(0xf27);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 3, 0);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 3);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_Jump(ACTOR_MOTHER, 4, 0);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 3);
    Actor_SetAnimation(ACTOR_MOTHER, 7);
    Event_Wait(5);
    Event_ShowTwoMessagesAndWait(ACTOR_MOTHER, 14, 2, 24, 2, ACTOR_GARET, 10, 14, 4, 14, 0);
    actor = Actor_Get(ACTOR_MOTHER);
    actor->sprite->flags = 0;
    actor->unknown_5a &= 0xfe;
    Actor_SetSpeed(ACTOR_MOTHER, 0x30000, 0x18000);
    Actor_WalkToAndWait(ACTOR_MOTHER, 364, 815);
    Event_Wait(4);
    for (i = 0; i != 4; i++) {
        actor->z.fixed += 0x18000;
        actor->scale_y -= 0x1999;
        Event_Wait(1);
    }
    Actor_SetPosition(ACTOR_MOTHER, 0, 0);
    Actor_SetSpeed(ACTOR_GARET, 0x30000, 0x18000);
    Actor_Jump(ACTOR_GARET, 6, 0);
    Actor_WalkToAndWait(ACTOR_GARET, 374, 827);
    Event_ShowMessage(ACTOR_JENNA, 0);
    Actor_FaceDirection(ACTOR_GARET, FACING_NORTHWEST + FACING_STEP, 0);
    Actor_ShowEmote(ACTOR_JENNA, EMOTE_IN_FRONT | 0, 0);
    Actor_StartRepeatedMotion(ACTOR_JENNA, 2);
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 0, 10);
    Actor_SetAnimation(ACTOR_GARET, 13);
    Actor_Jump(ACTOR_GARET, 2, 5);
    MapRender_SetValues(0, 0x40000, 0x10000);
    Map_CopyCellsTo(1, 102, 83, 41, 1, 1);
    Actor_SetSpriteFlags(Actor_Get(ACTOR_GARET), 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 10);
    Actor_StartRepeatedMotion(ACTOR_GARET, 3);
    MapRender_SetValues(-1, -1, 0xe666);
    MapRender_WaitForValues();
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 2, 80);
    Actor_SetAnimation(ACTOR_MOTHER, 8);
    actor->scale_y = 0x8000;
    Actor_SetPosition(ACTOR_MOTHER, PIXELS(364), PIXELS(811));
    for (i = 0; i != 5; i++) {
        actor->scale_y += 0x1999;
        Event_Wait(1);
    }
    Event_Wait(60);
    Actor_RunRepeatedMotion(ACTOR_GARET, 2);
    Actor_FaceDirection(ACTOR_GARET, FACING_SOUTH + FACING_STEP, 30);
    Actor_StartRepeatedMotion(ACTOR_GARET, 2);
    Actor_RunRepeatedMotion(ACTOR_JENNA, 2);
    Event_Wait(60);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Camera_SetSpeed(0x4ccc, 0x999);
    Camera_MoveTo(PIXELS(372), PIXELS(160), PIXELS(859), 1);
    Actor_SetSpeed(ACTOR_MOTHER, 0x30000, 0x18000);
    Actor_Jump(ACTOR_MOTHER, 6, 0);
    Actor_WalkToAndWait(ACTOR_MOTHER, 359, 835);
    Event_Wait(30);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 30);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 2);
    Event_Wait(30);
    actor = Actor_Get(ACTOR_MOTHER);
    actor->priority_flags &= ~ACTOR_PRIORITY_AUTOMATIC;
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 80);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 1, 80);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_EAST, 60);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 3);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAttachedEffect(ACTOR_MOTHER, EMOTE_IN_FRONT | 2);
    Event_Wait(80);
    Actor_FaceDirection(ACTOR_GARET, FACING_SOUTH + FACING_STEP, 30);
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 2, 80);
    Actor_RunRepeatedMotion(ACTOR_GARET, 2);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_GARET, 0, 20);
    Actor_FaceDirection(ACTOR_GARET, FACING_WEST, 30);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    Event_Wait(10);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    Actor_SetSpriteFlags(Actor_Get(ACTOR_GARET), 1);
    Actor_Jump(ACTOR_GARET, 6, 0);
    Actor_SetAnimation(ACTOR_GARET, ANIM_STAND);
    Actor_SetSpeed(ACTOR_GARET, 0x40000, 0x20000);
    actor = Actor_Get(ACTOR_GARET);
    actor->unknown_5a &= 0xfe;
    Actor_SetDestination(ACTOR_GARET, 403, 827);
    Actor_SetAttachedEffect(ACTOR_JENNA, EMOTE_IN_FRONT | 2);
    Actor_FaceDirection(ACTOR_JENNA, FACING_NORTH, 20);
    Event_ShowMessageAndWait(ACTOR_JENNA, 0, 1);
    Actor_WaitForMove(ACTOR_GARET);
    Actor_FaceDirection(ACTOR_GARET, FACING_SOUTH + FACING_STEP, 20);
    Event_ShowMessageAndWait(ACTOR_GARET, 0, 20);
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 0, 0);
    Actor_SetAnimation(ACTOR_GARET, 13);
    Actor_Jump(ACTOR_GARET, 2, 5);
    Actor_SetSpriteFlags(Actor_Get(ACTOR_GARET), 0);
    Map_CopyCellsTo(2, 102, 84, 41, 2, 1);
    MapRender_SetValues(0, 0x40000, 0x10000);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    MapRender_SetValues(-1, -1, 0xe666);
    MapRender_WaitForValues();
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 2, 30);
    Actor_SetSpeed(ACTOR_JENNA, 0x4ccc, 0x2666);
    Actor_WalkToAndWait(ACTOR_JENNA, 408, 855);
    Event_Wait(60);
    Actor_RunRepeatedMotion(ACTOR_GARET, 2);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 5, 60);
    Actor_StartRepeatedMotion(ACTOR_JENNA, 3);
    Actor_RunRepeatedMotion(ACTOR_PARTY_LEADER, 3);
    Event_Wait(80);
    Actor_FaceDirection(ACTOR_GARET, FACING_SOUTH, 30);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    Event_Wait(10);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_SHAKE_HEAD);
    Event_Wait(80);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_FaceDirection(ACTOR_JENNA, FACING_NORTHWEST + FACING_STEP, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_EAST, 60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 60);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_EAST, 80);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 5, 80);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 60);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_ShowEmote(ACTOR_PARTY_LEADER, EMOTE_IN_FRONT | 1, 0);
    Actor_ShowEmote(ACTOR_JENNA, EMOTE_IN_FRONT | 1, 0);
    Actor_ShowEmote(ACTOR_GARET, EMOTE_IN_FRONT | 1, 60);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 70);
    Actor_StartRepeatedMotion(ACTOR_GARET, 2);
    Actor_RunRepeatedMotion(ACTOR_JENNA, 2);
    Event_Wait(20);
    Actor_FaceDirection(ACTOR_JENNA, FACING_WEST, 60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 30);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_JENNA, 0, 20);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_EAST, 30);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_SHAKE_HEAD);
    Event_Wait(20);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 20);
    Actor_SetAnimationAndWait(ACTOR_JENNA, ANIM_NOD);
    Event_Wait(20);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    Event_Wait(10);
    Event_ShowMessageAndWait(ACTOR_GARET, 0, 20);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH + FACING_STEP, 0);
    Actor_ShowEmote(ACTOR_MOTHER, EMOTE_IN_FRONT | 0, 0);
    Actor_RunRepeatedMotion(ACTOR_MOTHER, 3);
    Event_Wait(30);
    Event_ShowMessageAndWait(ACTOR_MOTHER, 0, 60);
    Actor_RunRepeatedMotion(ACTOR_GARET, 3);
    Actor_SetSpeed(ACTOR_GARET, 0x10000, 0x8000);
    Actor_SetSpriteFlags(Actor_Get(ACTOR_GARET), 0);
    Actor_Jump(ACTOR_GARET, 4, 0);
    Actor_WalkToAndWait(ACTOR_GARET, 398, 828);
    Event_Wait(60);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 60);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(60);
    Actor_SetAnimationAndWait(ACTOR_MOTHER, ANIM_NOD);
    Event_Wait(60);

    /* The three set off together. */
    actor = Actor_Get(ACTOR_GARET);
    actor->unknown_5a |= 1;
    actor = Actor_Get(ACTOR_JENNA);
    actor->unknown_5a |= 1;
    actor = Actor_Get(ACTOR_PARTY_LEADER);
    Actor_SetSpeed(ACTOR_GARET, 0x10000, 0x8000);
    Actor_SetSpeed(ACTOR_JENNA, 0x10000, 0x8000);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_EAST, 0);
    Actor_WalkTo(ACTOR_JENNA, actor->x.part.pixel + 16, actor->z.part.pixel);
    Actor_WalkToAndWait(ACTOR_GARET, actor->x.part.pixel + 16, actor->z.part.pixel - 16);
    Actor_WaitForMove(ACTOR_GARET);
    Actor_FaceDirection(ACTOR_GARET, FACING_SOUTH + FACING_STEP, 30);
    Actor_SetAnimation(ACTOR_GARET, ANIM_NOD);
    Actor_SetAnimation(ACTOR_JENNA, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(40);
    Actor_WalkToAndWait(ACTOR_JENNA, actor->x.part.pixel, actor->z.part.pixel);
    Actor_SetPosition(ACTOR_JENNA, 0, 0);
    Actor_WalkToAndWait(ACTOR_GARET, actor->x.part.pixel, actor->z.part.pixel);
    Actor_SetPosition(ACTOR_GARET, 0, 0);
    Party_AddMembers(ACTOR_GARET, ACTOR_JENNA);
    Camera_MoveTo(PIXELS(377), PIXELS(160), PIXELS(887), 1);
    HouseScene_ClimbDown(ACTOR_PARTY_LEADER, 13, 10, 0);
    Actor_WalkToAndWait(ACTOR_PARTY_LEADER, 376, 912);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 0);
    Actor_Get(ACTOR_MOTHER)->unknown_5a |= 1;
    HouseScene_ClimbDown(ACTOR_MOTHER, 6, 5, 0);
    Actor_WalkToAndWait(ACTOR_MOTHER, 373, 887);
    Actor_FaceDirection(ACTOR_MOTHER, FACING_SOUTH, 0);
    Actor_FaceDirection(ACTOR_PARTY_LEADER, FACING_NORTH, 40);
    Actor_SetAnimation(ACTOR_MOTHER, ANIM_NOD);
    Actor_SetAnimationAndWait(ACTOR_PARTY_LEADER, ANIM_NOD);
    Event_Wait(20);
    Camera_FollowActor(ACTOR_PARTY_LEADER, 1);
    Camera_WaitForMove();
    Event_Wait(100);
    GameFlag_Set(0x202);
    GameFlag_Clear(0x12f);
    *leader_motion = leader_motion_flags;
    Event_End();
}
