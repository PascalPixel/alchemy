...
pub fn entry(args: &[String]) -> ExitCode {
    let result = (|| -> Result<(), String> {
        ...
        if args.first().map(String::as_str) != Some("build-stdout") || args.len() < 2 || args.len() > 3 {
        ...
        let data = if let Some(text) = args.get(2) {
            ...
            return Err("build-stdout requires ADDRESS when the package has multiple typed sections".into());
        };
