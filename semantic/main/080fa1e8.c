 ...
 * at 0x080fa644 and at 0x080fb7ec; the interpreter calls the entry with the music
 * player in the first argument and the track in the second.
 *
 * One operand byte is consumed and stored as the depth of the second modulation at
 * track+0x17.  A zero operand means the second modulation is switched off, and that case
...
u8 Func_080fa1c8(void *player, struct Track_080fa1e8 *track);
void Func_080fa1ac(void *player, struct Track_080fa1e8 *track);

void Func_080fa1e8(void *player, struct Track_080fa1e8 *track)
{
