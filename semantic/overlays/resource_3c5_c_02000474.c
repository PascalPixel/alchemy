...
/*
 * resource_3c5 owner at 0x02000474, 404 bytes: the slide raycast.
 * Code 0x02000474-0x020005f9, an alignment halfword at 0x020005fa and the three
 * pool words 0x0200b02c, 0x0200afd4 and 0xffff0000 at 0x020005fc-0x02000607.
 *
 ...
 *
 * What was changed:
 *  - Data_0200af50 (footprint boxes) becomes Data_0200b02c, file offset 0x302c;
 *  - Data_0200aef8 (packed direction words) becomes Data_0200afd4, offset 0x2fd4;
 *    both under the proven 0x02008000 overlay link base.  The direction table
 *    address is independently corroborated: resource_3c5's 0x020000c4 owner
 *    loads the same 0x0200afd4 for the same purpose.
 *  - the calls were re-resolved with 'bun tools/overlay_call_targets.ts
 *    resource_3c5 0474': the same two sites, 0x0200034c (this overlay's own
...
extern s32 Data_0200afd4[];   /* 16 packed direction words */
extern s32 Data_0200b02c[];   /* 16-byte footprint boxes, four words each */
...
s32 Func_02000474(s32 *request)
{
    ...
    *state = 2;
    ...
    for (;;) {
        ...
        request[4] = probe[2] + (Data_0200b02c[request[0] * 4 + 1] << 16);

        for (z = 0; z < spanZ; z++) {
            request[2] = probe[0] + (Data_0200b02c[request[0] * 4] << 16);
        ...
        probe[2] += Data_0200afd4[heading] << 16;
    }
}
