...
//
// Not to be confused with tools/overlay/overlay_entry.ts, which repairs a function
...
//
//   bun tools/overlay/overlay_driver.ts resource_380      # one overlay
//   bun tools/overlay/overlay_driver.ts --all             # every overlay, with owner status
//   bun tools/overlay/overlay_driver.ts --all --unowned   # only the drivers nobody owns
//   bun tools/overlay/overlay_driver.ts --self-test
import { existsSync, readFileSync, readdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { overlayImage, OVERLAY_BASE, BASE_SHIFT } from "../lib/overlay_call_targets.ts";
