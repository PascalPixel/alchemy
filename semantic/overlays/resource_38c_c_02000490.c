...
/*
 * Resource 38c two-line beat at 0x02000490.
 *
 * Complete owner: `push {lr}` at 0x02000490, interworking return
 * `pop {r0} / bx r0` at 0x020004ba.  The control-flow walk reaches every
 * halfword to 0x020004bc; 0x020004be-0x020004c7 is the literal pool
 * (alignment halfword, 0x0000146f, 0x000013d9).  Handler pool word:
 * 0x02008491 = `Func_02000490 + 1`.
 *
 * Signature: `pop {r0} / bx r0` — `void`.
 *
 * The flag tested here is the small id 3, loaded as a plain immediate rather
 * than from the pool — not the 0x845 the band guards use.
 *
 * Both arms fall into the same act/end pair, which the assembly reaches by a
 * `b.n` from the first arm; it is written once.
 */
...
void Func_02000490(void)
{
    ...
    if (Func_080770c0(3) != 0) {
        Func_0808a170(0x146f);
    } else {
        Func_0808a170(0x13d9);
    }
