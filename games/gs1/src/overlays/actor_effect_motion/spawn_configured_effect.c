...
/*
 * resource_382 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_382_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x02009bb4 and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_382_c_02000048.c's adopted
 * Func_02005a2e pair and resource_382_c_02005158.c already use on this
...
extern struct Descriptor *Data_02009bb4[];

struct Effect *Func_02001b3e();
struct Effect *Func_02001abc();
void Func_02001ac6();
void Func_02001ae0();
void Func_02001c62();
s32 Func_02001b7c();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_02001b3e(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02001abc(options->kind, x, y, z);
    } else {
        effect = Func_02001abc(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_02001ac6(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02001ae0(effect, Data_02009bb4[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_02001c62(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02001b7c(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02001b94(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_02001ba2(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02001be0(effect, 1);
        Func_02001bf0(effect, options->callback_arg);
    }
