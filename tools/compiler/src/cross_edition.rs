...
pub fn run(args: &[String]) -> Result<(), String> {
    ...
    let edition_build = options.edition_build.as_deref().map(|path| write_edition_build(path, owner, &object_path, &report, &roms)).transpose()?;
    if options.json {
...
fn analyze_owner(owner: &str, owner_address: u64, object_path: &Path, roms: &EditionRoms, calls: bool, hints: Option<&BTreeMap<&str, usize>>) -> Result<Report, String> {
    ...
    for edition in EDITIONS.into_iter().filter(|edition| *edition != "en") {
        ...
        let located = if anchors.is_empty() { Err("no relocation-free anchor found in ROM".into()) } else { locate(en_owner, &mask, &anchors, rom) };
        let (start, support) = located.or_else(|global_error| hints.and_then(|values| values.get(edition).copied()).ok_or(global_error).and_then(|predicted| locate_near_exact(en_owner, &mask, rom, predicted, 0x1000, ROM_BASE))).map_err(|error| format!("{edition}: {error}"))?;
        found.insert(edition, FoundEdition { start, bytes: rom[start..start + size].to_vec(), anchor_matches: support });
    }

    let ja = found.get("ja").ok_or("JA counterpart was not located")?;
    let context = ComparisonContext { base_start: ja.start, base: &ja.bytes, mask: &mask, relocation_mask: &relocation_mask, relocations: &relocations, literals: &literals };
...
fn compile_edition_object(owner: &str, edition: &str, source: &Path) -> Result<PathBuf, String> {
    ...
    fs::write(&wrapper, format!("#define GS1_EDITION_{} 1\n#include \"{}\"\n", edition.to_ascii_uppercase(), include)).map_err(|error| format!("{}: {error}", wrapper.display()))?;
...
fn edition_build_report(owner: &str, object_path: &Path, report: &Report, roms: &EditionRoms, compiled: &mut BTreeMap<(String, String), Result<PathBuf, String>>) -> Result<EditionBuildReport, String> {
    ...
    for edition in EDITIONS {
        ...
        let built = derive_external_symbols(&variant_relocations, reference, start).and_then(|values| link_owner_for_edition(&output_root, edition, &variant_object, &owner_symbol, ROM_BASE + start as u64, variant_symbol_offset, variant_size, &values).map(|linked| (values, linked)));
        match built {
            Ok((values, linked)) => {
                let differing_bytes = reference.iter().zip(&linked).filter(|(left, right)| left != right).count() + reference.len().abs_diff(linked.len());
                editions.push(EditionBuildEntry {
                    edition: edition.into(),
                    start: format!("0x{:08x}", ROM_BASE + start as u64),
                    size: variant_size,
                    external_symbols: values.into_iter().map(|(name, value)| (name, format!("0x{value:08x}"))).collect(),
                    differing_bytes: Some(differing_bytes),
                    byte_exact: differing_bytes == 0,
                    error: None,
                });
            }
            Err(error) => editions.push(EditionBuildEntry { edition: edition.into(), start: format!("0x{:08x}", ROM_BASE + start as u64), size: variant_size, external_symbols: BTreeMap::new(), differing_bytes: None, byte_exact: false, error: Some(error) }),
        }
    }

    let build = EditionBuildReport { schema_version: 1, game: "gs1", source_edition: "en", source: source_report_path, object: object_path.display().to_string(), owner_symbol, size, edition_variant, all_exact: editions.iter().all(|edition| edition.byte_exact), editions };
    Ok(build)
}
...
fn write_corpus_edition_build(path: &Path, objects: &BTreeMap<String, PathBuf>, reports: &BTreeMap<String, Report>, owners_total: usize, owner_symbol_bytes: usize, unresolved_owners: usize, roms: &EditionRoms) -> Result<CorpusEditionBuildReport, String> {
    ...
    let editions = EDITIONS
        .into_iter()
        .map(|edition| {
            let mut summary = EditionBuildSummary { edition: edition.into(), checked_owners: 0, checked_bytes: 0, byte_exact_owners: 0, byte_exact_bytes: 0, nonexact_owners: 0, nonexact_bytes: 0, differing_bytes: 0, unbuildable_owners: 0 };
            for owner in &owners {
                let entry = owner.editions.iter().find(|entry| entry.edition == edition).expect("every edition build contains every edition");
    ...
    let build = CorpusEditionBuildReport {
        ...
        all_exact_owners,
        all_exact_bytes,
        all_exact: unresolved_owners == 0 && failures.is_empty() && owners.len() == owners_total && all_exact_owners == owners_total,
        editions,
        owners,
        failures,
    };
    write_json(path, &build, "corpus edition build")?;
    println!("edition_builds={} owners={} built={} all_exact={} unresolved={} failures={}", path.display(), build.owners_total, build.built_owners, build.all_exact_owners, build.unresolved_owners, build.failures.len());
    Ok(build)
}

fn derive_external_symbols(relocations: &[RelocationSite], reference: &[u8], start: usize) -> Result<BTreeMap<String, u64>, String> {
    ...
    for site in relocations.iter().filter(|site| site.external) {
        let value = if is_thumb_call(site) {
            ...
            let bytes = reference.get(site.offset..site.offset + 4).ok_or_else(|| format!("{} relocation extends past owner", site.symbol))?;
...
fn link_owner_for_edition(output_root: &Path, edition: &str, object_path: &Path, owner_symbol: &str, start: u64, symbol_offset: u64, size: usize, values: &BTreeMap<String, u64>) -> Result<Vec<u8>, String> {
    ...
    run_tool(Command::new("arm-none-eabi-as").args(["-mcpu=arm7tdmi", "-mthumb-interwork", "-o"]).arg(&symbols_object).arg(&symbols_source), "assemble edition symbols")?;
    run_tool(
        Command::new("arm-none-eabi-ld").arg(format!("-Ttext=0x{:08x}", start.checked_sub(symbol_offset).ok_or("owner symbol offset exceeds destination address")?)).arg("--unresolved-symbols=ignore-all").args(["-e", owner_symbol, "-o"]).arg(&elf).arg(object_path).arg(&symbols_object),
        "link edition owner",
    )?;
    run_tool(Command::new("arm-none-eabi-objcopy").args(["-O", "binary", "-j", ".text"]).arg(&elf).arg(&binary), "extract edition owner")?;
    ...
    let owner = bytes.get(offset..offset + size).map(Vec::from).ok_or_else(|| format!("{}: linked owner extends past .text", binary.display()))?;
...
fn parse_explicit_overlay_owner(value: &str, span: Option<usize>) -> Result<OverlayOwner, String> {
    ...
    let en_offset = u64::from(address).checked_sub(OVERLAY_BASE).ok_or_else(|| format!("{value}: address is below the overlay base"))? as usize;
    Ok(OverlayOwner { name: source.id(), resource, en_offset, size: span.ok_or_else(|| format!("explicit overlay owners require --span\n{USAGE}"))? })
}
...
fn location_method_codes() -> BTreeMap<&'static str, &'static str> {
    [("a", "global_anchor"), ("n", "neighbor_exact"), ("o", "resource_offset_exact"), ("s", "source"), ("t", "global_thumb_anchor")].into_iter().collect()
}
...
fn run_all(options: &Options, roms: &EditionRoms) -> Result<(), String> {
    ...
    for (owner, report) in reports {
        ...
        owners.push(CorpusOwner { en_owner: canonical_main_owner_id(&owner)?, size: report.size, start_overrides, location_methods, core_diff_bytes_from_ja });
    }

    let unresolved = failures.into_iter().map(|(owner, error)| Ok(UnresolvedOwner { en_owner: canonical_main_owner_id(&owner)?, error })).collect::<Result<Vec<_>, String>>()?;

    let editions = EDITIONS
        .into_iter()
        .map(|edition| {
            let totals = edition_totals[edition];
            EditionSummary { edition: edition.into(), matched_owners: totals[0], matched_bytes: totals[1], core_identical_owners: totals[2], core_identical_bytes: totals[3], core_different_owners: totals[4], core_different_bytes: totals[5] }
        })
        .collect();
    ...
    let json = serde_json::to_string_pretty(&report).map_err(|error| format!("serialize corpus report: {error}"))? + "\n";
    if let Some(path) = &options.write {
        if let Some(parent) = path.parent().filter(|parent| !parent.as_os_str().is_empty()) {
            fs::create_dir_all(parent).map_err(|error| format!("{}: {error}", parent.display()))?;
        }
        fs::write(path, json).map_err(|error| format!("{}: {error}", path.display()))?;
        println!("correspondence={} owners={} matched={} shared_core={} regional_core={} unresolved={}", path.display(), report.owners_total, report.matched_owners, report.shared_core_owners, report.regional_core_owners, report.unresolved_owners);
    } else {
        print!("{json}");
    }
    Ok(())
...
fn run_all_overlays(options: &Options, roms: &EditionRoms) -> Result<(), String> {
    ...
    for found in matches.into_values() {
        ...
        owners.push(OverlayCorpusOwner { en_owner: found.owner.name.clone(), size: found.owner.size, core_bytes: found.mask.iter().filter(|masked| !**masked).count(), start_overrides, location_methods: methods, core_diff_bytes_from_ja: differences });
    }
    owners.sort_by(|left, right| left.en_owner.cmp(&right.en_owner));

    let unresolved = owner_list.iter().filter_map(|owner| failures.get(&owner.name).map(|error| OverlayUnresolvedOwner { en_owner: owner.name.clone(), size: owner.size, error: error.clone() })).collect::<Vec<_>>();
    let editions = EDITIONS
        .into_iter()
        .map(|edition| {
            let totals = edition_totals[edition];
    ...
    let json = serde_json::to_string_pretty(&report).map_err(|error| format!("serialize overlay corpus report: {error}"))? + "\n";
    if let Some(path) = &options.write {
        if let Some(parent) = path.parent().filter(|parent| !parent.as_os_str().is_empty()) {
            fs::create_dir_all(parent).map_err(|error| format!("{}: {error}", parent.display()))?;
        }
...
fn run_overlay_owner(options: &Options, roms: &EditionRoms, value: &str) -> Result<(), String> {
    ...
    let editions = EDITIONS
        ...
        .map(|edition| {
            ...
            Ok(OverlayOwnerEdition { edition: edition.into(), start: format!("0x{:08x}", OVERLAY_BASE + start as u64), location_method: found.methods[edition].into(), core_diff_bytes: difference, core_identical: difference == 0 })
        })
        .collect::<Result<Vec<_>, String>>()?;
    let report = OverlayOwnerReport { schema_version: CORRESPONDENCE_SCHEMA_VERSION, game: "gs1", owner: owner.name, size: owner.size, core_bytes: found.mask.iter().filter(|masked| !**masked).count(), core_identical: editions.iter().all(|edition| edition.core_identical), editions };
    if options.json {
        println!("{}", serde_json::to_string_pretty(&report).map_err(|error| format!("serialize overlay owner report: {error}"))?);
    } else {
        println!("owner={} size={} core_bytes={} core_identical={}", report.owner, report.size, report.core_bytes, if report.core_identical { "yes" } else { "no" });
        for edition in &report.editions {
            println!("edition={} start={} location_method={} core_diff_bytes={} core_identical={}", edition.edition, edition.start, edition.location_method, edition.core_diff_bytes, if edition.core_identical { "yes" } else { "no" });
        }
    }
    if report.core_identical {
        Ok(())
...
fn exact_overlay_owners() -> Result<Vec<OverlayOwner>, String> {
    ...
    for source in source_paths.all_sources()? {
        ...
        let en_offset = absolute.checked_sub(OVERLAY_BASE).ok_or_else(|| format!("{}: address is below overlay base", source.owner.id()))? as usize;
        ...
        let start = lines.iter().position(|line| line.trim() == tag).ok_or_else(|| format!("{}: missing {tag} placeholder", source.owner.id()))?;
        let mut size = 0usize;
...
fn parse_usize(value: &str) -> Result<usize, String> {
    value.strip_prefix("0x").map_or_else(|| value.parse(), |hex| usize::from_str_radix(hex, 16)).map_err(|error| format!("invalid integer {value}: {error}"))
}
...
fn resource_table(rom: &[u8]) -> Result<usize, String> {
    (0..rom.len().saturating_sub(8))
        .step_by(4)
        .find(|offset| u32::from_le_bytes(rom[*offset..*offset + 4].try_into().unwrap()) as u64 == ROM_BASE && u32::from_le_bytes(rom[*offset + 4..*offset + 8].try_into().unwrap()) as u64 == ROM_BASE + *offset as u64)
...
fn resource_pointer(rom: &[u8], table: usize, resource: usize) -> Result<usize, String> {
    ...
    let address = u32::from_le_bytes(rom.get(at..at + 4).ok_or("resource directory extends past ROM")?.try_into().unwrap()) as u64;
    rom_offset(address, rom.len())
}

fn overlay_window<'a>(decoded: &'a DecodedOverlays, edition: &str, resource: usize, start: usize, size: usize) -> Result<&'a [u8], String> {
    decoded[edition][&resource].get(start..start + size).ok_or_else(|| format!("{edition}: resource {resource:03x} owner extends past container"))
}
...
fn analyze_overlay_owner(owner: &OverlayOwner, decoded: &DecodedOverlays, hints: Option<&BTreeMap<&str, usize>>) -> Result<OverlayMatch, String> {
    let en = overlay_window(decoded, "en", owner.resource, owner.en_offset, owner.size)?;
    let mask = overlay_mask(en, owner.en_offset);
    let core_bytes = mask.iter().filter(|masked| !**masked).count();
    if core_bytes < 4 {
        let raw_identical = EDITIONS.into_iter().all(|edition| overlay_window(decoded, edition, owner.resource, owner.en_offset, owner.size) == Ok(en));
        if raw_identical {
            return Ok(OverlayMatch {
                owner: OverlayOwner { name: owner.name.clone(), resource: owner.resource, en_offset: owner.en_offset, size: owner.size },
                mask,
                starts: EDITIONS.into_iter().map(|edition| (edition, owner.en_offset)).collect(),
                methods: EDITIONS.into_iter().map(|edition| (edition, if edition == "en" { "source" } else { "resource_offset_exact" })).collect(),
            });
        }
    ...
    for edition in EDITIONS.into_iter().filter(|edition| *edition != "en") {
        ...
        let located = if owner_anchors.is_empty() { Err("no relocation-free anchor found in decoded resource".into()) } else { locate(en, &mask, &owner_anchors, container) };
        let (start, method) = match located {
            Ok((start, _)) => (start, "global_thumb_anchor"),
            Err(_) if container.get(owner.en_offset..owner.en_offset + owner.size) == Some(en) => (owner.en_offset, "resource_offset_exact"),
            Err(global_error) => {
                let predicted = hints.and_then(|values| values.get(edition).copied()).ok_or(global_error).map_err(|error| format!("{edition}: {error}"))?;
                let (start, _) = locate_near_exact(en, &mask, container, predicted, 0x1000, OVERLAY_BASE).map_err(|error| format!("{edition}: {error}"))?;
    ...
    Ok(OverlayMatch { owner: OverlayOwner { name: owner.name.clone(), resource: owner.resource, en_offset: owner.en_offset, size: owner.size }, mask, starts, methods })
}
...
fn overlay_location_anchors(matches: &BTreeMap<String, OverlayMatch>) -> Vec<OverlayLocationAnchor> {
    matches.values().map(|found| OverlayLocationAnchor { resource: found.owner.resource, en_offset: found.owner.en_offset, starts: found.starts.clone() }).collect()
}

fn nearest_overlay_hints(owner: &OverlayOwner, anchors: &[OverlayLocationAnchor]) -> Result<BTreeMap<&'static str, usize>, String> {
    let anchor = anchors.iter().filter(|anchor| anchor.resource == owner.resource).min_by_key(|anchor| anchor.en_offset.abs_diff(owner.en_offset)).ok_or_else(|| format!("resource_{:03x}: no global anchors", owner.resource))?;
...
fn remove_overlay_order_conflicts(matches: &mut BTreeMap<String, OverlayMatch>, failures: &mut BTreeMap<String, String>, phase: &str) -> Result<(), String> {
    loop {
        let mut conflict = None;
        for resource in OVERLAY_FIRST..=OVERLAY_LAST {
            let names = matches.iter().filter(|(_, found)| found.owner.resource == resource).map(|(name, _)| name.clone()).collect::<Vec<_>>();
            for edition in EDITIONS.into_iter().filter(|edition| *edition != "en") {
        ...
        failures.insert(rejected, format!("{edition}: {phase} counterpart conflicts with neighboring proved owner order"));
    }
}

fn overlay_prediction_error(owner: &str, edition: &str, matches: &BTreeMap<String, OverlayMatch>, excluded: [&str; 2]) -> usize {
    let found = &matches[owner];
    let neighbor = matches.iter().filter(|(name, candidate)| candidate.owner.resource == found.owner.resource && !excluded.contains(&name.as_str())).min_by_key(|(_, candidate)| candidate.owner.en_offset.abs_diff(found.owner.en_offset));
...
fn location_anchors(reports: &BTreeMap<String, Report>) -> Result<Vec<LocationAnchor>, String> {
    reports
        ...
        .map(|report| {
            let mut starts = BTreeMap::new();
            for edition in EDITIONS {
                let value = report.editions.iter().find(|entry| entry.edition == edition).ok_or_else(|| format!("{} lacks {edition} location", report.owner))?;
...
fn remove_order_conflicts(reports: &mut BTreeMap<String, Report>, failures: &mut BTreeMap<String, String>, phase: &str) -> Result<(), String> {
    loop {
...
fn location_prediction_error(owner: &str, edition: &str, reports: &BTreeMap<String, Report>, excluded: [&str; 2]) -> Result<usize, String> {
    ...
    let neighbor = reports
        ...
        .min_by_key(|(name, _)| owner_address(name).ok().and_then(|address| rom_offset(address, usize::MAX).ok()).map_or(usize::MAX, |offset| offset.abs_diff(en_offset)))
...
fn report_start(report: &Report, edition: &str) -> Result<usize, String> {
    let entry = report.editions.iter().find(|entry| entry.edition == edition).ok_or_else(|| format!("{} lacks {edition} location", report.owner))?;
    parse_rom_address(&entry.start)
}

fn nearest_location_hints(owner_address: u64, anchors: &[LocationAnchor]) -> Result<BTreeMap<&'static str, usize>, String> {
    let en_offset = rom_offset(owner_address, usize::MAX)?;
    let mut hints = BTreeMap::new();
    for edition in EDITIONS.into_iter().filter(|edition| *edition != "en") {
        let anchor = anchors.iter().min_by_key(|anchor| anchor.en_offset.abs_diff(en_offset)).ok_or("no global correspondence anchors are available")?;
...
fn parse_rom_address(value: &str) -> Result<usize, String> {
    let address = u64::from_str_radix(value.trim_start_matches("0x"), 16).map_err(|error| format!("invalid ROM address {value}: {error}"))?;
...
fn relocation_mask(path: &Path, owner: &str) -> Result<(usize, u64, Vec<u8>, Vec<bool>, Vec<RelocationSite>), String> {
    ...
    let (symbol_name, symbol_index) = registered_symbol
        ...
        .ok_or_else(|| format!("{}: missing owner symbol {}", path.display(), registered_symbol.unwrap_or(&legacy_symbol)))?;
    ...
    let size = if symbol.size == 0 { section.data.len().checked_sub(section_offset).ok_or("owner symbol extends past its section")? } else { usize::try_from(symbol.size).map_err(|_| "owner is too large")? };
    if size == 0 {
        return Err(format!("{symbol_name} has zero size"));
    }
    let bytes = section.data.get(section_offset..section_offset + size).ok_or("owner symbol extends past its section")?.to_vec();
    ...
    for relocation in &section.relocations {
        ...
        let kind = object.arch.reloc_name(relocation.flags).map(str::to_string).unwrap_or_else(|| format!("{:?}", relocation.flags));
        ...
        sites.push(RelocationSite { offset: start, size: end - start, kind, symbol, addend: relocation.addend, external: target.is_some_and(|symbol| symbol.section.is_none()) });
    }
...
fn literal_sites(path: &Path, symbol_address: u64, size: usize) -> Result<Vec<LiteralSite>, String> {
    let output = Command::new("arm-none-eabi-objdump").args(["-dr"]).arg(path).output().map_err(|error| format!("arm-none-eabi-objdump: {error}"))?;
    if !output.status.success() {
...
fn anchors(owner: &[u8], mask: &[bool]) -> Vec<Anchor> {
    ...
    runs.into_iter().take(16).map(|(size, offset)| Anchor { offset, size: size.min(32).min(owner.len() - offset) }).collect()
}
...
fn locate(owner: &[u8], mask: &[bool], anchors: &[Anchor], rom: &[u8]) -> Result<(usize, usize), String> {
    ...
    for anchor in anchors {
        ...
        let exact = support.keys().copied().filter(|start| core_diff_bytes(owner, &rom[*start..*start + owner.len()], mask) == 0).collect::<Vec<_>>();
        if let [start] = exact.as_slice() {
            return Ok((*start, support[start]));
        }
    }
    let mut scored = support.into_iter().map(|(start, anchor_matches)| (core_diff_bytes(owner, &rom[start..start + owner.len()], mask), usize::MAX - anchor_matches, start, anchor_matches)).collect::<Vec<_>>();
...
fn locate_near_exact(owner: &[u8], mask: &[bool], rom: &[u8], predicted: usize, radius: usize, address_base: u64) -> Result<(usize, usize), String> {
    ...
    let mut exact = (first..=last).step_by(2).filter(|start| core_diff_bytes(owner, &rom[*start..*start + owner.len()], mask) == 0).map(|start| (start.abs_diff(predicted), start)).collect::<Vec<_>>();
    exact.sort_unstable();
    let Some(&(distance, start)) = exact.first() else {
        return Err(format!("no exact core within {radius:#x} bytes of predicted 0x{:08x}", address_base + predicted as u64));
    };
...
fn find_all(haystack: &[u8], needle: &[u8]) -> Vec<usize> {
    ...
    let first = needle[0];
    haystack.windows(needle.len()).enumerate().filter_map(|(index, window)| (window[0] == first && window == needle).then_some(index)).collect()
}

fn edition_report(edition: &str, start: usize, other: &[u8], context: &ComparisonContext<'_>, anchor_matches: usize) -> EditionReport {
    let base = context.base;
    let raw_diff_bytes = base.iter().zip(other).filter(|(left, right)| left != right).count();
    let raw_diff_halfwords = base.chunks_exact(2).zip(other.chunks_exact(2)).filter(|(left, right)| left != right).count();
    let relocation_diff_bytes = base.iter().zip(other).zip(context.relocation_mask).filter(|((left, right), relocated)| **relocated && left != right).count();
    let literal_diff_bytes = context.literals.iter().map(|site| base[site.offset..site.offset + site.size].iter().zip(&other[site.offset..site.offset + site.size]).filter(|(left, right)| left != right).count()).sum();
    let core_diff_bytes = core_diff_bytes(base, other, context.mask);
    let changed = context.relocations.iter().filter(|site| base[site.offset..site.offset + site.size] != other[site.offset..site.offset + site.size]).collect::<Vec<_>>();
    EditionReport {
        edition: edition.into(),
        start: format!("0x{:08x}", ROM_BASE + start as u64),
        ...
        literal_fields_changed: context.literals.iter().filter(|site| base[site.offset..site.offset + site.size] != other[site.offset..site.offset + site.size]).count(),
...
fn call_targets(relocations: &[RelocationSite], found: &BTreeMap<&str, FoundEdition>) -> Result<Vec<CallTarget>, String> {
    ...
    groups
        ...
        .map(|(symbol, offsets)| {
            ...
            for edition in EDITIONS {
                let entry = &found[edition];
                let mut targets = offsets.iter().map(|offset| thumb_bl_target(&entry.bytes, entry.start, *offset).map_err(|error| format!("{edition}: {error}"))).collect::<Result<Vec<_>, _>>()?;
...
fn print_report(report: &Report, calls: bool) {
    ...
    println!("callee_correspondences={} core_identical={}", report.call_targets.len(), if report.core_identical { "yes" } else { "no" });
    if calls {
        for call in &report.call_targets {
            let addresses = call.editions.iter().map(|target| format!("{}={}", target.edition, target.address)).collect::<Vec<_>>().join(" ");
...
mod tests {
    ...
    fn accepts_corpus_edition_build_output() {
        let options = parse(&["--all".into(), "--edition-build".into(), "out/builds.json".into()]).expect("corpus edition build options");
        assert!(options.all);
    ...
    fn correspondence_rows_serialize_only_canonical_owner_identity() {
        ...
        let owner = CorpusOwner { en_owner: SourceOwner::Main(0x0800_2ee4).id(), size: 4, start_overrides: BTreeMap::new(), location_methods: "asaaaa".into(), core_diff_bytes_from_ja: BTreeMap::new() };
        ...
        assert_identity(&UnresolvedOwner { en_owner: SourceOwner::Main(0x0800_2ee4).id(), error: "unresolved".into() }, "main:08002ee4");
        let overlay_owner = SourceOwner::Overlay { resource: 0x36f, address: 0x0200_0030 }.id();
        assert_identity(&OverlayCorpusOwner { en_owner: overlay_owner.clone(), size: 4, core_bytes: 4, start_overrides: BTreeMap::new(), location_methods: "sooooo".into(), core_diff_bytes_from_ja: BTreeMap::new() }, "resource_36f:02000030");
        assert_identity(&OverlayUnresolvedOwner { en_owner: overlay_owner, size: 4, error: "unresolved".into() }, "resource_36f:02000030");
        assert_identity(&EditionBuildFailure { en_owner: SourceOwner::Main(0x0800_2ee4).id(), error: "unbuildable".into() }, "main:08002ee4");
        assert_eq!(CORRESPONDENCE_SCHEMA_VERSION, 3);
        assert_eq!(CORPUS_EDITION_BUILD_SCHEMA_VERSION, 3);
        assert_eq!(location_method_codes(), [("a", "global_anchor"), ("n", "neighbor_exact"), ("o", "resource_offset_exact"), ("s", "source"), ("t", "global_thumb_anchor"),].into_iter().collect());
    }
