    ...
    ('A'..='Z')
        ...
        .find(|candidate| {
            ...
            let path = root.join(format!(
                "games/THE BROKEN SEAL/SRC/overlays/unidentified/run_scene_{resource}_sequence_{letter}.c"
            ));
...
pub fn adopt(root: &Path, request: &Request) -> Result<Vec<String>, String> {
    ...
    let destination = root.join("games/THE BROKEN SEAL/SRC").join(&relative);
