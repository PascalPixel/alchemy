...
/// `tools-rs/compare-roms/src/lib.rs`, which was written and tested against the
...
pub fn to_fixed(value: f64, digits: usize) -> String {
    ...
    let body = if digits == 0 { integral } else { format!("{integral}.{kept}") };
...
pub fn parse_int_auto(text: &str) -> Option<i64> {
    ...
    let (body, radix) = match trimmed.strip_prefix("0x").or_else(|| trimmed.strip_prefix("0X")) {
        Some(rest) => (rest, 16),
        None => (trimmed, 10),
    };
...
mod tests {
    ...
    fn utf16_cmp_orders_by_code_unit() {
        ...
        assert_eq!(utf16_cmp("\u{fffd}", "\u{10000}"), std::cmp::Ordering::Greater);
