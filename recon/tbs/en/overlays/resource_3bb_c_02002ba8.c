/* NONMATCHING: 472 bytes, candidate 476, 120 differing halfwords, 58
 * halfword edits (2026-09-25). KorosseoKabe_Func02002ba8, meant for
 * FIELD/KOROSSEO_KABE/F_02BA8.C as a single-overlay unit binding its names
 * at their runtime addresses (an import veneer's listing offset plus
 * 0x8000). Remaining: ported from 3ba:02002910 twin; BLDALPHA pool word sits
 * 4 bytes past owner (+4 size); blend mask and IO-queue schedule still off
 * WALL: copy_versus_rematerialise: IO-queue count store schedule and
 * 0xf3-vs-negs mask; plus +4 BLDALPHA pool outside span */
#include "TYPES.H"
...
void KorosseoKabe_Func02002ba8(s32 id, s32 x, s32 z)
{
    u8 *state;
    u8 *actor;
    u8 *sprite;
