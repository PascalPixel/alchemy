...
fn expected_keys(value: &Value, expected: &[&str], name: &str) -> Result<()> {
    ...
    if actual_keys != expected_keys {
        return fail(format!("{name} has unexpected fields"));
    }
    Ok(())
}
...
fn atlas(path: &Path, spec: &(usize, usize, &str, usize, usize, usize, usize)) -> Result<Vec<u8>> {
    ...
    if output.len() != size {
        return fail("battle-menu atlas size differs");
    }
    Ok(output)
}
...
fn selector_ids(value: Option<&Value>) -> Result<Vec<u8>> {
    ...
    for left in 0..ids.len() {
        if ids[left + 1..].contains(&ids[left]) {
            return fail("selector IDs must be unique before their terminator");
        }
    }
