...
// `--self-test`, and `bun run` drives it in CI without a test runner. Porting
...
fn tile(label: &str, bytes: i64, categories: &[(&str, i64)]) -> Tile {
    let mut built = Tile { label: label.to_string(), bytes, ..Tile::default() };
...
pub fn self_test() -> Result<String, String> {
    ...
    refuse(options_of(&[String::from("--unknown")]).is_ok(), "unknown argument was accepted")?;
    let parsed = options_of(&[String::from("--semantic-ref"), String::from("none")])
        .map_err(|_| "option parsing failed".to_string())?;
    refuse(parsed.semantic.as_deref() != Some("none"), "option parsing failed")?;
...
fn spans_and_regions() -> Result<(), String> {
    ...
    refuse(found.start != 0x0800_0010 || found.end != 0x0800_0030, "region span failed")?;
    refuse(
...
fn placeholders_and_buckets() -> Result<(), String> {
    ...
    refuse(asset_bucket("golden-sun-pcm-wave-series").id != "audio", "audio bucket failed")?;
    refuse(asset_bucket("brand-new-package").id != "other", "unknown bucket failed")
}
...
fn whole_overlay_claims() -> Result<(), String> {
    ...
    extent.insert("resource_375".to_string(), vec![span(0x0200_0000, 0x0200_0100)]);
    ...
    refuse(
        span_bytes(claimed.overlays.get(&"resource_375".to_string()).map(Vec::as_slice).unwrap_or(&[]))
            != 256,
    ...
    refuse(
        !unbacked.overlays.get(&"resource_375".to_string()).map(Vec::is_empty).unwrap_or(true),
    ...
    refuse(
        !no_extent.overlays.get(&"resource_999".to_string()).map(Vec::is_empty).unwrap_or(true),
        "a whole-code-overlay claim without an audited extent credited bytes",
    )?;

    let unlisted = semantic_spans(&source_tree("{}", &["resource_375_c_02000030.c"]), &[], &[], &extent);
    refuse(unlisted.unresolved.len() != 1, "an unlisted overlay owner is no longer reported")?;
    ...
    refuse(
        span_bytes(
            per_owner.overlays.get(&"resource_375".to_string()).map(Vec::as_slice).unwrap_or(&[]),
        ) != 64,
...
fn readme_and_ref_tree() -> Result<(), String> {
    ...
    for index in b'1'..=b'4' {
        let needle =
            format!("<img src=\"assets/readme/djinn_10{}_idle.gif\" width=\"", index as char);
        let Some(at) = readme.find(&needle) else { continue };
        found += 1;
        let rest = &readme[at + needle.len()..];
        let width: String = rest.chars().take_while(char::is_ascii_digit).collect();
        let after = &rest[width.len()..];
        let Some(after) = after.strip_prefix("\" height=\"") else { continue };
        let height: String = after.chars().take_while(char::is_ascii_digit).collect();
        if width == "64" && height == "64" {
            correct += 1;
        }
    }
    refuse(found != 4 || correct != 4, "README Djinn must remain exactly 64 by 64 pixels")?;
    ...
    refuse(
        resolve_exact_tree(None, None).map(|tree| tree.id().to_string()).as_deref() != Ok("worktree"),
        "the exact source did not default to the worktree",
    )?;
    refuse(
        resolve_exact_tree(None, Some("worktree")).map(|tree| tree.id().to_string()).as_deref()
            != Ok("worktree"),
...
fn layout_and_tiles() -> Result<(), String> {
    ...
    let placed_area: f64 =
        placed.iter().map(|item| item.rect.width * item.rect.height).sum();
    refuse((placed_area - 10000.0).abs() > 1.0, "treemap does not fill its rectangle")?;
    for item in &placed {
        refuse(
            item.rect.x < -0.01
                || item.rect.y < -0.01
    ...
    let subpixel =
        squarify(&sizes, |item| *item, Rect { x: 0.0, y: 0.0, width: 1600.0, height: 900.0 })?;
    ...
    refuse(
        category_total(&overlay_classification, "exact_c") != 20
            ...
            || overlay_classification.iter().map(|tile| tile.bytes).sum::<i64>() != 100,
    ...
    let widths: Vec<String> = owner_tiles.iter().map(|tile| tile.bytes.to_string()).collect();
    refuse(
        owner_tiles.len() != 3
    ...
    let mut groups: Vec<String> =
        crossing.iter().map(|tile| tile.group.clone().unwrap_or_default()).collect();
    groups.dedup();
    ...
    let retained_tree = default_box_tree(
        &area("retained", "Retained", vec![tile("r", 32, &[("retained_asm", 32)])]),
    ...
    refuse(!retained_tree.contains("DONE 100.0%"), "permanent assembly was omitted from DONE")?;
    refuse(
        retained_tree.contains("Unknown 0.0%"),
        "zero-byte dashboard category leaked into the legend",
    )?;
    let semantic_tree = default_box_tree(
        &area("semantic", "Semantic", vec![tile("s", 32, &[("semantic_c", 32)])]),
...
fn renderers(map: &CoverageMap) -> Result<(), String> {
    ...
    let legend_order: Vec<Option<usize>> =
        ["Semantic 40.0%", "Permanent ASM 30.0%", "Exact 30.0%"]
            .iter()
    ...
    let folder_tree = crate::boxtree::render_box_tree(
        &area(
            ...
            "Folders",
            vec![
                Tile { group: Some("first".to_string()), ..tile("one", 60, &[("exact_c", 60)]) },
                Tile { group: Some("second".to_string()), ..tile("two", 40, &[("assembly", 40)]) },
            ],
        ),
        "folder self-test",
        &CORE_HUE,
    ...
    for category in CATEGORY_ORDER {
        ...
        refuse(
            !svg.to_uppercase().contains(&crate::jstext::escape_text(style.label).to_uppercase()),
...
mod tests {
    ...
    fn the_self_test_passes() {
        assert_eq!(super::self_test().expect("self-test"), "self-test=ok tool=coverage_map");
    }
}
