...
//
// The surface `Bun.serve` is asked for here is: seven `GET` routes on
// 127.0.0.1, no TLS, no request bodies, no cookies, no content negotiation, no
// compression, no HTTP/2, no WebSocket upgrade. Six routes are a single
// buffered response. The seventh, `/events`, is Server-Sent Events: an
// unbounded `text/event-stream` body that Bun emits with
...
pub fn read_request(reader: &mut BufReader<&TcpStream>) -> Result<Request, ParseError> {
    ...
    Ok(Request { method: method.to_string(), path: percent_decode(&path), query })
}
...
/// `new URL(url).pathname` leaves percent escapes in place; the TypeScript
/// then compares the pathname to literals, so `/%73napshot` does NOT match
/// `/snapshot` in Bun.
///
...
impl Response {
    pub fn new(status: u16, reason: &'static str, headers: Vec<(String, String)>, body: Vec<u8>) -> Self {
...
/// `headers(contentType?, cacheControl = "no-store")` from the TypeScript.
///
/// PORT NOTE: the spread `...(contentType ? {...} : {})` means an EMPTY STRING
/// content type is falsy and emits NO header at all, not an empty one. The
/// `Option<&str>` here is `None` for both `undefined` and `""`; the caller
/// never passes `""`, and a test pins the rule.
...
/// `event: NAME\ndata: JSON\n\n` -- the exact framing the TypeScript encodes.
/// Note the single `\n`, not `\r\n`: a "correction" to CRLF changes bytes the
...
mod tests {
    ...
    fn sse_frames_use_bare_newlines() {
        assert_eq!(sse_frame("update", "{}"), b"event: update\ndata: {}\n\n".to_vec());
        assert!(!String::from_utf8(sse_frame("update", "{}")).unwrap().contains('\r'));
    }

    #[test]
    fn percent_escapes_are_left_alone_because_url_pathname_leaves_them() {
        // `/%73napshot` must NOT route to `/snapshot`. A decoder here would be
        // a security-shaped divergence from the oracle.
