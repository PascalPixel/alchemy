 ...
 * `cargo run --release --manifest-path tools/overlay-call-targets/Cargo.toml -- resource_38d 009c --json`; both are
...
/*
 * resource_38d owner at 0x02000088, 8 bytes: `ldr r0, [pc, #0] / bx lr` plus the
 * one-word literal pool at 0x200008c holding 0x200a638.
 *
 ...
 *
 * The pool word is an ADDRESS -- 0x200a638 is image offset
 * 0x2638 under the base + 0x8000 spelling -- loaded and returned
...
/*
 * resource_38d owner at 0x02000094, 8 bytes: `ldr r0, [pc, #0] / bx lr` plus the
 * one-word literal pool at 0x2000098 holding 0x200a920.
 *
 ...
 * Recording 4 would orphan a word and manufacture a phantom gap.
 *
 * The pool word is an ADDRESS -- 0x200a920 is image offset
 * 0x2920 under the base + 0x8000 spelling -- loaded and returned
 * without being dereferenced, so this is a getter for an in-image table.
 *
 * One of the 191 rows sharing this exact body across the tree, and every
 * one of them returns a DIFFERENT address. Identical bytes are not
