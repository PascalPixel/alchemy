...
fn list<'a>(value: &'a Value, size: usize, label: &str) -> Result<&'a Vec<Value>> {
    let entries = value.as_array().ok_or_else(|| format!("{label} must contain {size} entries"))?;
    if entries.len() != size {
...
fn item_definition<'a>(value: &'a Value, index: usize) -> Result<&'a Map<String, Value>> {
    ...
    let name = field(item, "name")?.as_str().ok_or_else(|| format!("{label} name must be a string"))?;
...
fn parse_gameplay_databases(value: &Value) -> Result<&Map<String, Value>> {
    ...
    let matrix = list(field(source, "class_family_matrix")?, 8, "class_family_matrix")?;
    for (index, row) in matrix.iter().enumerate() {
        list(row, 4, &format!("class_family_matrix[{index}]"))?;
    }
    list(field(source, "elemental_profiles")?, 44, "elemental_profiles")?;
    list(field(source, "signed_scale_curve")?, 5, "signed_scale_curve")?;
...
fn build_item_definition(value: &Value, index: usize) -> Result<Vec<u8>> {
    ...
    write_u16(&mut result, 0, integer_or(item, "price", 0, 0, 0xffff, "item price")? as u16);
    result[2] = integer_or(item, "type", 0, 0, 0xff, "item type")? as u8;
    result[3] = integer_or(item, "flags", 0, 0, 0xff, "item flags")? as u8;
    write_u16(&mut result, 4, integer_or(item, "equip_mask", 0, 0, 0xffff, "item equip_mask")? as u16);
    write_u16(&mut result, 6, integer_or(item, "icon", 0, 0, 0xffff, "item icon")? as u16);
    write_i16(&mut result, 8, integer_or(item, "primary_bonus", 0, -0x8000, 0x7fff, "item primary_bonus")? as i16);
    ...
    write_u16(&mut result, 40, integer_or(item, "action_id", 0, 0, 0xffff, "item action_id")? as u16);
    Ok(result)
}
...
fn build_ability(value: &Value, index: usize) -> Result<Vec<u8>> {
    ...
    write_u16(&mut result, 4, value(4, 0xffff, "ability animation")? as u16);
...
fn build_hero_growth(value: &Value, index: usize) -> Result<Vec<u8>> {
    ...
    for (offset, key) in [(80, "hp"), (92, "pp"), (104, "attack"), (116, "defense"), (128, "agility")] {
        for (slot, entry) in list(field(growth, key)?, 6, &format!("hero growth {key}"))?.iter().enumerate() {
            write_u16(&mut result, offset + slot * 2, u16(entry, &format!("hero growth {key}"))?);
        }
    }
...
fn build_class(value: &Value, index: usize) -> Result<Vec<u8>> {
    ...
    if let Some(requirements) = item.get("djinn_requirements") {
        for (slot, entry) in list(requirements, 4, "class djinn_requirements")?.iter().enumerate() {
            result[4 + slot] = u8(entry, "class djinn requirement")?;
        }
    }
    if let Some(multipliers) = item.get("stat_multipliers") {
        for (slot, entry) in list(multipliers, 6, "class stat_multipliers")?.iter().enumerate() {
...
fn build_elemental_profile(value: &Value, index: usize) -> Result<Vec<u8>> {
    ...
    exact_keys(item, &["element", "levels", "stats"], &format!("elemental_profiles[{index}]"))?;
    let mut result = vec![0; 24];
    write_i32(&mut result, 0, s32(field(item, "element")?, "elemental profile element")?);
    for (slot, entry) in list(field(item, "levels")?, 4, "elemental profile levels")?.iter().enumerate() {
        result[4 + slot] = u8(entry, "elemental profile level")?;
    }
    for (slot, raw) in list(field(item, "stats")?, 4, "elemental profile stats")?.iter().enumerate() {
        let stat = list(raw, 2, "elemental profile stat")?;
        write_u16(&mut result, 8 + slot * 4, u16(&stat[0], "elemental profile power")?);
        write_u16(&mut result, 10 + slot * 4, u16(&stat[1], "elemental profile resistance")?);
    }
    Ok(result)
}
...
pub fn build_gameplay_databases(value: &Value) -> Result<Vec<u8>> {
    ...
    for (hero, row) in list(field(source, "level_experience")?, 8, "level_experience")?.iter().enumerate() {
        for (level, entry) in list(row, 99, &format!("level_experience[{hero}]"))?.iter().enumerate() {
            write_u32(&mut experience, (hero * 99 + level) * 4, u32(entry, "level experience")?);
        }
    }
    ...
    for (index, entry) in list(field(source, "party_order")?, 6, "party_order")?.iter().enumerate() {
    ...
    for (row_index, row) in list(field(source, "class_family_matrix")?, 8, "class_family_matrix")?.iter().enumerate() {
        for (column, entry) in list(row, 4, &format!("class_family_matrix[{row_index}]"))?.iter().enumerate() {
            write_i32(&mut matrix, (row_index * 4 + column) * 4, s32(entry, "class family matrix")?);
        }
    }
    ...
    for (index, raw) in list(field(source, "djinn")?, 80, "djinn")?.iter().enumerate() {
        let djinni = list(raw, 2, &format!("djinn[{index}]"))?;
        let mut bytes = vec![0; 12];
        write_u16(&mut bytes, 0, u16(&djinni[0], "Djinn name_message")?);
        for (slot, entry) in list(&djinni[1], 6, "Djinn stat_bonuses")?.iter().enumerate() {
            bytes[4 + slot] = s8(entry, "Djinn stat bonus")? as u8;
        }
        result.extend(bytes);
    }
    let alignment = unsigned(field(source, "alignment_bytes")?, (END - ALIGNMENT) as u64, "alignment_bytes")? as usize;
    result.extend(vec![0; alignment]);
    if result.len() != RESOURCE_SIZE {
        return err(format!("gameplay databases rebuilt 0x{:x} bytes", result.len()));
    }
    Ok(result)
}
...
pub fn run(args: Vec<String>) -> Result<()> {
    ...
    match command.as_str() {
        "build-stdout" => io::stdout().write_all(&build_gameplay_databases(&value)?).map_err(|error| error.to_string()),
...
mod tests {
    ...
    fn sparse_records_expand_to_binary_defaults() {
        ...
        assert!(combatant[14..27].iter().chain(&combatant[28..]).all(|byte| *byte == 0));
    }
