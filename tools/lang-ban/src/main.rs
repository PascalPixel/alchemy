...
// that is not in the allowlist. The allowlist is the grandfathered
// set captured when the ban landed; it may shrink and must never grow. A port
// lands in `tools/`, its callers move, and only then does the original leave
// the tree and this file.
//
...
//     our codebase. lang-ban walks `tools/` under the
...
const SCANNED: [&str; 1] = ["tools"];

fn repo_root() -> PathBuf {
    // The crate sits at <root>/tools/lang-ban.
...
fn main() -> ExitCode {
    ...
    eprintln!("New tooling goes in tools/ as a workspace crate. See AGENTS.md, Hard rules.");
    ExitCode::FAILURE
}
...
mod tests {
    ...
    fn the_tool_root_is_scanned() {
        ...
        assert_eq!(
            found,
            ["tools/b.ts".to_string(), "tools/a.ts".to_string()]
                .into_iter()
                .collect::<BTreeSet<_>>(),
            "only tools/ is policed"
        );
    }
