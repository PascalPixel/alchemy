...
Measured from `out/full/asm/manifest.json` (written by `tools/build-asm`,
`verification=rom`), the classifier in `tools/remaining-survey/target/release/remaining-survey`, and
`tools/overlay-inventory/target/release/overlay-inventory`. Region counts are scheduling diagnostics, not
...
`retainedStructuralBytes`. `tools/audit-residuals` asserts the two sum to
`asm_bytes`.

So the per-kind census printed by `make build-asm` reconciles exactly,
...
`discoverOverlay()` in `tools/overlay-inventory/target/release/overlay-inventory` constructs
...
There is no unwalked main-image region. `tools/audit-residuals` proves
`unowned_bytes=0`: the claimed, assembly, and asset manifests together span all
8 MiB with no gap, and `tools/build-asm` rejects any overlap. Each debt
...
`tools/build-asm/target/release/build-asm --source asm/080bbb0c.s` returns
...
One further gap worth naming: `tools/remaining-survey/target/release/remaining-survey` filters
...
classes are `tools/remaining-survey/target/release/remaining-survey`'s, extended to every debt retention.
...
**File `tools/overlay-inventory/target/release/overlay-inventory`, function `discoverOverlay()` — the
...
The data needed to fix it already exists: `Discovery` in the native
`tools/discover` crate
...
- `tools/remaining-survey/target/release/remaining-survey` filters `retention === "c_candidate"`. Widening it
