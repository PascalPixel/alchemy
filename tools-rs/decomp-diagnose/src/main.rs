...
// `tools/lib/match_m2c.ts`, because `verifyCandidate` sits on top of
// `tools/lib/alchemy_gcc.ts` (the compiler routing layer), which is being
// ported separately. This binary shells out to a generated Bun bridge for that
// one step and does everything else natively. When an `alchemy-gcc` crate
// lands, the bridge is the only thing that has to go.
//
...
fn run() -> Result<Option<String>, String> {
    ...
    let diagnosis = diagnose_candidate(source, &rom, &rom_path, &scratch, &root)?;
...
const BRIDGE: &str = r#"// Generated by tools-rs/decomp-diagnose; safe to delete.
...
fn verify_candidate(
    ...
    if !output.status.success() {
        // Forward the bridge's diagnostics so the failure text stays useful.
        eprint!("{}", String::from_utf8_lossy(&output.stderr));
        return Err("verifyCandidate failed".to_string());
    }
