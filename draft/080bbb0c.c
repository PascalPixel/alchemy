...
void Func_080072f0(void *dst, void *src, s32 size,
                   void *desc); /* 記述子付きコピー */
...
#define MSG_HP_RECOVER                                                         \
  ((s32) & Value_0000081d) /* 「HPが N かいふくした！」                      \
                            */
#define MSG_PP_RECOVER                                                         \
  ((s32) & Value_0000081e)                    /* 「PPが N かいふくした！」   \
                                               */
...
#define MSG_BITTER_BLOW                                                        \
  ((s32) & Value_00000823)                     /* 「つうこんのいちげき！」   \
                                                */
...
#define MSG_PP_LOSS_E                                                          \
  ((s32) & Value_00000829) /* 「PPを Nうしなった！」敵側                     \
                            */
#define MSG_PP_LOSS_P                                                          \
  ((s32) & Value_0000082a) /* 「PPを Nうしなった！」味方側 */
#define MSG_DMG_EMPH_E                                                         \
  ((s32) & Value_00000831) /* ±affinity で 。/！/！！！ 敵側 */
#define MSG_DMG_EMPH_P                                                         \
  ((s32) & Value_00000834) /* ±affinity で 。/！/！！！ 味方側 */
#define MSG_SPIRIT_DRAIN                                                       \
  ((s32) & Value_0000084c)                   /* 「せいしんを すいとられた！」 */
#define MSG_KO_DOWN ((s32) & Value_0000084f) /* 「たおれた！」 */
#define MSG_SUFFOCATE ((s32) & Value_00000850) /* 「いきができない！」 */
#define MSG_NO_EFFECT                                                          \
  ((s32) & Value_00000854) /* 「しかし こうかがなかった！」 */
#define MSG_LEECH_GAIN                                                         \
  ((s32) & Value_0000085e) /* 「さらに PPを Nかいふく！」 */
#define MSG_LEECH_TAKE ((s32) & Value_0000085f) /* 「PPを Nすいとられた！」 */
#define MSG_WAKES                                                              \
  ((s32) &                                                                     \
   Value_00000883) /* 「めをさました！」 */   /* 「PPを Nすいとられた！」 */
...
#define MSG_DELUSION                                                           \
  ((s32) & Value_00000868)                    /* 「げんわくに つつまれた！」 \
                                               */
...
#define MSG_PSY_BLOCK                                                          \
  ((s32) & Value_0000086d) /* 「エナジーを ふうじられた！」 */
#define MSG_REFRAIN                                                            \
  ((s32) & Value_0000086f) /* 「リフレインに はばまれた！」                  \
                            */
#define MSG_REFLECT ((s32) & Value_00000870) /* 「リフレクの こうか！」 */
#define MSG_EVIL_SPIRIT                                                        \
  ((s32) & Value_00000872) /* 「あくりょうに とりつかれた！」 */
#define MSG_DEATH_CURSE                                                        \
  ((s32) & Value_00000873)                 /* 「しのせんこくを うけた！」 */
#define MSG_VENOM ((s32) & Value_00000874) /* 「もうどくに おかされた！」 */
#define MSG_DEATH_COUNT                                                        \
  ((s32) & Value_00000875) /* 「しにがみが てまねく… N」 */
#define MSG_PSY_SEAL                                                           \
  ((s32) & Value_00000876)                  /* 「エナジーを シールされた！」 */
...
#define MSG_CHALLENGE                                                          \
  ((s32) & Value_0000087e)                    /* 「たたかいに もえてきた！」 \
                                               */
#define MSG_IMMOBILE ((s32) & Value_0000087f) /* 「うごけなくなった！」 */
#define MSG_AURA                                                               \
  ((s32) & Value_00000881) /* 「まもりのオーラに つつまれた！」 */
#define MSG_AURA_2                                                             \
  ((s32) & Value_00000882) /* 「まもりのオーラに つつまれた！」強 */
#define MSG_CURE_POISON ((s32) & Value_00000884) /* 「どくが きえた！」 */
#define MSG_CURE_DELUSION                                                      \
  ((s32) & Value_0000088b) /* 「めが みえるようになった！」 */
#define MSG_CURE_SEAL ((s32) & Value_0000088c) /* 「シールが とけた！」 */
#define MSG_CURE_STUN ((s32) & Value_0000088d) /* 「スタンが なおった！」 */
#define MSG_CURE_SPIRIT                                                        \
  ((s32) & Value_0000088f) /* 「しにがみが はなれた！」                      \
                            */
#define MSG_CURE_CURSE                                                         \
  ((s32) & Value_00000894) /* 「しのせんこくを ふりはらった！」 */
#define MSG_BUFFS_RESET                                                        \
  ((s32) & Value_00000896) /* 「のうりょくが もとにもどった！」 */
#define MSG_SPLIT_OFF ((s32) & Value_000008f3) /* 「ぶんれつした！」 */
#define MSG_SPLIT_FAIL                                                         \
  ((s32) & Value_000008f4)                   /* 「ぶんれつに しっぱいした！」 */
#define MSG_APPEARS ((s32) & Value_000008f5) /* 「あらわれた！！」 */
#define MSG_NO_ONE_CAME                                                        \
  ((s32) & Value_000008f6) /* 「しかし だれもこなかった！」 */
...
after_power:
  ...
  nibble = action->target_flags & 15;
  {
    ...
    if (first == -1)
      hit = Battle_HitCheck(actor_id, target_id, range, action->effect,
                            HitFalloff[offset]);
    else
      hit = first;
  }
  ...
  if (skip == 0 &&
    ...
    case DK_ATTACK_X: {
      ...
      do {
        ...
        if (nibble == 4)
          dmg = Math_Div(
              Battle_CalcAttack(actor->attack, scale, 0, bonus) * apwr, 10);
        else
          dmg = Battle_CalcAttack(actor->attack, scale, apwr, bonus);
        dmg *= adjust;
        if (modifier != 0) {
          if (modifier == 1)
            dmg = dmg * 5 / 4;
  ...
  case EFX_RES_DOWN1:
    ADJUST_RES(-1, (copy->res_modifier - target->res_modifier) * 20,
               MSG_RES_DOWN);
    break;
  ...
  case EFX_RES_DOWN2: {
    ...
    if (target->res_modifier > 4)
      target->res_modifier = 4;
    BattleEvent_Push(BATTLE_EVENT_VALUE,
                     (copy->res_modifier - target->res_modifier) * 20);
  ...
  case EFX_RES_UP2: {
    ...
    if (target->res_modifier > 4)
      target->res_modifier = 4;
    BattleEvent_Push(BATTLE_EVENT_VALUE,
                     (target->res_modifier - copy->res_modifier) * 20);
  ...
  if (target->hp != 0) {
    if (target->sleep != 0)
      if (target->sleep <= 6 && dealt > 0 && (3 & BattleRandom_Next()) == 0) {
        target->sleep = 0;
        BattleEvent_Push(BATTLE_EVENT_UNIT, target_id);
