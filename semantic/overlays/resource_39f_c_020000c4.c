...
/*
 * resource_39f owner at 0x020000c4, 384 bytes: code 0x020000c4-0x02000235, an
 * alignment halfword at 0x02000236 and the three pool words 0x0200aef8,
 ...
 * This owner is the clearest demonstration of why that rule is needed.  Its
 * three lookups at 0x02000108, 0x02000134 and 0x02000160 all carry the same
 * stored displacement, so pc-relative decoding prints three different targets -
 * 0x02000176, 0x020001a2 and 0x020001ce - each of which lands at a plain join
 * point inside this very function, which no call can mean.  HANDOVER records
 * exactly that puzzle as unexplained for resource_373.  Under the encoding rule
 * all three resolve to 0x0200006c, and assets/code/resource_39f_c_0200006c.c
 * is the byte-exact source for it: it takes a three-word probe position and
 * returns the occupying slot record or 0, which is precisely how each result is
 * used here.
 *
 * Data_0200aef8 is the packed direction word table also used by 0x020002a8:
 * sixteen entries indexed by `heading >> 12`, X step in the high halfword and
 * Z step in the low halfword.  The X step is promoted to 16.16 by masking with
