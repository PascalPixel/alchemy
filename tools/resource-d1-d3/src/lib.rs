...
fn field<'a>(object: &'a Map<String, Value>, name: &str) -> Result<&'a Value, Error> {
    object.get(name).ok_or_else(|| err(format!("missing field {name}")))
}

fn string<'a>(object: &'a Map<String, Value>, name: &str) -> Result<&'a str, Error> {
    field(object, name)?.as_str().ok_or_else(|| err(format!("{name} must be a string")))
}
...
fn parse_palette_tokens(value: &Value, decoded_size: usize) -> Result<Vec<PaletteGroup>, Error> {
    let groups = value.as_array().ok_or_else(|| err("D1 palette tokens must be an array"))?;
    ...
    for (group_position, group_value) in groups.iter().enumerate() {
        ...
        let group = group_value.as_array().ok_or_else(|| err(format!("palette group {group_position} is invalid")))?;
                    ...
                    match token.first().and_then(Value::as_str) {
                        Some("l") if token.len() == 1 => {
                            if output >= decoded_size {
                                return Err(err(format!("palette token {group_position}:{token_position} is invalid")));
                            }
                            output += 1;
                        ...
                        _ => return Err(err(format!("palette token {group_position}:{token_position} is invalid"))),
                    }
                }
...
fn parse_stream(value: &Value) -> Result<StreamPlan, Error> {
    ...
    if string(palette, "source")? != "iro.rgba.png" || string(palette, "size")? != hex(D1_PALETTE_SIZE as u32, 2) {
    ...
    let suffix = if suffix.contains_key("policy") {
        ...
        if number(field(suffix, "size")?, "D1 suffix size")? != 3 || string(suffix, "policy")? != "fallback" {
            return Err(err("D1 suffix policy differs"));
        }
        Suffix::Fallback
    } else {
        Suffix::Tail(parse_alignment_tail(field(plan, "suffix")?, 3, 3, "D1 suffix").map_err(|error| err(error.0))?)
    };
    let image = object(field(plan, "image")?, "D1 image")?;
    exact_keys(image, &["source", "encoding", "width", "height", "columns"], "D1 image")?;
...
fn pair(value: &Value, label: &str, minimum: i64, maximum: i64) -> Result<[i64; 2], Error> {
    let pair = value.as_array().ok_or_else(|| err(format!("{label} must be a coordinate pair")))?;
    if pair.len() != 2 {
...
fn parse_motion(value: &Value, spec: Spec) -> Result<MotionPlan, Error> {
    ...
    if string(consumer, "function")? != function
        ...
        || number(field(consumer, "boundary_delta_pairs")?, "boundary_delta_pairs")? != boundary_count as i64
    {
    ...
    if 4 + (deltas.len() + boundary_deltas.len()) * 2 != spec.source_size {
        return Err(err("motion-path records do not fill their resource boundary"));
    }
    Ok(MotionPlan { initial: [initial[0] as i16, initial[1] as i16], deltas, boundary_deltas })
}
...
fn build_stream(path: &Path) -> Result<(Vec<u8>, Vec<PathBuf>), Error> {
    ...
    let root = path.parent().ok_or_else(|| err("D1 stream plan has no parent"))?;
...
fn build_motion(path: &Path, spec: Spec) -> Result<Vec<u8>, Error> {
    let plan = parse_motion(&json_document(path, &format!("{} motion path", id_text(spec.id)))?, spec)?;
...
pub fn build_resource_d1_d3(index_path: &Path) -> Result<Vec<BuiltResource>, Error> {
    ...
    let root = index_path.parent().ok_or_else(|| err("D1-D3 index has no parent"))?;
...
pub fn run(args: Vec<String>) -> Result<(), Error> {
    ...
    let index = args.get(1).ok_or_else(|| err("build-stdout requires an index"))?;
