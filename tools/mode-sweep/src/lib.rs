...
pub fn root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("tools/mode-sweep is two levels below the repository root")
        .to_path_buf()
}
...
pub const FORK_MODES: &[&str] = &[
    ...
    // Routed by the compiler tables but never registered here, so no sweep could
...
/// Native equivalent of the former `evidencedRoutingFlags`, built on the
/// routing tables owned by `alchemy-routing`.
///
