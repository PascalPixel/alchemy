...
#[rustfmt::skip]
fn parse_hex(value: &str) -> Result<Vec<u8>> { if !value.len().is_multiple_of(2) { return err("hex data has an odd length"); } (0..value.len()).step_by(2).map(|i| u8::from_str_radix(&value[i..i+2],16).map_err(|_| "invalid hex data".to_string())).collect() }
#[rustfmt::skip]
fn u16s(data: &[u8]) -> Result<Vec<u16>> { if !data.len().is_multiple_of(2) { return err("partial u16"); } Ok(data.chunks_exact(2).map(|c| u16::from_le_bytes([c[0],c[1]])).collect()) }
#[rustfmt::skip] fn pack_u16(words: &[u16]) -> Vec<u8> { words.iter().flat_map(|word| word.to_le_bytes()).collect() }
