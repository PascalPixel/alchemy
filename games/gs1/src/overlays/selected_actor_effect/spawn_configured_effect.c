...
/*
 * resource_3c0 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_3c0_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x02009424 and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_3c0_c_02000048.c's adopted
 * Func_02005a2e pair and resource_3c0_c_02005158.c already use on this
...
extern struct Descriptor *Data_02009424[];

struct Effect *Func_020013ea();
struct Effect *Func_020013a8();
void Func_020013b2();
void Func_020013cc();
void Func_0200150e();
s32 Func_02001468();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_020013ea(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_020013a8(options->kind, x, y, z);
    } else {
        effect = Func_020013a8(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_020013b2(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_020013cc(effect, Data_02009424[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_0200150e(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02001468(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02001480(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_0200148e(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_020014cc(effect, 1);
        Func_020014dc(effect, options->callback_arg);
    }
