/*
 * resource_3a7 scene-entity presentation step at 0x02000554, 76 bytes.
 *
 * Complete owner: `push {r5, lr}` / `sub sp, #8` at 0x02000554 through the
 ...
 * A single literal-pool word sits at 0x0200059c, inside the span and after
 ...
 *
 * Shape:
 *   record = Func_0808a080(17);          // scene-entity record by selector
 *   Func_080091c0(26, 30, 1, 1, 23, 34);  // six-argument request
 *   if (record != 0) { 
 *       Func_0808a080(17)[85] = 0;      // reloaded, same selector
 *       record[35] = 1;
 *   }
 *   Func_080770c8(0x201);            // raise the completion flag
 *
 * Call targets resolved with
 *   bun tools/overlay_show.ts resource_3a7 0554 -n 76 |
...
{
    unsigned char *record = Func_0808a080(17);

    Func_080091c0(26, 30, 1, 1, 23, 34);
