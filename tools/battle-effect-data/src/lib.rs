...
pub fn parse(value: &Value) -> Res<()> {
    ...
    for item in direct.unwrap() {
        exact_keys(
            item,
            &["address", "end", "source", "bpp", "columns", "role"],
            "direct graphic",
        )?;
    }
    let weighted = value
        .get("weighted_records")
        .ok_or("weighted-record source has unknown fields")?;
    exact_keys(
        weighted,
    ...
    for record in array(weighted.get("records")).ok_or("weighted-record layout differs")? {
        exact_keys(
            record,
            &["base_result", "rank_origin", "results", "weights"],
            "weighted record",
        )?;
    }
    ...
    let halfword = value
        ...
        .ok_or("halfword graphic has unknown fields")?;
    exact_keys(
        halfword,
        &[
            "address",
    ...
    let sentinel = value
        .get("sentinel_lookup")
        .ok_or("sentinel lookup has unknown fields")?;
    exact_keys(
        sentinel,
        &[
            "address",
    ...
    for script in scripts.unwrap() {
        exact_keys(
            script,
            &["address", "end", "entry_points", "commands"],
            "object script",
        )?;
    ...
    exact_keys(
        value
            .get("sine_table")
            .ok_or("sine table has unknown fields")?,
        &["address", "end", "count", "type", "formula"],
        "sine table",
    )?;
    let gradients = value
        .get("gradients")
        .ok_or("gradient source has unknown fields")?;
    exact_keys(
        gradients,
        &["address", "end", "stride", "consumers", "records"],
        "gradient source",
    )?;
    for record in array(gradients.get("records")).ok_or("gradient layout differs")? {
        exact_keys(
            record,
            &["first_color", "second_color", "runs"],
            "gradient record",
        )?;
    ...
    exact_keys(
        value
            ...
            .ok_or("symbolic pointers has unknown fields")?,
        &["address", "end", "pointers"],
        "symbolic pointers",
    )?;
    let sparse = value
        .get("sparse_table")
        .ok_or("sparse table has unknown fields")?;
    exact_keys(
        sparse,
        &[
            "address",
    ...
    for token in tokens {
        match *token {
            ...
            HalfwordToken::Copy { length, distance } => {
                let length = integer(
                    Some(&Value::Num(f64::from(length))),
                    2.0,
                    33.0,
...
fn build_sentinel_lookup(source: &Value) -> Res<Vec<u8>> {
    ...
    if !text_is(table.get("address"), &hex(0x0809_e686))
        ...
        || table.get("fields").map(minified)
            != Some(minified(&string_list(&["key:s16", "value:s16"])))
...
fn build_sparse_table(source: &Value) -> Res<Vec<u8>> {
    ...
    if !text_is(table.get("address"), &hex(SPARSE_TABLE_ADDRESS))
        ...
        || table.get("fields").map(minified)
            != Some(minified(&string_list(&["key:s32", "value:s32"])))
...
fn export_halfword_graphic(rom: &[u8], root: &Path) -> Res<Value> {
    ...
    let image = tile_png(&decoded, 4.0, 12, None)
        .map_err(|error| error.0)?
        .0;
    write_file(
        &root.join(format!("graphics/battle/effects/{name}")),
        &image,
    )?;
    Ok(Value::obj(vec![
        ...
        (
            "source",
            Value::Str(format!("graphics/battle/effects/{name}")),
        ),
        ("bpp", Value::Num(4.0)),
        ...
        (
            "tokens",
            Value::Arr(tokens.iter().map(halfword_token_value).collect()),
        ),
    ]))
}
...
fn export_sentinel_lookup(rom: &[u8]) -> Res<Value> {
    ...
    Ok(Value::obj(vec![
        ...
        (
            "sentinel",
            Value::Num(f64::from(read_u16(&raw, 48) as u16 as i16)),
        ),
    ]))
}
    ...
    for (start, end, entries) in SCRIPT_LAYOUT {
        ...
        while cursor < raw.len() {
            ...
            for _ in 0..arity {
                ...
                cursor += 4;
                let callback = CALLBACK_SYMBOLS
                    .iter()
                    .find(|(address, _)| *address == word);
...
fn export_gradients(rom: &[u8]) -> Res<Value> {
    ...
    while offset < raw.len() {
        ...
        records.push(Value::obj(vec![
            ...
            (
                "second_color",
                Value::Num(f64::from(read_u16(&raw, offset + 2))),
            ),
            ("runs", Value::Arr(runs)),
        ]));
        offset += 28;
...
fn export_palette_graphics(rom: &[u8], root: &Path) -> Res<Value> {
    let specs: [(u32, u32, u64, &str); 3] = [
        (
            PALETTE_GRAPHICS_ADDRESS,
            0x080a_001e,
            0x300,
            "particle_slash.4bpp.png",
        ),
        (0x080a_001e, 0x080a_00b8, 0x300, "particle_spark.4bpp.png"),
        (
            0x080a_00b8,
            PALETTE_GRAPHICS_END,
            0x200,
            "particle_beam.4bpp.png",
        ),
    ];
    ...
    for (start, end, decoded_bytes, name) in specs {
        ...
        graphics.push(Value::obj(vec![
            ...
            (
                "tokens",
                Value::Arr(tokens.iter().map(palette_group_value).collect()),
            ),
        ]));
    }
...
pub fn export_battle_effect_data(rom: &[u8], root: &Path) -> Res<Value> {
    ...
    for (name, start, end) in [
        (
            "obj_tiles_0809c410.4bpp.png",
            BATTLE_DATA_ADDRESS,
            BATTLE_DATA_ADDRESS + 0x100,
        ),
        (
            "obj_tiles_0809c510.4bpp.png",
            BATTLE_DATA_ADDRESS + 0x100,
            DIRECT_GRAPHICS_END,
        ),
    ] {
    ...
    let source = Value::obj(vec![
        ...
        (
            "kind",
            Value::Str("golden-sun-battle-effect-data".to_string()),
        ),
        ...
        (
            ...
            Value::obj(vec![
                ...
                (
                    "consumers",
                    string_list(&["Func_0808ae74", "Func_0808b05c"]),
                ),
                ("records", Value::Arr(records)),
            ]),
        ),
        ...
        (
            "tail_tables",
            export_table_collection(rom, TAIL_TABLE_LAYOUT)?,
        ),
...
pub fn run(args: Vec<String>) -> Res<()> {
    ...
    if command == "build-stdout" {
        ...
        if input.is_empty() || root.is_empty() {
            return err("usage: battle-effect-data build-stdout SOURCE --root ASSETS");
        }
