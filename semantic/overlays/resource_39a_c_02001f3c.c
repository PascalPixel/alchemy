...
/*
 * Resource 39a overlay routine at 0x02001f3c.
 *
 * Complete owner: `push {lr}` at 0x02001f3c and `pop {r0} / bx r0` at
 * its epilogue, so the owner returns nothing.  A one-word literal pool holding
 * 0x17e3 follows the return and is data, not code.
 *
...
{
    Func_02004278();
    /* 0x17e3 is a plain pooled constant: it is below the overlay's 0x02008000
     * link base, so it is an identifier and not an in-image address. */
    Func_02004248((s32)0x17e3, 1);
    Func_0200428c();
}
