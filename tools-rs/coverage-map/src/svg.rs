...
pub fn category_style(category: &str) -> &'static CategoryStyle {
    const EXACT: CategoryStyle =
        CategoryStyle { fill: "#0072f5", ink: "#eaf2ff", label: "byte-exact C" };
    const SEMANTIC: CategoryStyle =
        CategoryStyle { fill: "#ededed", ink: "#333333", label: "semantic C" };
    const ASSEMBLY: CategoryStyle =
        CategoryStyle { fill: "#333333", ink: "#a1a1a1", label: "assembly" };
    const RETAINED: CategoryStyle =
        CategoryStyle { fill: "#50e3c2", ink: "#04241d", label: "permanent asm" };
    const ASSET: CategoryStyle =
        CategoryStyle { fill: "#ff0080", ink: "#2b0016", label: "data / assets" };
    match category {
        "exact_c" => &EXACT,
        "semantic_c" => &SEMANTIC,
...
pub fn fit_text(text: &str, width: f64, font_size: f64) -> Option<String> {
    ...
    if utf16_len(text) <= room {
        ...
        Some(format!("{}…", utf16_slice_to(text, room.saturating_sub(1).max(1))))
    }
}
...
pub fn label_tag(x: f64, y: f64, text: &str, options: &LabelOptions) -> String {
    let mut parts = vec![
        ...
        format!("font-size=\"{}\"", js_number_string(options.size.unwrap_or(11.0))),
...
fn is_address_label(text: &str) -> bool {
    ...
    let Some(rest) = hex_run(text) else { return false };
    if rest.is_empty() {
        return true;
    }
...
fn card(
    ...
    lines.push(rect_tag(
        frame,
        CARD,
        &RectOptions { stroke: Some(BORDER), radius: Some(8.0), opacity: None },
    ));
    lines.push(label_tag(
        frame.x + 16.0,
        frame.y + 25.0,
    ...
    for placed_area in squarify(&sorted, |item| item.bytes as f64, body)? {
        ...
        let heading = fit_text(
            ...
            inset.width - if size_fits { utf16_len(&size) as f64 * 5.2 + 16.0 } else { 4.0 },
            9.5,
        );
...
fn category_bar(map: &CoverageMap, frame: Rect, lines: &mut Vec<String>) {
    ...
    for category in ["exact_c", "semantic_c", "assembly", "retained_asm"] {
        ...
        lines.push(rect_tag(
            Rect { x: cursor, width: category_width, ..frame },
            category_style(category).fill,
            &RectOptions::default(),
        ));
...
pub fn render_svg(map: &CoverageMap) -> Result<String, String> {
    ...
    let combined_percent =
        js_number_string(crate::intervals::round_half_up_percent(combined, executable)?);
    ...
    lines.push(label_tag(
        margin,
        64.0,
        &format!("{} · {} ROM image", document_string(map, &["target"]), megabytes(rom_bytes)),
        &LabelOptions { size: Some(10.0), fill: Some(MUTED), ..LabelOptions::default() },
    ));
    lines.push(label_tag(
        width - margin,
        46.0,
    ...
    let percent = |category: &str| {
        js_number_string(document_number(map, &["categories", category, "percent_of_executable"]))
    };
    let metrics: Vec<(&str, String, Option<&str>)> = vec![
        ("exact_c", format!("{}%", percent("exact_c")), Some("FULL-C BYTE SHARE")),
        ("semantic_c", format!("{}%", percent("semantic_c")), None),
        ("assembly", format!("{}%", percent("assembly")), None),
        ("retained_asm", format!("{}%", percent("retained_asm")), Some("BY DESIGN")),
        (
            "asset_data",
    ...
    for (index, (category, value, note)) in metrics.iter().enumerate() {
        ...
        lines.push(rect_tag(
            Rect { x, y: 96.0, width: 8.0, height: 8.0 },
            category_style(category).fill,
            &RectOptions { radius: Some(2.0), ..RectOptions::default() },
        ));
        lines.push(label_tag(
            x + 14.0,
            104.0,
    ...
    category_bar(
        map,
        Rect { x: margin, y: 146.0, width: width - margin * 2.0, height: 6.0 },
        &mut lines,
    );
    ...
    card(
        ...
        &map.rom_areas,
        Rect { x: margin, y: card_top, width: 372.0, height: card_height },
        &mut lines,
    )?;
    card(
...
mod tests {
    ...
    fn fit_text_matches_the_javascript_boundaries() {
        assert_eq!(fit_text("abcdef", 3.0, 9.0), None, "room below 3 yields undefined");
    ...
    fn the_address_label_regex_is_case_insensitive_and_wants_an_en_dash() {
        ...
        assert!(is_address_label("08000000–0800FFFF"), "uppercase hex still matches");
        assert!(!is_address_label("08000000-0800ffff"), "a hyphen is not an en dash");
    ...
    fn a_falsy_radius_or_stroke_emits_nothing() {
        let frame = Rect { x: 1.0, y: 2.0, width: 3.0, height: 4.0 };
        let plain = rect_tag(frame, "#fff", &RectOptions::default());
        assert_eq!(plain, "<rect x=\"1\" y=\"2\" width=\"3\" height=\"4\" fill=\"#fff\"/>");
        let zero = rect_tag(
            frame,
            "#fff",
            &RectOptions { radius: Some(0.0), stroke: Some(""), opacity: None },
        );
    ...
    fn a_start_anchor_is_omitted_but_a_zero_opacity_is_not() {
        ...
        assert!(text.contains("opacity=\"0\""), "opacity checks !== undefined, not truthiness");
    }
}
