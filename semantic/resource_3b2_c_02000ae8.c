...
/*
 * resource_3b2 owner at 0x02000ae8, 472 bytes: code 0x02000ae8-0x02000cb3 and
 * the three pool words 0x0200b2d4, 0x02008ab1 and 0xffff0000 at
 ...
 * 14 places, 13 of which are BL halfwords and exactly one of which is the low
 ...
 *
 * What was changed:
 *  - Data_0200b058 becomes Data_0200b2d4 (file offset 0x32d4 under the
 *    proven 0x02008000 link base);
 *  - the calls were re-resolved with 'bun tools/overlay_call_targets.ts
 *    resource_3b2 0ae8': the same 10 sites, 6 distinct targets, all veneers
...
extern u32 Data_0200b2d4[];   /* first three words are descriptor pointers */
...
extern u8 * Func_02003b92();
extern u8 * Func_02003b38();
extern void Func_02003b42();
extern void Func_02003b5c();
extern void Func_02003c8e();
extern s32 Func_02003bd8();
extern s32 Func_02003bf0();
extern s32 Func_02003bfe();
extern void Func_02003c5c();
extern void Func_02003c6c();
                   ...
                   u32 flags, const u8 *options)
{
    ...
    effect = Func_02003b38(kind, x, y, z);
    if (effect == 0) return;

    block = *(u8 **)(effect + 80);

    Func_02003b42(effect, (flags + 1) & 15);
    Func_02003b5c(effect, Data_0200b2d4[flags & 15]);
    ...
    *tag = 0;
    ...
    if ((flags & 0x10000) != 0) {                   /* 128 << 9 */
        Func_02003c8e(effect, *(s32 *)(options + 4));
    }
    ...
    if ((flags & 0x40000) != 0) {                   /* 128 << 11 */
        const s32 *descriptor = (const s32 *)Data_0200b2d4[flags & 15];
        s32 delta;
        ...
        if ((flags & 0x80000) != 0) {
            *(s32 *)(effect + 48) =
                Func_02003bd8(*(s32 *)(options + 16) - *(s32 *)(effect + 24),
                              descriptor[3]);
            delta = *(s32 *)(options + 20) - *(s32 *)(effect + 28);
        } else {
            *(s32 *)(effect + 48) =
                Func_02003bf0(*(s32 *)(options + 16) + (s32)0xffff0000,
                              descriptor[3]);
         ...
         * once. */
        *(s32 *)(effect + 52) = Func_02003bfe(delta, descriptor[3]);
    }

    if ((flags & 0x200000) != 0) {                  /* 128 << 14 */
        Func_02003c5c(effect, 1);
        Func_02003c6c(effect, *(s32 *)(options + 28));
    }
