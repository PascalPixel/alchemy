//! The native, toolchain-free compiler lints.
//!
//! WHY A REBUILD. The former checks shipped green while looking at less than
//! they claimed, which is the same defect class `cache-key-lint` and
...
//!     so `SCHED_CALL_DEST_DESCENDING_OVERLAY_SOURCES`,
            ...
            State::Code => match (byte, next) {
                ...
                _ => {
                    ...
                    if state == State::Code && !(byte as char).is_whitespace() {
                        value_position = !matches!(byte, b')' | b']' | b'}') && !is_word_byte(byte);
                    }
                }
            },
            State::Line => {
...
pub fn declarations(blanked: &str) -> Vec<Declaration> {
    ...
    while let Some(hit) = blanked[index..].find("new ") {
        ...
        let Some(end) = balanced_end(blanked, after) else {
            continue;
        };
        let Some(name) = binding_name(blanked, start) else {
            continue;
        };
...
mod tests {
    ...
    fn comments_are_blanked_and_strings_are_kept() {
        ...
        assert!(
            !blanked.contains("08005678"),
            "a stem named in a comment must not"
        );
    ...
    fn map_values_are_not_mistaken_for_stems() {
        ...
        let found = declarations(&blank_comments(text));
        let keys: Vec<String> = routing_keys(&found[0])
            .into_iter()
            .map(|(_, value)| value)
            .collect();
        assert_eq!(
            keys,
            vec!["080f9a30"],
            "the register order is data, not a stem"
        );
    }
