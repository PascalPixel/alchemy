...
pub const CATEGORY_ORDER: [&str; 5] =
    ["exact_c", "semantic_c", "assembly", "retained_asm", "asset_data"];
...
mod tests {
    ...
    fn area_omits_zero_categories_and_drops_empty_tiles() {
        let mut solid = Tile { label: "a".into(), bytes: 10, ..Tile::default() };
        solid.set_category("exact_c", 10);
        solid.set_category("assembly", 0);
        let empty = Tile { label: "b".into(), bytes: 0, ..Tile::default() };
    ...
    fn categories_keep_the_stacking_order_not_alphabetical_order() {
        ...
        assert_eq!(keys, vec!["retained_asm", "exact_c"], "insertion order, not sorted");
    }
}
