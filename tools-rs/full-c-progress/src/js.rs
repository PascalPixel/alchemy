...
// `tools-rs/full-c-history/src/js.rs`, which inlined the same helpers; they are
...
pub fn split_lines(text: &str) -> Vec<&str> {
    ...
    while cursor < bytes.len() {
        if bytes[cursor] == b'\n' {
            let end = if cursor > start && bytes[cursor - 1] == b'\r' { cursor - 1 } else { cursor };
...
pub fn subject_match(subject: &str) -> Option<(&str, &str)> {
    ...
    while let Some(offset) = subject[from..].find(SUBJECT_OPEN) {
        ...
        let Some((numerator, rest)) = canonical_number_run(rest) else { continue };
        let Some(rest) = rest.strip_prefix(" / ") else { continue };
        let Some((denominator, rest)) = canonical_number_run(rest) else { continue };
        if rest == " ]" {
...
fn take_run(text: &str, accept: impl Fn(char) -> bool) -> (&str, &str) {
    let taken = text.find(|character: char| !accept(character)).unwrap_or(text.len());
    text.split_at(taken)
}
...
pub fn branch_label(line: &str) -> Option<&str> {
    ...
    for skip in candidates {
        ...
        for suffix in [2usize, 0] {
            ...
            let Some((length, tail)) = local_label_body(trimmed) else { continue };
...
fn directive_word(line: &str, words: &[&str]) -> bool {
    let Some(rest) = trim_js_start(line).strip_prefix('.') else { return false };
...
pub fn pc_relative_literal(line: &str) -> Option<i64> {
    ...
    while let Some(offset) = lowered[from..].find("ldr") {
        ...
        let Some(rest) = trimmed.strip_prefix('r') else { continue };
        ...
        let Some(rest) = rest.strip_prefix("[pc,") else { continue };
        let rest = trim_js_start(rest);
        let Some(rest) = rest.strip_prefix('#') else { continue };
...
pub fn overlay_source_name(name: &str) -> bool {
    let Some(rest) = name.strip_prefix("resource_") else { return false };
    let Some(stem) = rest.strip_suffix("_overlay.s") else { return false };
...
mod tests {
    ...
    fn overlay_name_helpers() {
        ...
        assert_eq!(strip_overlay_suffix("resource_36f_overlay.s"), "resource_36f");
        assert_eq!(
