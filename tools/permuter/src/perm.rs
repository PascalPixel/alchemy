...
enum Node {
    ...
    Combine(Vec<Node>),
    General(Vec<Node>),
    Once { id: usize, key: String, inner: Box<Node> },
    Var { name: Box<Node>, value: Option<Box<Node>> },
    LineSwap(Vec<Node>),
...
fn checked_add(left: usize, right: usize) -> Result<usize, String> {
    left.checked_add(right).ok_or_else(|| "permutation count overflow".to_string())
}

fn checked_mul(left: usize, right: usize) -> Result<usize, String> {
    left.checked_mul(right).ok_or_else(|| "permutation count overflow".to_string())
}
...
impl Node {
    fn count(&self) -> Result<usize, String> {
        match self {
            ...
            Self::General(parts) => parts.iter().try_fold(0usize, |total, part| checked_add(total, part.count()?)),
        ...
        match self {
            ...
            Self::Var { name, value } => {
                ...
                if let Some(value) = value {
                    ...
                    state.vars.get(&key).cloned().ok_or_else(|| format!("undefined PERM_VAR {key}"))
                }
            }
            Self::LineSwap(lines) => {
            ...
            Self::Ignore(inner) => Ok(format!("{IGNORE_START}{}{IGNORE_END}", inner.evaluate(seed, state)?)),
            Self::Pretend(inner) => Ok(format!("{PRETEND_START}{}{PRETEND_END}", inner.evaluate(seed, state)?)),
            Self::Randomize(inner) => Ok(format!("{RANDOM_START}{}{RANDOM_END}", inner.evaluate(seed, state)?)),
...
impl Parser {
    fn parse(&mut self, text: &str) -> Result<Node, String> {
        ...
        Ok(if parts.len() == 1 { parts.remove(0) } else { Node::Combine(parts) })
    }

    fn comma_nodes(&mut self, text: &str) -> Result<Vec<Node>, String> {
        split_top(text, b',')?.into_iter().map(|part| self.parse(&part)).collect()
    }
    ...
    fn macro_node(&mut self, name: &str, text: &str) -> Result<Node, String> {
        match name {
            ...
            "PERM_INT" => {
                ...
                let low = parts[0].trim().parse::<i64>().map_err(|_| "bad PERM_INT low")?;
                let high = parts[1].trim().parse::<i64>().map_err(|_| "bad PERM_INT high")?;
                if low > high {
            ...
            "PERM_ONCE" => {
                ...
                self.next_once += 1;
                Ok(Node::Once { id, key, inner: Box::new(self.parse(value)?) })
            }
            "PERM_VAR" => {
                let parts = split_top(text, b',')?;
                if !(1..=2).contains(&parts.len()) {
...
pub fn parse(source: &str) -> Result<Permutation, String> {
    ...
    Ok(Permutation { root, count, random, once })
}
...
pub fn self_test() -> Result<(), String> {
    ...
    if !rendered.contains("kept();") || rendered.contains("removed();") || !rendered.contains("live();") {
