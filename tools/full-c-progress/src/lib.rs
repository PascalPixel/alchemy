...
/// the crate manifest directory baked in at compile time: `tools/full-c-progress`
/// -> repository root.
fn root() -> PathBuf {
...
/// Permanent-assembly bytes for the subject prefix, from the coverage map.
///
/// The progress report itself counts only exact C, so DONE's second term has to
/// come from the map. Both are tracked artifacts regenerated by `make coverage`
/// and gated by `coverage-check`, so a stale map blocks the commit rather than
/// letting a wrong percentage through.
fn permanent_asm_bytes(root: &Path, target: &str) -> Result<f64, String> {
    let path = root
        .join("metrics")
    ...
    document
        .get("categories")
        .and_then(|value| value.get("retained_asm"))
        .and_then(|value| value.get("bytes"))
        .and_then(json::Value::as_f64)
        .ok_or_else(|| format!("{} has no categories.retained_asm.bytes", path.display()))
}
...
fn run(argv: &[String]) -> Result<(), String> {
    ...
    if options.write_report {
        ...
        println!(
            ...
            format_subject(
                report.full_c_bytes,
                permanent_asm_bytes(&root, &options.target)?,
...
fn self_test() -> Result<(), String> {
    ...
    if format_subject(123_456.0, 0.0, 1_234_567.0)? != "☀️ 10% –" {
