...
fn path(root: &Path, source: &str) -> Result<PathBuf, Error> {
    let tail = source
        .strip_prefix("graphics/fonts/localization_font/")
        .or_else(|| source.strip_prefix("graphics/fonts_localization_font_"))
    ...
    Ok(if source.starts_with("graphics/fonts/localization_font/") {
...
fn build_direct(source: &Map<String, Value>, root: &Path) -> Result<Vec<Vec<u8>>, Error> {
    ...
    for (index, item) in array(source, "direct_tiles", "localization-font source")?.iter().enumerate() {
        ...
        if address(item.get("address").unwrap(), "direct tile")? != start
            || address(item.get("end").unwrap(), "direct tile end")? != end
        ...
        (Some("g"), 2) => {
            ...
            for item in items[1].as_array().ok_or_else(|| Error("packed-image tokens differ".into()))? {
                ...
                match (
                    ...
                    item.get(2).and_then(Value::as_u64),
                ) {
...
fn build_packed(source: &Map<String, Value>, root: &Path) -> Result<Vec<u8>, Error> {
    ...
    if address(packed.get("address").unwrap(), "packed image")? != PACKED_ADDRESS
        || address(packed.get("end").unwrap(), "packed image end")? != PACKED_END
        || images != 8
    {
...
fn build_font(source: &Map<String, Value>, root: &Path) -> Result<Vec<u8>, Error> {
    ...
    if address(font.get("address").unwrap(), "font")? != FONT_ADDRESS
        || usize_field(font, "first_code", "font")? != 32
        || usize_field(font, "glyphs", "font")? != 224
        || advances.len() != 224
        || advances.iter().any(|value| *value > 0xffff)
    {
        return err("font metadata differs");
    }
    let frames = atlas_frames(
        &path(root, string_field(font, "source", "font")?)?,
        224,
        16,
        16,
...
fn build_articles(source: &Map<String, Value>) -> Result<Vec<u8>, Error> {
    ...
    if address(article.get("data_address").unwrap(), "article data")? != ARTICLE_DATA_ADDRESS
        || address(article.get("table_address").unwrap(), "article table")? != ARTICLE_TABLE_ADDRESS
        || usize_field(article, "data_size", "article")? != data_size
    {
