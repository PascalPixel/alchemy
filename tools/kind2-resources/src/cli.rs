...
use crate::{self_test, verify_kind2_resource, verify_kind2_series, write_build_stdout, write_encode_stdout, Result};
use std::path::Path;
...
fn run(mut args: Vec<String>) -> Result<()> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("verify-resource") => {
            let rom = args.get(1).ok_or_else(|| USAGE.to_string())?;
            println!("{}", verify_kind2_resource(Path::new(rom), Path::new(&option(&args, "--plan")?))?);
            Ok(())
        }
        _ => Err(USAGE.into()),
    }
