...
/*
 * resource_3be owner at 0x02000ba4, 284 bytes. It maps the current actor's
 * signed key through a six-entry table, derives a position and two extents
 * from the matching displacement record, then issues one placement call and
 * two indexed setup calls. The return ends at 0x02000cb2; three pool words
 * extend the owner through 0x02000cbf.
 */
...
{
    const s32 *const keys = (const s32 *)0x02009700;
