...
pub fn union_intervals(intervals: &[Interval]) -> Result<Vec<Span>, String> {
    ...
    for interval in sorted {
        match result.last_mut() {
            ...
            _ => result.push(Span { start: interval.start, end: interval.end }),
        }
    }
    Ok(result)
}
...
pub fn interval_bytes(intervals: &[Interval]) -> Result<f64, String> {
    Ok(union_intervals(intervals)?.iter().map(|span| span.end - span.start).sum())
}
...
fn canonical_count(value: &str, label: &str) -> Result<f64, String> {
    ...
    let parsed = if digits.is_empty() { 0.0 } else { digits.parse::<f64>().unwrap_or(f64::NAN) };
    let canonical = is_safe_integer(parsed)
        && parsed >= 0.0
        && crate::js::commas(parsed as i64) == value;
    if !canonical {
...
pub fn parse_subject(subject: &str) -> Result<Option<Subject>, String> {
    ...
    Ok(Some(Subject { full_c_kilobytes, executable_kilobytes }))
}
        ...
        match result.last_mut() {
            Some(previous) if previous.end == interval.start && previous.kind == interval.kind => {
                ...
                if previous.evidence != interval.evidence {
                    previous.evidence =
                        format!("adjacent audited {} spans; see namespace evidence", interval.kind);
                }
            }
...
mod tests {
    ...
    fn overlap_diagnostics_ignore_identical_aliases() {
        assert_eq!(overlap_diagnostics(&[item(0, 4), item(2, 6)]).unwrap().len(), 1);
        assert_eq!(overlap_diagnostics(&[item(0, 4), item(0, 4)]).unwrap().len(), 0);
    ...
    fn empty_and_reversed_and_unclassified_are_rejected() {
        ...
        let blank = Interval { start: 0.0, end: 4.0, kind: String::new(), evidence: "x".into() };
        assert_eq!(
    ...
    fn subject_round_trips_and_rejects_non_canonical_markers() {
        assert_eq!(format_subject(123456.0, 1234567.0).unwrap(), "[ \u{2600}\u{fe0f} 123 / 1,234 ]");
        let parsed = parse_subject("decomp: x [ \u{2600}\u{fe0f} 123 / 1,234 ]").unwrap().unwrap();
    ...
    fn kilobytes_floor_and_percent_round_half_up() {
        ...
        assert_eq!(to_fixed_2(round_half_up_percent(1.0, 8.0).unwrap()), "12.50");
    ...
    fn merge_coalesces_same_kind_and_generalises_evidence() {
        ...
        assert_eq!(merged[0].evidence, "adjacent audited thumb spans; see namespace evidence");
    ...
    fn merge_orders_equal_spans_by_kind() {
        let merged =
            merge_classified(&[kinded(0, 4, "thumb"), kinded(0, 4, "arm")]).unwrap_err();
