...
//
// Inlined from other modules: nothing. The TypeScript imports only `node:fs`
// and `node:path`; the scoring step shells out to `bun tools/lib/candidate_show.ts`
// as a *subprocess*, and this port spawns exactly the same command line with the
// same working directory, so no part of that library was transcribed.
//
...
impl Sweeper {
    ...
    fn score(&self, source: &str, slot: usize) -> Option<Score> {
        ...
        command
            .arg("tools/lib/candidate_show.ts")
            .arg(&path)
            .arg("--work")
            .arg(directory.join("w"))
        ...
        let mut child = match command.spawn() {
            Ok(child) => child,
            Err(error) => die(&format!("failed to spawn bun: {error}")),
        };
        let mut stdout = child.stdout.take().expect("piped");
        let mut stderr = child.stderr.take().expect("piped");
        // Drain stderr on a helper thread so a chatty child cannot deadlock.
        ...
        // `new Response(...).text()` decodes UTF-8 with replacement, as does this.
        parse_score(&String::from_utf8_lossy(&bytes))
    }
}
