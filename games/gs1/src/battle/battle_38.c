    ...
    struct BattlePresentationUnitEntry *entries)
{
    ...
    for (unit_index = 0; unit_index < unit_count; unit_index++) {
        u8 *unit = Battle_unk2_4(unit_ids[unit_index]);
        s32 copy_index;
    ...
    struct BattlePresentationOpponentEntry *entries)
{
    ...
    for (i = 31; i >= 0; i--) {
        u32 first = (u32)(unit_count *Battle_unk2_4()) >> 16;
        u32 second = (u32)(unit_count *Battle_unk2_4()) >> 16;
    ...
    if (battle[0x45] == 2) {
        s32 limit = ((u32)(Battle_unk2_4() * 5) >> 16) + 1;
