 ...
 *
 * Matching shape, witnessed against the reference bytes:
 * - the selector mask must be built as `shade = 12; shade &= raw;` with the
 *   raw byte in its own local, so the immediate is the and destination
 *   (`movs r1, #12 / ands r1, r2`); folding it into the load keeps the loaded
 *   register as destination and misallocates the whole tail;
 * - the +21 store must be spelled value-first, `(s32)(shade) | (u8)(...)`,
 *   which pins the sprite-record pointers to r0 and the selector to r1; and
 * - the final `orrs r2, r1` needs the routed `-fthumb-orr-dead-input-reuse`
 *   mode (an already-admitted route with three exact overlay users) to reuse
 *   the dead masked input as the or destination.
 *
...
void Func_020006e8(u8 *self)
{
    ...
    second_sprite[21] = (s32)(shade) | (u8)((second_sprite[21] & clear));
}
