...
//
//   bun tools/decomp/index.ts <subcommand> [args...]
//   bun tools/decomp/index.ts --list
...
async function main(): Promise<void> {
  ...
  if (subcommand === undefined || subcommand === "--list" || subcommand === "--help") {
    console.log("usage: decomp <subcommand> [args...]");
    for (const name of available) console.log(`  ${name}`);
    return;
  }
  if (subcommand === "--self-test") {
    if (available.length === 0) throw new Error("decomp has no subcommands");
    console.log(`decomp self-test ok (${available.length} subcommands)`);
    return;
  }
