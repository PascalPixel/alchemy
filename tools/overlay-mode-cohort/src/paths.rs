...
/// `tools/overlay-mode-cohort`, which is two levels deep, so it takes two.
///
...
/// exact failure `tools/cache-key-lint` exists to prevent, and sharing a key
...
mod tests {
    ...
    fn root_holds_the_tool_this_crate_ports() {
        assert!(root().join("tools/overlay-mode-cohort/Cargo.toml").exists());
    }
