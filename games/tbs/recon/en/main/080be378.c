 ...
 * epilogue at the tail of games/tbs/asm/080bef88.s (0x080bf1e8), spanning
 * 3696 bytes across games/tbs/asm/{080be378,080be76c,080beb08,080bef88}.s.
 * See games/tbs/recon/en/dossiers.json#main:080be378 for the full map.
 *
 ...
 * "Split functions" note: assembled games/tbs/asm/080be378.s and
 * games/tbs/asm/080be76c.s standalone (both already 4-byte aligned at their
 ...
 * LiteralPool_080be738 (games/tbs/asm/080be378.s, right after the head's own
 ...
 * truth: assembled games/tbs/asm/080beb08.s standalone (already 4-byte
...
void Func_080be378(u8 *req, u8 *tgt)
{
    ...
    *(u32 *)(tgt + 80) = 4;
     ...
     * games/tbs/asm/080be378.s (9 named targets, a 90-entry `.rept` block
...
L_080be76c:
     ...
     * games/tbs/asm/080be76c.s shows `ldr r4,[sp,#12]` (actor) feeding
...
L_080be888:
    {
        ...
        }
        {
             ...
             * games/tbs/asm/080be76c.s and objdumping it shows this array
...
L_080be984:
     ...
     * games/tbs/asm/080be76c.s (0x080be9ce-0x080bea9a) with every
     * literal-pool constant resolved from that region's own pool (see
     * games/tbs/recon/en/dossiers.json#main:080be378's attempt log for the
...
L_080bec90:
    ...
    *(u32 *)(tgt + 80) = *((u8 *)abilityData + 2);
     ...
     * games/tbs/asm/080beb08.s -- treated as an alias of L_080bee00's entry
...
L_080becea:
     ...
     * Pool constants resolved directly from roms/tbs-en.gba (per-owner .o
...
L_080befb4_shared:
     ...
     * games/tbs/asm/080bef88.s against its real load address 0x080bef88 to
     * get exact instruction offsets (see games/tbs/recon/en/dossiers.json#main:080be378
     ...
     *
     * Pool constants resolved directly from roms/tbs-en.gba, computed against
...
L_080bf1a8:
    ...
    *(u16 *)(tgt + 72) = *(u16 *)(req + 6);
     ...
     * (dead-unit tail-call / early-return, inside games/tbs/asm/080beb08.s)
     * and 0x080bf1d4 / 0x080bf1d6 (inside games/tbs/asm/080bef88.s,
