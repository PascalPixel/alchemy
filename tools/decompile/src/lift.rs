...
impl<'a> Lifter<'a> {
    ...
    fn loop_at(&mut self, i: usize, from: usize, cond: Cond, end: usize) -> usize {
        ...
        // A comparison alone may test a call result, not an induction variable.
        // Only remove the tail update when it really increments this counter.
        let simple = tail_ins.len() == 2
            && matches!(self.ins[tail_ins[0]].kind, Kind::AddImm8 { rd, imm: 1 } if rd == counter);
        let head = if simple {
            ...
            format!("for (i = 0; {c}; i++) {{")
        } else if counter == 3 && cond == Cond::Ne && tail_ins.len() > 2 {
...
mod tests {
    ...
    #[test]
    fn legacy_high_register_constant_case() {
        let text = lifted(&[
            0x2280, 0x0212, 0x4690, 0x4641, 0x2001, 0xf000, 0xfff9, 0x4770,
        ]);
        assert!(text.contains("(1, 0x8000)"), "{text}");
    }

    #[test]
    fn legacy_fixed_point_counter_case() {
        let text = lifted(&[
            0x2500, 0x2001, 0xf000, 0xfff9, 0x2380, 0x025b, 0x18ed, 0x0c2b, 0x2b3c, 0xd1f6, 0x4770,
        ]);
        assert!(text.contains("(i >> 16) != 60; i += 0x10000"), "{text}");
    }

    #[test]
    fn legacy_called_predicate_case() {
        let text = lifted(&[
            0x2001, 0xf000, 0xfff9, 0x2000, 0xf000, 0xfff9, 0x2800, 0xd1f7, 0x4770,
        ]);
