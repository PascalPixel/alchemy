//! The plan layer of `tools/lib/alchemy_gcc.ts`, ported to Rust.
//!
//! WHY THIS CRATE EXISTS. `alchemy_gcc.ts` is a 4,404-line module being moved to
//! Rust one layer at a time so that each layer can be proved equivalent before
//! the next one moves. `alchemy-routing` owns which flags a source gets,
...
//! all.
//!
//! RETIREMENT: when `tools/lib/alchemy_gcc.ts` is deleted, delete [`drift`] and
//! `src/bin/plan_drift.rs` with it, and drop the `plan-drift` `[[bin]]` entry. A
//! parity gate with one side missing cannot fail, which is the exact failure it
//! exists to prevent.
