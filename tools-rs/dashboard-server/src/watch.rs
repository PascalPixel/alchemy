// `watchRepository()`.
//
// PORT NOTE, and the one honest behavioural divergence in this crate: Node's
// `fs.watch(dir, { recursive: true })` is FSEvents on macOS, a kernel
// notification stream the standard library does not expose. Without a
// dependency the only faithful-in-effect mechanism is polling. So the
// mechanism differs; the OBSERVABLE behaviour -- a change under a watched path
// schedules a debounced coverage rebuild, and a change to a RESTART_FILES
// entry exits the process with status 0 -- is the same, and the tests below
// prove both, including that an UNWATCHED file does neither.
//
// Polling interval is 250ms, matching the TypeScript's debounce window, so a
// change is seen within one window rather than instantly. That is stated
// rather than hidden.
...
pub fn fingerprint(path: &Path) -> Fingerprint {
    ...
    while let Some(current) = stack.pop() {
        let Ok(metadata) = std::fs::symlink_metadata(&current) else { continue };
        entries += 1;
        newest = js_max(newest, mtime(&current));
        if metadata.is_dir() {
            let Ok(listing) = std::fs::read_dir(&current) else { continue };
...
pub enum Tick {
    ...
    Page,
    /// A RESTART_FILES entry changed: `process.exit(0)`.
    Restart,
}
...
impl Watcher {
    pub fn new(root: &Path) -> Watcher {
        let mut coverage: Vec<(PathBuf, Fingerprint)> = paths::COVERAGE_DIRECTORIES
            ...
            .collect();
        // The completed-full-build evidence files, filtered by existence
        // exactly as `COVERAGE_BUILD_FILES.filter(existsSync)` is.
        for file in paths::coverage_build_files() {
            if file.exists() {
        ...
        Watcher {
            coverage,
            restart: paths::restart_files().into_iter().map(|f| {
                let seen = mtime(&f);
                (f, seen)
            }).collect(),
            page: paths::page_files().into_iter().map(|f| {
                let seen = mtime(&f);
                (f, seen)
            }).collect(),
        }
    ...
    /// One poll. Restart wins, then page, then coverage, mirroring the
    /// TypeScript's ordering: the restart watcher calls `process.exit(0)`
    /// synchronously and nothing after it runs.
...
mod tests {
    ...
    fn with_root<R>(root: &Path, body: impl FnOnce() -> R) -> R {
        let guard = ROOT_LOCK.lock().unwrap_or_else(|poison| poison.into_inner());
    ...
    fn a_change_to_a_watched_restart_file_fires_and_an_unwatched_file_does_not() {
        ...
        with_root(&root, || {
            ...
            assert!(matches!(watcher.tick(), Tick::Idle), "a quiet tree must be idle");
            ...
            // Now the real thing: a RESTART_FILES entry.
            settle();
            touch(&root.join("tools/metrics/coverage_map.ts")).unwrap();
            settle();
            assert!(
    ...
    fn a_change_under_a_watched_coverage_directory_schedules_a_rebuild() {
        ...
        with_root(&root, || {
            ...
            assert!(matches!(watcher.tick(), Tick::Idle), "a change is reported once");
        });
    }
