    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::WrongRecordCount(count) => {
                write!(f, "map graphics load table requires {RECORDS} records, found {count}")
            }
            Self::NonSequentialIndex(index) => {
                write!(f, "map graphics load table indices must be sequential at {index}")
            }
            Self::OffsetOutsideU16 { record, field } => {
                write!(f, "record {record} field {field}: resource offset is outside u16")
            }
...
fn table_slice(rom: &[u8]) -> Result<&[u8], TableError> {
    ...
    rom.get(start..start + RECORDS * RECORD_SIZE).ok_or(TableError::RomTooSmall)
}
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir()
            .join(format!("alchemy-map-load-table-{name}-{}", std::process::id()));
    ...
    fn tampered_documents_are_rejected() {
        ...
        assert!(matches!(build_table(&source), Err(TableError::WrongRecordCount(185))));
        ...
        assert!(matches!(
            build_table(&source),
            Err(TableError::OffsetOutsideU16 { record: 3, field: "palette" })
        ));

        document["records"][3]["palette"] = json!("128");
        document["records"][3]["map_index"] = json!(9);
        fs::write(&source, canonical_json(&document)).unwrap();
        assert!(matches!(build_table(&source), Err(TableError::NonSequentialIndex(3))));
    ...
    fn records_group_by_container_in_order() {
        ...
        assert_eq!(grouped.iter().map(|(_, rows)| rows.len()).sum::<usize>(), RECORDS);
        assert!(grouped.iter().all(|(name, _)| name.chars().all(|c| !c.is_ascii_uppercase())));
        fs::remove_dir_all(&dir).unwrap();
    }
}
