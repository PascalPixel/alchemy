...
export const ROOT = dirname(dirname(dirname(Bun.fileURLToPath(import.meta.url))));
...
// routing set here nor to FORK_MODES in tools/lib/mode_sweep.ts, so no sweep could
...
export function callbackArityLint(): void {
  ...
  const toolsDirectory = dirname(dirname(dirname(Bun.fileURLToPath(import.meta.url))));
...
export function overlayStemCollisionLint(): void {
  ...
  const source = readFileSync(join(root, "tools/lib/alchemy_gcc.ts"), "utf8");
