...
// `tools/discover` port; copied rather than shared by path so this crate
...
impl Discovery {
    ...
    fn note_jump_table(&mut self, pc: i64, table: i64, count: i64) -> Option<Vec<i64>> {
        ...
        let targets: Vec<i64> = (0..count)
            .map(|index| self.u32(table + index * 4))
            .collect();
        if targets.iter().any(|&target| {
            (target & 1) != 0 || !self.inside(target, 2) || (target - pc).abs() > 0x10000
        }) {
            return None;
        }
