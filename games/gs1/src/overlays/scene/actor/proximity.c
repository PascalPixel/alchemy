                                    ...
                                    s32 range, s32 force)
{
    ...
    if (SceneActor_GetPositionDistance(targetPos, actorPos) < range || force != 0) {
        ...
        if ((u8 *)target == SceneActor_Find(0) && (farRight == facing || farLeft == facing)) {
            actor->active = 1;
            SceneActor_SetMode(actor, 1);
            result = 1;
        }
