...
impl ClassificationRule {
    fn classification(&self) -> Classification {
        Classification { kind: self.kind.clone(), origin: self.origin.clone(), retention: self.retention.clone(), confidence: self.confidence.clone(), evidence: self.evidence.clone() }
    }
}
...
pub fn repository_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).parent().and_then(Path::parent).expect("build-asm lives under tools").to_path_buf()
}

pub fn parse_args(argv: &[String]) -> Result<ParseOutcome, String> {
    let mut options = Options { rom: "roms/gs1-en.gba".into(), output: "out/gs1-en/asm".into(), source: None, source_only: false };
...
fn run(root: &Path, command: &[String]) -> Result<String, String> {
    ...
    let output = Command::new(program).args(&command[1..]).current_dir(root).output().map_err(|error| format!("{program}: {error}"))?;
...
fn integer(value: &Value, name: &str) -> Result<u64, String> {
    let parsed = match value {
        ...
        Value::String(text) => text.strip_prefix("0x").or_else(|| text.strip_prefix("0X")).map_or_else(|| text.parse::<u64>().ok(), |digits| u64::from_str_radix(digits, 16).ok()),
        _ => None,
    };
...
fn load_layout(root: &Path) -> Result<BTreeMap<String, Placement>, String> {
    ...
    let regions: Vec<LayoutRegion> = serde_json::from_value(value["regions"].clone()).map_err(|_| "games/gs1/asm/manifest.json: unsupported format".to_string())?;
    let mut result = BTreeMap::new();
    for item in regions {
        if result.contains_key(&item.source) {
            return Err("games/gs1/asm/manifest.json: invalid or duplicate source".into());
        }
        let inferred = u64::from_str_radix(&stem(Path::new(&item.source)), 16).map_err(|_| format!("{}: invalid address", item.source))?;
...
fn load_alignments(path: &Path) -> Result<Vec<(u64, Vec<u8>)>, String> {
    let value: Value = read_json(path)?;
    if value["format"].as_u64() != Some(1) || value["kind"].as_str() != Some("thumb-function-alignment") || value["width"].as_u64() != Some(2) || value["value"].as_u64() != Some(0) || !value["addresses"].is_array() {
...
fn long_call_veneer(data: &[u8]) -> bool {
    data.len() == 8 && u16::from_le_bytes([data[0], data[1]]) == 0x4c00 && u16::from_le_bytes([data[2], data[3]]) == 0x4720 && u32::from_le_bytes([data[4], data[5], data[6], data[7]]) & 1 != 0
}
...
fn thumb_standalone_wide_transfer(source: &str) -> bool {
    ...
    significant.iter().enumerate().any(|(index, transfer)| {
        ...
        let paired_as_load = significant.get(index + 1).and_then(Option::as_ref).is_some_and(|next| approved_thumb_block_copy_pair(transfer, next));
        let paired_as_store = index.checked_sub(1).and_then(|previous| significant.get(previous)).and_then(Option::as_ref).is_some_and(|previous| approved_thumb_block_copy_pair(previous, transfer));
        !paired_as_load && !paired_as_store
    })
}

fn classify(name: &str, data: &[u8], source: &str, config: &ClassificationConfig, explicit: &BTreeMap<String, ClassificationRule>) -> Result<Classification, String> {
    if let Some(fixed) = explicit.get(name) {
        return Ok(fixed.classification());
    }
    if long_call_veneer(data) {
        return config.structural.iter().find(|rule| rule.kind == "linker_veneer").map(ClassificationRule::classification).ok_or_else(|| "missing linker veneer classification".into());
    }
    if alignment_padding(data) {
        return config.structural.iter().find(|rule| rule.kind == "alignment_padding").map(ClassificationRule::classification).ok_or_else(|| "missing alignment padding classification".into());
    }
    if thumb_standalone_wide_transfer(source) {
        return config.groups.iter().find(|rule| rule.matcher.as_deref() == Some("thumb_standalone_wide_transfer")).map(ClassificationRule::classification).ok_or_else(|| "missing Thumb multi-register classification".into());
    }
...
fn validate_counts(config: &ClassificationConfig, counts: &BTreeMap<String, Count>) -> Result<(), String> {
    for rule in config.structural.iter().chain(&config.groups) {
        ...
        if count.files != rule.expected_files || count.bytes != rule.expected_bytes {
            return Err(format!("{}: expected {} files/{} bytes, got {} files/{} bytes", rule.kind, rule.expected_files, rule.expected_bytes, count.files, count.bytes));
        }
    }
    Ok(())
}
...
const ASSEMBLY_BINUTILS: [&str; 4] = ["arm-none-eabi-as", "arm-none-eabi-nm", "arm-none-eabi-ld", "arm-none-eabi-objcopy"];

fn production_binutil_signatures() -> Result<Vec<(String, String)>, String> {
    ASSEMBLY_BINUTILS.iter().map(|name| host_executable_signature(&[*name]).map(|signature| ((*name).to_string(), signature)).map_err(|error| format!("{name}: {error}"))).collect()
}
...
fn read_region_cache(payload: &Path, record: &Path) -> Option<Vec<u8>> {
    ...
    if value.get("format").and_then(Value::as_u64) != Some(1) || value.get("size").and_then(Value::as_u64) != Some(data.len() as u64) || value.get("sha256").and_then(Value::as_str) != Some(sha256::hex(&data).as_str()) {
        return None;
    }
    Some(data)
}
...
pub fn region_cache_key_with_signatures(source: &[u8], linked_address: u64, binutils: &[(String, String)]) -> Result<String, String> {
...
fn valid_external(name: &str) -> bool {
    ...
    ["Func", "Data", "Value"].contains(&prefix) && address.len() == 8 && address.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
}

fn build_region(root: &Path, source: &Path, source_bytes: &[u8], output_dir: &Path, cache_dir: &Path, run_address: Option<u64>, binutils: &[(String, String)]) -> Result<BuiltRegion, String> {
    let name = stem(source);
    let address = u64::from_str_radix(&name, 16).map_err(|_| format!("{}: invalid assembly filename", source.display()))?;
    ...
    let cached = cache_dir.join(format!("{}.bin", region_cache_key_with_signatures(source_bytes, linked_address, binutils)?));
    ...
    run(root, &["arm-none-eabi-as".into(), "-mcpu=arm7tdmi".into(), "-mthumb-interwork".into(), "-o".into(), object.to_string_lossy().into_owned(), source.to_string_lossy().into_owned()])?;
    let undefined = run(root, &["arm-none-eabi-nm".into(), "-u".into(), object.to_string_lossy().into_owned()])?;
    let names: Vec<String> = undefined.lines().filter(|line| !line.is_empty()).filter_map(|line| line.split_whitespace().last().map(str::to_string)).collect();
    for external in &names {
        if !valid_external(external) {
            return Err(format!("{}: unsupported external symbol {external}", source.file_name().unwrap().to_string_lossy()));
        }
    }
    ...
    if !names.is_empty() {
        ...
        for external in &names {
            body.push_str(&format!(".global {external}\n{}.set {external}, 0x{}\n", if external.starts_with("Func_") { ".thumb_func\n" } else { "" }, external.rsplit_once('_').unwrap().1));
        }
        std::fs::write(&symbols_source, body).map_err(|error| format!("{}: {error}", symbols_source.display()))?;
        run(root, &["arm-none-eabi-as".into(), "-mcpu=arm7tdmi".into(), "-mthumb-interwork".into(), "-o".into(), symbols_object.to_string_lossy().into_owned(), symbols_source.to_string_lossy().into_owned()])?;
        objects.push(symbols_object);
    }
    let formatted = format!("{linked_address:08x}");
    let mut link = vec!["arm-none-eabi-ld".into(), format!("-Ttext=0x{formatted}"), "-e".into(), format!("0x{formatted}"), "-o".into(), elf.to_string_lossy().into_owned()];
    link.extend(objects.iter().map(|path| path.to_string_lossy().into_owned()));
    run(root, &link)?;
    run(root, &["arm-none-eabi-objcopy".into(), "-O".into(), "binary".into(), "-j".into(), ".text".into(), elf.to_string_lossy().into_owned(), binary.to_string_lossy().into_owned()])?;
    ...
    write_cache_entry_atomically(&record, region_cache_record(&data).as_bytes()).map_err(|error| format!("{}: {error}", record.display()))?;
...
fn region_value(output: &Path, source: &str, built: &BuiltRegion, category: &Classification) -> Value {
    ...
    object.insert("output".into(), Value::String(output.join(format!("{:08x}.bin", built.address)).to_string_lossy().into_owned()));
...
pub fn build(root: &Path, cwd: &Path, options: &Options) -> Result<BuildReport, String> {
    ...
    for source in &sources {
        ...
        let built = build_region(root, source, source_text.as_bytes(), &output, &cache, placement.map(|item| item.run_address), &binutils).map_err(|error| format!("{source_name}: {error}"))?;
        ...
        if built.address < ROM_BASE || built.address >= limit || built.data.is_empty() || built.address + built.data.len() as u64 > limit {
    ...
    if options.source.is_none() {
        ...
        let category = classification.structural.iter().find(|item| item.kind == "alignment_padding").map(ClassificationRule::classification).ok_or("missing alignment padding classification")?;
    ...
    std::fs::write(output.join("manifest.json"), format!("{}\n", canonical_json(&Value::Object(document)))).map_err(|error| format!("{}: {error}", output.join("manifest.json").display()))?;
    let bytes = regions.iter().map(|item| item.1["size"].as_u64().unwrap() as usize).sum();
    let counts_text = counts.iter().map(|(kind, count)| format!("{kind}={}/{}", count.files, count.bytes)).collect::<Vec<_>>().join(" ");
