...
`graphics/chr_081a/`, `graphics/chr_zenhan/`, `graphics/chr/`, and
`graphics/chr_0828/` reconstruct 22, 18, 33, and 36 descriptor-linked static
character banks as one palette-correct `koma.8bpp.png` (コマ) sheet per physical
bank. The 109 sheets hold 3,085 unique frames instead of creating one file per
frame. Their payload-free plans preserve logical pointer aliases and all three
...
`0x08244fc0..0x08287774`, and `0x0828cfd8..0x082b5060` series re-encode
825,480 bytes byte-for-byte; the eight-byte separator is the third series'
explicit prefix. `zenhan` (前半) is a period-style reconstruction grouping.
Numeric grouping and bank names remain because no historical character names
have been recovered.
