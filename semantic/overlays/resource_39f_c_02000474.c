...
/*
 * resource_39f owner at 0x02000474, 404 bytes: code 0x02000474-0x020005f9, an
 * alignment halfword at 0x020005fa and the three pool words 0x0200af50,
 * 0x0200aef8 and 0xffff0000 at 0x020005fc-0x02000607.
 *
 ...
 *
 * Its three callers - 0x02000f94, 0x02001454 and 0x020019b8 - use exactly that
 * contract: `if (Func_02000474(probe) != 0)`, then read probe[1] as the
 * subject kind and probe[2]/probe[4] as the resulting tile.
 *
 * See resource_39f_c_02000030.c for the link base and the `bl` encoding rule.
 *
 * Two in-image data tables, both confirmed by the 0x02008000 link base:
 *  - Data_0200aef8 (file offset 0x2ef8) is the packed direction word table
 *    also used by 0x020000c4 and 0x020002a8: X step in the high halfword,
 *    Z step in the low halfword.
 *  - Data_0200af50 (file offset 0x2f50) is a table of 16-byte footprint boxes
 *    indexed by the request's kind word: (minX, minZ, maxX, maxZ) in sixteenths
 *    of a tile.  The first six entries are (-32,-8,32,8), (-8,-32,8,32),
 *    (-32,-16,32,0), (-8,-32,8,32), (-32,-8,32,8) and (-8,-32,8,32), so the
 *    sweep counts below are 4x1, 1x4 and so on.
 *
 ...
 * head at 0x020005ae.  That is what the assembly says, and it is consistent
 * with a map that is always enclosed, but nothing in this routine guarantees
 * termination.
 *
...
extern s32 Data_0200af50[];   /* 16-byte footprint boxes, four words each */
...
s32 Func_02000474(s32 *request)
{
    ...
    *state = 2;
    ...
    for (;;) {
        ...
        request[4] = probe[2] + (Data_0200af50[request[0] * 4 + 1] << 16);

        for (z = 0; z < spanZ; z++) {
            request[2] = probe[0] + (Data_0200af50[request[0] * 4] << 16);
        ...
        probe[2] += Data_0200aef8[heading] << 16;
    }
}
