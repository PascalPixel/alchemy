...
import { deflateSync } from "../lib/zlib.ts";
import { chunk } from "../lib/export_asset.ts";
import { prune_files } from "../lib/generated_files.ts";
import { indexed_png } from "../lib/import_asset.ts";
...
export function preview_png(directory: string, count: number, columns = 8, scale = 2): Buffer {
  ...
  for (let index = 0; index < count; index++) {
    const [pixels, palette] = read_image(readFileSync(`${directory}_images_image_${index.toString().padStart(2, "0")}.png`));
    const colors: number[][] = [];
...
export function build_archive(plan: F0Plan, directory: string): Buffer {
  ...
  const packages = Array.from({ length: count }, (_, index) =>
    package_image(readFileSync(`${directory}_images_image_${index.toString().padStart(2, "0")}.png`)));
