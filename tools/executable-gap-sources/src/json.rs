...
//
// WHY: this crate is dependency-free by policy, and `executable_gap_sources.ts`
// only has to read two hand-edited documents (the executable gap plan and the
// source-ownership manifest) and emit two (`index.json` and `report.json`) in
// the layout `tools/lib/canonical_json.ts` defines. A general JSON library
// would add a dependency, and every off-the-shelf one sorts or rehashes object
// keys, which would silently rewrite the plan's key order on the first run.
...
//
// Lifted from `tools/full-c-history/src/json.rs`, which needed the same
...
/// `JSON.stringify(value)` with no spacer: the fully minified spelling.
pub fn stringify_min(value: &Value) -> String {
    let mut out = String::new();
    write_min(value, &mut out);
    out
}

fn write_min(value: &Value, out: &mut String) {
    match value {
        Value::Arr(items) => {
            out.push('[');
            for (index, item) in items.iter().enumerate() {
        ...
        Value::Obj(members) => {
            ...
            for (index, (key, member)) in members.iter().enumerate() {
                ...
                out.push_str(&quote(key));
                out.push(':');
                write_min(member, out);
            }
            out.push('}');
        }
        primitive => primitive.write_primitive(out),
    }
}

/// `JSON.stringify(value, null, 2)`: the legacy two-space spelling.
pub fn stringify_pretty2(value: &Value) -> String {
    let mut out = String::new();
    write_pretty2(value, "", &mut out);
    out
}

fn write_pretty2(value: &Value, indent: &str, out: &mut String) {
    let deeper = format!("{indent}  ");
    match value {
        Value::Arr(items) => {
            // PORT NOTE: `JSON.stringify` renders an empty array as `[]` with no
            // interior newline, and likewise `{}` for an empty object.
            if items.is_empty() {
                out.push_str("[]");
                return;
            }
        ...
        Value::Obj(members) => {
            ...
            for (index, (key, member)) in members.iter().enumerate() {
                ...
                out.push_str(&quote(key));
                out.push_str(": ");
                write_pretty2(member, &deeper, out);
            }
            out.push('\n');
            out.push_str(indent);
            out.push('}');
...
/// `isCanonicalJsonText`: three spellings are accepted while the migration to
/// the compact form runs -- canonical compact, fully minified, and the legacy
/// two-space form, each with a single trailing newline.
pub fn is_canonical_json_text(text: &str, value: &Value) -> bool {
    text == format!("{}\n", canonical_json(value))
        || text == format!("{}\n", stringify_min(value))
        || text == format!("{}\n", stringify_pretty2(value))
}
