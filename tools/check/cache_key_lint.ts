...
//
//   bun tools/check/cache_key_lint.ts
//   bun tools/check/cache_key_lint.ts --self-test
...
function main(): void {
  ...
  // Recursive: tools live in domain folders, and a flat read would lint only
  // the dispatchers while reporting success.
  const walk = (at: string, prefix = ""): string[] =>
    readdirSync(at, { withFileTypes: true }).flatMap((entry) =>
      entry.isDirectory()
        ? walk(join(at, entry.name), prefix === "" ? entry.name : `${prefix}/${entry.name}`)
        : [prefix === "" ? entry.name : `${prefix}/${entry.name}`]);
  const names = walk(directory).filter((name) => name.endsWith(".ts")).sort();
  ...
  if (findings.length > 0) {
    console.log(
      "HAND-MAINTAINED CACHE KEY — this is a FAILURE.\n" +
        ...
        "  See tools/lib/overlay_disasm.ts for the pattern, and HANDOVER §5h for why.",
    );
    process.exitCode = 1;
  }
}
