    ...
    while index < queue.len() {
        ...
        for (row_index, member) in row.iter().enumerate() {
            ...
            placed.push(Placed {
                index: *member,
                rect: if horizontal {
                    Rect { x: free.x, y: free.y + offset, width: thickness, height: side }
                } else {
                    Rect { x: free.x + offset, y: free.y, width: side, height: thickness }
                },
            });
            offset += side;
        }
...
mod tests {
    ...
    fn the_placement_fills_the_rectangle_and_stays_inside_it() {
        ...
        let frame = Rect { x: 4.0, y: 7.0, width: 120.0, height: 80.0 };
    ...
    fn an_empty_or_degenerate_rectangle_places_nothing() {
        ...
        for frame in [
            ...
            Rect { x: 0.0, y: 0.0, width: 10.0, height: 0.0 },
        ] {
            assert!(squarify(&items, identity, frame).expect("placement").is_empty());
        }
        assert!(squarify::<f64>(&[], identity, Rect { x: 0.0, y: 0.0, width: 4.0, height: 4.0 })
            .expect("placement")
            .is_empty());
    }
