...
fn score_from_json(value: &Json, cache_key: &str) -> Option<Score> {
    if !crate::accepted_cache(value, cache_key) {
        return None;
    }
    Some(Score {
...
fn score_one(context: &ScoreContext<'_>, config: Config) -> Score {
    let cache_key = crate::score_cache_key(
...
fn rank(mut scores: Vec<Score>) -> Vec<Score> {
    ...
    scores.sort_by(|left, right| {
        ...
        b.exact
            ...
            .then_with(|| crate::collate(&left.config.key(), &right.config.key()))
    });
    scores
}
...
fn report_json(input: &ReportInputs<'_>) -> Json {
    ...
    let all_results = single
        ...
        .collect::<Vec<_>>();
    Json::Object(vec![
        ("format".into(), Json::Number(crate::FORMAT as f64)),
...
fn run(options: Options, root: &Path) -> Result<(), String> {
    ...
    let output = PathBuf::from(crate::mode_sweep_output_directory(
        root,
        &options.source,
        &source_bytes,
    ));
    ...
    let implementation_digest = crate::current_binary_digest()
