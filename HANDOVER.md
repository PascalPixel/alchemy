...
Updated: 2026-07-30 (semantic-C speed lane plus remote session 3)
...
and continuing the byte-exact decompilation on `main`. Exact means fully linked
machine-code byte equality, not semantic similarity or equal object size.
...
- Semantic-C lane: **256,468 executable bytes across 557 compiling sources**:
  243,676 main bytes and 12,792 overlay bytes. Combined with exact C,
  **432,162 / 1,339,542 executable bytes** are now expressed as C, with
  907,380 remaining. Seven semantic main sources were removed during the
...
- The tenth queue-driven pass adds three complete main owners and 2,168 bytes:
  a 64-particle directional renderer (`080cb4ec`), a wave/point renderer
  (`080ce85c`), and a 32-particle animated effect (`080d59b0`). Their six
  call-via sites all reduce to the same two clean families already established:
...
- The thirteenth queue-driven pass adds five complete main owners and 4,340
  bytes: a two-object transition (`080cb1a4`), a 16x40 curtain effect
  (`080da2ac`), a scheduled-orbit/512-particle renderer (`080dfe2c`), a
  nine-anchor/144-particle animation (`080e08c0`), and a three-mode actor
...
- The 212-byte `080b386c` owner reconstructs an inventory-value detail panel:
  ...
  preserved.
- The 320-byte `080a9598` owner reconstructs an inventory page renderer:
  page-relative visibility, item-icon rows or fallback text, page/count
  indicators, owner display, and two owner values. Its true three-argument ABI
  keeps the page pointer in `r2`, and all 11 static call sites are preserved.
- The 260-byte `080b1868` owner reconstructs validated inventory-item
  application: selected/replacement restrictions, confirmation, item
  application, optional detail refresh, flagged-item feedback, and completion.
  Its full owner/slot ABI and distinct metadata flag tests are explicit, and
  all 16 static call sites are preserved.
- The 208-byte `08021cb8` owner reconstructs decoded resource remapping:
  relative resource lookup, 1,024-byte palette-index remapping, dynamic palette
  allocation with overflow behavior, VRAM DMA, and ordered cleanup. Its mapping
  counter and decoded-source layouts are explicit, and all six static call
  sites are preserved.
- The 248-byte `080b5e14` owner reconstructs record transfer and name-prefix
  installation: a three-owner cohort, qualifying-result count, decoded
  halfword tokens, backwards 15-byte name shifting, a second transfer, and
  paired allocation cleanup. Its record layout and 48-byte token buffer are
  explicit, and all 13 static call sites are preserved.
- The 284-byte `08077f70` owner reconstructs two-owner state initialization:
  flag setup, copied/scaled halfword values, bounded inventory replacement,
  per-owner rebuilding, four final assignments, and a global time increment.
  Its signed division inputs and explicit owner/index mutation ABI are
  preserved across all 17 static call sites.
- The 360-byte `080b29a8` owner reconstructs a modal object-backed menu:
  nested resource lookup, two-stage window fallback, display-object setup,
  signed state dispatch, detail selection, menu refresh, and ordered teardown.
  Its object/runtime layouts and true object-ID ABI are explicit, and all 22
  static call sites are preserved.
- The 360-byte `080a6b64` owner reconstructs a styled item-page renderer:
  alternate heading labels, page-relative visibility, page/count indicators,
...
- The 316-byte `0807a664` owner reconstructs a marker-gated inventory snapshot:
  ...
  are preserved.
- The 380-byte `080a8f40` owner reconstructs an icon-rich item-page renderer:
...
- The 328-byte `08091890` owner reconstructs owner-state recovery:
  ...
  are preserved.
- The 304-byte `080a1d08` owner reconstructs a character display/prompt:
  variant-dependent handle setup, character rendering, optional input waiting,
