...
/*
 * resource_37b owner at 0x02000030, 8 bytes: a constant getter --
 * `ldr r0, [pc, #0] / bx lr` over its own one-word pool, returning the
 * in-image address 0x0200a5c0 (overlay offset 0x25c0).
 *
 * Complete owner: NO PROLOGUE. It saves nothing, touches no stack, never
 * writes lr, and returns with a bare `bx lr` at 0x02000032. The span
 * is 8 bytes and NOT 4: the literal pool word sits PAST the
 * `bx lr`, at 0x02000034, and belongs to this owner. Next owner at
 * 0x02000038. Takes no argument.
 *
 ...
 * at overlay offset 0xc. That word is one field of a DESCRIPTOR TABLE
 ...
 * overlay's own image (0x25c0 of 0x2978); nothing here establishes
 * what lives there, and it is deliberately not named.
 */

u8 *Func_02000030(void)
{
    return (u8 *)0x0200a5c0;
}
