...
/*
 * resource_387 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_387_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x020092dc and `idx = flags & 0xf` -- the low nibble of the `flags`
...
extern struct Descriptor *Data_020092dc[];

struct Effect *Func_020012a2();
struct Effect *Func_02001238();
void Func_02001242();
void Func_0200125c();
void Func_020013d6();
s32 Func_02001300();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_020012a2(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02001238(options->kind, x, y, z);
    } else {
        effect = Func_02001238(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_02001242(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_0200125c(effect, Data_020092dc[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_020013d6(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02001300(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02001318(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_02001326(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_0200135c(effect, 1);
        Func_0200136c(effect, options->callback_arg);
    }
