...
//! them inherited its `alchemy_gcc.ts` import. That import is why seven earlier
//! attempts to port this module ran out of context before writing a line. The
//! 4,404-line module has since been taken apart into five crates, so the
//! closure is now finite:
//!
...
//!   applied by `alchemy_gcc.ts` callers, not by `overlay_disasm.ts`.
//!
//! Two further crates cover the non-`alchemy_gcc` imports: `discover` for
//! `Discovery` and `cache-entry` for `write_cache_entry_atomically`.
//!
//! `ROOT` is the one `alchemy_gcc.ts` export that has no crate: it is
//! `dirname(dirname(dirname(alchemy_gcc.ts)))`, i.e. the repository root, and
//! it is recomputed here from `CARGO_MANIFEST_DIR` (see [`paths::root`]).
//!
//! THE CACHE KEY IS LOAD-BEARING. `tools-rs/cache-key-lint` cites this module
