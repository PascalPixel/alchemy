...
// `/snapshot` body and every SSE `data:` frame are compared byte for byte
// against Bun. `serde_json` sorts object keys unless the `preserve_order`
// feature is on, and its number writer is ryu, which is not ECMAScript
// `ToString(Number)`. Both would change bytes silently.
//
...
impl Json {
    pub fn obj(members: Vec<(&str, Json)>) -> Json {
        Json::Obj(members.into_iter().map(|(key, value)| (key.to_string(), value)).collect())
    }
...
mod tests {
    ...
    fn insertion_order_is_not_sorted_order() {
        ...
        let value =
            Json::obj(vec![("zeta", Json::Num(1.0)), ("alpha", Json::Num(2.0))]);
    ...
    fn the_page_version_float_survives_round_trip() {
        ...
        assert_eq!(Json::Num(1_785_756_380_214.988_8).stringify(), "1785756380214.9888");
    }
    ...
    fn control_characters_and_quotes_escape_but_astral_text_does_not() {
        assert_eq!(quote("a\u{22}b\u{5c}c\nd\u{1}"), "\"a\\\"b\\\\c\\nd\\u0001\"");
        assert_eq!(quote("Reading the stones… ☀️"), "\"Reading the stones… ☀️\"");
    }
}
