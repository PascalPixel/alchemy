...
/*
 * resource_390 owner at 0x0200012c, 72 bytes: an interaction handler.  If the
 ...
 * all 36 halfwords except: the entry call's two immediates (13, 16 -> 15, 18),
 * the message-id pool word (0x16ad -> 0x16b1), the act call's first immediate
 * (16 -> 18).  Every call was
 * re-resolved with 'bun tools/overlay_call_targets.ts resource_390 012c'
 * rather than transposed with the body.
 *
 * Complete owner: 'push {r5, lr}' and the single interworking epilogue at
 * 0x02000160.  Control-flow walk: one forward diamond rejoining before
 ...
 * 'bun tools/overlay_call_targets.ts resource_390 012c'.  The inventory
