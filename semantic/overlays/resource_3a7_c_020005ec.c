/*
 * resource_3a7 scene-entity presentation step at 0x020005ec, 84 bytes.
 *
 * Complete owner: `push {r5, lr}` / `sub sp, #8` at 0x020005ec through the
 ...
 * A single literal-pool word sits at 0x0200063c, inside the span and after
 ...
 *
 * Shape:
 *   record = Func_0808a080(19);          // scene-entity record by selector
 *   Func_080091c0(26, 30, 1, 1, 26, 32);  // six-argument request
 *   if (record != 0) { Func_080091e0(record, 0);
 *       Func_0808a080(19)[85] = 0;      // reloaded, same selector
 *       record[35] = 1;
 *   }
 *   Func_080770c8(0x203);            // raise the completion flag
 *
 * Call targets resolved with
 *   bun tools/overlay_show.ts resource_3a7 05ec -n 84 |
...
{
    unsigned char *record = Func_0808a080(19);

    Func_080091c0(26, 30, 1, 1, 26, 32);

    if (record != 0) {
        Func_080091e0(record, 0);
        /* The record is reloaded with the same selector before this store. */
        Func_0808a080(19)[85] = 0;
        record[35] = 1;
    }
