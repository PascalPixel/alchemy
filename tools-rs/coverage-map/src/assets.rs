...
pub fn has_extension(name: &str, max: usize) -> bool {
    ...
    let Some(dot) = lowered.rfind('.') else { return false };
...
impl ManifestWalk<'_> {
        ...
        match node {
            ...
            Value::Str(text) => {
                ...
                if ends_with_ci(&text, ".json") {
                    ...
                    let Some(body) = self.tree.read(&text) else { return };
    ...
    fn push(
        ...
        let (Some(start), Some(size)) = (start, size) else { return };
    ...
    fn visit(&mut self, node: &Value, kind: &str, family: &str) {
        if let Value::Arr(items) = node {
            ...
            for index in 0..items.len().saturating_sub(1) {
                ...
                let (Some(start_value), Some(size_value)) = (start, size) else { continue };
                ...
                self.push(start, size, kind, family, format!("{identity} · {kind}"), sources);
            }
        ...
        let local = node.get("kind").and_then(|value| value.as_str()).unwrap_or(kind).to_string();
        ...
        for key in ["index", "source", "plan"] {
            let Some(reference) = node.get(key).and_then(|value| value.as_str()) else { continue };
            ...
            let Some(body) = self.tree.read(&reference) else { continue };
            let Ok(document) = crate::json::parse(&body) else { continue };
...
pub fn overlay_streams(tree: &SourceTree) -> Result<OrderedMap<String, Stream>, String> {
    ...
    let series = manifest.get("series").and_then(|value| value.as_array()).unwrap_or(&[]).to_vec();
    for entry in &series {
        if entry.get("kind").and_then(|value| value.as_str()) != Some(OVERLAY_SERIES) {
            continue;
        }
        for resource in entry.get("resources").and_then(|value| value.as_array()).unwrap_or(&[]) {
            let Some(row) = resource.as_array() else { continue };
            ...
            let (Some(start), Some(rom_bytes)) = (start, rom_bytes) else { continue };
            streams.insert(
                format!("resource_{id}"),
                Stream { start, rom_bytes, decoded_bytes: decoded },
            );
        }
    }
    ...
    let mut tail = series;
    tail.extend(
        manifest.get("closure_packages").and_then(|value| value.as_array()).unwrap_or(&[]).to_vec(),
    );
    for entry in &tail {
        ...
        let Some(source) = entry.get("source").and_then(|value| value.as_str()) else { continue };
        let Some(body) = tree.read(source) else { continue };
...
const BUCKET_MATCHES: [(&str, &str, &[&str]); 6] = [
    ("sprites", "Sprites & battle art", &["sprite", "character-catalog", "mtf4", "chr", "enemy"]),
    (
        "backgrounds",
        "Backgrounds & stills",
        &["still", "background", "title", "tile", "palette", "canvas", "4bpp", "8bpp"],
    ),
    ("maps", "Maps & world", &["map", "grid", "charblock", "chiiki", "tokushu", "world"]),
    ("audio", "Music & sound", &["sound", "audio", "wave", "pcm", "music", "midi", "sequence"]),
    ("text", "Text & fonts", &["message", "font", "localization", "kana", "glyph", "namae", "staff"]),
    (
        "tables",
        "Tables & databases",
        &["data", "table", "database", "directory", "runtime", "encounter", "header", "resource"],
    ),
];
...
pub fn asset_bucket(kind: &str) -> Bucket {
    ...
    Bucket { id: "other", label: "Other data" }
}

fn tier_rank(tier: &str) -> usize {
    crate::model::ASSET_TIERS.iter().position(|name| *name == tier).unwrap_or(0)
}
...
pub fn asset_tier_of(sources: &[String], kind: &str) -> &'static str {
    ...
    if any_of(&lowered, &["music", "sequence", "midi", "wave", "pcm", "audio"])
    ...
    if any_of(&lowered, &["sprite", "character-catalog", "chr"])
        && sources.iter().any(|name| any_of(name, &["koma_", "frame_"]))
    {
        return "asset_objects";
    }
...
fn object_png(lowered: &str) -> bool {
    let Some(stem) = lowered.strip_suffix(".png") else { return false };
...
pub fn manifest_asset_tiles(
    ...
    for range in manifest_ranges(tree, rom_size)? {
        ...
        match by_extent.get(&key).cloned() {
            ...
            Some(mut previous) => {
                ...
                previous.sources.sort_by(|l, r| crate::jsnum::utf16_cmp(l, r));
    ...
    for range in &ranges {
        ...
        let mut tile = Tile {
            label: format!("{} · 0x{} · {} bytes", range.label, hex8(range.start), commas(bytes as f64)),
            bytes,
            group: Some(range.family.clone()),
            subgroup: if range.family == range.kind { None } else { Some(range.kind.clone()) },
...
fn maturity_record(
    ...
    let mut tile = buckets.get(&bucket.id.to_string()).cloned().unwrap_or(Tile {
...
fn maturity_visit(
    ...
    if let Some(size) = size {
        if size > 0 {
            ...
            for key in ["index", "source", "plan"] {
                let Some(Value::Str(reference)) = node.get(key) else { continue };
                if !reference.ends_with(".json") {
                    continue;
                }
...
pub fn asset_maturity_tiles(tree: &SourceTree) -> Result<Vec<Tile>, String> {
    ...
    Ok(buckets.values().filter(|tile| tile.bytes > 0).cloned().collect())
}
...
pub fn retained_main_spans() -> Vec<Span> {
    let repo = root();
    let manifest_path = repo.join("..").join("out").join("full").join("asm").join("manifest.json");
    ...
    if let Ok(text) = std::fs::read_to_string(&non_c_path) {
        if let Ok(document) = crate::json::parse(&text) {
            for region in document.get("non_c_ranges").and_then(|v| v.as_array()).unwrap_or(&[]) {
                ...
                let (Some(start), Some(size)) = (start, size) else { continue };
                if !is_safe_integer(start) {
                    continue;
                }
                let start = start as i64;
                let kind = region.get("kind").and_then(|v| v.as_str()).unwrap_or("");
                let evidence = region.get("evidence").and_then(|v| v.as_str()).unwrap_or("");
    ...
    for region in manifest.get("regions").and_then(|v| v.as_array()).unwrap_or(&[]) {
        ...
        let retention = region.get("retention").and_then(|v| v.as_str()).unwrap_or("");
        let kind = region.get("kind").and_then(|v| v.as_str()).unwrap_or("");
        let evidence_text = region.get("evidence").and_then(|v| v.as_str()).unwrap_or("");
        let evidence_string = region.get("evidence").map(js_string).unwrap_or_else(|| "".into());
...
mod tests {
    ...
    fn hex_value_accepts_integral_floats_and_hex_strings() {
        ...
        assert_eq!(hex_value(&parse("\"0x08000000\"").expect("literal")), Some(0x0800_0000));
        assert_eq!(hex_value(&parse("\"0X10\"").expect("literal")), Some(16));
        assert_eq!(hex_value(&parse("\"10\"").expect("literal")), None, "no 0x prefix, no value");
    ...
    fn asset_bucket_is_ordered_leftmost_alternation() {
        ...
        assert_eq!(asset_bucket("TILE-map").id, "backgrounds", "case-insensitive, and tile wins");
    ...
    fn asset_tier_takes_the_highest_rank_seen() {
        let sources: Vec<String> =
            ["a/koma_12.png".into(), "b/plain.png".into(), "c/data.bin".into()].to_vec();
        assert_eq!(asset_tier_of(&sources, "tiles"), "asset_objects");
        assert_eq!(asset_tier_of(&["b/plain.png".to_string()], "tiles"), "asset_color");
        assert_eq!(asset_tier_of(&["b/mask_a.png".to_string()], "tiles"), "asset_bw");
        assert_eq!(asset_tier_of(&["b/data.bin".to_string()], "tiles"), "asset_bytes");
        assert_eq!(asset_tier_of(&[], "midi-sequences"), "asset_objects", "kind alone decides");
        assert_eq!(asset_tier_of(&[], "sound-font"), "asset_objects");
        // A sprite kind needs per-frame sources before it counts as objects.
        assert_eq!(asset_tier_of(&["x/sheet.png".to_string()], "sprite"), "asset_color");
        assert_eq!(asset_tier_of(&["x/koma_1.png".to_string()], "sprite"), "asset_objects");
    }
    ...
    fn extension_test_is_ascii_only_and_length_bounded() {
        ...
        assert!(!has_extension("a.tilemap", 5), "8 characters exceeds the bound");
        assert!(!has_extension("a.x", 5), "one character is below the bound");
        assert!(!has_extension("a.pn\u{0660}", 5), "a non-ASCII digit is not \\d");
    }
    ...
    fn object_and_bw_png_matchers_follow_the_js_classes() {
        ...
        assert!(!object_png("object/two.png"), "[^/]* cannot cross a separator");
