/*
 * resource_3a7 scene-entity presentation step at 0x0200050c, 72 bytes.
 *
 * Complete owner: `push {r5, lr}` / `sub sp, #8` at 0x0200050c through the
 ...
 *
 * Shape:
 *   record = Func_0808a080(16);          // scene-entity record by selector
 *   Func_080091c0(26, 30, 1, 1, 23, 32);  // six-argument request
 *   if (record != 0) { 
 *       Func_0808a080(16)[85] = 0;      // reloaded, same selector
 *       record[35] = 1;
 *   }
 *   Func_080770c8(0x200);            // raise the completion flag
 *
 * Call targets resolved with
 *   bun tools/overlay_show.ts resource_3a7 050c -n 72 |
...
{
    unsigned char *record = Func_0808a080(16);

    Func_080091c0(26, 30, 1, 1, 23, 32);

    if (record != 0) {
        /* The record is reloaded with the same selector before this store. */
        Func_0808a080(16)[85] = 0;
        record[35] = 1;
    }
