...
fn atlas_image(frames: &[Vec<u8>], width: usize, height: usize, columns: usize, colors: &[Rgb]) -> Result<Vec<u8>> {
    if frames.is_empty() || !width.is_multiple_of(8) || !height.is_multiple_of(8) || columns == 0 || columns > frames.len() { return fail("invalid sprite atlas layout"); }
    let rows = frames.len().div_ceil(columns); let atlas_width = columns * width; let atlas_height = rows * height;
...
fn atlas_frames(data: &[u8], width: usize, height: usize, count: usize, columns: usize, colors: &[Rgb]) -> Result<Vec<Vec<u8>>> {
    ...
    if image.palette != colors || image.width as usize != columns * width || image.height as usize != count.div_ceil(columns) * height { return fail("sprite atlas dimensions or palette differ from its plan"); }
    ...
    for index in count..(count.div_ceil(columns) * columns) { let left = index % columns * width; let top = index / columns * height; for y in 0..height { if pixels[(top + y) * image.width as usize + left..(top + y) * image.width as usize + left + width].iter().any(|p| *p != 0) { return fail("sprite atlas padding must be transparent"); } } }
    Ok(output)
}

fn zero_decode(data: &[u8], offset: usize, max: usize) -> Result<(Vec<u8>, usize)> {
    let mut out = Vec::new(); let mut cursor = offset; while cursor < data.len() { let value = data[cursor]; cursor += 1; if value == 0 { return Ok((out, cursor - offset)); } else if value <= 0xdf { if out.len() >= max { return fail("zero-skip pixel limit exceeded"); } out.push(value); } else { let count = value as usize - 0xdf; if out.len() + count > max { return fail("zero-skip pixel limit exceeded"); } out.extend(std::iter::repeat_n(0, count)); } } fail("zero-skip stream has no terminator")
}
...
fn copy_word(distance: usize, length: usize) -> Result<(u16, Option<u8>)> { if !(1..=0xfff).contains(&distance) || !(3..=273).contains(&length) { fail("mode-3 copy token is invalid") } else if length <= 17 { Ok((((length - 2) << 12 | distance) as u16, None)) } else { Ok((distance as u16, Some((length - 18) as u8))) } }
...
pub fn build_series(index: &Value, index_path: &Path, palette_path: &Path) -> Result<Vec<u8>> {
    ...
    result.extend(std::iter::repeat_n(0, suffix)); if result.len() != size { return fail("static-sprite series size differs"); } else if suffix > result.len() || result[result.len()-suffix..].iter().any(|b| *b != 0) { return fail("static-sprite suffix alignment is not zero"); } Ok(result)
}
