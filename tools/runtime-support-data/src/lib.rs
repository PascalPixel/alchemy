...
//! the native asset builder invokes directly.
    ...
    match args.first().map(String::as_str) {
        ...
        Some("build-stdout") => {
            ...
            while index < args.len() {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| Error(format!("{} requires a value", args[index])))?;
                match args[index].as_str() {
                    "--address" => address = Some(parse_cli_integer(value, "build address")?),
                    "--size" => size = Some(parse_cli_integer(value, "build size")? as usize),
                    _ => return fail(format!("unknown build option {}", args[index])),
                }
                index += 2;
            }
            let bytes = build_runtime_support_component(
                &source,
                address.ok_or_else(|| Error("build address is required".into()))?,
                size.ok_or_else(|| Error("build size is required".into()))?,
            )?;
