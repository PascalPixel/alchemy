...
pub fn main_bands(
    ...
    fn flush(
        ...
        let Some((start, spans)) = current.take() else { return };
        if spans.is_empty() {
            return;
        }
...
fn short_overlay(overlay: &str) -> String {
    overlay.strip_prefix("resource_").unwrap_or(overlay).to_string()
}
...
pub fn overlay_owner_tiles(
    ...
    let flattened: Vec<Span> =
        exact_owners.iter().flat_map(|owner| owner.spans.iter().copied()).collect();
...
pub fn group_tiles(tiles: &[Tile], target: i64) -> Vec<Tile> {
    ...
    for tile in tiles {
        if current.is_none() {
            current = Some(Tile { label: tile.label.clone(), ..Tile::default() });
        ...
        if entry.bytes >= target {
            entry.label =
                if first == last { first.clone() } else { format!("{first}\u{2013}{last}") };
            grouped.push(current.take().expect("just checked"));
        }
    }
    if let Some(mut entry) = current {
        entry.label = if first == last { first } else { format!("{first}\u{2013}{last}") };
        grouped.push(entry);
    }
    grouped
}
...
mod tests {
    ...
    fn spans(runs: &[(i64, i64)]) -> Vec<Span> {
        runs.iter().map(|(start, end)| Span::new(*start, *end)).collect()
    }
    ...
    fn main_bands_cuts_at_the_target_and_labels_by_prefix() {
        ...
        assert_eq!(bands[2].category("exact_c"), 0, "later bands hold no exact C");
    ...
    fn retained_never_outranks_owned_bytes() {
        ...
        assert_eq!(bands[0].category("exact_c"), 0x80);
        assert_eq!(bands[0].category("retained_asm"), 0x80, "only the unowned half");
    ...
    fn owner_tiles_cut_at_banks_and_boundaries() {
        ...
        let starts: Vec<i64> = tiles.iter().map(|tile| tile.address.expect("set")).collect();
    ...
    fn overlay_tiles_credit_exact_then_semantic_then_retained_then_the_rest() {
        ...
        let tiles =
            overlay_owner_tiles("resource_2a", &extent, &[owner], &semantic, &retained);
        let labels: Vec<&str> = tiles.iter().map(|tile| tile.label.as_str()).collect();
        assert_eq!(labels.len(), 4, "one tile per credited layer");
        assert!(labels[0].starts_with("2a · sub_1 · byte-exact C"), "prefix is stripped");
        assert_eq!(tiles[0].category("exact_c"), 0x10);
        assert_eq!(tiles[1].category("semantic_c"), 0x10, "the exact half is not re-credited");
        ...
        assert_eq!(total, 0x40, "the whole extent is accounted for exactly once");
    }

    #[test]
    fn group_tiles_names_a_merged_run_by_its_ends() {
        let make = |label: &str, bytes: i64| {
            let mut tile = Tile { label: label.into(), bytes, ..Tile::default() };
            tile.set_category("exact_c", bytes);
        ...
        assert_eq!(grouped[1].label, "c", "a single-tile tail keeps its own label");
        let keys: Vec<String> = grouped[0].categories.keys().cloned().collect();
        assert_eq!(keys, vec!["exact_c"], "a zero contribution never creates the key");
    }
}
