...
/// Scan the repository root for legacy callback references retained by the
/// self-test's compatibility guard. The full recursive implementation lives in
/// `alchemy_lints::callback_arity`; this small check remains only for the
/// self-test's direct symbol assertions.
