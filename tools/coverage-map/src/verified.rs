...
fn families_visit(
    ...
    match value {
        ...
        Value::Str(text) => {
            ...
            let path = if is_rooted_reference(text) {
                text.clone()
            } else {
                join_path(base, text)
            };
...
pub fn asset_families_by_source(tree: &SourceTree) -> Result<OrderedMap<String, String>, String> {
    ...
    for section in ["series", "regions", "closure_packages"] {
        let Some(Value::Arr(entries)) = manifest.get(section) else {
            continue;
        };
...
pub fn verified_asset_tiles(tree: &SourceTree, target: &str) -> Result<Option<Vec<Tile>>, String> {
    let Some(text) = tree.read("out/full/assets/manifest.json") else {
        return Ok(None);
    };
    ...
    let main_executable = to_spans(crate::intervals::union_intervals(&intervals_of(
        &main_node,
    ))?);
    ...
    for section in ["series", "regions"] {
        let Some(Value::Arr(entries)) = asset_manifest.get(section) else {
            continue;
        };
        for entry in entries {
    ...
    let family_of = |region: &BuiltAssetRegion| -> String {
        ...
        enclosing
            .first()
            .map(|range| range.family.clone())
            .unwrap_or_else(|| region.kind.clone())
    };
...
mod tests {
    ...
    fn the_family_walk_uses_the_five_character_extension_bound() {
        ...
        assert!(
            has_extension("a.midix", 5),
            "five characters: the wider bound only"
        );
        assert!(
            !has_extension("a.midix", 4),
            "the maturity walk cannot see it"
        );
