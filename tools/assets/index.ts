...
//
//   bun tools/assets/index.ts <subcommand> [args...]
//   bun tools/assets/index.ts --list
...
// Native replacements remain visible through this dispatcher after their
// TypeScript entrypoints are removed. The make dispatcher has a separate
// migration boundary.
const RUST_SUBCOMMANDS: Record<string, string> = {
  bl_site_symbols: "tools-rs/bl-site-symbols/target/release/bl-site-symbols",
};
...
async function main(): Promise<void> {
  ...
  if (subcommand === undefined || subcommand === "--list" || subcommand === "--help") {
    console.log("usage: assets <subcommand> [args...]");
    for (const name of available) console.log(`  ${name}`);
    return;
  }
  if (subcommand === "--self-test") {
    if (available.length === 0) throw new Error("assets has no subcommands");
    console.log(`assets self-test ok (${available.length} subcommands)`);
    return;
  }
