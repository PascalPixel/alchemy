 ...
 * resolve across fourteen targets; the only call_via site is the explicit
 * callback argument.
 *
 * The fixed tables and most object fields remain positional.  Width and
 * depth are the sums of opposing absolute delta components in 1/16 units.
 * The reference object's high-nibble bucket selects a packed signed X/Z
 * step; each half is divided by two with truncation toward zero before it
 * is passed to the corresponding setup call.
 */
