...
async function main(): Promise<void> {
  ...
  await run([process.execPath, "tools/make/build_claimed.ts", romPath,
    "--jobs", String(args.jobs), "--output", "out/claimed"]);
  await run([process.execPath, "tools/make/build_asm.ts", romPath, "--output", "out/asm"]);
  await run([process.execPath, "tools/make/build_assets.ts", romPath, "--output", "out/assets"]);
