...
//
// WHY this is here and not a path dependency: `tools-rs/canonical-json`
// declares `version.workspace`, so it only resolves from inside
// `tools-rs/Cargo.toml`'s workspace; this crate carries a bare `[workspace]`
// line so it can build concurrently with its siblings and therefore cannot
// depend on it. `tools-rs/discover/src/json.rs` inlines the same thing for the
