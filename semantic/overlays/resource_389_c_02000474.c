...
/*
 * resource_389 owner at 0x02000474, 404 bytes: the slide raycast.
 * Code 0x02000474-0x020005f9, an alignment halfword at 0x020005fa and the three
 * pool words 0x020095ac, 0x02009554 and 0xffff0000 at 0x020005fc-0x02000607.
 *
 ...
 *
 * What was changed:
 *  - Data_0200af50 (footprint boxes) becomes Data_020095ac, file offset 0x15ac;
 *  - Data_0200aef8 (packed direction words) becomes Data_02009554, offset 0x1554;
 *    both under the proven 0x02008000 overlay link base.  The direction table
 *    address is independently corroborated: resource_389's 0x020000c4 owner
 *    loads the same 0x02009554 for the same purpose.
 *  - the calls were re-resolved with 'bun tools/overlay_call_targets.ts
 *    resource_389 0474': the same two sites, 0x0200034c (this overlay's own
...
extern s32 Data_02009554[];   /* 16 packed direction words */
extern s32 Data_020095ac[];   /* 16-byte footprint boxes, four words each */
...
s32 Func_02000474(s32 *request)
{
    ...
    *state = 2;
    ...
    for (;;) {
        ...
        request[4] = probe[2] + (Data_020095ac[request[0] * 4 + 1] << 16);

        for (z = 0; z < spanZ; z++) {
            request[2] = probe[0] + (Data_020095ac[request[0] * 4] << 16);
        ...
        probe[2] += Data_02009554[heading] << 16;
    }
}
