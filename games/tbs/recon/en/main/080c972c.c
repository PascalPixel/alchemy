 ...
 * established by games/tbs/SRC/BATTLE/EFFECT/MEMBER_ORBIT.C
 * (main:080ce85c) and games/tbs/SRC/BATTLE/EFFECT/PUFF_ARC.C
 ...
 * (games/tbs/asm/080c972c.s) rather than the member_orbit template, since
 ...
 * _call_via_rN trampoline bundle at games/tbs/asm/080072e4.s
 ...
 * as in puff_arc/run.c and games/tbs/recon/en/main/080ccc38.c.
 *
 * Four spellings are load-bearing and were each proved against the
 * retained assembly:
 *
 *   - `var_i` is one shared loop counter for every loop in the function.
 *     The reference keeps all six of them in r8, which a per-loop variable
 *     cannot reproduce: separate counters take low registers and push the
 *     work pointer out of r10 into r8.  Sharing one variable is worth 83
 *     instructions of agreement on its own.
 *   - The two `>> 3` divisions are shifts, not `/ 8`.  The `if (v < 0)
 *     v += 7;` above each one is the reference's own rounding; spelling
 *     the division as `/ 8` makes GCC emit a second rounding sequence.
 *   - The 160-entry scanline table at 0x02010000 is `s16 *`, so its cursor
 *     advances by one element (`adds r1, #2`), not by two.
 *   - `Value_000000cc`, `Value_00000076` and `Value_0000027f` are the
 *     puff_arc/member_orbit spelling for a literal the reference loads from
 *     its pool: an ordinary `0xCC` becomes `movs r0, #204`, and an ordinary
 *     `<= 0x27F` is canonicalised to a shiftable `< 0x280`.  The halfword
 *     stores are deliberately NOT spelled that way -- GAS assembles Thumb
 *     `ldrh rN, <pool>` as the same `ldr rN, [pc, #imm]`, and the HImode
 *     pool entry is what places the first pool at 0x64 with the reference's
 *     own [0x100, 0x3f44, 0x3337, ...] ordering.
 */
...
/* Value_ symbols carry a literal the reference loads from its pool rather
   ...
   games/tbs/SRC/BATTLE/EFFECT/PUFF_ARC.C. */
    ...
    *(s16 *)0x04000048 = 0x3337;
    ...
    if (temp_r1_268 == 0) {
        ...
        if (temp_r1_268 == 1) {
            ...
            M2C_FIELD(temp_r1_17, s32 *, 0x7784) = 0x32;
        } else {
            M2C_FIELD(temp_r1_17, s32 *, 0x7780) = 2;
            ...
            if (temp_r3_362 == -1) {
                ...
                if (M2C_FIELD(var_r7_347, s32 *, 4) <= (s32)&Value_0000027f) {
                    ...
                    if (Data_080ededc[(M2C_FIELD(M2C_FIELD(temp_r1_17, void **, 0x7828), s32 *, 0x18) * 4) + 1] != 0) {
                        do {
                            temp_r3_436 = ((var_i * Data_080ededc[(M2C_FIELD(M2C_FIELD(temp_r1_17, void **, 0x7828), s32 *, 0x18) * 4) + 1]) + var_r4_412) * 0x1C;
                ...
                if ((u32) temp_r3_362 <= 3U) {
                    ...
                    sp10(sp1C, (u8 *)temp_r1_17 + 0xA84, temp_r6_355 - 9, temp_r5_360 - 9, 42, 42);
                }
                if (M2C_FIELD(var_r7_347, s32 *, 0x18) <= 0xE) {
                    M2C_FIELD(var_r7_347, s32 *, 0x18) += 1;
                }
            }
            var_i += 1;
            var_r7_347 = (u8 *)var_r7_347 + 0x1C;
            if (var_i != Data_080ededc[M2C_FIELD(M2C_FIELD(temp_r1_17, void **, 0x7828), s32 *, 0x18) * 4]) {
                goto loop_38;
            }
        }
