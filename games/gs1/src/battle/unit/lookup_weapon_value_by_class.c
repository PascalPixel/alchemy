...
/* battle/placement_contains_id.c */
...
struct StatusIconEffect *Battle_RunPlacementContainsId(struct EffectContext *context, s32 effect_id);
...
update:
    ...
    if (record->icon_effect != 0 && changed != 0) {
        Battle_ApplyPlacementContainsId(context, record->icon_effect);
        record->icon_effect = 0;
    }

    if (effect_id >= 0 && changed != 0) {
        effect = Battle_RunPlacementContainsId(context, effect_id);
    ...
    if (slot != NULL) {
        ...
        if (object != NULL) {
            ...
            while ((record = GetMotionRecord(object, record_index)) != NULL) {
                Battle_DoPlacementContainsId(record);
                record_index += 1;
            }
...
{
  struct BattleObjectSlot *slot;
  Battle_CheckPlacementContainsId();
  slot = GetBattleObjectSlot(object_id);
  slot->active = 1;
  return 0;
}
