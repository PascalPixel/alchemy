...
pub fn main_source(name: &str) -> Option<&str> {
    let body = name.strip_suffix(".c").or_else(|| name.strip_suffix(".C"))?;
...
pub fn assembly_source(name: &str) -> Option<&str> {
    let body = name.strip_suffix(".s").or_else(|| name.strip_suffix(".S"))?;
...
mod tests {
    ...
    fn overlay_assembly_captures_the_resource_id() {
        assert_eq!(overlay_assembly("resource_1a2_overlay.s"), Some("resource_1a2"));
