...
void FieldScene_RunSecondarySequence(void)
{
    ...
    *(u8 *)(Scene_GetRecord_1_020007c4(22) + 85) = 2;
    ...
    if (record != 0) {
        Actor_SetDestination(1, REC_S16(record, 10), REC_S16(record, 18));
    }
    Actor_WaitForMove(1);
    ...
    if (record != 0) {
        Actor_SetDestination(3, REC_S16(record, 10), REC_S16(record, 18));
    }
    Actor_WaitForMove(3);
    ...
    if (record != 0) {
        Actor_SetDestination(2, REC_S16(record, 10), REC_S16(record, 18));
    }
    Actor_WaitForMove(2);
