...
// `tools-rs/<crate>/<path>.rs`; that was removed. This is the gate that decides
...
mod tests {
    ...
    fn repo_root() -> std::path::PathBuf {
        // <crate>/tools-rs/source-citations -> repository root.
    ...
    fn citation_pattern_is_not_widened_to_rust_paths() {
        // The de-widening PORT NOTE, asserted. `tools-rs/a/b.rs` contains no
        // `tools/` substring, so nothing matches; `tools-rs/a/b.ts` likewise,
        // because the hyphen is outside the body class.
        assert!(citations("tools-rs/a/b.rs").is_empty());
        assert!(citations("tools-rs/a/b.ts").is_empty());
    }
