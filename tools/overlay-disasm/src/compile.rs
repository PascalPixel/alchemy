...
const SELF_SOURCE: [&[u8]; 6] = [include_bytes!("lib.rs"), include_bytes!("compile.rs"), include_bytes!("disasm.rs"), include_bytes!("paths.rs"), include_bytes!("regex.rs"), include_bytes!("cli.rs")];
const LINKED_POSTPROCESS_SOURCE: [&[u8]; 3] = [include_bytes!("../../compiler-core/src/lib.rs"), include_bytes!("../../compiler-core/src/call_via_data.rs"), include_bytes!("../../compiler-core/src/symbols.rs")];
...
fn overlay_c_sources_checked(source: &OverlaySource) -> Result<Vec<PathBuf>, String> {
    ...
    for address in placeholder_addresses(&text) {
        ...
        if !path.exists() {
            return Err(format!("{overlay}:{address:08x} has an AlchemyC placeholder but no exact C source at {}", path.display()));
        }
        found.push(path);
    }
    Ok(found)
...
fn placeholder_addresses(assembly: &str) -> Vec<u32> {
    let mut found = assembly.lines().filter_map(|line| line.trim().strip_prefix("AlchemyC_")?.strip_suffix(':').and_then(|address| u32::from_str_radix(address, 16).ok())).collect::<Vec<_>>();
    found.sort_unstable();
    found.dedup();
    found
}
...
fn address_stem(path: &Path, overlay: &str) -> Result<(SourceOwner, String, i64), String> {
    ...
    let resource = overlay.strip_prefix("resource_").and_then(|value| u16::from_str_radix(value, 16).ok()).ok_or_else(|| format!("invalid overlay owner {overlay:?}"))?;
    let owner = SourceOwner::from_legacy_stem(stem).filter(|owner| owner.overlay_id().as_deref() == Some(overlay)).unwrap_or(SourceOwner::Overlay { resource, address: address as u32 });
...
fn checked(command: &[String], cwd: &Path) -> Result<String, String> {
    ...
    let output = Command::new(binary).args(rest).current_dir(cwd).output().map_err(|error| format!("{} failed: {error}", basename(binary)))?;
...
pub(crate) fn spawn_raw(command: &[String], cwd: &Path) -> Result<Vec<u8>, String> {
    ...
    let output = Command::new(binary).args(rest).current_dir(cwd).output().map_err(|error| format!("{binary}: {error}"))?;
    if output.status.code() != Some(0) {
...
const OVERLAY_HOST_TOOLS: [&str; 4] = ["arm-none-eabi-as", "arm-none-eabi-nm", "arm-none-eabi-ld", "arm-none-eabi-objcopy"];
...
fn overlay_cache_key(compiler_signature: &str, host_signature: &str, plan_signature: &str, address: i64, call_via_base: i64, source_inputs: &[u8]) -> String {
    let mut key = Vec::new();
...
fn compiler_include_dirs(commands: &[Vec<String>]) -> Vec<PathBuf> {
    ...
    for command in commands {
        ...
        while let Some(argument) = arguments.next() {
            let include = if argument == "-I" { arguments.next().map(String::as_str) } else { argument.strip_prefix("-I").filter(|path| !path.is_empty()) };
...
fn collect_source_inputs(path: &Path, include_dirs: &[PathBuf], seen: &mut BTreeSet<PathBuf>, inputs: &mut Vec<(PathBuf, Vec<u8>)>) -> Result<(), String> {
    ...
    for include in text.lines().filter_map(quoted_include) {
        ...
        if let Some(resolved) = std::iter::once(local).chain(include_dirs.iter().map(|directory| directory.join(include))).find(|candidate| candidate.is_file()) {
...
fn source_defines_symbol(text: &str, symbol: &str) -> bool {
    definition_guard(symbol).is_match(text) || aliases_for_symbol(text, symbol).iter().any(|alias| definition_guard(alias).is_match(text))
}
...
pub fn compile_overlay_c(source: &Path, work: &Path, overlay: &str, routing_source: Option<&Path>, extra_flags: &[String]) -> Result<Compiled, String> {
    let mutations = if extra_flags.is_empty() { None } else { Some(CompilerFlagMutations { add_flags: extra_flags.to_vec(), remove_flags: Vec::new() }) };
    compile_overlay_with_mutations(source, work, overlay, routing_source, mutations.as_ref())
}
fn compile_overlay_with_mutations(source: &Path, work: &Path, overlay: &str, routing_source: Option<&Path>, mutations: Option<&CompilerFlagMutations>) -> Result<Compiled, String> {
    ...
    let routed_owner = routing_source.map(|path| source_paths.overlay_owner_for_path(overlay, path)).transpose()?.flatten();
    ...
    let routing_source = match routing_source {
        Some(path) => source_paths.overlay_owner_for_path(overlay, path)?.map(SourceOwner::routing_path).unwrap_or_else(|| path.to_path_buf()),
        None => owner.routing_path(),
    }
    .to_string_lossy()
    .into_owned();
    let call_via_base = source_paths.registered_call_via(owner).map(u64::from).unwrap_or_else(|| overlay_call_via_base(overlay)) as i64;
    ...
    let mut options = SourceToAssemblyPlanOptions::new(CompilerTarget::Gs1, routing_source, source_display.clone(), assembly.clone());
    ...
    let host_signature = compiler_core::bundle::host_executable_signature(&OVERLAY_HOST_TOOLS).map_err(|error| format!("overlay host tool signature: {error}"))?;
    let cached = overlay_c_cache_dir().join(format!("{}.bin", overlay_cache_key(&compiler_core::bundle::compiler_bundle_signature(), &host_signature, &plan_signature, address, call_via_base, &source_inputs,)));
    ...
    let undefined_symbols: Vec<String> = split_lines(&checked(&strings(&["arm-none-eabi-nm", "-u", &object]), work)?)
        ...
        .map(|line| crate::regex::js_trim(&line).split(|c: char| crate::regex::is_js_space(c)).rfind(|part| !part.is_empty()).expect("a non-empty trimmed line has a last field").to_string())
        .collect();
    ...
    checked(&strings(&["arm-none-eabi-as", "-mcpu=arm7tdmi", "-mthumb-interwork", "-o", &symbols_object, &symbols_source]), work)?;
    checked(&strings(&["arm-none-eabi-ld", &format!("-Ttext=0x{}", hex(address, 8)), "-e", &symbol, "-o", &elf, &object, &symbols_object]), work)?;
    ...
    let row = split_lines(&listing).into_iter().find(|line| line.ends_with(&needle)).ok_or_else(|| format!("missing linked overlay C symbol: {symbol}"))?;
    let size_field = crate::regex::js_trim(&row).split(|c: char| crate::regex::is_js_space(c)).filter(|part| !part.is_empty()).nth(1).ok_or_else(|| format!("nm -S row has no size field: {row}"))?.to_string();
...
pub fn compile_overlay_candidate(source: &Path, work: &Path, overlay: &str, routing_source: Option<&Path>, extra_flags: &[String]) -> Result<Compiled, String> {
    compile_overlay_c(source, work, overlay, routing_source, extra_flags)
}
pub fn compile_overlay_mutated(source: &Path, work: &Path, overlay: &str, routing_source: Option<&Path>, mutations: &CompilerFlagMutations) -> Result<Compiled, String> {
...
pub fn assemble_overlay(source: &OverlaySource, base: i64) -> Result<Vec<u8>, String> {
    ...
    spawn_raw(&strings(&["arm-none-eabi-as", "-mcpu=arm7tdmi", "-mthumb-interwork", "-o", &object, &assembly]), work.path())?;
    spawn_raw(&strings(&["arm-none-eabi-ld", &format!("-Ttext=0x{}", hex(base, 8)), "-o", &elf, &object]), work.path())?;
    ...
    for c_source in overlay_c_sources_checked(source)? {
        let compiled = compile_overlay_c(&c_source, work.path(), &overlay, None, &[]).map_err(|cause| format!("{overlay}: {}: {cause}", basename(&c_source.to_string_lossy())))?;
...
mod source_activation_tests {
    ...
    fn only_explicit_overlay_placeholders_activate_exact_c() {
        assert_eq!(placeholder_addresses("Func_02000104:\n  bx lr\nAlchemyC_02000104:\n  .space 8\nAlchemyC_02000314:\n"), vec![0x0200_0104, 0x0200_0314]);
