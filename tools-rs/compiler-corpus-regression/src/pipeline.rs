...
/// The four TypeScript sources whose bytes go into `compilerSignature`.
///
/// PORT NOTE -- CORRECTED `b3ab4841b` BREAKAGE. The TypeScript still spells
/// these as `join(ROOT, "tools", name)` with bare basenames, but `b3ab4841b`
/// moved three of them into `tools/lib/` and this tool itself into
/// `tools/compiler/`. The TypeScript therefore throws
/// `ENOENT: no such file or directory, open '.../tools/alchemy_gcc.ts'`
/// the instant it reaches a non-empty corpus. The corrected relative paths are
/// used here. This is the one class of defect the task allows fixing, and the
