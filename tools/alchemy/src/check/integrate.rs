...
fn cleanup(stem: &str, work_root: &Path) -> Result<usize, String> {
    let mut removed = 0;
    if work_root.exists() {
...
fn run_pipeline(directory: &str, apply: bool) -> Result<PipelineReport, String> {
    ...
    for (stem, candidate) in candidates {
        ...
        let asm = repository.join("recon/tbs/raw").join(format!("{stem}.s"));
    ...
    if apply {
        for (stem, _, candidate) in &accepted {
            ...
            let removed = cleanup(stem, &repository.join("scratch"))?
                + cleanup(stem, &repository.join("draft"))?;
            if removed > 0 {
                lines.push(format!("clean {stem} scratch={removed}"));
            }
        }
    }
    lines.extend(
