...
//
// WHY: `stableInventoryShape` prints `createHash("sha256")` of the canonical
// inventory JSON, and that digest appears verbatim in the `--write-inventory`
// line. This crate takes no third-party dependencies, so FIPS 180-4 lives here
// rather than in `sha2`. Roughly sixty lines against one more crate in the
// tree is the right trade for a hash we call once per run.
