...
pub fn exact_main_spans(tree: &SourceTree, executable: &[Span]) -> OrderedMap<i64, Vec<Span>> {
    ...
    for name in tree.list("exact") {
        let Some(digits) = main_source(&name) else {
            continue;
        };
        // Register-pinned, inline-assembly and fakematch C is not a C claim.
        let Some(source) = tree.read(&format!("exact/{name}")) else {
            continue;
        };
...
pub fn exact_overlay_owners(tree: &SourceTree) -> OrderedMap<String, Vec<OverlayOwner>> {
    ...
    for name in tree.list("assets/code") {
        let Some(overlay) = overlay_assembly(&name) else {
            continue;
        };
        ...
        let owners: Vec<OverlayOwner> = overlay_placeholder_owners(&source)
            ...
            .filter(|owner| {
                ...
                tree.read(&path)
                    .is_some_and(|text| js::canonical_c_source(&text))
            })
            .collect();
        if !owners.is_empty() {
...
pub fn semantic_spans(
                ...
                .and_then(|value| value.as_array())
            {
                for owner in list {
                    ...
                    let ranges: Vec<Span> = owner
                        ...
                        .map(|range| {
                            let start = parse_hex(
                                range.get("address").and_then(|v| v.as_str()).unwrap_or(""),
                            );
        ...
        if let Ok(document) = crate::json::parse(text) {
                ...
                .and_then(|value| value.as_array())
            {
                for region in list {
                    ...
                    let entry =
                        parse_hex(region.get("entry").and_then(|v| v.as_str()).unwrap_or(""));
                    reviewed.insert(
                        format!("{overlay}@{entry}"),
                        size_of(region.get("span_bytes")),
                    );
                }
            }
    ...
    for name in tree.list("semantic") {
        let Some((overlay, entry)) = semantic_overlay_source(&name) else {
            continue;
        };
        sources += 1;
        let overlay = overlay.to_string();
...
mod tests {
    ...
    fn fixture(root: &str, files: &[(&str, &str)]) -> (PathBuf, SourceTree) {
        ...
        for (path, body) in files {
            ...
            std::fs::create_dir_all(full.parent().expect("fixture paths are nested"))
                .expect("mkdir");
...
";
        ...
        assert_eq!(
            owners[0].spans,
            vec![
                Span::new(0x0800_1000, 0x0800_1010),
                Span::new(0x0800_1010, 0x0800_1018)
            ]
        );
    ...
    fn a_foreign_line_closes_the_open_owner() {
        ...
        assert!(
            overlay_placeholder_owners(source).is_empty(),
            "no reservation, no owner"
        );
        let source = "AlchemyC_08001000:\n\t.space 0x10\n\tbx lr\n\t.space 0x10\n";
        let owners = overlay_placeholder_owners(source);
    ...
    fn exact_main_spans_requires_canonical_c() {
        ...
        assert_eq!(
            owned.get(&0x0800_0100),
            Some(&vec![Span::new(0x0800_0100, 0x0800_0200)])
        );
    }
    ...
    fn exact_overlay_owners_requires_a_matching_c_file() {
        ...
        let owners = owned
            .get(&"resource_1a2".to_string())
            .expect("overlay present");
        assert_eq!(owners.len(), 1, "the owner without a C file is dropped");
        assert_eq!(owners[0].entry, 0x0800_1000);
        let spans = exact_overlay_spans(&tree);
        assert_eq!(
            spans.get(&"resource_1a2".to_string()),
            Some(&vec![Span::new(0x0800_1000, 0x0800_1010)])
        );
    }
    ...
    fn semantic_spans_covers_every_branch() {
        ...
        overlay_executable.insert(
            "resource_2".to_string(),
            vec![Span::new(0x0800_2000, 0x0800_2100)],
        );
        overlay_executable.insert(
            "resource_9".to_string(),
            vec![Span::new(0x0800_9000, 0x0800_9100)],
        );
        ...
        assert_eq!(
            ...
            Some(&vec![
                Span::new(0x0800_0500, 0x0800_0510),
                Span::new(0x0800_0600, 0x0800_0610)
            ]),
        );
        // Outside the executable extent -> unresolved, not estimated.
        assert!(coverage
            .unresolved
            .contains(&"semantic/08000900.c".to_string()));
        ...
        assert!(coverage
            .unresolved
            .contains(&"semantic/resource_3_c_08003000.c".to_string()));
    }
}
