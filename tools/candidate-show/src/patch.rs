...
pub fn strip_level_for_basename(header_path: &str, basename: &str) -> Option<usize> {
    let path = header_path.trim().strip_prefix("a/").or_else(|| header_path.trim().strip_prefix("b/")).unwrap_or(header_path.trim());
...
pub fn apply_unified_diff(source: &str, patch_text: &str, dest: &Path) -> Result<(), String> {
    let basename = dest.file_name().and_then(|name| name.to_str()).ok_or_else(|| format!("{}: not a file path", dest.display()))?;
    ...
    let header = patch_text.lines().find_map(|line| line.strip_prefix("--- ").or_else(|| line.strip_prefix("+++ "))).ok_or("patch: no ---/+++ header")?;
    let path = header.split_whitespace().next().unwrap_or(header);
    let strip = strip_level_for_basename(path, basename).ok_or_else(|| format!("patch: header {path:?} does not end in {basename}"))?;
    ...
    let output = Command::new("git").args(["apply", "--unsafe-paths", "--whitespace=nowarn", &format!("-p{strip}"), "incoming.diff"]).current_dir(parent).output().map_err(|error| format!("git apply: {error}"))?;
