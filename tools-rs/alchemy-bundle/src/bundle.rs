//! The compiler-bundle layer of `tools/lib/alchemy_gcc.ts`: which executables a
...
/// reproduces the ROM byte-identically under `bun run verify`, and pins the
/// resulting digests.
...
/// the order they are declared in `alchemy_gcc.ts`.
...
/// `alchemy_gcc.ts`, not in this port, and it is left alone deliberately:
