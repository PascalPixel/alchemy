...
fn catalog_path() -> PathBuf {
    std::env::var_os("ALCHEMY_ROOT").map(PathBuf::from).unwrap_or_else(|| Path::new(env!("CARGO_MANIFEST_DIR")).ancestors().nth(2).unwrap().to_path_buf()).join("games/gs1/assets/data/late_runtime_catalog.json")
}
...
pub fn entry(args: &[String]) -> ExitCode {
    let result = (|| -> Result<(), String> {
        ...
        let wanted = u32::from_str_radix(args[2].strip_prefix("0x").ok_or("invalid address")?, 16).map_err(|_| "invalid address".to_string())?;
        let built = build_late_runtime_residual(Path::new(&args[1]), &catalog_path())?;
        let (_, data) = built.regions.iter().find(|(address, _)| *address == wanted).ok_or("late-runtime asset address is not a produced region")?;
