...
/*
 * Exact reviewed skeleton homolog of resource_39c:0x020008ec.  This overlay's
 * independently mapped tables are 0x0200acf8 (four-delta records) and
 * 0x0200aca0 (heading steps); its callback literal remains 0x0200858d.
 *
 * The owner configures one selected actor, derives and publishes a tile
 * rectangle in local and global coordinates, optionally invokes the caller's
 * callback, and finalizes the actor placement.  Its unusual stack-before-push
 * prologue begins at 0x020008ec, the interworking return is at 0x02000b84,
 * and alignment plus the seven-word pool extends through 0x02000ba3.  All 31
 * calls, including the one explicit indirect callback, retain machine order.
 */
void Func_020008ec(s32 deltaIndex, s32 objectIndex, s32 x, s32 y, s32 z,
                   void (*callback)(void))
{
    const s32 *deltaTable = (const s32 *)0x0200acf8;
