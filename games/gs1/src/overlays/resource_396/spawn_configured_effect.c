...
/*
 * resource_396 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_396_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x02009da8 and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_396_c_02000048.c's adopted
 * Func_02005a2e pair and resource_396_c_02005158.c already use on this
...
extern struct Descriptor *Data_02009da8[];

struct Effect *Func_02001c56();
struct Effect *Func_02001bf4();
void Func_02001bfe();
void Func_02001c18();
void Func_02001d62();
s32 Func_02001c8c();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_02001c56(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02001bf4(options->kind, x, y, z);
    } else {
        effect = Func_02001bf4(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_02001bfe(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02001c18(effect, Data_02009da8[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_02001d62(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02001c8c(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02001ca4(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_02001cb2(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02001d18(effect, 1);
        Func_02001d28(effect, options->callback_arg);
    }
