...
/*
 * resource_3b4 owner at 0x02001198, 64 bytes: code 0x02001198-0x020011d3 and
 * the pool word 0x00000eeb at 0x020011d4.
 *
 * Repaint a 1x2 collision strip through Func_080091b8 (four register
 * arguments plus the two stack words), then, only while scene flag 0xeeb is
 * still clear, place slot 12 at (0x1980000, 0x3080000) -- both materialised as
 * `movs`/`lsls` pairs, 204 << 17 and 194 << 18.  Func_0808a010(1) runs on both
 * paths.
 */
...
void Func_02001198(void)
{
    Func_080091b8(25, 45, 1, 2, 25, 48);

    if (Func_080770c0(0xeeb) == 0) {
        Func_0808a0f0(12, 0x1980000, 0x3080000);
    }
