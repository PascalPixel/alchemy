...
void StoryScene_SetBranchValueFromZ(
    u8 *actor_object, s32 value_when_other_z_lower,
    s32 value_when_other_z_not_lower)
{
    u8 *scene_state;
    s16 *scene_table;
    ...
    if (other_actor->z < subject_actor->z) {
        *(u16 *)(scene_state + 0x170) = value_when_other_z_lower;
    } else {
        *(u16 *)(scene_state + 0x170) = value_when_other_z_not_lower;
    }
    Func_020049d2(0x7B);
}
