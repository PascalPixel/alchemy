...
/// Repository root, the value `alchemy_gcc.ts` exports as `ROOT`
/// (`dirname(dirname(dirname(tools/lib/alchemy_gcc.ts)))`).
///
...
/// path segments. `overlay_disasm.ts` was NOT among the casualties: it takes
/// `ROOT` from `alchemy_gcc.ts`, which never moved, and its only use of it is
/// `join(ROOT, "exact")`, a directory that still exists. Verified by running
/// `bun tools/lib/overlay_disasm.ts --self-test` before porting.
pub fn root() -> PathBuf {
