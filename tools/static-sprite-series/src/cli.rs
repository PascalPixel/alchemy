...
fn option(args: &[String], name: &str) -> Result<String, Error> {
    let index = args.iter().position(|arg| arg == name).ok_or_else(|| Error(format!("{name} is required")))?;
...
fn optional(args: &[String], name: &str) -> Result<Option<String>, Error> {
    match args.iter().position(|arg| arg == name) {
        Some(index) => args.get(index + 1).cloned().map(Some).ok_or_else(|| Error(format!("{name} requires a value"))),
        None => Ok(None),
    }
}
