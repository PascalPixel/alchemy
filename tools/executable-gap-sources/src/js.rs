...
pub fn branch_match(text: &str, sized: bool) -> Option<BranchMatch> {
    ...
    for start in 0..bytes.len() {
        ...
        if text[..start].chars().next_back().is_some_and(is_js_word) {
            continue;
        }
...
mod tests {
    ...
    fn objdump_rows_split_the_halfword_field() {
        ...
        let (address, word, text) = arm_objdump_row(" 8002000:\te3a00001 \tmov\tr0, #1").unwrap();
    ...
    fn trailing_comments_are_cut_at_the_whitespace_run() {
        assert_eq!(
            strip_trailing_comment("bl\t0x8002400 ; note", true),
            "bl\t0x8002400"
        );
        assert_eq!(
