...
/*
 * Move one selected object to a requested world position while repainting
 * its old and new authored footprint rectangles.  The optional callback is
 * the owner's sole call-through-register site.  The owner begins with stack
 * reservation at 0x020008ec, before the register-saving push at 0x020008ee,
 * and runs through the return, alignment and seven-word pool at 0x02000ba3.
 */
void Func_020008ec(s32 deltaIndex, s32 objectIndex, s32 x, s32 y, s32 z,
                   void (*callback)(void))
{
    const s32 *deltaTable = (const s32 *)0x0200a758;
    const s32 *referenceDeltas = (const s32 *)0x0200a700;
    ...
    *(s32 *)(current + 108) = 0x0200858d;
    Func_0808a010(4);
    if ((unsigned int)(referenceBucket - 6) <= 7)
    ...
    Func_0808a0e0(0, (s16)(packedStep >> 16) / 2, (s16)packedStep / 2);
