...
pub fn mkdtemp(prefix: &str) -> Result<PathBuf, String> {
    ...
    for _ in 0..64 {
        let tag =
            COUNTER.fetch_add(1, Ordering::SeqCst) ^ nanos ^ (u64::from(std::process::id()) << 20);
    ...
    Err(format!(
        "could not create a temporary directory under {root}"
    ))
}
