 ...
 * cover it -- resolved with `bun tools/overlay_show.ts resource_3bc 2e18 |
 * bun tools/overlay_call_targets.ts resource_3bc 2e18 2e52 --annotate`):
...
s32 Func_08077028();           /* established (handle, item) */
void Func_08077050();          /* established (handle, slot) */
...
void Func_02002e18(s32 handle, s32 item)
{
    ...
    record = Func_08077008(handle);
    Func_08077028(handle, item);

    for (slot = 0; slot <= 14; slot++) {
        if (*(u16 *)(record + 216 + slot * 2) == item) {
            Func_08077050(handle, slot);
        }
    }
}
