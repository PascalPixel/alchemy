...
/*
 * resource_385 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_385_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x0200929c and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_385_c_02000048.c's adopted
 * Func_02005a2e pair and resource_385_c_02005158.c already use on this
...
extern struct Descriptor *Data_0200929c[];

struct Effect *Func_0200125e();
struct Effect *Func_02001204();
void Func_0200120e();
void Func_02001228();
void Func_0200137a();
s32 Func_020012d4();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_0200125e(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02001204(options->kind, x, y, z);
    } else {
        effect = Func_02001204(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_0200120e(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02001228(effect, Data_0200929c[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_0200137a(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_020012d4(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_020012ec(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_020012fa(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02001328(effect, 1);
        Func_02001338(effect, options->callback_arg);
    }
