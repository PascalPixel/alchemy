/*
 * resource_3a7 scene-entity presentation step at 0x02000690, 84 bytes.
 *
 * Complete owner: `push {r5, lr}` / `sub sp, #8` at 0x02000690 through the
 ...
 * A single literal-pool word sits at 0x020006e0, inside the span and after
 ...
 *
 * Shape:
 *   record = Func_0808a080(21);          // scene-entity record by selector
 *   Func_080091c0(26, 30, 1, 1, 28, 33);  // six-argument request
 *   if (record != 0) { Func_080091e0(record, 0);
 *       Func_0808a080(21)[85] = 0;      // reloaded, same selector
 *       record[35] = 1;
 *   }
 *   Func_080770c8(0x205);            // raise the completion flag
 *
 * Call targets resolved with
 *   bun tools/overlay_show.ts resource_3a7 0690 -n 84 |
...
{
    unsigned char *record = Func_0808a080(21);

    Func_080091c0(26, 30, 1, 1, 28, 33);
