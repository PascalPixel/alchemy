...
/*
 * resource_3b3 owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_3b3_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x0200ad58 and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_3b3_c_02000048.c's adopted
 * Func_02005a2e pair and resource_3b3_c_02005158.c already use on this
...
extern struct Descriptor *Data_0200ad58[];

struct Effect *Func_02002c7a();
struct Effect *Func_02002bf8();
void Func_02002c02();
void Func_02002c1c();
void Func_02002d86();
s32 Func_02002ca8();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_02002c7a(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02002bf8(options->kind, x, y, z);
    } else {
        effect = Func_02002bf8(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_02002c02(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02002c1c(effect, Data_0200ad58[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_02002d86(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02002ca8(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02002cc0(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_02002cce(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02002d1c(effect, 1);
        Func_02002d2c(effect, options->callback_arg);
    }
