...
function selfTest(): void {
  ...
  // The passing fixture is a well-formed address rather than a NOTABLE one:
  // this exit code turns purely on the 08xxxxxx spelling, so 0x08000000 pins
  // the direction without depending on any owner surviving in the tree.
  if (run("resource_ffffff") === 0)
    throw new Error("veneer self-test: an unrecognised argument must NOT exit 0");
  if (run("0x08000000") !== 0)
    throw new Error("veneer self-test: a well-formed address must exit 0");
