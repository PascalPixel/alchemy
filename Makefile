...
# and out/cross-edition regenerated by exhaustive audits, never evicted).
...
.PHONY: help verify audit reports test lint lint-production lint-all-targets build-tools tool-tests tooling-size \
	...
	families family-check twins twins-check coverage coverage-check dashboard clean clean-preview doctor
...
twins: build-full | $(REPORT_DIR)
	$(COMPILER) twins --write $(REPORT_DIR)/main-twins.json

twins-check: twins
	@printf 'main twin audit ok\n'
...
audit: verify test targets classification-check candidate-corpus-check \
	correspondence-check twins-check progress-report coverage-check

reports: correspondence families twins progress-report coverage
