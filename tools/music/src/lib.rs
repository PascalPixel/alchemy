...
/// Build the small audited reserve stream used by the music residual package.
/// Keeping this encoder beside the native music code lets residual tooling use
/// the same sequence representation without carrying a second byte emitter.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub fn build_reserve_sequence(base: u32) -> (Vec<u8>, SequenceBuildReport) {
    let data = vec![
        ...
        0xbb, 30, // tempo
        0xbd, 21, // voice
        0xd0, 61, 127,  // note, duration 1
        0x81, // wait 1
        66,   // running note, duration 1
        0x81, // wait 1
        0xee, 69,   // note, duration 44
        0xa0, // wait 48
        0xb1, // fine
    ];
