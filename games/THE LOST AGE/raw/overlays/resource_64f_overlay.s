.syntax unified
.include "games/THE LOST AGE/SRC/SYSTEM/OVERLAY.INC"
	.thumb
	.set sub_02000426, 0x02000426
	.set sub_02000a84, 0x02000a84
	.set sub_02000a88, 0x02000a88
	.set sub_02000a92, 0x02000a92
	.set sub_02000a94, 0x02000a94
	.set sub_02000af0, 0x02000af0
	.set sub_02000af6, 0x02000af6
	.set sub_02000b00, 0x02000b00
	.set sub_02000b02, 0x02000b02
	.set sub_02000b16, 0x02000b16
	.set sub_02000b6a, 0x02000b6a
	.set sub_02000b80, 0x02000b80
	.set sub_02000b82, 0x02000b82
	.set sub_02000b8e, 0x02000b8e
	.set sub_02000b92, 0x02000b92
	.set sub_02000b9c, 0x02000b9c
	.set sub_02000ba6, 0x02000ba6
	.set sub_02000bb2, 0x02000bb2
	.set sub_02000bbc, 0x02000bbc
	.set sub_02000bcc, 0x02000bcc
	.set sub_02000be6, 0x02000be6
	.set sub_02000bea, 0x02000bea
	.set sub_02000c08, 0x02000c08
	.set sub_02000c10, 0x02000c10
	.set sub_02000c18, 0x02000c18
	.set sub_02000c20, 0x02000c20
	.set sub_02000c28, 0x02000c28
	.set sub_02000c68, 0x02000c68
	.set sub_02000c6e, 0x02000c6e
	.set sub_02000c72, 0x02000c72
	.set sub_02000c84, 0x02000c84
	.set sub_02000c98, 0x02000c98
	.set sub_02000c9c, 0x02000c9c
	.set sub_02000cae, 0x02000cae
	.set sub_02000cb2, 0x02000cb2
	.set sub_02000cb8, 0x02000cb8
	.set sub_02000cc4, 0x02000cc4
	.set sub_02000cfa, 0x02000cfa
	.set sub_02000d26, 0x02000d26
	.set sub_02000d38, 0x02000d38
	.set sub_02000d80, 0x02000d80
	.set sub_02000d82, 0x02000d82
	.set sub_02000d90, 0x02000d90
	.set sub_02000daa, 0x02000daa
	.set sub_02000db6, 0x02000db6
	.set sub_02000dbc, 0x02000dbc
	.set sub_02000dbe, 0x02000dbe
	.set sub_02000dcc, 0x02000dcc
	.set sub_02000dd8, 0x02000dd8
	.set sub_02000de0, 0x02000de0
	.set sub_02000df2, 0x02000df2
	.set sub_02000e0a, 0x02000e0a
	.set sub_02000e0c, 0x02000e0c
	.set sub_02000e0e, 0x02000e0e
	.set sub_02000e18, 0x02000e18
	.set sub_02000e20, 0x02000e20
	.set sub_02000e22, 0x02000e22
	.set sub_02000e26, 0x02000e26
	.set sub_02000e38, 0x02000e38
	.set sub_02000e3a, 0x02000e3a
	.set sub_02000e3e, 0x02000e3e
	.set sub_02000e50, 0x02000e50
	.set sub_02000e5c, 0x02000e5c
	.set sub_02000e5e, 0x02000e5e
	.set sub_02000e6e, 0x02000e6e
	.set sub_02000ea4, 0x02000ea4
	.set sub_02000eae, 0x02000eae
	.set sub_02000eba, 0x02000eba
	.set sub_02000ebe, 0x02000ebe
	.set sub_02000ec2, 0x02000ec2
	.set sub_02000ecc, 0x02000ecc
	.set sub_02000ee8, 0x02000ee8
	.set sub_02000eea, 0x02000eea
	.set sub_02000f0a, 0x02000f0a
	.set sub_02000f0e, 0x02000f0e
	.set sub_02000f12, 0x02000f12
	.set sub_02000f1c, 0x02000f1c
	.set sub_02000f34, 0x02000f34
	.set sub_02000f38, 0x02000f38
	.set sub_02000f4e, 0x02000f4e
	.set sub_02000f52, 0x02000f52
	.set sub_02000f58, 0x02000f58
	.set sub_02000f64, 0x02000f64
	.set sub_02000f68, 0x02000f68
	.set sub_02000f7a, 0x02000f7a
	.set sub_02000f7c, 0x02000f7c
	.set sub_02000f84, 0x02000f84
	.set sub_02000f8c, 0x02000f8c
	.set sub_02000f98, 0x02000f98
	.set sub_02000f9c, 0x02000f9c
	.set sub_02000fb2, 0x02000fb2
	.set sub_02000fd0, 0x02000fd0
	.set sub_02000fda, 0x02000fda
	.set sub_02000fdc, 0x02000fdc
	.set sub_02000fee, 0x02000fee
	.set sub_02001004, 0x02001004
	.set sub_02001010, 0x02001010
	.set sub_02001014, 0x02001014
	.set sub_02001028, 0x02001028
	.set sub_02001038, 0x02001038
	.set sub_02001046, 0x02001046
	.set sub_02001068, 0x02001068
	.set sub_02001070, 0x02001070
	.set sub_02001078, 0x02001078
	.set sub_0200107c, 0x0200107c
	.set sub_02001080, 0x02001080
	.set sub_02001086, 0x02001086
	.set sub_0200108a, 0x0200108a
	.set sub_0200109a, 0x0200109a
	.set sub_0200109e, 0x0200109e
	.set sub_020010a4, 0x020010a4
	.set sub_020010a8, 0x020010a8
	.set sub_020010b4, 0x020010b4
	.set sub_020010b6, 0x020010b6
	.set sub_020010b8, 0x020010b8
	.set sub_020010ba, 0x020010ba
	.set sub_020010c8, 0x020010c8
	.set sub_020010ca, 0x020010ca
	.set sub_020010d0, 0x020010d0
	.set sub_020010de, 0x020010de
	.set sub_020010e8, 0x020010e8
	.set sub_020010f2, 0x020010f2
	.set sub_02001104, 0x02001104
	.set sub_0200110e, 0x0200110e
	.set sub_0200111c, 0x0200111c
	.set sub_0200112c, 0x0200112c
	.set sub_02001144, 0x02001144
	.set sub_0200114e, 0x0200114e
	.set sub_0200116c, 0x0200116c
	.set sub_02001170, 0x02001170
	.set sub_02001180, 0x02001180
	.set sub_02001184, 0x02001184
	.set sub_0200118c, 0x0200118c
	.set sub_02001196, 0x02001196
	.set sub_0200119a, 0x0200119a
	.set sub_0200119c, 0x0200119c
	.set sub_0200119e, 0x0200119e
	.set sub_020011ae, 0x020011ae
	.set sub_020011b6, 0x020011b6
	.set sub_020011ce, 0x020011ce
	.set sub_020011d4, 0x020011d4
	.set sub_020011e2, 0x020011e2
	.set sub_020011ec, 0x020011ec
	.set sub_020011ee, 0x020011ee
	.set sub_020011fa, 0x020011fa
	.set sub_02001204, 0x02001204
	.set sub_02001210, 0x02001210
	.set sub_0200122a, 0x0200122a
	.set sub_0200122e, 0x0200122e
	.set sub_0200123a, 0x0200123a
	.set sub_02001244, 0x02001244
	.set sub_0200124c, 0x0200124c
	.set sub_0200124e, 0x0200124e
	.set sub_02001254, 0x02001254
	.set sub_02001258, 0x02001258
	.set sub_02001264, 0x02001264
	.set sub_02001272, 0x02001272
	.set sub_02001274, 0x02001274
	.set sub_0200127a, 0x0200127a
	.set sub_02001284, 0x02001284
	.set sub_02001298, 0x02001298
	.set sub_020012a4, 0x020012a4
	.set sub_020012ac, 0x020012ac
	.set sub_020012b8, 0x020012b8
	.set sub_020012bc, 0x020012bc
	.set sub_020012c6, 0x020012c6
	.set sub_020012da, 0x020012da
	.set sub_020012dc, 0x020012dc
	.set sub_020012de, 0x020012de
	.set sub_020012f4, 0x020012f4
	.set sub_02001306, 0x02001306
	.set sub_02001346, 0x02001346
	.set sub_02001362, 0x02001362
	.set sub_020013c6, 0x020013c6
	.global Overlay_02000000
Overlay_02000000:
.include "games/THE LOST AGE/SRC/FIELD/SUHARA_GATE/ENTRY.INC"
	push	{lr}
	movs	r0, #13
	movs	r1, #26
	bl	sub_02000a88
	pop	{pc}
	push	{lr}
	ldr	r3, [pc, #36]	@ (0x200006c)
	movs	r1, #240	@ 0xf0
	lsls	r1, r1, #1
	adds	r3, r3, r1
	movs	r1, #0
	ldrsh	r2, [r3, r1]
	ldr	r3, [pc, #28]	@ (0x2000070)
	cmp	r2, r3
	bne.n	.L_0200005c
	ldr	r0, [pc, #24]	@ (0x2000074)
	b.n	.L_02000068
.L_0200005c:
	ldr	r3, [pc, #24]	@ (0x2000078)
	cmp	r2, r3
	bne.n	.L_02000066
	ldr	r0, [pc, #24]	@ (0x200007c)
	b.n	.L_02000068
.L_02000066:
	ldr	r0, [pc, #24]	@ (0x2000080)
.L_02000068:
	pop	{pc}
	.2byte 0x0000
	.4byte 0x02000240
	.4byte 0x00000007
	.4byte 0x02008d98
	.4byte 0x00000008
	.4byte 0x02008e40
	.4byte 0x02008cd8
	.4byte 0x47702000
	.4byte 0x47704800
	.2byte 0x8ed0
	.2byte 0x0200
	push	{lr}
	ldr	r3, [pc, #36]	@ (0x20000b8)
	movs	r1, #240	@ 0xf0
	lsls	r1, r1, #1
	adds	r3, r3, r1
	movs	r1, #0
	ldrsh	r2, [r3, r1]
	ldr	r3, [pc, #28]	@ (0x20000bc)
	cmp	r2, r3
	bne.n	.L_020000a8
	ldr	r0, [pc, #24]	@ (0x20000c0)
	b.n	.L_020000b4
.L_020000a8:
	ldr	r3, [pc, #24]	@ (0x20000c4)
	cmp	r2, r3
	bne.n	.L_020000b2
	ldr	r0, [pc, #24]	@ (0x20000c8)
	b.n	.L_020000b4
.L_020000b2:
	ldr	r0, [pc, #24]	@ (0x20000cc)
.L_020000b4:
	pop	{pc}
	.2byte 0x0000
	.4byte 0x02000240
	.4byte 0x00000007
	.4byte 0x02008f58
	.4byte 0x00000006
	.4byte 0x02009048
	.2byte 0x8f10
	.2byte 0x0200
	push	{lr}
	ldr	r3, [pc, #36]	@ (0x20000f8)
	movs	r1, #240	@ 0xf0
	lsls	r1, r1, #1
	adds	r3, r3, r1
	movs	r1, #0
	ldrsh	r2, [r3, r1]
	ldr	r3, [pc, #28]	@ (0x20000fc)
	cmp	r2, r3
	bne.n	.L_020000e8
	ldr	r0, [pc, #24]	@ (0x2000100)
	b.n	.L_020000f4
.L_020000e8:
	ldr	r3, [pc, #24]	@ (0x2000104)
	cmp	r2, r3
	bne.n	.L_020000f2
	ldr	r0, [pc, #24]	@ (0x2000108)
	b.n	.L_020000f4
.L_020000f2:
	ldr	r0, [pc, #24]	@ (0x200010c)
.L_020000f4:
	pop	{pc}
	.2byte 0x0000
	.4byte 0x02000240
	.4byte 0x00000007
	.4byte 0x020090e4
	.4byte 0x00000008
	.4byte 0x02009180
	.2byte 0x9078
	.2byte 0x0200
	push	{lr}
	ldr	r3, [pc, #80]	@ (0x2000164)
	movs	r2, #241	@ 0xf1
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #99	@ 0x63
	bne.n	.L_0200012c
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #148	@ 0x94
	bl	sub_02000a92
.L_0200012c:
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #148	@ 0x94
	bl	sub_02000a94
	cmp	r0, #0
	beq.n	.L_02000160
	movs	r1, #228	@ 0xe4
	movs	r2, #156	@ 0x9c
	movs	r0, #8
	lsls	r1, r1, #17
	lsls	r2, r2, #17
	bl	sub_02000b16
	movs	r1, #128	@ 0x80
	lsls	r1, r1, #7
	movs	r0, #8
	movs	r2, #0
	bl	sub_02000bea
	movs	r0, #8
	bl	sub_02000af0
	movs	r1, #8
	bl	sub_02000be6
.L_02000160:
	pop	{pc}
	.2byte 0x0000
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, lr}
	sub	sp, #8
	movs	r3, #102	@ 0x66
	movs	r2, #56	@ 0x38
	str	r3, [sp, #0]
	str	r2, [sp, #4]
	movs	r3, #1
	movs	r2, #1
	movs	r0, #108	@ 0x6c
	movs	r1, #38	@ 0x26
	bl	sub_02000af6
	ldr	r3, [pc, #212]	@ (0x2000258)
	movs	r2, #241	@ 0xf1
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #99	@ 0x63
	bne.n	.L_0200019a
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #149	@ 0x95
	bl	sub_02000b00
.L_0200019a:
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #149	@ 0x95
	bl	sub_02000b02
	cmp	r0, #0
	beq.n	.L_0200022a
	movs	r2, #230	@ 0xe6
	movs	r0, #13
	ldr	r1, [pc, #172]	@ (0x200025c)
	lsls	r2, r2, #18
	bl	sub_02000b82
	movs	r1, #156	@ 0x9c
	movs	r0, #14
	lsls	r1, r1, #18
	ldr	r2, [pc, #164]	@ (0x2000260)
	bl	sub_02000b8e
	movs	r1, #158	@ 0x9e
	movs	r2, #230	@ 0xe6
	lsls	r2, r2, #18
	lsls	r1, r1, #18
	movs	r0, #15
	bl	sub_02000b9c
	movs	r0, #13
	bl	sub_02000b6a
	movs	r3, #128	@ 0x80
	lsls	r3, r3, #7
	movs	r1, #192	@ 0xc0
	strh	r3, [r0, #6]
	lsls	r1, r1, #7
	movs	r0, #13
	bl	sub_02000c72
	movs	r0, #14
	bl	sub_02000b80
	movs	r5, #0
	strh	r5, [r0, #6]
	movs	r1, #0
	movs	r0, #14
	bl	sub_02000c84
	movs	r0, #15
	bl	sub_02000b92
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #8
	strh	r3, [r0, #6]
	movs	r1, #0
	movs	r0, #15
	bl	sub_02000c98
	movs	r0, #13
	bl	sub_02000ba6
	movs	r5, #1
	adds	r0, #89	@ 0x59
	strb	r5, [r0, #0]
	movs	r0, #14
	bl	sub_02000bb2
	adds	r0, #89	@ 0x59
	strb	r5, [r0, #0]
	movs	r0, #15
	bl	sub_02000bbc
	adds	r0, #89	@ 0x59
	strb	r5, [r0, #0]
.L_0200022a:
	movs	r0, #8
	movs	r1, #4
	bl	sub_02000c08
	movs	r0, #9
	movs	r1, #4
	bl	sub_02000c10
	movs	r0, #10
	movs	r1, #3
	bl	sub_02000c18
	movs	r0, #11
	movs	r1, #4
	bl	sub_02000c20
	movs	r0, #12
	movs	r1, #3
	bl	sub_02000c28
	add	sp, #8
	pop	{r5, pc}
	.2byte 0x0000
	.4byte 0x02000240
	.4byte 0x024e0000
	.2byte 0x0000
	.2byte 0x03ab
	push	{lr}
	movs	r0, #10
	adds	r0, #255	@ 0xff
	bl	sub_02000bcc
	cmp	r0, #0
	bne.n	.L_02000286
	ldr	r3, [pc, #20]	@ (0x2000288)
	movs	r2, #241	@ 0xf1
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r2, #0
	ldrsh	r3, [r3, r2]
	cmp	r3, #99	@ 0x63
	bne.n	.L_02000286
	bl	sub_02000a84
.L_02000286:
	pop	{pc}
	.2byte 0x0240
	.2byte 0x0200
	push	{lr}
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #108]	@ 0x6c
	movs	r1, #214	@ 0xd6
	lsls	r1, r1, #1
	movs	r2, #128	@ 0x80
	adds	r3, r3, r1
	lsls	r2, r2, #1
	str	r2, [r3, #0]
	ldr	r3, [pc, #44]	@ (0x20002d0)
	adds	r2, #224	@ 0xe0
	adds	r3, r3, r2
	movs	r1, #0
	ldrsh	r2, [r3, r1]
	ldr	r3, [pc, #40]	@ (0x20002d4)
	cmp	r2, r3
	bne.n	.L_020002b6
	bl	.L_020003c2
	b.n	.L_020002cc
.L_020002b6:
	ldr	r3, [pc, #32]	@ (0x20002d8)
	cmp	r2, r3
	bne.n	.L_020002c2
	bl	sub_02000426
	b.n	.L_020002cc
.L_020002c2:
	ldr	r3, [pc, #24]	@ (0x20002dc)
	cmp	r2, r3
	bne.n	.L_020002cc
	bl	.L_0200052e
.L_020002cc:
	movs	r0, #0
	pop	{pc}
	.4byte 0x02000240
	.4byte 0x00000006
	.4byte 0x00000007
	.2byte 0x0008
	.2byte 0x0000
	push	{lr}
	sub	sp, #8
	movs	r3, #102	@ 0x66
	movs	r2, #56	@ 0x38
	str	r3, [sp, #0]
	str	r2, [sp, #4]
	movs	r1, #38	@ 0x26
	movs	r2, #1
	movs	r3, #1
	movs	r0, #108	@ 0x6c
	bl	sub_02000c6e
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #222	@ 0xde
	bl	sub_02000c68
	add	sp, #8
	pop	{pc}
	.2byte 0x0000
	push	{r5, r6, lr}
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #108]	@ 0x6c
	movs	r2, #170	@ 0xaa
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r0, #0
	sub	sp, #8
	movs	r2, #0
	ldrsh	r6, [r3, r2]
	bl	sub_02000cb8
	movs	r3, #0
	adds	r0, #85	@ 0x55
	strb	r3, [r0, #0]
	movs	r0, #158	@ 0x9e
	bl	sub_02000dcc
	movs	r5, #2
	movs	r1, #36	@ 0x24
	movs	r2, #71	@ 0x47
	movs	r3, #8
	movs	r0, #66	@ 0x42
	str	r5, [sp, #0]
	str	r5, [sp, #4]
	bl	sub_02000cae
	movs	r0, #4
	bl	sub_02000c9c
	movs	r3, #8
	movs	r1, #36	@ 0x24
	movs	r2, #71	@ 0x47
	movs	r0, #68	@ 0x44
	str	r5, [sp, #0]
	str	r5, [sp, #4]
	bl	sub_02000cc4
	movs	r0, #4
	bl	sub_02000cb2
	movs	r2, #16
	movs	r1, #3
	negs	r2, r2
	movs	r0, #0
	bl	sub_02000d26
	movs	r0, #123	@ 0x7b
	bl	sub_02000e0c
	adds	r0, r6, #0
	bl	sub_02000daa
	add	sp, #8
	pop	{r5, r6, pc}
	push	{r5, lr}
	movs	r3, #192	@ 0xc0
	lsls	r3, r3, #18
	ldr	r3, [r3, #108]	@ 0x6c
	movs	r2, #170	@ 0xaa
	lsls	r2, r2, #1
	adds	r3, r3, r2
	movs	r2, #0
	ldrsh	r5, [r3, r2]
	ldr	r3, [pc, #24]	@ (0x20003a4)
	movs	r2, #133	@ 0x85
	lsls	r2, r2, #2
	adds	r3, r3, r2
	movs	r2, #4
	str	r2, [r3, #0]
	movs	r0, #123	@ 0x7b
	bl	sub_02000e3a
	adds	r0, r5, #0
	bl	sub_02000dd8
	pop	{r5, pc}
	.4byte 0x02000240
	.2byte 0x2000
	.2byte 0x4770
	push	{r5, r6, lr}
	bl	sub_02000d38
	movs	r0, #0
	bl	sub_02000e26
	ldr	r0, [pc, #200]	@ (0x2000484)
	bl	sub_02000dbc
	ldr	r6, [pc, #200]	@ (0x2000488)
	movs	r3, #133	@ 0x85
.L_020003c2:
	lsls	r3, r3, #2
	adds	r5, r6, r3
	movs	r1, #236	@ 0xec
	movs	r2, #160	@ 0xa0
	ldr	r0, [r5, #0]
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	bl	sub_02000d82
	movs	r0, #228	@ 0xe4
	movs	r1, #1
	movs	r2, #140	@ 0x8c
	movs	r3, #1
	negs	r1, r1
	lsls	r2, r2, #17
	lsls	r0, r0, #17
	bl	sub_02000e0c
	bl	sub_02000e18
	ldr	r0, [r5, #0]
	bl	sub_02000db6
	movs	r1, #160	@ 0xa0
	ldr	r0, [r5, #0]
	lsls	r1, r1, #8
	movs	r2, #0
	bl	sub_02000e0a
	movs	r0, #10
	bl	sub_02000d80
	movs	r1, #6
	movs	r2, #20
	movs	r0, #8
	bl	sub_02000df2
	movs	r0, #20
	bl	sub_02000d90
	movs	r0, #8
	movs	r1, #0
	movs	r2, #10
	bl	sub_02000e22
	movs	r1, #128	@ 0x80
	lsls	r1, r1, #1
	movs	r2, #50	@ 0x32
	movs	r0, #8
	bl	sub_02000e3e
	movs	r2, #10
	movs	r0, #8
	movs	r1, #0
	bl	sub_02000e38
	movs	r1, #3
	movs	r0, #8
	bl	sub_02000e18
	movs	r0, #30
	bl	sub_02000dbe
	movs	r0, #8
	movs	r1, #0
	movs	r2, #10
	bl	sub_02000e50
	movs	r1, #228	@ 0xe4
	movs	r2, #156	@ 0x9c
	lsls	r2, r2, #1
	movs	r0, #8
	lsls	r1, r1, #1
	bl	sub_02000e0e
	ldr	r0, [pc, #48]	@ (0x200048c)
	movs	r1, #99	@ 0x63
	bl	sub_02000eae
	ldr	r0, [pc, #44]	@ (0x2000490)
	movs	r1, #98	@ 0x62
	bl	sub_02000ebe
	movs	r3, #166	@ 0xa6
	lsls	r3, r3, #1
	adds	r3, #255	@ 0xff
	adds	r6, r6, r3
	movs	r3, #2
	strb	r3, [r6, #0]
	movs	r0, #9
	movs	r1, #1
	bl	sub_02000eba
	bl	sub_02000e0e
	pop	{r5, r6, pc}
	.2byte 0x0000
	.4byte 0x00001609
	.4byte 0x02000240
	.4byte 0x00000006
	.2byte 0x0004
	.2byte 0x0000
	push	{r5, r6, lr}
	bl	sub_02000e20
	movs	r0, #0
	bl	sub_02000f0e
	ldr	r0, [pc, #308]	@ (0x20005d8)
	bl	sub_02000ea4
	ldr	r6, [pc, #308]	@ (0x20005dc)
	movs	r3, #133	@ 0x85
	lsls	r3, r3, #2
	adds	r5, r6, r3
	movs	r2, #4
	movs	r1, #0
	negs	r2, r2
	ldr	r0, [r5, #0]
	bl	sub_02000f38
	movs	r0, #30
	bl	sub_02000e3e
	ldr	r0, [r5, #0]
	bl	sub_02000e5c
	adds	r0, #90	@ 0x5a
	ldrb	r2, [r0, #0]
	movs	r3, #254	@ 0xfe
	ands	r3, r2
	strb	r3, [r0, #0]
	movs	r1, #0
	ldr	r0, [r5, #0]
	movs	r2, #12
	bl	sub_02000f58
	movs	r0, #10
	bl	sub_02000e5e
	ldr	r0, [r5, #0]
	movs	r1, #0
	movs	r2, #16
	bl	sub_02000f68
	movs	r0, #30
	bl	sub_02000e6e
	movs	r1, #154	@ 0x9a
	movs	r2, #228	@ 0xe4
	movs	r0, #13
	lsls	r1, r1, #18
	lsls	r2, r2, #18
	bl	sub_02000ecc
	movs	r1, #150	@ 0x96
	movs	r2, #230	@ 0xe6
	movs	r0, #13
	lsls	r1, r1, #2
	lsls	r2, r2, #2
	bl	sub_02000ec2
	movs	r1, #154	@ 0x9a
	movs	r2, #228	@ 0xe4
	movs	r0, #15
	lsls	r1, r1, #18
	lsls	r2, r2, #18
	bl	sub_02000ee8
	movs	r1, #128	@ 0x80
	movs	r0, #13
	lsls	r1, r1, #7
	movs	r2, #0
	bl	sub_02000f34
	movs	r1, #158	@ 0x9e
	movs	r2, #230	@ 0xe6
	movs	r0, #15
	lsls	r1, r1, #2
.L_0200052e:
	lsls	r2, r2, #2
	bl	sub_02000eea
	movs	r1, #128	@ 0x80
	movs	r0, #15
	lsls	r1, r1, #7
	movs	r2, #0
	bl	sub_02000f4e
	movs	r1, #154	@ 0x9a
	movs	r2, #228	@ 0xe4
	movs	r0, #14
	lsls	r1, r1, #18
	lsls	r2, r2, #18
	bl	sub_02000f1c
	movs	r1, #154	@ 0x9a
	movs	r2, #230	@ 0xe6
	lsls	r2, r2, #2
	movs	r0, #14
	lsls	r1, r1, #2
	bl	sub_02000f12
	movs	r1, #2
	movs	r0, #13
	bl	sub_02000f52
	movs	r0, #20
	bl	sub_02000ee8
	movs	r0, #13
	movs	r1, #0
	movs	r2, #10
	bl	sub_02000f7a
	movs	r2, #10
	movs	r0, #14
	movs	r1, #0
	bl	sub_02000f84
	movs	r1, #3
	movs	r0, #15
	bl	sub_02000f64
	movs	r0, #30
	bl	sub_02000f0a
	movs	r2, #10
	movs	r0, #15
	movs	r1, #0
	bl	sub_02000f9c
	movs	r0, #13
	movs	r1, #3
	bl	sub_02000f7c
	movs	r0, #14
	movs	r1, #3
	bl	sub_02000f7c
	movs	r0, #15
	movs	r1, #3
	bl	sub_02000f8c
	ldr	r0, [pc, #48]	@ (0x20005e0)
	movs	r1, #99	@ 0x63
	bl	sub_02001004
	ldr	r0, [pc, #44]	@ (0x20005e4)
	movs	r1, #98	@ 0x62
	bl	sub_02001014
	movs	r3, #166	@ 0xa6
	lsls	r3, r3, #1
	adds	r3, #255	@ 0xff
	adds	r6, r6, r3
	movs	r3, #2
	strb	r3, [r6, #0]
	movs	r0, #9
	movs	r1, #2
	bl	sub_02001010
	bl	sub_02000f64
	pop	{r5, r6, pc}
	.4byte 0x0000160d
	.4byte 0x02000240
	.4byte 0x00000007
	.2byte 0x0004
	.2byte 0x0000
	push	{r5, lr}
	movs	r0, #8
	movs	r2, #0
	movs	r1, #4
	bl	sub_02000fda
	ldr	r5, [pc, #76]	@ (0x2000644)
	movs	r2, #133	@ 0x85
	lsls	r2, r2, #2
	adds	r3, r5, r2
	ldr	r0, [r3, #0]
	bl	sub_02000f98
	cmp	r0, #0
	beq.n	.L_02000614
	movs	r3, #10
	ldrsh	r1, [r0, r3]
	movs	r3, #18
	ldrsh	r2, [r0, r3]
	movs	r0, #8
	bl	sub_02000fb2
.L_02000614:
	movs	r0, #20
	bl	sub_02000f98
	ldr	r0, [pc, #44]	@ (0x2000648)
	movs	r1, #99	@ 0x63
	bl	sub_02001070
	ldr	r0, [pc, #40]	@ (0x200064c)
	movs	r1, #98	@ 0x62
	bl	sub_02001080
	movs	r3, #166	@ 0xa6
	lsls	r3, r3, #1
	adds	r3, #255	@ 0xff
	adds	r2, r5, r3
	movs	r3, #2
	strb	r3, [r2, #0]
	movs	r0, #9
	movs	r1, #3
	bl	sub_0200107c
	bl	sub_02000fd0
	pop	{r5, pc}
	.4byte 0x02000240
	.4byte 0x00000008
	.2byte 0x0004
	.2byte 0x0000
	push	{r5, lr}
	bl	sub_02000fdc
	movs	r0, #0
	bl	sub_020010ca
	movs	r1, #236	@ 0xec
	movs	r2, #240	@ 0xf0
	lsls	r1, r1, #17
	lsls	r2, r2, #17
	movs	r0, #8
	bl	sub_02001038
	movs	r0, #16
	bl	sub_02000fee
	ldr	r5, [pc, #164]	@ (0x2000718)
	movs	r3, #133	@ 0x85
	lsls	r3, r3, #2
	adds	r5, r5, r3
	ldr	r0, [r5, #0]
	movs	r1, #8
	movs	r2, #0
	bl	sub_02001078
	movs	r1, #128	@ 0x80
	movs	r2, #0
	ldr	r0, [r5, #0]
	lsls	r1, r1, #1
	bl	sub_020010a4
	movs	r0, #8
	movs	r1, #1
	bl	sub_020010b4
	bl	sub_020010c8
	movs	r1, #4
	movs	r2, #0
	movs	r0, #8
	bl	sub_0200108a
	movs	r0, #16
	bl	sub_02001028
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_0200109a
	movs	r1, #240	@ 0xf0
	movs	r2, #236	@ 0xec
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	movs	r0, #8
	bl	sub_02001068
	movs	r0, #30
	bl	sub_02001046
	movs	r0, #8
	movs	r1, #2
	movs	r2, #0
	bl	sub_020010b8
	movs	r1, #248	@ 0xf8
	movs	r2, #232	@ 0xe8
	movs	r0, #8
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	bl	sub_02001086
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_020010d0
	movs	r1, #244	@ 0xf4
	movs	r2, #228	@ 0xe4
	movs	r0, #8
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	bl	sub_0200109e
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_020010e8
	movs	r1, #248	@ 0xf8
	movs	r2, #224	@ 0xe0
	movs	r0, #8
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	bl	sub_020010b6
	bl	sub_02000cfa
	pop	{r5, pc}
	.2byte 0x0000
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, lr}
	bl	sub_020010a8
	movs	r0, #0
	bl	sub_02001196
	movs	r1, #236	@ 0xec
	movs	r2, #240	@ 0xf0
	lsls	r1, r1, #17
	lsls	r2, r2, #17
	movs	r0, #8
	bl	sub_02001104
	movs	r0, #16
	bl	sub_020010ba
	ldr	r5, [pc, #188]	@ (0x20007fc)
	movs	r3, #133	@ 0x85
	lsls	r3, r3, #2
	adds	r5, r5, r3
	ldr	r0, [r5, #0]
	movs	r1, #8
	movs	r2, #0
	bl	sub_02001144
	movs	r1, #128	@ 0x80
	lsls	r1, r1, #1
	movs	r2, #0
	ldr	r0, [r5, #0]
	bl	sub_02001170
	movs	r0, #16
	bl	sub_020010de
	movs	r1, #244	@ 0xf4
	movs	r2, #220	@ 0xdc
	lsls	r2, r2, #1
	ldr	r0, [r5, #0]
	lsls	r1, r1, #1
	bl	sub_0200111c
	movs	r0, #16
	bl	sub_020010f2
	movs	r0, #8
	movs	r1, #1
	bl	sub_0200119a
	bl	sub_020011ae
	movs	r1, #4
	movs	r2, #0
	movs	r0, #8
	bl	sub_02001170
	movs	r0, #16
	bl	sub_0200110e
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_02001180
	movs	r1, #244	@ 0xf4
	movs	r2, #240	@ 0xf0
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	movs	r0, #8
	bl	sub_0200114e
	movs	r0, #30
	bl	sub_0200112c
	movs	r0, #8
	movs	r1, #2
	movs	r2, #0
	bl	sub_0200119e
	movs	r1, #252	@ 0xfc
	movs	r2, #236	@ 0xec
	movs	r0, #8
	lsls	r1, r1, #1
	lsls	r2, r2, #1
	bl	sub_0200116c
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_020011b6
	movs	r1, #130	@ 0x82
	movs	r2, #232	@ 0xe8
	movs	r0, #8
	lsls	r1, r1, #2
	lsls	r2, r2, #1
	bl	sub_02001184
	movs	r0, #8
	movs	r1, #4
	movs	r2, #0
	bl	sub_020011ce
	movs	r1, #132	@ 0x84
	movs	r2, #228	@ 0xe4
	movs	r0, #8
	lsls	r1, r1, #2
	lsls	r2, r2, #1
	bl	sub_0200119c
	bl	sub_02000de0
	pop	{r5, pc}
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, lr}
	bl	sub_0200118c
	movs	r0, #0
	bl	sub_0200127a
	ldr	r0, [pc, #180]	@ (0x20008c4)
	bl	sub_02001210
	movs	r1, #128	@ 0x80
	movs	r2, #216	@ 0xd8
	movs	r0, #9
	lsls	r1, r1, #18
	lsls	r2, r2, #17
	bl	sub_020011ee
	ldr	r5, [pc, #164]	@ (0x20008c8)
	movs	r3, #133	@ 0x85
	lsls	r3, r3, #2
	adds	r5, r5, r3
	movs	r1, #132	@ 0x84
	movs	r2, #216	@ 0xd8
	ldr	r0, [r5, #0]
	lsls	r1, r1, #18
	lsls	r2, r2, #17
	bl	sub_02001204
	bl	sub_02001298
	bl	sub_020012a4
	movs	r2, #10
	movs	r0, #9
	movs	r1, #0
	bl	sub_0200124e
	movs	r1, #3
	ldr	r0, [r5, #0]
	bl	sub_0200122e
	movs	r0, #30
	bl	sub_020011d4
	movs	r1, #2
	movs	r0, #9
	bl	sub_0200124c
	movs	r0, #20
	bl	sub_020011e2
	movs	r2, #10
	movs	r0, #9
	movs	r1, #0
	bl	sub_02001274
	movs	r1, #3
	ldr	r0, [r5, #0]
	bl	sub_02001254
	movs	r0, #30
	bl	sub_020011fa
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #4
	adds	r0, #150	@ 0x96
	bl	sub_020011ec
	movs	r0, #9
	movs	r1, #2
	bl	sub_02001264
	ldr	r0, [r5, #0]
	bl	sub_0200122a
	cmp	r0, #0
	beq.n	.L_020008a6
	movs	r3, #10
	ldrsh	r1, [r0, r3]
	movs	r3, #18
	ldrsh	r2, [r0, r3]
	movs	r0, #9
	bl	sub_02001244
.L_020008a6:
	movs	r0, #9
	bl	sub_02001272
	movs	r1, #0
	movs	r2, #0
	movs	r0, #9
	bl	sub_02001284
	movs	r0, #10
	bl	sub_0200123a
	bl	sub_0200124e
	pop	{r5, pc}
	.2byte 0x0000
	.4byte 0x00001613
	.2byte 0x0240
	.2byte 0x0200
	push	{r5, lr}
	bl	sub_02001258
	movs	r0, #0
	bl	sub_02001346
	ldr	r0, [pc, #116]	@ (0x2000950)
	bl	sub_020012dc
	movs	r1, #16
	movs	r3, #0
	negs	r1, r1
	movs	r2, #0
	movs	r0, #16
	bl	sub_02001362
	movs	r0, #16
	bl	sub_020012b8
	movs	r0, #128	@ 0x80
	lsls	r0, r0, #5
	movs	r2, #10
	adds	r0, #16
	movs	r1, #0
	bl	sub_02001306
	movs	r0, #16
	movs	r1, #2
	bl	sub_020012de
	ldr	r3, [pc, #72]	@ (0x2000954)
	movs	r2, #133	@ 0x85
	lsls	r2, r2, #2
	adds	r5, r3, r2
	ldr	r0, [r5, #0]
	bl	sub_020012ac
	cmp	r0, #0
	beq.n	.L_02000928
	movs	r3, #10
	ldrsh	r1, [r0, r3]
	movs	r3, #18
	ldrsh	r2, [r0, r3]
	movs	r0, #16
	bl	sub_020012c6
.L_02000928:
	movs	r0, #16
	bl	sub_020012f4
	movs	r1, #0
	movs	r2, #0
	movs	r0, #16
	bl	sub_02001306
	movs	r0, #10
	bl	sub_020012bc
	ldr	r0, [r5, #0]
	movs	r1, #16
	movs	r2, #0
	bl	sub_020013c6
	bl	sub_020012da
	pop	{r5, pc}
	.2byte 0x0000
	.4byte 0x00001616
	.4byte 0x02000240
.include "games/THE LOST AGE/SRC/FIELD/SUHARA_GATE/IMPORT.INC"
	.4byte 0x80010000
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0xffffc000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0x00002000
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00002000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfd70f001
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000004
	.4byte 0x80010000
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0xffffc000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0x00002000
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00002000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfdb2f001
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000008
	.4byte 0x0000000d
	.4byte 0x0000ffff
	.4byte 0xc0010000
	.4byte 0x00000000
	.4byte 0x00000005
	.4byte 0x80010000
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0xffffc000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0x00002000
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00002000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfdfef001
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000004
	.4byte 0x80010000
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0xffffc000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0x00002000
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00002000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfe40f001
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000002
	.4byte 0x00000017
	.4byte 0x00000009
	.4byte 0x00001000
	.4byte 0x00000017
	.4byte 0x0000000a
	.4byte 0xfffff800
	.4byte 0x00000000
	.4byte 0x00000008
	.4byte 0x0000000d
	.4byte 0x0000ffff
	.4byte 0xc0010000
	.4byte 0xffff0000
	.4byte 0x00000098
	.4byte 0x400001f8
	.4byte 0x00200000
	.4byte 0x01100020
	.4byte 0x00000220
	.4byte 0xffff0001
	.4byte 0x00000098
	.4byte 0xc0000208
	.4byte 0x00200000
	.4byte 0x01100020
	.4byte 0x00000220
	.4byte 0xffff0002
	.4byte 0x000000f8
	.4byte 0x80000118
	.4byte 0x00200000
	.4byte 0x01100020
	.4byte 0x00000220
	.4byte 0xffff0003
	.4byte 0x00000088
	.4byte 0x40000130
	.4byte 0x00200000
	.4byte 0x01100020
	.4byte 0x00000220
	.4byte 0xffff0004
	.4byte 0x00000168
	.4byte 0x00000128
	.4byte 0x01500000
	.4byte 0x02e00020
	.4byte 0x000001f0
	.4byte 0xffff0005
	.4byte 0x00000298
	.4byte 0x800001a8
	.4byte 0x01500000
	.4byte 0x02e00020
	.4byte 0x000001f0
	.4byte 0xffff0063
	.4byte 0x000001d8
	.4byte 0x80000138
	.4byte 0x01500000
	.4byte 0x02e00020
	.4byte 0x000001f0
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0xffff0001
	.4byte 0x00000028
	.4byte 0x00000130
	.4byte 0x00100000
	.4byte 0x01a00020
	.4byte 0x000001e0
	.4byte 0xffff0002
	.4byte 0x00000188
	.4byte 0x80000170
	.4byte 0x00100000
	.4byte 0x01a00020
	.4byte 0x000001e0
	.4byte 0xffff0003
	.4byte 0x00000028
	.4byte 0x00000390
	.4byte 0x00100000
	.4byte 0x02d00260
	.4byte 0x000003f0
	.4byte 0xffff0004
	.4byte 0x000002a8
	.4byte 0x80000388
	.4byte 0x00100000
	.4byte 0x02d00260
	.4byte 0x000003f0
	.4byte 0xffff0005
	.4byte 0x00000268
	.4byte 0x40000398
	.4byte 0x00100000
	.4byte 0x02d00260
	.4byte 0x000003f0
	.4byte 0xffff0063
	.4byte 0x00000268
	.4byte 0x40000398
	.4byte 0x00100000
	.4byte 0x02d00260
	.4byte 0x000003f0
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0xffff0001
	.4byte 0x00000048
	.4byte 0xc0000108
	.4byte 0x00180000
	.4byte 0x01180018
	.4byte 0x00000100
	.4byte 0xffff0002
	.4byte 0x000000b8
	.4byte 0x400000a8
	.4byte 0x00180000
	.4byte 0x01180018
	.4byte 0x00000100
	.4byte 0xffff0003
	.4byte 0x00000118
	.4byte 0x40000158
	.4byte 0x00d00000
	.4byte 0x02c00120
	.4byte 0x00000280
	.4byte 0xffff0004
	.4byte 0x00000298
	.4byte 0xc0000268
	.4byte 0x00d00000
	.4byte 0x02c00120
	.4byte 0x00000280
	.4byte 0xffff0063
	.4byte 0x00000210
	.4byte 0x400001b0
	.4byte 0x00d00000
	.4byte 0x02c00120
	.4byte 0x00000280
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000006
	.4byte 0x00201007
	.4byte 0x00404007
	.4byte 0x00502004
	.4byte 0x00000007
	.4byte 0x00102006
	.4byte 0x00203007
	.4byte 0x00302007
	.4byte 0x00404006
	.4byte 0x00501008
	.4byte 0x00000008
	.4byte 0x00105007
	.4byte 0x00203008
	.4byte 0x00302008
	.4byte 0x0040b009
	.4byte 0x000001ff
	.4byte 0xffff00f7
	.4byte 0x00000001
	.4byte 0x00080000
	.4byte 0x00000000
	.4byte 0x00080000
	.4byte 0x0000c000
	.4byte 0xffff0038
	.4byte 0x00000001
	.4byte 0x00080000
	.4byte 0x00000000
	.4byte 0x00080000
	.4byte 0x00022000
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0xffff012b
	.4byte 0x00000007
	.4byte 0x01080000
	.4byte 0x00000000
	.4byte 0x01900000
	.4byte 0x00025000
	.4byte 0xffff012b
	.4byte 0x00000007
	.4byte 0x01180000
	.4byte 0x00000000
	.4byte 0x01700000
	.4byte 0x00025000
	.4byte 0xffff012b
	.4byte 0x00000007
	.4byte 0x01d80000
	.4byte 0x00000000
	.4byte 0x03900000
	.4byte 0x00025000
	.4byte 0xffff012b
	.4byte 0x00000007
	.4byte 0x02180000
	.4byte 0x00000000
	.4byte 0x03900000
	.4byte 0x00025000
	.4byte 0xffff012b
	.4byte 0x00000007
	.4byte 0x02280000
	.4byte 0x00000000
	.4byte 0x03700000
	.4byte 0x00025000
	.4byte 0xffff005d
	.4byte 0x00000001
	.4byte 0x00180000
	.4byte 0x00000000
	.4byte 0x00180000
	.4byte 0x00022000
	.4byte 0xffff005d
	.4byte 0x00000001
	.4byte 0x00180000
	.4byte 0x00000000
	.4byte 0x00180000
	.4byte 0x00022000
	.4byte 0xffff005d
	.4byte 0x00000001
	.4byte 0x00180000
	.4byte 0x00000000
	.4byte 0x00180000
	.4byte 0x00022000
	.4byte 0xffff0038
	.4byte 0x00000001
	.4byte 0x00080000
	.4byte 0x00000000
	.4byte 0x00080000
	.4byte 0x00022000
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0xffff005d
	.4byte 0x00000001
	.4byte 0x01a80000
	.4byte 0x00000000
	.4byte 0x01080000
	.4byte 0x00022000
	.4byte 0x0000ffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000001
	.4byte 0xffff0001
	.4byte 0x00000001
	.4byte 0x00000001
	.4byte 0xffff0002
	.4byte 0x00000002
	.4byte 0x00000001
	.4byte 0xffff0004
	.4byte 0x00000004
	.4byte 0x00000001
	.4byte 0xffff0005
	.4byte 0x00000005
	.4byte 0x0000c602
	.4byte 0xffff0003
	.4byte 0x02008309
	.4byte 0x00000002
	.4byte 0x08940014
	.4byte 0x020083ad
	.4byte 0x00000002
	.4byte 0xffff000c
	.4byte 0x020088cd
	.4byte 0x00000000
	.4byte 0xffff0008
	.4byte 0x0000160c
	.4byte 0xffffffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000001
	.4byte 0xffff0001
	.4byte 0x00000001
	.4byte 0x00000001
	.4byte 0xffff0002
	.4byte 0x00000002
	.4byte 0x00000001
	.4byte 0xffff0003
	.4byte 0x00000003
	.4byte 0x00000001
	.4byte 0xffff0004
	.4byte 0x00000004
	.4byte 0x00000002
	.4byte 0x08950005
	.4byte 0x02008495
	.4byte 0x00000001
	.4byte 0xffff0005
	.4byte 0x00000005
	.4byte 0x00000002
	.4byte 0xffff0015
	.4byte 0x020088cd
	.4byte 0x00000000
	.4byte 0xffff000d
	.4byte 0x00001610
	.4byte 0x00000000
	.4byte 0xffff000e
	.4byte 0x00001611
	.4byte 0x00000000
	.4byte 0xffff000f
	.4byte 0x00001612
	.4byte 0x00000000
	.4byte 0xffff000d
	.4byte 0x02008039
	.4byte 0x00000013
	.4byte 0x0fb40064
	.4byte 0x001000c3
	.4byte 0xffffffff
	.4byte 0x00000000
	.4byte 0x00000000
	.4byte 0x00000001
	.4byte 0xffff0001
	.4byte 0x00000001
	.4byte 0x00000031
	.4byte 0xffff0002
	.4byte 0x00000002
	.4byte 0x00000021
	.4byte 0xffff0003
	.4byte 0x00000003
	.4byte 0x00000002
	.4byte 0xffff0004
	.4byte 0x02008379
	.4byte 0x00000002
	.4byte 0x08960016
	.4byte 0x02008651
	.4byte 0x00000002
	.4byte 0x08960017
	.4byte 0x0200871d
	.4byte 0xffffffff
	.4byte 0x00000000
	.4byte 0x00000000
