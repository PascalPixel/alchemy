...
//
// PORT NOTE: `assembleOverlay` is NOT ported. It runs the whole alchemy-gcc
// compile plan for every exact-C sibling and patches the results into the
// placeholder image; reimplementing it here would mean porting thousands more
// lines that nothing else in this tool needs. Instead `overlay_images.ts` in
// this crate is invoked ONCE, up front, with every overlay that has a C
// sibling. The consequence is an ordering divergence: in the TypeScript an
// assembler failure on overlay N is reported before a compile failure on
// overlay N+1, whereas here every compile failure surfaces first. Success runs
// are identical.
