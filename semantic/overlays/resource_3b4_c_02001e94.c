...
/*
 * resource_3b4 owner at 0x02001e94, 42 bytes.
 *
 * Copy a placement from slot 9 onto slot 14: word +8 verbatim, word +16 biased
 * by 0x10000 (`movs r2,#0x80 / lsls r2,#9`), and word +12 set to the constant
 * 0x200000 (`movs r3,#0x80 / lsls r3,#14`).
 *
 * The epilogue is `pop {r5} / pop {r0} / bx r0`, so this owner is void.  The
 * words are read and written as 32-bit quantities; the project's overlay
 * sources treat such a pair as a 16.16 fixed-point coordinate whose integer
 * part is the s16 two bytes further on, but nothing here observes that view.
 */
...
{
    s32 *target = Func_0808a080(14);
    s32 *source = Func_0808a080(9);

    target[3] = 0x200000;            /* +12 */
    target[2] = source[2];           /* +8 */
    target[4] = source[4] + 0x10000; /* +16 */
}
