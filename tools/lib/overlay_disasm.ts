...
const OVERLAY_C_CACHE = join(dirname(dirname(dirname(Bun.fileURLToPath(import.meta.url)))), "out/cache/overlay-c");
...
function compileOverlayC(
  ...
  // number describing it.
    // The compiler BINARY is part of the key, not just the argv that names it.
    // Rebuilding the fork leaves every command identical while changing the bytes
    // it emits, so without this a rebuild keeps serving the previous cc1's output:
    // 14,339 entries survived a rebuild and made resource_39c fail its asset round
    // trip long after `verify` had reported green on them.
  keyDigest.update(`overlay-c:${selfDigest()}:${compilerBundleSignature()}:${hex(address)}:${hex(callViaBase)}\0`);
 ...
 * `tools/lib/overlay_published.ts` had to report UNRULED and defer to a human.
