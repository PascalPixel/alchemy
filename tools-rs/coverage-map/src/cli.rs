...
pub fn options_of(argv: &[String]) -> Result<Options, String> {
    let mut options = Options { target: "gs1-en".to_string(), ..Options::default() };
    let mut index = 0;
...
pub fn resolve_exact_tree(
    ...
    let wanted = match requested {
        ...
        None => recorded.filter(|value| *value != "worktree").map(str::to_string),
    };
...
pub fn resolve_semantic_tree(
    ...
    if let Some(value) = wanted {
        let tree = if value == "worktree" { Some(work_tree()) } else { ref_tree(&value) };
        return tree.ok_or_else(|| format!("cannot resolve semantic source ref {value}")).map(Some);
    }
    if exact.list("semantic").iter().any(|name| main_source(name).is_some()) {
...
pub fn summarize(document: &Value) -> Result<String, String> {
    ...
    Ok([
        format!(
            "target={}",
            document.get("target").and_then(|v| v.as_str()).unwrap_or("undefined")
        ),
        format!("rom={}", crate::js::commas(field(document, &["rom_bytes"]) as i64)),
...
pub fn tracked_document(map: &CoverageMap) -> Value {
    ...
    let summarize_area = |area: &Value| -> Value {
        let tiles = area.get("tiles").and_then(|v| v.as_array()).map(|list| list.len()).unwrap_or(0);
        Value::Obj(vec![
            ("id".into(), area.get("id").cloned().unwrap_or(Value::Null)),
            ("label".into(), area.get("label").cloned().unwrap_or(Value::Null)),
            ("bytes".into(), area.get("bytes").cloned().unwrap_or(Value::Null)),
            ("categories".into(), area.get("categories").cloned().unwrap_or(Value::Null)),
...
fn map_path(target: &str) -> std::path::PathBuf {
    root().join("metrics").join(format!("{target}-coverage-map.json"))
}
...
pub fn run(argv: &[String]) -> Result<String, String> {
    ...
    let recorded_semantic = tracked_on_disk
        ...
        .map(str::to_string);
    let semantic =
        resolve_semantic_tree(&exact, options.semantic.as_deref(), recorded_semantic.as_deref())?;
...
fn tree_text(trees: &[(&'static str, String)], name: &str) -> String {
    trees.iter().find(|(id, _)| *id == name).map(|(_, text)| text.clone()).unwrap_or_default()
}
...
fn expected_readme(
    ...
    let versions: Vec<(&'static str, String)> = crate::boxtree::BOX_TREES
        .iter()
        .map(|tree| (*tree, crate::boxtree::svg_cache_version(&tree_text(trees, tree))))
        .collect();
...
fn check(
    ...
    let tracked_trees: Vec<(&'static str, String)> = crate::boxtree::BOX_TREES
        .iter()
        .map(|tree| {
            read_text(&crate::boxtree::box_tree_path(&options.target, tree)).map(|text| (*tree, text))
        })
        .collect::<Result<_, _>>()?;
    ...
    if !stale.is_empty() {
        return Err(format!(
            "tracked coverage map is stale ({}); run: bun run coverage",
            stale.join(", ")
        ));
    }
    Ok(format!("coverage-map=current {}", summarize(&map.document)?))
}
...
fn write(
    ...
    let tracked_on_disk = std::fs::read_to_string(map_path(&options.target))
        ...
        .and_then(|text| crate::json::parse(&text).ok());
    if let Some(refusal) =
        semantic_erase_refusal(semantic_resolved, options.semantic.as_deref(), tracked_on_disk.as_ref())
    {
        return Err(refusal);
    }
    let map_file = map_path(&options.target);
    std::fs::write(&map_file, json).map_err(|error| format!("cannot write {}: {error}", map_file.display()))?;
    ...
    let relative = |path: std::path::PathBuf| -> String {
        path.strip_prefix(root()).map(|rest| rest.display().to_string()).unwrap_or_else(|_| path.display().to_string())
    };
...
mod tests {
    ...
    fn options_parse_every_flag() {
        ...
        assert!(missing.contains("undefined"), "a missing value reads as undefined: {missing}");
        let bad = options_of(&argv(&["--target", "gs3"])).expect_err("rejected");
        assert!(bad.contains("\"gs3\""), "the target is quoted like JSON.stringify: {bad}");
    }

    #[test]
    fn a_trailing_ref_flag_reads_as_not_requested() {
        let parsed = options_of(&argv(&["--exact-ref"])).expect("valid");
        assert_eq!(parsed.exact, None, "argv past the end is undefined, not an error");
    }

    #[test]
    fn help_short_circuits_before_a_bad_argument() {
        assert!(options_of(&argv(&["--help", "--nope"])).expect("help wins").help);
        assert!(options_of(&argv(&["--nope", "--help"])).is_err(), "order still matters");
    ...
    fn semantic_erase_refusal_only_fires_when_coverage_would_vanish() {
        ...
        .expect("fixture");
        assert!(semantic_erase_refusal(true, None, Some(&tracked)).is_none(), "resolved");
        assert!(semantic_erase_refusal(false, Some("none"), Some(&tracked)).is_none(), "opted out");
        let refusal = semantic_erase_refusal(false, None, Some(&tracked)).expect("refused");
        assert!(refusal.contains("records 40 semantic bytes from origin/x"), "{refusal}");
        let empty = crate::json::parse(r#"{"categories":{"semantic_c":{"bytes":0}}}"#)
            .expect("fixture");
        assert!(semantic_erase_refusal(false, None, Some(&empty)).is_none(), "nothing to erase");
        assert!(semantic_erase_refusal(false, None, None).is_none(), "no tracked map");
    }

    #[test]
    fn resolve_exact_tree_errors_rather_than_falling_back() {
        let error = resolve_exact_tree(Some("origin/does-not-exist-xyz"), None)
            .expect_err("unresolvable");
        assert!(error.contains("git fetch origin does-not-exist-xyz"), "{error}");
        assert_eq!(resolve_exact_tree(None, None).expect("worktree").id(), "worktree");
        assert_eq!(
            resolve_exact_tree(None, Some("worktree")).expect("worktree").id(),
            "worktree",
    ...
    fn tracked_document_replaces_tiles_with_a_count_in_place() {
        ...
        let Value::Obj(fields) = &tracked else { panic!("object") };
        ...
        let areas = tracked.get("rom_areas").and_then(|v| v.as_array()).expect("areas");
...
mod real_tree_tests {
    ...
    /// The strongest available parity assertion that does not need the
    /// renderers: build the map from the live tree and compare the emitted
    /// document, byte for byte, with what the TypeScript produces from the same
    /// tree right now.
    ///
    /// PORT NOTE: the comparison is against a freshly generated golden, not
    /// against `metrics/gs1-en-coverage-map.json`. The tracked file is stale
    /// with respect to the current worktree -- the TypeScript itself disagrees
    /// with it by 30 executable bytes and 724 exact bytes -- so asserting
    /// against it would fail for a reason that has nothing to do with the port.
    /// The golden is regenerated by running the TypeScript here, so the test
    /// can never pass by comparing against a stale file and can never silently
    /// skip because someone ran `cargo clean`.
    #[test]
    fn the_emitted_document_matches_the_tracked_json() {
        let exact = work_tree();
        let golden = root().join("tools-rs/coverage-map/target/fixtures/ts-tracked.json");
        let expected = generate_golden(&golden);
        let semantic = resolve_semantic_tree(&exact, None, Some("worktree")).expect("semantic");
        let map = build_coverage_map(&BuildOptions {
        ...
        .expect("map");
        let produced = tracked_json(&map);
        // Written next to the golden so a failure can be diffed directly.
        let _ = std::fs::write(
            root().join("tools-rs/coverage-map/target/fixtures/rs-tracked.json"),
            &produced,
        );
        assert_eq!(produced.len(), expected.len(), "emitted JSON length");
        assert!(produced == expected, "emitted JSON differs from the tracked document");
    }
    ...
    fn every_emitted_number_stringifies_like_javascript() {
        ...
        assert!(checked > 1000, "only {checked} numbers walked; the walk is not reaching the tiles");
        assert!(fractional > 0, "no fractional value seen, so the check would be vacuous");
    }

    /// Run the TypeScript and capture `canonicalJson(trackedDocument(...))`.
    ///
    /// This shells out to `bun` deliberately: a golden the test writes itself
    /// from the Rust side would be a tautology, and the point of the assertion
    /// is that the *other* implementation agrees.
    fn generate_golden(path: &std::path::Path) -> String {
        let script = concat!(
            "const m = await import('./tools/metrics/coverage_map.ts');",
            "const { canonicalJson } = await import('./tools/lib/canonical_json.ts');",
            "const exact = m.workTree();",
            "const map = m.buildCoverageMap({ target: 'gs1-en', exact, semantic: exact,",
            " validateTrackedProgress: true, preferVerifiedAssets: true });",
            "process.stdout.write(canonicalJson(m.trackedDocument(map)));",
        );
        let output = std::process::Command::new("bun")
            ...
            .current_dir(root())
            .output()
            .expect("bun must be on PATH for the golden to be regenerated");
        assert!(
            output.status.success(),
            "bun failed: {}",
            String::from_utf8_lossy(&output.stderr)
        );
        let text = String::from_utf8(output.stdout).expect("golden is UTF-8");
        assert!(text.len() > 500, "golden looks truncated ({} bytes)", text.len());
