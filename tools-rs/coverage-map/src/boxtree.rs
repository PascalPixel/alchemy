...
pub const CORE_HUE: HueBand =
    HueBand { hsl_hue: 275.0, ok_hue: 295.0, ok_cmax: 0.26, edge: "#6d4fc2" };
pub const OVERLAY_HUE: HueBand =
    HueBand { hsl_hue: 190.0, ok_hue: 200.0, ok_cmax: 0.17, edge: "#1f7f93" };
pub const ASSET_HUE: HueBand =
    HueBand { hsl_hue: 330.0, ok_hue: 355.0, ok_cmax: 0.30, edge: "#bb2f77" };
pub const MUSIC_HUE: HueBand =
    HueBand { hsl_hue: 28.0, ok_hue: 55.0, ok_cmax: 0.20, edge: "#c85d00" };
...
const BOX_TREE_COMPACT_LEGEND: [(&str, &str); 3] =
    [("asset_color", "Color"), ("asset_bytes", "Encoded"), ("asset_unclassified", "Unknown")];
...
pub fn embedded_weyard_font() -> Result<&'static str, String> {
    ...
    let path = crate::tree::root().join("assets").join("fonts").join("weyard.otf");
...
enum BoxNode {
    Group { label: String, children: Vec<BoxNode> },
    Leaf(Tile),
}
...
fn box_tree_nodes(tiles: &[Tile], folders: &[&str], depth: usize) -> Vec<BoxNode> {
    if depth >= folders.len() {
        return tiles.iter().map(|tile| BoxNode::Leaf(tile.clone())).collect();
    }
    ...
    for tile in tiles {
        let label = if key == "group" { tile.group.clone() } else { tile.subgroup.clone() };
    ...
    for (label, children) in groups {
        ...
        out.push(BoxNode::Group { label, children: nested });
    }
    out
}
...
fn folder_display_name(label: &str) -> String {
    ...
    let mut text = label.strip_prefix("golden-sun-").unwrap_or(label).to_string();
...
impl BoxContext<'_> {
    ...
    fn draw_rectangle_label(&mut self, rectangle: Rect, text: &str) {
        ...
        let Some(display) = fit_weyard_text(text, rectangle.width - 6.0, 8.0) else { return };
        let banner = Rect { height: 10.0, ..rectangle };
    ...
    fn draw_leaf(&mut self, tile: &Tile, rectangle: Rect) {
        let populated: Vec<&str> = self
            ...
            .collect();
        let total: i64 = populated.iter().map(|category| tile.category(category)).sum();
        self.lines.push(format!(
            "<g aria-label=\"{}\">",
            escape_text(&format!("{}: {} bytes", tile.label, commas(tile.bytes as f64)))
        ));
        if total <= 0 {
        ...
        for (index, category) in populated.iter().enumerate() {
            ...
            let band =
                Rect { x: rectangle.x, y: top, width: rectangle.width, height: bottom - top };
...
fn leaf_display_name(tile: &Tile) -> String {
    ...
    let without_group = match &tile.group {
        Some(group) => {
            ...
            tile.label.strip_prefix(&prefix).unwrap_or(&tile.label).to_string()
        }
    ...
    let head = without_group.split(" \u{b7} ").next().unwrap_or("").to_string();
    head.replace("0x", "")
}
...
fn capture_after_ci(text: &str, marker: &str) -> Option<String> {
    ...
    let run: String = text[start..].chars().take_while(char::is_ascii_hexdigit).collect();
    if run.is_empty() {
        None
    } else {
        Some(run)
        ...
        if let Some(body) = rest.strip_prefix("0x").or_else(|| rest.strip_prefix("0X")) {
            ...
            if !first.is_empty() {
                ...
                if let Some(after) = tail.strip_prefix("\u{2013}") {
                    if let Some(hex) = after.strip_prefix("0x").or_else(|| after.strip_prefix("0X"))
                    {
                        let second: String =
...
pub fn render_box_tree(
    ...
    let width = 540.0f64;
    let height = 304.0f64;
    let plot = Rect { x: 3.0, y: 22.0, width: width - 6.0, height: height - 46.0 };
    let mut context = BoxContext { hue, category_fraction, category_order, lines: Vec::new() };
    ...
    context.lines.push(precise_rect(
        Rect { x: 1.0, y: 1.0, width: width - 2.0, height: height - 2.0 },
        "fill=\"none\" stroke=\"#1c1c26\" stroke-width=\"2\" rx=\"7\"",
    ));
    context.lines.push(precise_rect(plot, &format!("fill=\"{GROUND}\" rx=\"3\"")));

    let nodes = box_tree_nodes(&item.tiles, folders, 0);
    let total: f64 = nodes.iter().map(box_node_bytes).sum();
    if total != item.bytes as f64 {
        return Err(format!("{} hierarchy does not conserve its byte total", item.id));
    }
    ...
    if let Some(category) = completion_category {
        let completion_label = if category == "exact_c" {
            ...
            lookup_text(&BOX_TREE_LEGEND, category).unwrap_or(category).to_uppercase()
        };
    ...
    for category in category_order {
        let Some(legend_label) = lookup_text(&BOX_TREE_LEGEND, category) else { continue };
        ...
        let swatch =
            Rect { x: legend_x, y: legend_center_y - 5.0, width: 10.0, height: 10.0 };
...
pub fn box_tree_path(target: &str, tree: &str) -> std::path::PathBuf {
    crate::tree::root().join("assets").join("readme").join(format!("{target}-{tree}.svg"))
}
...
fn asset_tree_context(
    ...
    let catalogued_bytes: i64 = maturity
        .iter()
        .map(|tile| if tile.category("asset_unclassified") > 0 { 0 } else { tile.bytes })
        .sum();
    let embedded_unclassified: i64 =
        maturity.iter().map(|tile| tile.category("asset_unclassified")).sum();
    ...
    let folders: Vec<&'static str> =
        if maturity_with_remainder.iter().any(|tile| tile.group.is_some()) {
            vec!["group", "subgroup"]
        } else {
            vec![]
        };
    Ok(AssetTreeContext {
        ...
        category_order: if detailed {
            vec!["asset_unclassified", "asset_bytes", "asset_bw", "asset_color", "asset_objects"]
        } else {
            vec!["asset_data"]
        },
        folders,
...
fn is_music_asset(tile: &Tile) -> bool {
    ...
    ["sound", "audio", "wave", "pcm", "music", "midi", "sequence", "soundfont", "sound-font"]
        .iter()
...
pub fn render_box_trees(
    ...
    let overlays = map.executable_areas.iter().find(|item| item.id == "overlays");
    ...
    let music_tiles: Vec<Tile> =
        assets.area.tiles.iter().filter(|tile| is_music_asset(tile)).cloned().collect();
    let image_tiles: Vec<Tile> =
        assets.area.tiles.iter().filter(|tile| !is_music_asset(tile)).cloned().collect();
    ...
    Ok(vec![
        ...
        ("images", render_asset_tree(&assets, &images, "Images", &ASSET_HUE, "pink")?),
        ("music", render_asset_tree(&assets, &music, "Music", &MUSIC_HUE, "orange")?),
    ])
}
...
pub fn readme_with_cache_buster(
    readme: &str,
    target: &str,
    versions: &[(&str, String)],
) -> String {
    ...
    for tree in BOX_TREES {
        let Some((_, version)) = versions.iter().find(|(name, _)| *name == tree) else { continue };
        ...
        while let Some(found) = rest.find(&needle) {
            ...
            let tail = if let Some(hex) = after.strip_prefix("?v=") {
                let run: usize =
                    hex.chars().take_while(char::is_ascii_hexdigit).map(char::len_utf8).sum();
...
pub fn readme_with_done_headline(readme: &str, map: &CoverageMap) -> Result<String, String> {
    ...
    let done = js_round(crate::intervals::round_half_up_percent(done_bytes, executable)?);
...
fn svg_path(target: &str) -> std::path::PathBuf {
    crate::tree::root().join("assets").join("readme").join(format!("{target}-coverage.svg"))
}
...
mod tests {
    ...
    fn a_rectangle_shares_its_far_edge_rather_than_summing_a_rounded_width() {
        ...
        let tag = precise_rect(
            Rect { x: 0.1234567, y: 0.0, width: 0.1234567, height: 4.0 },
            "fill=\"none\"",
        );
    ...
    fn the_weyard_fitter_has_no_max_on_its_slice_bound() {
        assert_eq!(fit_weyard_text("abcdef", 100.0, 8.0), Some("abcdef".to_string()));
        assert_eq!(fit_weyard_text("abcdef", 16.0, 8.0), Some("abc\u{2026}".to_string()));
        assert_eq!(fit_weyard_text("abcdef", 11.0, 8.0), None, "room 2 is below the floor");
    }
    ...
    fn folder_names_uppercase_ascii_word_starts_only() {
        ...
        assert_eq!(folder_display_name("\u{e9}clair-bank"), "\u{e9}Clair Bank",
    ...
    fn the_leaf_name_strips_its_group_and_every_hex_prefix() {
        ...
        let plain =
            Tile { label: "0x0800 \u{b7} tail".to_string(), bytes: 1, ..Tile::default() };
    ...
    fn the_cache_buster_replaces_a_present_version_and_adds_a_missing_one() {
        ...
        let versions = vec![("core", "11112222".to_string()), ("music", "33334444".to_string())];
