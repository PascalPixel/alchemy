...
/*
 * resource_38a owner at 0x0200013c, 472 bytes: the overlay's general
 * "spawn and fully configure" routine -- the same spawn-then-owner-record
 * shape resource_38a_c_02000048.c/02005158.c/02005388.c already document,
 ...
 * 0x0200880c and `idx = flags & 0xf` -- the low nibble of the `flags`
 ...
 * clear-two-bits-then-copy idiom resource_38a_c_02000048.c's adopted
 * Func_02005a2e pair and resource_38a_c_02005158.c already use on this
...
extern struct Descriptor *Data_0200880c[];

struct Effect *Func_02000882();
struct Effect *Func_02000860();
void Func_0200086a();
void Func_02000884();
void Func_0200096e();
s32 Func_02000928();
                   ...
                   const struct Options *options)
{
    ...
    s32 first_delta;
    s32 accumulated;
    party = Func_02000882(0);

    if ((flags & 0x100000) != 0 && options != 0) {
        effect = Func_02000860(options->kind, x, y, z);
    } else {
        effect = Func_02000860(222, x, y, z);
    }
    if (effect == 0) return;
    ...
    Func_0200086a(effect, (flags + 1) & 15);
    table_offset = (flags & 15) << 2;
    Func_02000884(effect, Data_0200880c[table_offset >> 2]);
    ...
    if ((flags & 0x10000) != 0) {
        Func_0200096e(effect, options->mode);
    }
    ...
    if ((flags & 0x40000) != 0) {
        ...
        if ((flags & 0x80000) != 0) {
            ...
            effect->rate30 = Func_02000928(first_delta,
                                           descriptor->duration);
            ...
            effect->rate30 = Func_02000940(first_delta,
                                           descriptor->duration);
            delta = options->target34;
            duration = descriptor->duration;
        ...
        effect->rate34 = Func_0200094e(delta, duration);
    }

    if ((flags & 0x200000) != 0) {
        Func_02000984(effect, 1);
        Func_02000994(effect, options->callback_arg);
    }
