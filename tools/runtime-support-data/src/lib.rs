...
const FLASH_REGIONS: [(u32, u32); 3] =
    [(0x0800_7aa4, 0x0800_7b14), (0x0800_7b3c, 0x0800_7bc4), (0x0800_7bcc, 0x0800_7c3c)];
...
fn canonical_address(value: &Value, label: &str) -> Result<u32> {
    ...
    if text.len() != 10
        ...
        || !text[5..].bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
...
fn byte_id(value: &Value, label: &str) -> Result<u8> {
    ...
    if text.len() != 4
        || !text.starts_with("0x")
        || !text[2..].bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
...
fn function_pointer(value: &Value, label: &str) -> Result<u32> {
    ...
    if text.len() != 13
        ...
        || !text[8..].bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte))
    {
...
fn build_number_format(source: &Map<String, Value>) -> Result<Vec<u8>> {
    ...
    for (index, divisor) in
        field(number, "decimal_divisors", "decimal divisor table")?.as_array().unwrap().iter().enumerate()
    {
...
fn timing_buffer(source: &Map<String, Value>, name: &str) -> Result<Vec<u8>> {
    ...
    let timing = object(
        timings.get(name).ok_or_else(|| Error(format!("unknown timing set {name}")))?,
...
fn device_buffer(source: &Map<String, Value>, name: &str) -> Result<Vec<u8>> {
    ...
    let device = object(
        devices.get(name).ok_or_else(|| Error(format!("unknown flash device {name}")))?,
    ...
    put_u32(
        &mut output,
        20,
        uinteger(field(geometry, "total_bytes", "total bytes")?, 0, 0xffff_ffff, "total bytes")? as u32,
    );
    put_u16(
        &mut output,
        24,
        uinteger(field(geometry, "sector_bytes", "sector bytes")?, 0, 0xffff, "sector bytes")? as u16,
    );
    put_u16(&mut output, 28, uinteger(field(geometry, "sector_shift", "sector shift")?, 0, 31, "sector shift")? as u16);
    put_u16(
        &mut output,
        30,
        uinteger(field(geometry, "sector_count", "sector count")?, 0, 0xffff, "sector count")? as u16,
    );
    put_u32(
        &mut output,
        32,
        uinteger(field(geometry, "top_sector", "top sector")?, 0, 0xffff_ffff, "top sector")? as u32,
    );
...
fn build_transfer(source: &Map<String, Value>) -> Result<Vec<u8>> {
    ...
    let identity_width =
        integer(field(transfer, "identity_field_bytes", "identity width")?, 1, 32, "identity width")? as usize;
    let marker = ascii(field(transfer, "record_marker", "transfer marker")?, "transfer marker")?;
    let reserved =
        integer(field(transfer, "reserved_zero_bytes", "reserved bytes")?, 0, 32, "reserved bytes")? as usize;
    let packets = field(transfer, "packets", "transfer packets")?.as_array().unwrap();
    let version_width =
        integer(field(transfer, "flash_version_field_bytes", "version width")?, 1, 32, "version width")? as usize;
    let mut output = vec![0; (TRANSFER_END - TRANSFER_ADDRESS) as usize];
    let mut cursor = 0;
    let identity =
        fixed_ascii(ascii(field(transfer, "identity", "identity")?, "identity")?, identity_width, "transfer identity")?;
    ...
    let version = fixed_ascii(
        ascii(field(transfer, "flash_version", "flash version")?, "flash version")?,
        version_width,
        "flash version",
    )?;
...
fn build_flash_regions(source: &Map<String, Value>) -> Result<BTreeMap<u32, Vec<u8>>> {
    ...
    for (name, timing) in timings {
        pieces.push((
            canonical_address(field(object(timing, "timing")?, "address", "timing address")?, "timing")?,
            timing_buffer(source, name)?,
        ));
    }
    for (name, device) in devices {
        pieces.push((
            canonical_address(field(object(device, "device")?, "address", "device address")?, "device")?,
...
fn parse_cli_integer(value: &str, label: &str) -> Result<u32> {
    ...
    if digits.is_empty()
        || !digits
            .bytes()
            .all(|byte| byte.is_ascii_digit() || (value.starts_with("0x") && (b'a'..=b'f').contains(&byte)))
    {
