#!/usr/bin/env bun
import { canonicalJson, isCanonicalJsonText } from "../lib/canonical_json.ts";
...
function stageStamp(manifestPath: string, sourceOnly: boolean): string {
  ...
  const inputs = [
    ...
    // `exact/` belongs here for the same reason assets/code and semantic/ do:
    // adopting an owner writes exact/<owner>.c, that owner compiles into an overlay
    // image, and several asset regions compress those images. Without it the stamp
    // reported a hit for work an adoption had just invalidated -- the action most
    // likely to change the compressed bytes was the one input it could not see.
    ...walk(join(ROOT, "semantic")),
    ...walk(join(ROOT, "exact")),
  ];
