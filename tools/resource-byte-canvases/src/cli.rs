...
use crate::{export_resource_byte_canvases, self_test, verify_resource_byte_canvases, Error, RESOURCES, SOURCE_BYTES};
...
fn run(args: &[String]) -> Result<(), Error> {
    if args.len() == 1 && args[0] == "--self-test" {
        self_test()?;
        println!("self-test=ok resources={} source_bytes={SOURCE_BYTES}", RESOURCES.len());
        return Ok(());
    }
    ...
    if args.len() == 4 && args[0] == "verify" && args[2] == "--directory" {
        ...
        println!("identical=true resources={} source_bytes={SOURCE_BYTES}", RESOURCES.len());
        return Ok(());
    }
