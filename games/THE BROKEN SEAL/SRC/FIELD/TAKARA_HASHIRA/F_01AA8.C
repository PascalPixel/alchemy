...
s32 TakaraHashira_Func02001938(struct FieldActor *actor, struct FieldActor *other);
void TakaraHashira_Func020019f0(struct FieldActor *actor, struct FieldActor *other);
...
s32 TakaraHashira_Func02001aa8(struct FieldActor *actor)
{
    ...
    hit = TakaraHashira_Func02001938(actor, leader);
    for (id = 8; id <= 11; id++) {
        hit += TakaraHashira_Func02001938(actor, Engine_ActorGet(id));
    }
    if (hit != 0) {
        for (id = 8; id <= 11; id++) {
            TakaraHashira_Func020019f0(actor, Engine_ActorGet(id));
        }
    }
