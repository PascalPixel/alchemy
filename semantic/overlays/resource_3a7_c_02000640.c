/*
 * resource_3a7 scene-entity presentation step at 0x02000640, 80 bytes.
 *
 * Complete owner: `push {r5, lr}` / `sub sp, #8` at 0x02000640 through the
 ...
 * No literal pool sits inside the span - the flag id is built with
 * `movs / lsls` immediates.
 *
 ...
 *
 * Shape:
 *   record = Func_0808a080(20);          // scene-entity record by selector
 *   Func_080091c0(26, 30, 1, 1, 26, 34);  // six-argument request
 *   if (record != 0) { Func_080091e0(record, 0);
 *       Func_0808a080(20)[85] = 0;      // reloaded, same selector
 *       record[35] = 1;
 *   }
 *   Func_080770c8(0x204);            // raise the completion flag
 *
 * Call targets resolved with
 *   bun tools/overlay_show.ts resource_3a7 0640 -n 80 |
...
{
    unsigned char *record = Func_0808a080(20);

    Func_080091c0(26, 30, 1, 1, 26, 34);
