...
// keepalive interval.
//
// PORT NOTE: the TypeScript holds a `Set` of stream controllers and deletes a
// controller when `enqueue` throws. Here a client is a channel to its own
// connection thread, and a send failure removes it. `Vec` and not `HashSet`:
// insertion order is the delivery order, and `Set` would need identity
// semantics a `HashSet` does not give a closure.
...
pub fn register() -> (u64, Receiver<Vec<u8>>) {
    // A bounded channel, so a client that stops reading cannot make the
    // server buffer without limit. `Bun.serve` applies its own backpressure;
    // the shared behaviour that matters is that a wedged client is dropped
    // rather than allowed to grow the process.
    let (sender, receiver) = std::sync::mpsc::sync_channel(64);
    let id = {
        let mut next = NEXT_ID.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let id = *next;
        *next += 1;
        id
    };
...
mod tests {
    ...
    fn a_registered_client_receives_events_in_order_and_unregisters_cleanly() {
        ...
        keepalive();
        assert_eq!(receiver.recv().unwrap(), b"event: update\ndata: {\"a\":1}\n\n".to_vec());
    ...
    fn a_dropped_receiver_removes_its_client_rather_than_wedging_the_broadcast() {
        ...
        drop(receiver);
        send_event("update", "{}");
        assert_eq!(count(), before, "a disconnected client must be reaped, as the catch does");
    }
}
